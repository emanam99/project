...
export default function UgtLaporanMasalahFields({ items, onChange }) {
  ...
  const updateField = (idx, field, value) => {
    setItems(list.map((row, i) => (i === idx ? { ...row, [field]: value } : row)))
  }
  ...
  return (
    <motion.div layout className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Masalah</label>
        <span className="inline-flex items-center justify-center min-w-7 h-7 px-2 rounded-full bg-teal-100 dark:bg-teal-900/40 text-teal-800 dark:text-teal-200 text-xs font-semibold tabular-nums">
          {list.length}
        </span>
      </div>
        ...
        <AnimatePresence mode="popLayout" initial={false}>
          {list.map((item, idx) => (
            ...
            >
              ...
              <textarea value={item.masalah} onChange={(e) => updateField(idx, 'masalah', e.target.value)} rows={2} placeholder="Masalah yang ditemukan" className={fieldCls} />
              <textarea value={item.solusi} onChange={(e) => updateField(idx, 'solusi', e.target.value)} rows={2} placeholder="Solusi yang sudah dilakukan" className={fieldCls} />
              <textarea value={item.saran} onChange={(e) => updateField(idx, 'saran', e.target.value)} rows={2} placeholder="Saran tindak lanjut bagi pengurus UGB" className={fieldCls} />
            </motion.div>
          ))}
        </AnimatePresence>
      </motion.div>
