...
import { clearStoredLoginUsername } from '../../utils/passkeyLoginPrefs'
...
export default function AuthLeftPanel() {
  ...
  const isPjgtPortal = pathname.includes('pjgt')
  const isTokoPortal = pathname.includes('toko')
  const isLoginRoute = pathname === '/login' || pathname === '/login-pjgt'

  return (
    <div className="hidden md:flex flex-1 relative min-h-screen login-panel-left overflow-hidden">
      <div className="absolute inset-0 bg-linear-to-br from-[#1761ac] via-[#1a6eb5] to-[#125899]" />
      ...
      <div className="relative z-10 flex flex-col justify-center items-start px-14 text-white">
        ...
        >
          ...
          {isLoginRoute ? (
            <button
              type="button"
              onClick={() => clearStoredLoginUsername()}
              className="text-primary-300/80 text-xs font-mono mb-6 select-none cursor-default"
              tabIndex={-1}
              aria-hidden
            >
              v{APP_VERSION}
          ...
          <p className="text-primary-200/95 text-lg font-medium tracking-wide mb-6">
            {isPjgtPortal
              ? 'Aplikasi Guru Tugas dan pelaporan PJGT'
              : isTokoPortal
                ? 'Kasir, barang, dan transaksi cashless toko'
          ...
          <div className="flex items-center gap-2 text-primary-300/80 text-sm">
            ...
            <span>{isPjgtPortal ? 'Aplikasi PJGT' : isTokoPortal ? 'Aplikasi toko' : 'Aplikasi santri'}</span>
          </div>
        </motion.div>
      </div>
    </div>
