...
import { BarangIcon, KasirIcon, RiwayatTransaksiIcon } from '../../navigation/navIcons'

const tokoMenuItems = [
  { path: '/toko/penjualan', label: 'Penjualan', icon: KasirIcon },
  { path: '/toko/riwayat', label: 'Riwayat', icon: RiwayatTransaksiIcon },
  { path: '/toko/barang', label: 'Data Barang', icon: BarangIcon },
]
...
export default function Toko() {
    ...
    profilAPI
      ...
      .then((res) => {
        if (!cancelled && res.success) {
          setData({ user: res.user, nama: res.nama, foto_profil: res.foto_profil })
          patchUser({ foto_profil: res.foto_profil ?? null })
        }
  ...
  }, [user?.id, user?.has_toko, patchUser])
  ...
  const handleUploadFoto = async (blob) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const profileRes = await profilAPI.getProfil('toko')
        if (profileRes.success) {
          setData({ user: profileRes.user, nama: profileRes.nama, foto_profil: profileRes.foto_profil })
          patchUser({ foto_profil: profileRes.foto_profil ?? null })
        }
      } else {
      ...
      <div className="rounded-2xl bg-white dark:bg-gray-800/90 shadow-sm border border-gray-100 dark:border-gray-700/50 overflow-hidden">
        ...
        <div className="p-6 flex flex-col items-center">
          <div className="relative group shrink-0">
            <div
              className="w-28 h-28 rounded-full overflow-hidden bg-primary-500/10 dark:bg-primary-400/10 flex items-center justify-center text-4xl font-semibold text-primary-600 dark:text-primary-400 ring-2 ring-gray-200 dark:ring-gray-600 cursor-pointer"
          ...
          {tokoMenuItems.map((item) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center justify-center gap-2 py-4 px-3 rounded-xl bg-white dark:bg-gray-800/90 border border-gray-200 dark:border-gray-700 hover:bg-primary-50 dark:hover:bg-primary-900/20 active:bg-primary-100 dark:active:bg-primary-900/30 transition-colors"
              >
