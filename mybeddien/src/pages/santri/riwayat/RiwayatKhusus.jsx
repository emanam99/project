...
import { useSantriIds, useSantriKhususBundle } from '../../../hooks/useSantriCachedResources'
import { refreshSantriPembayaran } from '../../../services/santriDataService'
import { formatCurrency, formatDate } from '../../../utils/riwayatPembayaran'
...
export default function RiwayatKhusus() {
  ...
  const { bundle, loading } = useSantriKhususBundle()
  const qrMatch = useKwitansiQrSantriMatch()
  const [bayarFor, setBayarFor] = useState(null)
  const rincian = bundle?.rincian ?? []
  const total = bundle?.total ?? { total: 0, bayar: 0, kurang: 0 }
  ...
  return (
    <PageEnter className="max-w-2xl mx-auto px-4 sm:px-6 py-3 sm:py-4 pb-6">
      <PageEnterTitle className="mb-3">
        <Link
        ...
        <h1 className="text-lg font-semibold text-gray-900 dark:text-white tracking-tight">Khusus</h1>
      </PageEnterTitle>

      {loading ? (
        <PageEnterLoading className="flex justify-center py-8">
        ...
        <>
          ...
          <BayarOffcanvas
            ...
            onSuccess={() => { setBayarFor(null); refreshSantriPembayaran(idSantri, userId) }}
          />
        </>
      )}
    </PageEnter>
