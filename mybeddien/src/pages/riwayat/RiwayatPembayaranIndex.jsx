...
const MENU_ITEMS = [
  { key: 'pendaftaran', path: '/riwayat-pembayaran/pendaftaran', label: 'Pendaftaran', description: 'Riwayat pembayaran registrasi per tahun ajaran' },
  { key: 'uwaba', path: '/riwayat-pembayaran/uwaba', label: 'UWABA', descriptionBase: 'Riwayat pembayaran UWABA' },
  { key: 'khusus', path: '/riwayat-pembayaran/khusus', label: 'Khusus', description: 'Riwayat pembayaran khusus' },
  { key: 'tunggakan', path: '/riwayat-pembayaran/tunggakan', label: 'Tunggakan', description: 'Riwayat pembayaran tunggakan' },
]
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-4 sm:py-6 pb-8">
      ...
      {loading ? (
        <div className="flex justify-center py-12">
          <div className="animate-spin rounded-full h-8 w-8 border-2 border-teal-500 dark:border-teal-400 border-t-transparent" />
        </div>
      ) : (
        <>
                ...
                <li key={item.path}>
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-4 shrink-0 text-right">
                      ...
                      <span className="text-xs text-green-600 dark:text-green-400 whitespace-nowrap">Bayar {formatCurrency(tot.bayar)}</span>
          ...
          <section className="mt-8 pt-6 border-t-2 border-gray-200 dark:border-gray-600">
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="rounded-xl bg-white dark:bg-gray-800/60 border border-gray-200 dark:border-gray-600 p-4 text-center shadow-sm">
                <p className="text-xs text-green-600 dark:text-green-400 mb-1">Bayar</p>
                <p className="text-base sm:text-lg font-bold text-green-700 dark:text-green-300 tabular-nums" title={formatCurrency(totalKeseluruhan.bayar)}>{formatCurrency(totalKeseluruhan.bayar)}</p>
              </div>
              <div className="rounded-xl bg-white dark:bg-gray-800/60 border border-gray-200 dark:border-gray-600 p-4 text-center shadow-sm">
                {(() => {
