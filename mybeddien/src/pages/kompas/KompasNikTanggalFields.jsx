...
export default function KompasNikField({
  ...
  labelClass = 'mb-1 block text-xs text-slate-500 dark:text-slate-400',
  inputClass = '',
}) {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(async () => {
      try {
        const res = await kompasMybeddianAPI.checkNik({
          nik: nikDigits,
          tahunAjaran,
      ...
      <div className="mb-1 flex items-center justify-between gap-2">
        ...
        <span
          className={`font-mono text-[10px] tabular-nums ${
            nikDigits.length === 16 ? 'text-primary-600 dark:text-primary-400' : 'text-slate-400 dark:text-slate-500'
          }`}
        >
          {nikDigits.length}/16
        </span>
      ...
      <AnimatePresence mode="wait">
        {nikCheck === 'checking' ? (
          <motion.p
            ...
            exit={{ opacity: 0, y: -2 }}
            className="mt-1 flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400"
          >
        ...
        {nikCheck === 'ok' ? (
          <motion.p
            ...
            className="mt-1 flex items-start gap-1.5 text-[11px] font-medium text-emerald-600 dark:text-emerald-400"
          >
