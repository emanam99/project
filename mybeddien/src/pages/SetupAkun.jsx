...
export default function SetupAkun() {
  ...
  const [entityType, setEntityType] = useState('santri')
  ...
  useEffect(() => {
    ...
    authAPI.getSetupTokenSantri(token).then((res) => {
      if (cancelled) return
      setValid(!!res.valid)
      const rawEt = String(res.entity_type || 'santri')
      const et = rawEt === 'madrasah' || rawEt === 'toko' ? rawEt : 'santri'
      if (res.valid) {
        setEntityType(et)
        setNama(
          res.nama ||
            (et === 'madrasah' ? 'Madrasah' : et === 'toko' ? 'Toko' : 'Santri')
        )
      }
    }).catch(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const loginTarget =
          entityType === 'madrasah' || portalPjgt
            ? '/login-pjgt'
            : '/login'
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50 dark:bg-gray-900">
        ...
        >
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">
            Link sudah kadaluarsa (aktif 10 menit) atau sudah dipakai. Silakan minta link baru dari halaman Daftar lewat WhatsApp.
          </p>
          <a
            href={
              entityType === 'madrasah' || portalPjgt
                ? '/daftar-pjgt'
                : entityType === 'toko' || portalToko
                  ? '/daftar-toko'
                  : '/daftar'
            }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-linear-to-br from-primary-50 via-white to-primary-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      >
        ...
        <p className="text-gray-500 dark:text-gray-500 text-xs mb-6">
          Username minimal 5 karakter, tanpa spasi. Password minimal {PASSWORD_MIN} karakter (ketik ulang di konfirmasi; tidak bisa tempel).
        </p>
