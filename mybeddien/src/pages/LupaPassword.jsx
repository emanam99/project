...
/**
 * Lupa password — verifikasi identitas seperti daftar (santri / PJGT / toko), lalu kirim link reset ke WA.
 */
export function LupaPasswordFormCard() {
  ...
  const lupaUsernamePath =
    mode === 'pjgt' ? '/lupa-username-pjgt' : mode === 'toko' ? '/lupa-username-toko' : '/lupa-username'
  ...
  const modeHint =
    mode === 'pjgt'
      ? 'Data harus sama dengan saat daftar PJGT (identitas, nama madrasah, No. WA).'
      : mode === 'toko'
        ? 'Data harus sama dengan saat daftar toko (kode toko, nama toko, No. WA).'
        : 'Data harus sama dengan yang terdaftar di Mybeddien (NIS, NIK, No. WA).'
  const modeNotice =
    mode === 'pjgt'
      ? 'Identitas & nama madrasah harus persis seperti data resmi. Nomor WhatsApp harus sama dengan nomor akun PJGT.'
      : mode === 'toko'
        ? 'Kode & nama toko harus persis seperti data resmi. Nomor WhatsApp harus sama dengan nomor akun toko.'
        : 'NIK harus persis sama dengan data santri (tanpa spasi). Nomor WhatsApp harus sama dengan yang terhubung ke akun.'
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'pjgt') {
      const identitasTrim = identitas.trim()
      const namaTrim = namaMadrasah.trim()
      if (identitasTrim.length < 2) {
      ...
      setLoading(true)
      try {
        const res = await authAPI.lupaPasswordRequestPjgt(identitasTrim, namaTrim, noWa.trim())
        if (res.success) {
          if (res.wa_me_url) {
            setWaPrepare({
              message: res.message,
    ...
    if (mode === 'toko') {
      ...
      setLoading(true)
      try {
        const res = await authAPI.lupaPasswordRequestToko(kodeTrim, namaTrim, noWa.trim())
        if (res.success) {
          if (res.wa_me_url) {
            setWaPrepare({
              message: res.message,
    ...
    setLoading(true)
    try {
      const res = await authAPI.lupaPasswordRequestSantri(nisTrim, nikTrim, noWa.trim())
      if (res.success) {
        if (res.wa_me_url) {
          setWaPrepare({
            message: res.message,
