/**
 * Preferensi login passkey per perangkat (localStorage) — portal myBeddian.
 * ID baris DB (`credential_db_id`) hanya ditambah setelah daftar/login passkey sukses di browser ini.
 */

const STORAGE_KEY_USERNAME = 'mybeddian_last_login_username'
const STORAGE_KEY_ROW_IDS = 'mybeddian_passkey_row_ids'
...
/** Hapus username tersimpan (mis. ganti akun di perangkat yang sama). */
export function clearStoredLoginUsername() {
  if (typeof window === 'undefined') return
  try {
    localStorage.removeItem(STORAGE_KEY_USERNAME)
    window.dispatchEvent(new CustomEvent('mybeddian-stored-username-cleared'))
  } catch {
    /* ignore */
  }
}
...
/** Tombol login passkey: akun punya passkey di server DAN perangkat ini punya setidaknya satu ID yang tercatat. */
