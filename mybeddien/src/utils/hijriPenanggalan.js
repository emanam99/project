...
/**
 * Konversi tanggal Masehi (Y-m-d) → Hijriyah (Y-m-d) via kalender PSA.
 * @returns {Promise<string|null>}
 */
export async function fetchKalenderConvertMasehiToHijri(tanggalMasehi, waktu = '12:00:00') {
  const ymd = String(tanggalMasehi || '').trim().slice(0, 10)
  ...
  try {
    const apiUrl = `${base}/kalender?action=convert&tanggal=${encodeURIComponent(ymd)}&waktu=${encodeURIComponent(waktu)}`
    const response = await fetch(apiUrl, { credentials: 'include' })
