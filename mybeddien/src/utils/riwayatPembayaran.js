...
/**
 * Status ringkas untuk header accordion UWABA: Lunas (hijau), Kurang Rp… (kuning), Belum (merah).
 * `kurang` boleh dari API; jika tidak ada, dihitung dari wajib − bayar.
 */
export function statusUwabaAccordionHeader(wajib, bayar, kurang) {
  ...
  if (w <= 0) {
    return { label: '—', className: 'text-gray-400 dark:text-gray-500' }
  }
  if (b === 0) {
    return { label: 'Belum', className: 'text-red-600 dark:text-red-400 font-semibold' }
  }
  if (k <= 0 || b >= w) {
    return { label: 'Lunas', className: 'text-green-600 dark:text-green-400 font-semibold' }
  }
  return {
    label: `Kurang ${formatCurrency(k)}`,
