...
import { getStoredUser, isPortalUser, isSuperAdminRole } from '../utils/auth'
...
const SIDEBAR_KEY = 'wifi_sidebar_collapsed'
const HEADER_HIDE_AFTER = 48
const HEADER_SHOW_DELTA = 4
const BOTTOM_NAV_MAX = 5

type NavIcon = 'home' | 'bill' | 'people' | 'chart' | 'users' | 'settings' | 'more'
...
function splitBottomNav(items: NavItem[]): { slots: Array<NavItem | 'more'>; overflow: NavItem[] } {
  ...
  // Slot terakhir = Menu; sisanya di overflow (bukan menyembunyikan tengah)
  const visible = items.slice(0, BOTTOM_NAV_MAX - 1)
  const overflow = items.slice(BOTTOM_NAV_MAX - 1)
  return { slots: [...visible, 'more'], overflow }
}
...
function LayoutShell() {
  ...
  const portalUser = isPortalUser(user?.role)
  ...
  useOverlayHistory(moreOpen, () => setMoreOpen(false), 'nav-more')
  ...
  useEffect(() => {
    if (!moreOpen) return
    // Tutup sheet tanpa mengganggu navigasi route yang baru
    setMoreOpen(false)
  }, [location.pathname])
  ...
  const navItems: NavItem[] = useMemo(() => {
    if (portalUser) {
      return [{ to: '/saya', label: 'Tagihan saya', shortLabel: 'Saya', icon: 'bill' }]
    }
    return [
      { to: '/dashboard', label: 'Dashboard', shortLabel: 'Home', icon: 'home' },
      { to: '/tagihan', label: 'Tagihan', shortLabel: 'Tagihan', icon: 'bill' },
      { to: '/pelanggan', label: 'Pelanggan', shortLabel: 'Pelanggan', icon: 'people' },
      { to: '/rekap', label: 'Rekap', shortLabel: 'Rekap', icon: 'chart' },
      ...(superAdmin
        ...
        >
          ...
          <div className="px-3.5 py-2.5 flex items-center justify-between gap-2.5">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="md:hidden h-8 w-8 rounded-lg overflow-hidden shrink-0 bg-surface-soft">
                <img
                  src={gambarUrl('icon/connect.png')}
                  alt=""
                  width={32}
