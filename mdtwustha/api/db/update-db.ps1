param(
    ...
    [int]$SshPort=65002,
    [string]$RemoteApiPath="domains/mdtw.my.id/public_html/staging/api"
)
...
function Invoke-LocalImport {
    ...
    $dbName = if ($envMap.ContainsKey("DB_NAME")) { $envMap["DB_NAME"] } else { "mdtw" }
...
$resolvedSqlPath = if ($SqlPath -eq "") { Join-Path $scriptDir "..\eksport\db.sql" } else { $SqlPath }
