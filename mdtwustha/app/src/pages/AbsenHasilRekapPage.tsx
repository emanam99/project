...
import {
  deleteAbsenRekapPublish,
  getAbsenRekapPublish,
  getKelas,
  listAbsenRekapPublish,
  type AbsenRekapPublishBaris,
  type AbsenRekapPublishRow,
  type KelasRow,
} from '../api/apiClient'
import OffcanvasAbsenRekapPublish from '../components/OffcanvasAbsenRekapPublish'
...
export default function AbsenHasilRekapPage() {
  ...
  const [list, setList] = useState<AbsenRekapPublishRow[]>([])
  const [detail, setDetail] = useState<AbsenRekapPublishRow | null>(null)
  ...
  const loadList = useCallback(async () => {
    ...
    setLoading(true)
    setError('')
    const res = await listAbsenRekapPublish(akses)
    if (!res.success) {
      setList([])
  ...
  const loadDetail = useCallback(async () => {
    ...
    setLoading(true)
    setError('')
    const res = await getAbsenRekapPublish(id, akses)
  ...
  // Auto-reload detail when countdown selesai
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm('Hapus publish rekap ini? Tanggal akan bisa dipakai ulang.')) return
    setDeleting(true)
    const res = await deleteAbsenRekapPublish(id, akses)
    setDeleting(false)
    if (!res.success) {
      setError(res.message || 'Gagal menghapus')
      return
    }
    navigate('/absensi/hasil-rekap', { replace: true })
  }
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <h1 className="ui-title-lg">{detail?.judul || 'Detail Hasil Rekap'}</h1>
            {detail && (
              <p className="ui-subtitle mt-1">
                {formatKelasLabel(detail.nama_kelas, detail.kel)} ·{' '}
                {formatMasehiDateDisplay(detail.tanggal_awal)} — {formatMasehiDateDisplay(detail.tanggal_akhir)}
                      ...
                      {baris.map((b, idx) => (
                        <tr key={b.santri_id} className="ui-table-row">
                          ...
                          <td className="px-3 py-2">
                            <div className="font-medium text-slate-800 dark:text-slate-100">{b.nama}</div>
                            {b.nomer_induk && (
                              <div className="text-xs ui-text-muted">{b.nomer_induk}</div>
                            )}
                          </td>
                          ...
                          <td className="px-2 py-2 text-center tabular-nums font-semibold text-red-600 dark:text-red-400">
                            {b.a}
                          </td>
                        </tr>
                      ))}
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <Link
            ...
            className="text-sm text-blue-600 dark:text-blue-400 hover:underline mb-2 inline-flex items-center gap-1"
          >
            <MaterialIcon name="arrow_back" size={14} /> Rekap Absensi
          </Link>
          <h1 className="ui-title-lg">Hasil Rekap</h1>
          <p className="ui-subtitle mt-1">
            {isAdmin
              ? 'Semua publish rekap (termasuk yang belum tayang).'
              : 'Rekap absensi yang sudah dipublish dan waktunya sudah tiba.'}
          </p>
        </div>
        <Link
          to="/absensi/rekap"
      ...
      {!loading && list.length === 0 && (
        <div className="ui-card p-8 text-center text-sm ui-text-muted">Belum ada hasil rekap.</div>
      )}
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-xs ui-text-muted mt-0.5">
                    {formatKelasLabel(row.nama_kelas, row.kel)} · {formatMasehiDateDisplay(row.tanggal_awal)} —{' '}
