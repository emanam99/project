...
export type OffcanvasEditSantriProps = {
  ...
  zIndex?: number
  /**
   * offcanvas = slide-over (HP / halaman lain)
   * panel = kotak tetap di samping list (PC Data Santri)
   */
  variant?: 'offcanvas' | 'panel'
}
...
/** Lebar kotak edit = sama dengan max offcanvas form */
export const SANTRI_EDIT_PANEL_WIDTH = 420
...
/**
 * Offcanvas / panel kanan tambah/edit santri — bisa dipanggil dari halaman mana pun.
 */
export default function OffcanvasEditSantri({
  ...
  const idPrefix = variant === 'panel' ? 'panel-santri' : 'oc-santri'
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 pb-6">
        {FORM_FIELDS.slice(0, 2).map(({ key, label, type }) => (
          <div key={key} className="mb-4">
            <label htmlFor={`${idPrefix}-${key}`} className="ui-label mb-1.5">
              {label}
            </label>
            <input
              id={`${idPrefix}-${key}`}
        ...
        <div className="mb-4 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50/80 dark:bg-white/5 p-4">
          ...
          <label htmlFor={`${idPrefix}-kelas_id`} className="ui-label mb-1.5">
            Pilih kelas
          </label>
          <select
            id={`${idPrefix}-kelas_id`}
  ...
  if (variant === 'panel') {
    return (
      <aside
        className="ui-card flex flex-col overflow-hidden shrink-0 sticky top-0 self-start"
        style={{
          width: SANTRI_EDIT_PANEL_WIDTH,
          maxWidth: SANTRI_EDIT_PANEL_WIDTH,
          height: 'calc(100dvh - 8.5rem)',
          maxHeight: 'calc(100dvh - 8.5rem)',
        }}
        aria-label={open ? (mode === 'add' ? 'Tambah santri' : 'Edit santri') : 'Form santri'}
      >
        {open ? (
          <>
            {header}
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-2 px-6 py-10 text-center ui-text-muted">
            <MaterialIcon name="person_edit" size={36} className="opacity-50" />
            <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Form santri</p>
            <p className="text-xs leading-relaxed">
              Pilih santri di daftar untuk mengedit, atau klik <span className="font-medium">+ Tambah</span>.
            </p>
          </div>
        )}
      </aside>
