...
/** GET /chats — daftar chat dari cache event Baileys (halaman list chat mirip WA Web). */
export const getChatsList = async (req, res) => {
  try {
    if (!waEngineEnabled) return respondWaEngineStopped(res);
    const safeId = resolveSlotLenient(req.query?.sessionId || req.body?.sessionId || DEFAULT_SESSION);
    if (!isBaileysConnected(safeId)) {
      return res.status(200).json({
        success: false,
        message: 'Session belum terhubung. Scan QR di tab Koneksi.',
        data: [],
      });
    }
    const result = getChatsBaileys(safeId);
    if (!result.ok) {
      return res.status(200).json({
        success: false,
        message: result.error || 'Gagal mengambil daftar chat',
        data: [],
      });
    }
    return res.json({ success: true, data: result.data || [], message: result.message || 'OK' });
  } catch (err) {
    console.error('[WA] getChatsList error:', err?.message || err);
    return res.status(500).json({ success: false, message: err?.message || 'Gagal mengambil daftar chat', data: [] });
  }
};
...
export const getChatMessages = async (req, res) => {
  try {
    ...
    const { phoneNumber, chatId, limit: limitParam } = source;
    const limit = Math.min(Math.max(parseInt(limitParam, 10) || 50, 1), 100);
    const phoneOrChatId = (typeof chatId === 'string' && chatId.trim()) ? chatId.trim() : phoneNumber;
    if (!phoneOrChatId) {
      return res.status(400).json({ success: false, message: 'phoneNumber atau chatId wajib', data: [] });
    }
    const result = await getChatMessagesBaileys(safeId, phoneOrChatId, limit);
    if (!result.ok) {
      if (result.error === 'Nomor tidak valid' || result.error === 'Nomor atau chatId tidak valid') {
        return res.status(400).json({ success: false, message: result.error, data: [] });
      }
