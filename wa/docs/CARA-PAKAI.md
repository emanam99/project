...
1. **.env** di folder `wa/`:
   ```env
   # Production (container /var/www/wa):
   UWABA_API_BASE_URL=https://api.alutsmani.id/api
   # Staging (container /var/www/wa2):
   # UWABA_API_BASE_URL=https://api2.alutsmani.id/api
   PORT=3001
   WA_API_KEY=<kunci-rahasia-buat-php>
   ```
   **Penting:** `UWABA_API_BASE_URL` harus mengarah ke **API PHP** (`api.alutsmani.id` / `api2.alutsmani.id`), **bukan** ke SPA mybeddien/ebeddien (tidak ada route `/wa/incoming` di frontend).
