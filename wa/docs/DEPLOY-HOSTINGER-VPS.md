...
PORT=3001
NODE_ENV=production
# Webhook pesan masuk → API PHP (bukan domain mybeddien/ebeddien)
UWABA_API_BASE_URL=https://api.alutsmani.id/api
WA_API_KEY=<kunci-rahasia-sama-dengan-api-php>
FRONTEND_URL=https://ebeddien.alutsmani.id
```

Untuk **wa2** (staging): `PORT=3003`, `UWABA_API_BASE_URL=https://api2.alutsmani.id/api`.
