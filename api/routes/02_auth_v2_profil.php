...
return function (\Slim\App $app): void {
    ...
    $app->post('/api/v2/auth/webauthn/reauth/options', [WebAuthnController::class, 'reauthOptions'])->add(new AuthMiddleware());
    ...
    $app->post('/api/v2/auth/send-verify-email', [AuthControllerV2::class, 'postSendVerifyEmail'])->add(new AuthMiddleware());
    $app->post('/api/v2/auth/email-reminder-snooze', [AuthControllerV2::class, 'postEmailReminderSnooze'])->add(new AuthMiddleware());
    $app->get('/api/v2/auth/verify-email-token', [AuthControllerV2::class, 'getVerifyEmailToken']);
    $app->post('/api/v2/auth/verify-email', [AuthControllerV2::class, 'postVerifyEmail']);
    ...
    // GET /api/print — wajib autentikasi (dipakai oleh halaman print di ebeddien).
    // Sebelumnya tanpa auth → IDOR (siapa pun dengan id_santri/NIS bisa ambil rincian).
    // Otorisasi tambahan (role santri hanya boleh print dirinya) ditangani di PrintController::getPrintData.
    $app->get('/api/print', [PrintController::class, 'getPrintData'])->add(new AuthMiddleware());
};
