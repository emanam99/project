...
return function (\Slim\App $app): void {
    $psbStaffPanitiaTesMw = new EbeddienFiturMiddleware(
        EbeddienFiturAccess::merge(
            EbeddienFiturAccess::psbStaffSuperSelectors(),
            EbeddienFiturAccess::psbPanitiaTesApiSelectors(),
        ),
        array_values(array_unique(array_merge(
            LegacyRouteRoles::forKey(LegacyRouteRoleKeys::PSB_STAFF_SUPER_SELECTORS),
            LegacyRouteRoles::forKey(LegacyRouteRoleKeys::PSB_PANITIA_TES_SELECTORS),
        ))),
    );
    ...
    /** PSB staff + panitia tes — satu registrasi route (Slim tidak mengizinkan duplikat path+method). */
    $app->group('/api/pendaftaran', function ($group) {
        $group->get('/get-all-pendaftar', [PendaftaranController::class, 'getAllPendaftar']);
        $group->get('/get-tes-madin', [PendaftaranController::class, 'getTesMadin']);
        $group->post('/save-tes-madin', [PendaftaranController::class, 'saveTesMadin']);
    ...
    })->add($psbStaffPanitiaTesMw)->add(new AuthMiddleware());
    ...
    $app->group('/api/pendaftaran', function ($group) {
        ...
        $group->post('/update-registrasi-tahun-ajaran', [PendaftaranController::class, 'updateRegistrasiTahunAjaran']);
        ...
        $group->get('/analisis-pendaftar', [PendaftaranController::class, 'getAnalisisPendaftar']);
        ...
        $group->get('/merge-santri-preview', [PendaftaranController::class, 'getMergeSantriPreview']);
        ...
        $group->get('/nis-pengajuan', [MybeddianNisPengajuanController::class, 'listStaff']);
        $group->get('/nis-pengajuan/{id}', [MybeddianNisPengajuanController::class, 'getStaff']);
        $group->patch('/nis-pengajuan/{id}', [MybeddianNisPengajuanController::class, 'patchStaff']);
        $group->get('/nis-pengajuan/{id}/kk', [MybeddianNisPengajuanController::class, 'serveKk']);
        $group->get('/nis-pengajuan/{id}/kk-berkas-info', [MybeddianNisPengajuanController::class, 'getKkBerkasInfo']);
        $group->post('/nis-pengajuan/{id}/sync-kk-berkas', [MybeddianNisPengajuanController::class, 'syncKkBerkas']);
        $group->post('/nis-pengajuan/{id}/kirim-nis', [MybeddianNisPengajuanController::class, 'kirimNis']);
        $group->post('/nis-pengajuan/{id}/tolak', [MybeddianNisPengajuanController::class, 'tolakPengajuan']);
    ...
    /** Riwayat registrasi PSB per santri: cukup JWT + pembatasan ID di controller (santri hanya data sendiri). Tanpa Ebeddien PSB staff. */
    $app->group('/api/pendaftaran', function ($group) {
        $group->get('/get-all-registrasi-by-santri', [PendaftaranController::class, 'getAllRegistrasiBySantri']);
    })->add(new AuthMiddleware());

    /** Opsi tingkatan LTTQ untuk form biodata (UWABA/PSB/Santri) — bukan API modul LTTQ staff. */
    $app->get('/api/lttq-tingkatan-options', [PendaftaranController::class, 'getLttqTingkatanOptions'])
        ->add(new EbeddienFiturMiddleware(
            EbeddienFiturAccess::santriBiodataLttqOptionsSelectors(),
            array_values(array_unique(array_merge(
                LegacyRouteRoles::forKey(LegacyRouteRoleKeys::UWABA_STAFF_SUPER_SELECTORS),
                LegacyRouteRoles::forKey(LegacyRouteRoleKeys::PSB_STAFF_SUPER_SELECTORS),
                LegacyRouteRoles::forKey(LegacyRouteRoleKeys::PSB_PANITIA_TES_SELECTORS),
                LegacyRouteRoles::forKey(LegacyRouteRoleKeys::PSB_TARBIYAH_SUPER_SELECTORS_SANTRI),
            )))
        ))->add(new AuthMiddleware());
};
