...
use App\Controllers\StatusSantriMasterController;
use App\Controllers\BoyongController;
use App\Controllers\PelanggaranMasterController;
...
return function (\Slim\App $app): void {
    ...
    // Master Status Santri (tabel status) — super_admin only
    $app->group('/api/status-santri-master', function ($group) {
        $group->get('', [StatusSantriMasterController::class, 'getAll']);
        $group->post('', [StatusSantriMasterController::class, 'create']);
        $group->put('/{id}', [StatusSantriMasterController::class, 'update']);
        $group->patch('/{id}/status', [StatusSantriMasterController::class, 'setStatus']);
    })->add(new EbeddienFiturMiddleware(EbeddienFiturAccess::tarbiyahSuperSelectors(), LegacyRouteRoles::forKey(LegacyRouteRoleKeys::TARBIYAH_SUPER_SELECTORS)))->add(new AuthMiddleware());

    // Master pelanggaran (CRUD admin) — granular action.domisili.pelanggaran.*
    $app->group('/api/tarbiyah/pelanggaran-admin', function ($group) {
        $group->get('', [PelanggaranMasterController::class, 'getAll']);
        $group->post('', [PelanggaranMasterController::class, 'create']);
        $group->patch('/{id}/status', [PelanggaranMasterController::class, 'setStatus']);
        $group->put('/{id}', [PelanggaranMasterController::class, 'update']);
    })->add(new EbeddienFiturMiddleware(EbeddienFiturAccess::domisiliPelanggaranAdminApiSelectors(), LegacyRouteRoles::forKey(LegacyRouteRoleKeys::TARBIYAH_SUPER_SELECTORS)))->add(new AuthMiddleware());
