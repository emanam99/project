...
// Sembunyikan X-Powered-By sedini mungkin (sebelum framework)
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'OPTIONS') {
    ...
    header('Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With, X-CSRF-Token, X-Frontend-Base-URL, X-Frontend-Env, X-App-Source, X-Public-Payment-Token, Cache-Control, Pragma, X-Client-App');
...
// Muat config + .env di awal setiap request. Database::getInstance() singleton tidak memanggil ulang
// constructor di request berikutnya (PHP-FPM / reuse proses) — tanpa ini variabel baru di .env (mis. GEMINI_API_KEY) tidak terbaca.
...
if (!function_exists('sendErrorAlertWhatsApp')) {
    function sendErrorAlertWhatsApp(string $title, string $message, array $context = []): void
    {
        try {
            ...
            $targetWa = preg_replace('/\D/', '', (string)(getenv('ERROR_ALERT_WA_NUMBER') ?: ''));
            if ($targetWa === '') {
                return;
            }

            $appEnv = (string)(getenv('APP_ENV') ?: 'local');
            $rawPath = (string)($context['path'] ?? ($_SERVER['REQUEST_URI'] ?? '-'));
            // Normalisasi path: buang query string agar webhook unik tidak bypass throttle
            $path = $rawPath;
            if ($path !== '-' && $path !== '') {
                $qPos = strpos($path, '?');
                if ($qPos !== false) {
            ...
            $isDbFailure = (
                strpos($detailLower, 'database connection failed') !== false
                || strpos($detailLower, 'sqlstate[hy000] [2002]') !== false
                || strpos($detailLower, 'sqlstate[hy000] [1040]') !== false
                || strpos($detailLower, 'too many connections') !== false
                || strpos($detailLower, 'operation not permitted') !== false
            );

            // DB/host down: satu signature kasar + cooldown panjang (default 15 menit)
            $defaultCooldown = $isDbFailure ? 900 : 300;
            $cooldownSec = (int) (getenv('ERROR_ALERT_COOLDOWN_SEC') ?: 0);
            if ($cooldownSec <= 0) {
                $cooldownSec = $isDbFailure
                    ? (int) (getenv('ERROR_ALERT_DB_COOLDOWN_SEC') ?: $defaultCooldown)
                    : $defaultCooldown;
            }
            if ($cooldownSec < 60) {
                $cooldownSec = 60;
            ...
            if ($isDbFailure) {
                $signature = 'class:db_connection_failed';
            } else {
                // Tanpa detail penuh di signature — pesan stack sering sedikit berbeda per request
                $signature = sha1($title . '|' . $method . '|' . $path . '|' . $status);
            }
            ...
            // Batas global: max N alert / jam (default 8) — cegah banjir meski banyak signature
            $maxPerHour = (int) (getenv('ERROR_ALERT_MAX_PER_HOUR') ?: 8);
            if ($maxPerHour < 1) {
                $maxPerHour = 1;
            }
            $hourBucket = 'global_hour:' . date('YmdH', $now);
            $hourCount = (int) ($last[$hourBucket] ?? 0);
            if ($hourCount >= $maxPerHour) {
                return;
            }

            $last[$signature] = $now;
            $last[$hourBucket] = $hourCount + 1;
            // Prune entri lama (> 2 jam) agar file tidak membengkak
            foreach ($last as $k => $v) {
                if (is_int($v) && $v > 1000000000 && ($now - (int) $v) > 7200 && strpos((string) $k, 'global_hour:') !== 0) {
                    unset($last[$k]);
                }
                if (strpos((string) $k, 'global_hour:') === 0 && $k !== $hourBucket) {
                    $bucketHour = substr((string) $k, strlen('global_hour:'));
                    if ($bucketHour !== '' && $bucketHour < date('YmdH', $now - 3600)) {
                        unset($last[$k]);
                    }
                }
            }
            ...
            $text = "*ALERT API ERROR*\n"
                ...
                . "Detail: {$detail}";
            if ($isDbFailure) {
                $mins = (int) ceil($cooldownSec / 60);
                $text .= "\n_(DB/host: alert berikutnya ditahan ~{$mins} menit)_";
            }
...
require_once __DIR__ . '/../src/Helpers/ErrorLogRotator.php';
\App\Helpers\ErrorLogRotator::maybeRotate(__DIR__ . '/../error.log');
    ...
    if ($error !== NULL && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        ...
        sendErrorAlertWhatsApp(
            ...
            [
                ...
                // Path saja (tanpa query) — selaras throttle di sendErrorAlertWhatsApp
...
// Log akses API (setelah handler) — untuk Super Admin Aktivitas User
...
(require __DIR__ . '/../routes/24_mahrom.php')($app);
...
(require __DIR__ . '/../routes/38_ujian.php')($app);
...
(require __DIR__ . '/../routes/34_app_install_activity.php')($app);
(require __DIR__ . '/../routes/35_bisyaroh.php')($app);
(require __DIR__ . '/../routes/36_ugt_guru_tugas_tugasan.php')($app);
(require __DIR__ . '/../routes/37_website_admin.php')($app);
(require __DIR__ . '/../routes/39_lttq.php')($app);
(require __DIR__ . '/../routes/40_buku_tamu.php')($app);
(require __DIR__ . '/../routes/41_ugt_kompas.php')($app);
(require __DIR__ . '/../routes/42_alumni.php')($app);
(require __DIR__ . '/../routes/43_kurikulum_jadwal.php')($app);
