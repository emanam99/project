...
class RoleHelper
{
    ...
    /**
     * Ruang lingkup lembaga khusus tab Rekap Bisyaroh: hanya baris pengurus___role yang rolenya punya
     * `action.bisyaroh.tab.rekap`. Dengan begitu multi-role (mis. admin ULA + guru Wustha) tidak membuka
     * rekap Wustha kecuali role guru itu juga diberi tab Rekap.
     *
     * @return array{lembaga_scope_all: bool, lembaga_ids: string[]}
     */
    public static function computeBisyarohRekapLembagaScopeForPengurus(\PDO $db, int $pengurusId): array
    {
        if ($pengurusId <= 0) {
            return ['lembaga_scope_all' => false, 'lembaga_ids' => []];
        }
        try {
            $stmt = $db->prepare(
                'SELECT DISTINCT r.`key` AS role_key, pr.`lembaga_id`
                 ...
                 INNER JOIN `app` ap ON ap.`id` = f.`id_app` AND ap.`key` = \'ebeddien\'
                 WHERE pr.`pengurus_id` = ?
                   AND f.`code` = \'action.bisyaroh.tab.rekap\''
            );
            $stmt->execute([$pengurusId]);
            $rows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
        } catch (\Throwable $e) {
            error_log('RoleHelper::computeBisyarohRekapLembagaScopeForPengurus: ' . $e->getMessage());
    ...
    /**
     * ID lembaga dari jabatan aktif pengurus (override baris jabatan atau lembaga pada master jabatan).
     * Dipakai agar absen mandiri selaras "tempat kerja" di pengurus___jabatan, bukan hanya lembaga pada role token.
     *
     * @return list<string>
     */
    public static function getActiveJabatanLembagaIds(int $pengurusId): array
    {
        if ($pengurusId <= 0) {
            return [];
        }
        try {
            $db = self::getDb();
            $stmt = $db->prepare(
                'SELECT DISTINCT TRIM(COALESCE(NULLIF(TRIM(CAST(pj.lembaga_id AS CHAR)), \'\'), CAST(j.lembaga_id AS CHAR), \'\')) AS lid
                 ...
                 WHERE pj.pengurus_id = ?
                   ...
                   AND TRIM(COALESCE(NULLIF(TRIM(CAST(pj.lembaga_id AS CHAR)), \'\'), CAST(j.lembaga_id AS CHAR), \'\')) <> \'\''
            );
            $stmt->execute([$pengurusId]);
            $out = [];
            ...
            error_log('RoleHelper::getActiveJabatanLembagaIds: ' . $e->getMessage());
    ...
    /**
     * Ruang lingkup lembaga untuk absen mandiri GPS: irisan jabatan aktif dengan lembaga_ids pada token.
     * null = tidak dibatasi (super admin / lembaga_scope_all). [] = tidak boleh absen mandiri di titik manapun.
     *
     * @return null|list<string>
     */
    public static function resolveAbsenMandiriLembagaScope(?array $user, int $pengurusId): ?array
    {
        ...
        if (!\is_array($ids)) {
            ...
            $jwtScope = array_values(array_filter(array_map('strval', $ids)));
        }
        $jabatanIds = self::getActiveJabatanLembagaIds($pengurusId);
        if ($jabatanIds !== []) {
            return array_values(array_intersect($jabatanIds, $jwtScope));
        }
    ...
    /**
     * True = GET list laporan UGT dibatasi ke madrasah id_koordinator = pengurus login.
     * admin_ugt / super_admin / scope_all madrasah: tidak dibatasi.
     * Koordinator dengan aksi filter_koordinator_semua: lihat semua (dropdown filter aktif).
     */
    public static function tokenUgtLaporanApplyKoordinatorListScope(\PDO $db, array $user): bool
    {
    ...
    /**
     * Laporan UGT: boleh POST laporan baru per jenis tab (koordinator | gt | pjgt).
     * Tanpa penugasan fitur eBeddien di token → legacy izinkan (middleware sudah loloskan menu).
     */
    public static function tokenUgtLaporanCanTambah(\PDO $db, array $user, string $jenis): bool
    {
        if (!empty($user['is_real_super_admin'])) {
            return true;
        }
        $map = [
            'koordinator' => 'action.ugt.laporan.tambah.koordinator',
            'gt' => 'action.ugt.laporan.tambah.gt',
            'pjgt' => 'action.ugt.laporan.tambah.pjgt',
        ];
        $code = $map[$jenis] ?? '';
        if ($code === '') {
            return false;
        }
        if (!self::tokenUnionHasAnyEbeddienFiturAssignment($db, $user)) {
            return true;
        }
    ...
    public static function tokenMayToggleDraftNotifOnSave(\PDO $db, array $user): bool
    {
        ...
        foreach ([
            'action.pengeluaran.draft.notif.lembaga_sesuai_role',
            'action.pengeluaran.notif.semua_lembaga',
            'action.pengeluaran.notif.lembaga_sesuai_role',
        ] as $code) {
            if (self::tokenHasEbeddienFiturCode($db, $user, $code)) {
                return true;
            }
        }
