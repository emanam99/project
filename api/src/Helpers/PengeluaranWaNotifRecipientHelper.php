...
final class PengeluaranWaNotifRecipientHelper
{
    ...
    public static function filterRecipients(\PDO $db, ?string $lembagaId, array $recipients, bool $draftWaOnly = false): array
    {
        ...
        foreach (self::fetchEligiblePengurusWithWa($db, $lembagaId, $draftWaOnly) as $row) {
            if ($row['id'] > 0) {
                $allowed[$row['id']] = true;
                $waDb = trim((string) ($row['whatsapp'] ?? ''));
                if ($waDb !== '') {
                    $allowedWaById[$row['id']] = $waDb;
                }
            }
        }
        ...
        foreach ($recipients as $r) {
            ...
            // Nomor WA hanya dari DB pengurus (abaikan whatsapp di body klien).
            $whatsapp = isset($allowedWaById[$id]) ? trim((string) $allowedWaById[$id]) : '';
            if ($whatsapp === '') {
                continue;
            }
