...
final class FileUploadValidator
{
    ...
    private const SIGNATURES = [
        ...
        'pdf' => [
            // libmagic sering mengembalikan octet-stream walau header %PDF valid
    ...
    public static function validateMovedFile(string $absolutePath, string $expectedExtension): array
    {
        ...
        // Tolak polyglot: skrip PHP tersembunyi di file gambar/dokumen.
        if (self::containsEmbeddedPhp($absolutePath)) {
            return [
                'success' => false,
                'message' => 'Konten file tidak aman (terdeteksi skrip)',
            ];
        }

        // Untuk gambar: wajib lolos getimagesize (bukan sekadar ekstensi/MIME klien).
        if (in_array($expectedExtension, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true)) {
            $img = @getimagesize($absolutePath);
            if ($img === false || empty($img[0]) || empty($img[1])) {
                return [
                    'success' => false,
            ...
            $type = (int) ($img[2] ?? 0);
            $okType = match ($expectedExtension) {
                'jpg', 'jpeg' => $type === IMAGETYPE_JPEG,
                'png' => $type === IMAGETYPE_PNG,
                'gif' => $type === IMAGETYPE_GIF,
                'webp' => defined('IMAGETYPE_WEBP') ? $type === IMAGETYPE_WEBP : true,
                default => false,
            };
            if (!$okType) {
                return [
                    'success' => false,
                    'message' => 'Tipe gambar tidak sesuai dengan ekstensi',
                ];
            }
        }
    ...
    private static function containsEmbeddedPhp(string $absolutePath): bool
    {
        ...
        // Polyglot umum: JPEG/PNG + payload PHP
        return (bool) preg_match('/<\?(php|=)|<\s*script\b/i', $chunk);
    }
