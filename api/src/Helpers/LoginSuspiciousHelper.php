...
class LoginSuspiciousHelper
{
    ...
    public static function notifyIfThirdFailure(\PDO $db, string $ip, string $endpoint, string $identifier): void
    {
        ...
        try {
            ...
            $alertWaCopy = $alertWa;
            $messageCopy = $message;
            $logCtxCopy = $logContext;
            DeferredHttpTask::runAfterResponse(static function () use ($alertWaCopy, $messageCopy, $logCtxCopy): void {
                try {
                    WhatsAppService::sendMessage($alertWaCopy, $messageCopy, null, $logCtxCopy);
                } catch (\Throwable $e) {
                    error_log('LoginSuspiciousHelper::notifyIfThirdFailure deferred WA: ' . $e->getMessage());
                }
            });
        } catch (\Throwable $e) {
