...
class RoleMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if (!$hasAllowedRole && in_array('pjgt', $allowedRoles, true) && isset($user['madrasah_id']) && (int) $user['madrasah_id'] > 0) {
            $hasAllowedRole = true;
        }
        ...
        if ($union === []
            ...
            && (!isset($user['madrasah_id']) || (int) $user['madrasah_id'] <= 0)) {
        ...
        if (!empty($this->allowedRoles) && !$hasAllowedRole) {
            ...
            // Log ringkas (tanpa dump array JSON penuh) — sampling 1/10 di production untuk
            // mengurangi log bloat saat ada probing massal. Detail penuh hanya saat APP_ENV != production.
            $env = (string) getenv('APP_ENV');
            $shouldLog = $env !== 'production' || mt_rand(1, 10) === 1;
            if ($shouldLog) {
                $userId = $user['user_id'] ?? $user['id'] ?? '';
                $requiredCount = is_array($allowedRoles) ? count($allowedRoles) : 0;
                $dbRolesCount = is_array($dbRoleKeys) ? count($dbRoleKeys) : 0;
                error_log("RoleMiddleware: deny user_id=$userId pengurus_id=" . ($pengurusIdForLog ?? '') . " role=$userRole required_n=$requiredCount db_roles_n=$dbRolesCount db_checked=" . ($dbRolesChecked ? '1' : '0'));
            }
            $response = new Response();
            $response->getBody()->write(json_encode([
                'success' => false,
