...
class RateLimitMiddleware implements MiddlewareInterface
{
    ...
    public function __construct()
    {
        ...
        // Default fail-closed: rate limit AKTIF kalau key tidak diset (audit Mei 2026).
        // Sebelumnya `?? true` artinya rate limit di-bypass di production kalau
        // ENV/config-nya kosong — itu kebalik dari niat "longgarkan saat dev lokal".
        $this->disableLocalhost = $security['disable_rate_limit_localhost'] ?? false;
        $this->trustedProxies = $security['trusted_proxies'] ?? [];
        // Jangan connect DB di constructor — gagal koneksi Hostinger (2002) sebelumnya
        // melempar fatal di bootstrap dan membuat SEMUA request eBeddien 500.
    }
    ...
    private function ensureDb(): bool
    {
        ...
        try {
            ...
            error_log('RateLimitMiddleware: DB unavailable, skip rate limit: ' . $e->getMessage());
            $this->db = null;
            return false;
        }
    }
    ...
    {
        if ($this->tableEnsured || !($this->db instanceof \PDO)) {
            return;
        }
        try {
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if (!$this->ensureDb()) {
            // Auth / enumerasi sensitif: fail-closed saat DB rate-limit down
            if ($this->isAuthSensitiveWhenDbDown($path)) {
                $response = new Response();
                $response->getBody()->write(json_encode([
                    'success' => false,
                    'message' => 'Layanan sementara tidak tersedia. Coba lagi sebentar.',
                ], JSON_UNESCAPED_UNICODE));
                return $this->withCorsHeaders($request, $response->withStatus(503)->withHeader('Content-Type', 'application/json; charset=utf-8'));
            }
            // Endpoint lain: fail-open agar transient MySQL tidak mematikan seluruh API
        ...
        /** Rate limit per akun (JWT) untuk Deepseek — selaras IP limit */
        $deepseekAccountKey = null;
        if (
            $request->getMethod() === 'POST'
            && strpos($path, '/api/deepseek/') !== false
            && (
                strpos($path, '/api/deepseek/agent/') !== false
                || strpos($path, '/api/deepseek/api-chat') !== false
            )
        ) {
            $deepseekAccountKey = $this->deepseekJwtAccountKey($request);
        }
        ...
        // Rate limit lupa-password-request (public, cegah spam WA) — UWABA & Mybeddian
        if (strpos($path, '/api/v2/auth/lupa-password-request') !== false
            || strpos($path, '/api/mybeddian/v2/auth/lupa-password-request') !== false) {
            if (!$skipRateLimit) {
                $lupaEndpoint = strpos($path, '/api/mybeddian/') !== false
                    ? '/api/mybeddian/v2/auth/lupa-password-request'
                    : '/api/v2/auth/lupa-password-request';
                $lockout = $this->checkLockoutWithLimit($ip, $lupaEndpoint, 3, 900);
                if ($lockout !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
        ...
        // Rate limit lupa-username-request (public, cegah spam WA)
        if (strpos($path, '/api/mybeddian/v2/auth/lupa-username-request') !== false) {
            if (!$skipRateLimit) {
                $lockout = $this->checkLockoutWithLimit($ip, '/api/mybeddian/v2/auth/lupa-username-request', 3, 900);
                if ($lockout !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
        ...
        // Rate limit verify-password (cegah brute force oracle)
        if (strpos($path, '/api/user/verify-password') !== false) {
            if (!$skipRateLimit) {
                $lockout = $this->checkLockoutWithLimit($ip, '/api/user/verify-password', 10, 900);
                if ($lockout !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
                        'message' => 'Terlalu banyak percobaan verifikasi password. Coba lagi dalam ' . ceil($lockout / 60) . ' menit.'
                    ], JSON_UNESCAPED_UNICODE));
                    return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
                }
            }
        ...
        // Rate limit OTP ganti nomor WA (OTP bombing): 3x / 10 menit per IP
        if ($request->getMethod() === 'POST' && strpos($path, '/api/v2/auth/send-otp-ganti-wa') !== false) {
            if (!$skipRateLimit) {
                $lockout = $this->checkLockoutWithLimit($ip, '/api/v2/auth/send-otp-ganti-wa', 3, 600);
                if ($lockout !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
                        'message' => 'Terlalu banyak permintaan OTP. Coba lagi dalam ' . ceil($lockout / 60) . ' menit.'
                    ], JSON_UNESCAPED_UNICODE));
                    return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
                }
            }
        ...
        if ($this->isPublicSantriRateLimitPath($path)) {
            if (!$skipRateLimit) {
                $lockout = $this->checkLockoutWithLimit($ip, '/api/public/santri-data', 40, 300);
                if ($lockout !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
        ...
        // Rate limit login (termasuk portal myBeddian)
        if ($this->isLoginRateLimitPath($path)) {
            if (!$skipRateLimit) {
                $endpoint = $this->loginRateLimitEndpointKey($path);
        ...
        // Rate limit agen Chat AI eBeddien (per IP + per akun JWT bila ada)
        $isDeepseekAgentTurn = ($request->getMethod() === 'POST' && strpos($path, '/api/deepseek/agent/turn') !== false);
        $isDeepseekAgentConfirm = ($request->getMethod() === 'POST' && strpos($path, '/api/deepseek/agent/confirm') !== false);
        $isDeepseekAgentRollback = ($request->getMethod() === 'POST' && strpos($path, '/api/deepseek/agent/rollback') !== false);
        $isDeepseekAgentDiscard = ($request->getMethod() === 'POST' && strpos($path, '/api/deepseek/agent/discard') !== false);
        $isDeepseekApiChat = ($request->getMethod() === 'POST' && strpos($path, '/api/deepseek/api-chat') !== false);
        if ($isDeepseekAgentTurn && !$skipRateLimit) {
            $lockout = $this->checkLockoutWithLimit($ip, '/api/deepseek/agent/turn', 45, 300);
            if ($lockout !== null) {
                $response = new Response();
                $response->getBody()->write(json_encode([
                    'success' => false,
                    'message' => 'Terlalu banyak permintaan agen AI. Coba lagi dalam ' . ceil($lockout / 60) . ' menit.'
                ], JSON_UNESCAPED_UNICODE));
                return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
            }
            if ($deepseekAccountKey !== null) {
                $lockoutAcct = $this->checkLockoutWithLimit($deepseekAccountKey, '/api/deepseek/agent/turn#acct', 45, 300);
                if ($lockoutAcct !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
                        'message' => 'Terlalu banyak permintaan agen AI untuk akun ini. Coba lagi dalam ' . ceil($lockoutAcct / 60) . ' menit.'
                    ], JSON_UNESCAPED_UNICODE));
                    return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
                }
            }
        ...
        foreach ([
            ['/api/deepseek/agent/confirm', $isDeepseekAgentConfirm],
            ['/api/deepseek/agent/rollback', $isDeepseekAgentRollback],
            ['/api/deepseek/agent/discard', $isDeepseekAgentDiscard],
        ] as [$ep, $flag]) {
            if ($flag && !$skipRateLimit) {
                $lockout = $this->checkLockoutWithLimit($ip, $ep, 40, 300);
                if ($lockout !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
                        'message' => 'Terlalu banyak permintaan konfirmasi agen. Coba lagi dalam ' . ceil($lockout / 60) . ' menit.'
                    ], JSON_UNESCAPED_UNICODE));
                    return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
                }
                if ($deepseekAccountKey !== null) {
                    $lockoutAcct = $this->checkLockoutWithLimit($deepseekAccountKey, $ep . '#acct', 40, 300);
                    if ($lockoutAcct !== null) {
                        $response = new Response();
                        $response->getBody()->write(json_encode([
                            'success' => false,
                            'message' => 'Terlalu banyak permintaan konfirmasi agen untuk akun ini. Coba lagi dalam ' . ceil($lockoutAcct / 60) . ' menit.'
                        ], JSON_UNESCAPED_UNICODE));
                        return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
                    }
                }
        ...
        if ($isDeepseekApiChat && !$skipRateLimit) {
            $lockout = $this->checkLockoutWithLimit($ip, '/api/deepseek/api-chat', 90, 300);
            if ($lockout !== null) {
                $response = new Response();
                $response->getBody()->write(json_encode([
                    'success' => false,
                    'message' => 'Terlalu banyak permintaan chat AI. Coba lagi dalam ' . ceil($lockout / 60) . ' menit.'
                ], JSON_UNESCAPED_UNICODE));
                return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
            }
            if ($deepseekAccountKey !== null) {
                $lockoutAcct = $this->checkLockoutWithLimit($deepseekAccountKey, '/api/deepseek/api-chat#acct', 90, 300);
                if ($lockoutAcct !== null) {
                    $response = new Response();
                    $response->getBody()->write(json_encode([
                        'success' => false,
                        'message' => 'Terlalu banyak permintaan chat AI untuk akun ini. Coba lagi dalam ' . ceil($lockoutAcct / 60) . ' menit.'
                    ], JSON_UNESCAPED_UNICODE));
                    return $this->withCorsHeaders($request, $response->withStatus(429)->withHeader('Content-Type', 'application/json; charset=utf-8'));
                }
            }
        ...
        if (strpos($path, '/api/mybeddian/v2/auth/lupa-password-request') !== false && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/mybeddian/v2/auth/lupa-password-request', 3, 900);
        }
        if (strpos($path, '/api/mybeddian/v2/auth/lupa-username-request') !== false && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/mybeddian/v2/auth/lupa-username-request', 3, 900);
        }
        ...
        if ($this->isLoginRateLimitPath($path) && !$skipRateLimit) {
            $endpoint = $this->loginRateLimitEndpointKey($path);
        ...
        if (($isDeepseekAgentTurn ?? false) && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/deepseek/agent/turn', 45, 300);
            if ($deepseekAccountKey !== null) {
                $this->recordAttemptWithLimit($deepseekAccountKey, '/api/deepseek/agent/turn#acct', 45, 300);
            }
        }
        if (($isDeepseekAgentConfirm ?? false) && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/deepseek/agent/confirm', 40, 300);
            if ($deepseekAccountKey !== null) {
                $this->recordAttemptWithLimit($deepseekAccountKey, '/api/deepseek/agent/confirm#acct', 40, 300);
            }
        }
        if (($isDeepseekAgentRollback ?? false) && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/deepseek/agent/rollback', 40, 300);
            if ($deepseekAccountKey !== null) {
                $this->recordAttemptWithLimit($deepseekAccountKey, '/api/deepseek/agent/rollback#acct', 40, 300);
            }
        }
        if (($isDeepseekAgentDiscard ?? false) && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/deepseek/agent/discard', 40, 300);
            if ($deepseekAccountKey !== null) {
                $this->recordAttemptWithLimit($deepseekAccountKey, '/api/deepseek/agent/discard#acct', 40, 300);
            }
        }

        if (($isDeepseekApiChat ?? false) && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/deepseek/api-chat', 90, 300);
            if ($deepseekAccountKey !== null) {
                $this->recordAttemptWithLimit($deepseekAccountKey, '/api/deepseek/api-chat#acct', 90, 300);
            }
        }

        if (strpos($path, '/api/user/verify-password') !== false && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/user/verify-password', 10, 900);
        }
        ...
        if ($this->isPublicSantriRateLimitPath($path) && !$skipRateLimit) {
            $this->recordAttemptWithLimit($ip, '/api/public/santri-data', 40, 300);
        }
    ...
    private function withCorsHeaders(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        $allowOrigin = '*';
        if ($origin !== '' && $this->isTrustedCorsOrigin($origin)) {
            $allowOrigin = $origin;
        }
        $headers = [
            'Access-Control-Allow-Origin' => $allowOrigin,
            'Access-Control-Allow-Headers' => 'Content-Type, Authorization, X-Requested-With, X-CSRF-Token, X-Frontend-Base-URL, X-App-Source, X-Public-Payment-Token, Cache-Control, Pragma, X-Client-App',
    ...
    /**
     * Kunci rate limit per akun untuk Deepseek (kolom ip_address pada rate_limits dipakai sebagai identifier).
     */
    private function deepseekJwtAccountKey(ServerRequestInterface $request): ?string
    {
        ...
        try {
            $jwt = new JwtAuth();
            $payload = $jwt->validateToken(trim($m[1]));
            if (!is_array($payload)) {
                return null;
            }
            $usersId = isset($payload['users_id']) ? (int) $payload['users_id'] : 0;
            if ($usersId > 0) {
                return 'jwt_u:' . $usersId;
            }
            $pid = isset($payload['user_id']) ? (int) $payload['user_id'] : 0;
            if ($pid > 0) {
                return 'jwt_p:' . $pid;
            }
        } catch (\Throwable $e) {
    ...
    private function getDaftarRateLimitKey(ServerRequestInterface $request, string $ip, string $path): array
    {
        ...
        if ($isMybeddian) {
            $isPjgtDaftar = (strpos($path, 'daftar-check-pjgt') !== false
                || strpos($path, 'daftar-konfirmasi-pjgt') !== false
                || strpos($path, 'daftar-pjgt-lookup-madrasah') !== false);
            if ($isPjgtDaftar) {
                $ident = isset($data['identitas']) ? trim((string) $data['identitas']) : '';
                if ($ident === '' && strpos($path, 'daftar-pjgt-lookup-madrasah') !== false) {
                    $params = $request->getQueryParams();
                    $ident = isset($params['identitas']) ? trim((string) $params['identitas']) : '';
                }
                $ident = preg_replace('/\s+/u', ' ', $ident);
                $ident = substr($ident, 0, 120);
                if ($ident === '') {
                    return ['key' => $ip, 'per_identifier' => false, 'endpoint' => $endpoint];
                }
                return ['key' => 'madrasah_identitas:' . mb_strtolower($ident, 'UTF-8'), 'per_identifier' => true, 'endpoint' => $endpoint];
            }
            $isTokoDaftar = (strpos($path, 'daftar-check-toko') !== false
                || strpos($path, 'daftar-konfirmasi-toko') !== false
                || strpos($path, 'daftar-toko-hubung-akun') !== false);
            if ($isTokoDaftar) {
                $kode = isset($data['kode_toko']) ? trim((string) $data['kode_toko']) : '';
                $kode = preg_replace('/\s+/u', '', $kode);
                $kode = substr($kode, 0, 40);
                if ($kode === '') {
                    return ['key' => $ip, 'per_identifier' => false, 'endpoint' => $endpoint];
                }
                return ['key' => 'kode_toko:' . mb_strtolower($kode, 'UTF-8'), 'per_identifier' => true, 'endpoint' => $endpoint];
            }
    ...
    private function isLoginRateLimitPath(string $path): bool
    {
        return strpos($path, '/api/auth/login') !== false
            || strpos($path, '/api/auth/daftar-wa-') !== false
            || strpos($path, '/api/v2/auth/login') !== false
            || strpos($path, '/api/mybeddian/v2/auth/login') !== false;
    }

    private function loginRateLimitEndpointKey(string $path): string
    {
        if (strpos($path, '/api/mybeddian/v2/auth/login') !== false) {
            return '/api/mybeddian/v2/auth/login';
        }
        if (strpos($path, '/api/v2/auth/login') !== false) {
            return '/api/v2/auth/login';
        }
        if (strpos($path, '/api/auth/daftar-wa-') !== false) {
            return '/api/auth/daftar-wa';
        }

        return '/api/auth/login';
    }

    private function isPublicSantriRateLimitPath(string $path): bool
    {
        return strpos($path, '/api/public/santri') !== false
            || strpos($path, '/api/public/ijin') !== false
            || strpos($path, '/api/public/shohifah') !== false
            || strpos($path, '/api/public/juara') !== false
            || strpos($path, '/api/pendaftaran/check-nik') !== false
            || strpos($path, '/api/alumni/check-nik') !== false
            || strpos($path, '/api/alumni/login-nik') !== false;
    }

    /** Saat DB rate-limit gagal: jangan biarkan brute-force / enumerasi lewat. */
    private function isAuthSensitiveWhenDbDown(string $path): bool
    {
