...
class AuthMiddleware implements MiddlewareInterface
{
    ...
    private function ensureDb(): ?\PDO
    {
        ...
        try {
            ...
            error_log('AuthMiddleware: DB unavailable: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if (empty($authHeader) || !preg_match('/Bearer\s+(.*)$/i', $authHeader, $matches)) {
            return $this->jsonError(401, 'Token tidak ditemukan. Silakan login terlebih dahulu.');
        }
        ...
        if (!$payload) {
            ...
            return $this->jsonError(401, 'Token tidak valid atau sudah kadaluarsa. Silakan login kembali.');
        }

        if ($this->ensureDb() === null) {
            return $this->jsonError(503, 'Layanan sementara tidak tersedia. Coba lagi sebentar.');
        }
