...
class WhatsAppService
{
    ...
    private const KATEGORI_KIRIM_KE_NOMOR_PENGURUS_TANPA_ALIAS = [
        ...
        'pengeluaran_notif',
        'nis_pengajuan_admin_notif',
    ];

    /**
     * Pengajuan NIS myBeddien: kirim ke nomor WA yang diisi pemohon (no_wa di pengajuan),
     * tanpa resolveDeliveryTarget — hindari pesan ke nomor_kanonik lain (riwayat chat vs penerima beda).
     */
    private const KATEGORI_KIRIM_KE_NOMOR_PEMOHON_NIS = [
        'nis_pengajuan_kirim',
    ...
    /**
     * Kirim manual staff lewat POST /api/wa/send (riwayat chat, kwitansi, dll.):
     * ke nomor yang dipilih di UI, tanpa alias nomor_kanonik & tanpa blok siap_terima_notif.
     */
    private const KATEGORI_STAFF_MANUAL_WA = [
        'staff_manual_wa',
    ];
    ...
    /**
     * Opsional: paksa port 3002 → 3001 (template lama). Default OFF — hormati WA_API_URL apa adanya
     * (mis. PORT=3002 di wa/.env). Aktifkan hanya bila perlu: WA_LEGACY_PORT_FIX=true di api/.env.
     */
    private static function normalizeWaApiUrlForLegacyPort(string $url): string
    {
        $url = trim($url);
        if ($url === '') {
            return $url;
        }
        $raw = getenv('WA_LEGACY_PORT_FIX');
        if ($raw === false || $raw === '' || !filter_var($raw, FILTER_VALIDATE_BOOLEAN)) {
            return $url;
        }
    ...
    /** Pesan API untuk gagal koneksi ke Node WA (bukan teks mentah Guzzle/cURL). */
    private static function humanizeWaNodeConnectError(\Throwable $e): string
    {
        $m = $e->getMessage();
        if (preg_match('/Failed to connect|Could not connect|Connection refused|cURL error 7/i', $m)) {
            return 'Server WhatsApp Node tidak terjangkau. Jalankan layanan di folder wa/ (npm run dev), samakan port dengan WA_API_URL di api/.env (lokal biasanya http://127.0.0.1:3001/api/whatsapp/send).';
        }
    ...
    private static function getConfig(): array
    {
        ...
        $wa = $config['whatsapp'] ?? [];
        $apiUrl = getenv('WA_API_URL') ?: ($wa['api_url'] ?? 'https://wa.alutsmani.id/api/whatsapp/send');

        return [
            'api_url' => self::normalizeWaApiUrlForLegacyPort((string) $apiUrl),
    ...
    /** Varian URL cek Node (127.0.0.1 ↔ localhost) bila koneksi gagal di Windows/XAMPP. */
    private static function waNodeCheckUrlVariants(string $checkUrl): array
    {
        $urls = [$checkUrl];
        foreach (['127.0.0.1' => 'localhost', 'localhost' => '127.0.0.1'] as $from => $to) {
            $alt = str_replace($from, $to, $checkUrl);
            if ($alt !== $checkUrl && !in_array($alt, $urls, true)) {
                $urls[] = $alt;
            }
        }
    ...
    {
        $config = require __DIR__ . '/../../config.php';
        $url = $config['mybeddian_app_url'] ?? $config['app']['mybeddian_url'] ?? getenv('MYBEDDIAN_APP_URL') ?: 'https://mybeddien.alutsmani.id';
        return rtrim((string) $url, '/');
    }

    /**
     * Samakan identitas pengirim webhook (MSISDN atau LID) dengan nomor yang diharapkan (62…).
     * Berguna saat chat masuk lewat @lid tetapi nomor form/biodata adalah MSISDN.
     *
     * @return string|null MSISDN 62… jika cocok, else null
     */
    public static function resolveInboundSenderToExpectedMsisdn(
        string $senderRaw,
        ?string $fromJid,
        string $expectedWa62
    ): ?string {
        $expectedWa62 = self::formatPhoneNumber($expectedWa62);
        if ($expectedWa62 === '' || strpos($expectedWa62, '62') !== 0) {
            return null;
        }

        $digits = preg_replace('/\D/', '', $senderRaw) ?? '';
        $isLidJid = is_string($fromJid) && preg_match('/@lid$/i', trim((string) $fromJid)) === 1;

        if ($digits !== '' && strpos($digits, '62') === 0 && strlen($digits) >= 11) {
            return $digits === $expectedWa62 ? $digits : null;
        }
        $asPhone = self::formatPhoneNumber($senderRaw);
        if (!$isLidJid && strpos($asPhone, '62') === 0 && strlen($asPhone) >= 11) {
            return $asPhone === $expectedWa62 ? $asPhone : null;
        }

        $lid = $digits;
        if ($lid === '' || strlen($lid) < 10) {
            return null;
        }

        try {
            $db = Database::getInstance()->getConnection();
            if ($db->query("SHOW TABLES LIKE 'whatsapp___kontak'")->rowCount() === 0) {
                return null;
            }
            ...
            $stmt = $db->prepare(
                'SELECT nomor, nomor_kanonik FROM whatsapp___kontak
                 WHERE nomor = ?
                    OR nomor_kanonik = ?
                    OR nomor_kanonik = ?
                 LIMIT 8'
            );
            $stmt->execute([$expectedWa62, $lid, $lid . '@lid']);
            while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $nomor = self::formatPhoneNumber((string) ($row['nomor'] ?? ''));
                $kanonikRaw = trim((string) ($row['nomor_kanonik'] ?? ''));
                $kanonikDigits = preg_replace('/\D/', '', $kanonikRaw) ?? '';
                if ($nomor === $expectedWa62 && ($kanonikDigits === $lid || strcasecmp($kanonikRaw, $lid . '@lid') === 0)) {
                    return $expectedWa62;
                }
            }
        } catch (\Throwable $e) {
            error_log('WhatsAppService::resolveInboundSenderToExpectedMsisdn ' . $e->getMessage());
        }
    ...
    /**
     * Ambil nomor nyata dari tabel santri (untuk cek/kirim WA saat FE hanya punya nomor ter-mask).
     * field: no_telpon | no_wa_santri
     */
    public static function resolveSantriPhoneField(\PDO $db, int $idSantri, string $field): ?string
    {
        if ($idSantri <= 0) {
            return null;
        }
        $map = [
            'no_telpon' => 'no_telpon',
            'no_wa_santri' => 'no_wa_santri',
            'telpon' => 'no_telpon',
            'wa_santri' => 'no_wa_santri',
            'wa' => 'no_wa_santri',
        ];
        ...
        $col = $map[$key];
        try {
            $stmt = $db->prepare("SELECT {$col} AS phone FROM santri WHERE id = ? LIMIT 1");
            $stmt->execute([$idSantri]);
            $row = $stmt->fetch(\PDO::FETCH_ASSOC);
            if (!$row) {
                return null;
            ...
            if ($raw === '' || \App\Helpers\NikHelper::looksMasked($raw)) {
                return null;
            }
            $formatted = self::formatPhoneNumber($raw);

            return $formatted !== '' ? $formatted : null;
        } catch (\Throwable $e) {
            error_log('WhatsAppService::resolveSantriPhoneField ' . $e->getMessage());
    ...
    /** Label provider untuk pesan error (selaras Pengaturan → Notifikasi eBeddien). */
    public static function getNotificationProviderLabel(?string $provider = null): string
    {
        $p = $provider ?? self::getNotificationProvider();

        return match ($p) {
            'watzap' => 'WatZap',
            'evolution' => 'Evolution WA',
            default => 'WA server sendiri',
        };
    }

    /**
     * Cek apakah nomor terdaftar di WhatsApp.
     * Selalu mengikuti notification_provider di app___settings (Pengaturan → Notifikasi eBeddien):
    ...
    public static function checkNumber(string $noWa, ?string $sessionId = null): array
    {
        ...
        return match ($provider) {
            ...
            'evolution' => \App\Services\EvolutionApiService::checkWhatsAppNumber($noWa),
            default => self::checkNumberViaWaNode($noWa, $phone, $sessionId),
        };
    }

    /**
     * Cek nomor via server WA Node (provider wa_sendiri) — WA_API_URL / WA_API_KEY / WA_SESSION_ID.
     */
    private static function checkNumberViaWaNode(string $noWa, string $phone, ?string $sessionId): array
    {
        $cfg = self::getConfig();
        ...
        if (empty($checkUrl) || empty($apiKey)) {
            return [
                'success' => false,
                'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
                'message' => 'Backend WA server sendiri belum dikonfigurasi. Set WA_API_URL dan WA_API_KEY di api/.env (sama dengan wa/.env), lalu jalankan layanan di folder wa/.',
            ];
        }
        ...
        $checkUrls = self::waNodeCheckUrlVariants($checkUrl);
            ...
            foreach ($checkUrls as $tryUrl) {
                try {
                    ...
                    if ($attempt === 0 && self::nodeSendIndicatesReconnect($errMsg)) {
                        error_log('WhatsAppService::checkNumberViaWaNode: Node belum siap, retry setelah wake: ' . $errMsg);
                        continue 2;
                    }
                    if (self::nodeSendIndicatesReconnect($errMsg)) {
                        ...
                        foreach ($sessionCandidates as $sid) {
                            ...
                            try {
                                ...
                                $retryResponse = $client->post($tryUrl, [
                                    'headers' => [
                                        'Content-Type' => 'application/json',
                                        'X-API-Key' => $apiKey,
                                    ],
                    ...
                    return [
                        'success' => false,
                        'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
                        'message' => $errMsg !== '' ? $errMsg : 'Gagal mengecek nomor (WA server sendiri)',
                    ];
                } catch (\Throwable $e) {
                    $lastConnectError = $e;
                    if (preg_match('/Failed to connect|Could not connect|Connection refused|cURL error 7/i', $e->getMessage())) {
                        error_log('WhatsAppService::checkNumberViaWaNode URL ' . $tryUrl . ': ' . $e->getMessage());
                        continue;
                    }
                    error_log('WhatsAppService::checkNumberViaWaNode attempt ' . ($attempt + 1) . ': ' . $e->getMessage());

                    return [
                        'success' => false,
                        'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
                        'message' => self::humanizeWaNodeConnectError($e),
                    ];
                }
            }
            if ($lastConnectError !== null && $attempt === 1) {
                return [
                    'success' => false,
                    'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
                    'message' => self::humanizeWaNodeConnectError($lastConnectError),
                ];
            }
        }
        ...
        return [
            'success' => false,
            'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
            'message' => 'Gagal menghubungi WA server sendiri untuk cek nomor. Pastikan layanan wa/ berjalan dan sesi QR terhubung di eBeddien → WhatsApp.',
        ];
    }
    ...
    private static function shouldBlockSendBySiapTerimaNotif(?array $logContext, array $kontakStatus): bool
    {
        ...
        if (in_array($k, ['pengeluaran_rencana_notif', 'pengeluaran_notif', 'nis_pengajuan_admin_notif'], true)) {
            return false;
        }
        if (in_array($k, self::KATEGORI_KIRIM_KE_NOMOR_PEMOHON_NIS, true)) {
            return false;
        }
        if (in_array($k, self::KATEGORI_STAFF_MANUAL_WA, true)) {
            return false;
        }
        if ($k === 'staff_data_delete_audit') {
            return false;
        }
    ...
    public static function sendMessage(string $noWa, string $message, ?string $instance = null, ?array $logContext = null, ?string $chatId = null): array
    {
        ...
        if ($useRawLidTarget) {
            ...
            $phone = $originalPhone;
        } elseif ($logContext !== null && in_array(($logContext['kategori'] ?? ''), self::KATEGORI_KIRIM_KE_NOMOR_PEMOHON_NIS, true)) {
            $phone = $originalPhone;
            $chatId = null;
        } elseif ($logContext !== null && in_array(($logContext['kategori'] ?? ''), self::KATEGORI_STAFF_MANUAL_WA, true)) {
            $phone = $originalPhone;
            $chatId = null;
        } else {
        ...
        // Link preview Baileys sering membuat pesan "Menunggu pesan ini…" di beberapa HP Android.
        if (self::shouldDisableWaLinkPreview($logContext, $message)) {
    ...
    public static function processPending(): array
    {
        ...
        try {
            ...
            $tableCheck = $db->query("SHOW TABLES LIKE 'whatsapp___pending'");
            if ($tableCheck->rowCount() === 0) {
                $expiredEdited = \App\Helpers\WaSecurityLinkHelper::sweepExpiredLinkMessages($db);
                return ['success' => true, 'sent' => 0, 'skipped' => 0, 'expired_link_edited' => $expiredEdited];
            }
        ...
        $expiredEdited = 0;
        try {
            $expiredEdited = \App\Helpers\WaSecurityLinkHelper::sweepExpiredLinkMessages();
        } catch (\Throwable $e) {
     ...
     * @param bool $attachMethodLogo Lampirkan logo metode (VA/CStore/e-wallet) — untuk kirim staff eBeddien
     * @param bool $forceSendWa Abaikan PSB_FLOW_WA_NOTIF_DISABLED
     * @param string|null $emailOverride Email notifikasi (opsional)
     */
    public static function sendPsbPembayaranIpaymuOrder(array $phoneNumbers, string $nama, $amount, $adminFee, $total, string $paymentMethod, string $paymentChannel, string $vaOrCode, string $link = '', ?int $idSantri = null, bool $attachMethodLogo = false, bool $forceSendWa = false, ?string $emailOverride = null): void
    {
        if ($vaOrCode === '') {
            return;
        }
        ...
        if ($paymentMethod === 'va') {
            ...
            $instruksi = "Buka tautan pembayaran untuk menyelesaikan di aplikasi e-wallet (ShopeePay/DANA).";
        }
        ...
        if ($emailOverride !== null && trim($emailOverride) !== '') {
            self::sendPsbEmailNotification(trim($emailOverride), 'Detail Pembayaran', $message, $idSantri, 'pembayaran_ipaymu_order');
        } else {
            $email = self::getSantriEmailById($idSantri);
            self::sendPsbEmailNotification($email, 'Detail Pembayaran Pendaftaran', $message, $idSantri, 'pembayaran_ipaymu_order');
        }

        if (!$forceSendWa && self::PSB_FLOW_WA_NOTIF_DISABLED) {
            return;
        }
        $numbers = self::collectPhoneNumbers($phoneNumbers);
        $logoB64 = $attachMethodLogo ? self::paymentMethodLogoBase64($paymentMethod, $paymentChannel) : null;

        foreach ($numbers as $no) {
            try {
                if ($logoB64 !== null && $logoB64 !== '') {
                    self::sendMessageWithImage($no, $message, $logoB64, 'image/png', null, $logContext);
                } else {
    ...
    /**
     * Path logo metode di folder gambar/logo (selaras PaymentIcons frontend).
     */
    public static function paymentMethodLogoPath(string $paymentMethod, string $paymentChannel): ?string
    {
        $method = strtolower(trim($paymentMethod));
        $channel = strtolower(trim(str_replace(['-', ' '], '_', $paymentChannel)));
        $base = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'gambar' . DIRECTORY_SEPARATOR . 'logo';
        $base = realpath($base) ?: $base;
        $file = null;
        if ($method === 'qris') {
            $file = 'qris.png';
        } elseif ($method === 'cstore') {
            $map = ['alfamart' => 'alfamart.png', 'indomaret' => 'indomart.png', 'indomart' => 'indomart.png'];
            $file = $map[$channel] ?? 'alfamart.png';
        } elseif ($method === 'ewallet') {
            $map = [
                'dana' => 'dana.png',
                'shopeepay' => 'shopee-pay.png',
                'shopee_pay' => 'shopee-pay.png',
                'ovo' => 'ovo.png',
                'gopay' => 'gopay.png',
                'linkaja' => 'linkaja.png',
                'astrapay' => 'astrapay.png',
            ];
            $file = $map[$channel] ?? 'dana.png';
        } elseif ($method === 'va') {
            $map = [
                'bag' => 'ag.png', 'ag' => 'ag.png',
                'bca' => 'bca.png', 'bni' => 'bni.png', 'bri' => 'bri.png', 'bsi' => 'bsi.png',
                'btn' => 'btn.png', 'mandiri' => 'mandiri.png', 'permata' => 'permata.png',
                'cimb' => 'cimb.png', 'danamon' => 'danamon.png', 'muamalat' => 'muamalat.png',
            ];
            $file = $map[$channel] ?? 'bca.png';
        }
        if ($file === null) {
            return null;
        }
    ...
    public static function paymentMethodLogoBase64(string $paymentMethod, string $paymentChannel): ?string
    {
        $path = self::paymentMethodLogoPath($paymentMethod, $paymentChannel);
        if ($path === null) {
            return null;
        }
     ...
     * @param string $link Url halaman pembayaran (opsional)
     * @param int|null $idSantri id santri (untuk log)
     * @param bool $forceSendWa Abaikan PSB_FLOW_WA_NOTIF_DISABLED (kirim staff eBeddien)
     * @param string|null $emailOverride
     */
    public static function sendPsbPembayaranIpaymuQris(array $phoneNumbers, string $nama, $amount, $adminFee, $total, string $qrCode, string $link = '', ?int $idSantri = null, bool $forceSendWa = false, ?string $emailOverride = null): void
    {
        if ($link === '') {
            $link = self::getDaftarAppUrl() . '/pembayaran?payment=ipaymu';
        }
        ...
        if ($emailOverride !== null && trim($emailOverride) !== '') {
            self::sendPsbEmailNotification(trim($emailOverride), 'QRIS Pembayaran', $message, $idSantri, 'pembayaran_ipaymu_qris');
        } else {
            $email = self::getSantriEmailById($idSantri);
    ...
    private static function paymentChannelToLabel(string $channel, string $paymentMethod): string
    {
        ...
        if ($paymentMethod === 'ewallet' && isset($ewalletLabels[$c])) {
            return $ewalletLabels[$c];
        }
        if (isset($vaLabels[$c])) {
            return $vaLabels[$c];
        }
