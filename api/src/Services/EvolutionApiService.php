...
final class EvolutionApiService
{
    ...
    /** Apakah base URL + API key Evolution sudah di-set (tanpa cek instance). */
    ...
    /**
     * Cek nomor terdaftar di WhatsApp lewat Evolution API v2.
     * POST /chat/whatsappNumbers/{instance} — selaras pengaturan Setting → Evolution WA.
     *
     * @return array{success: bool, data: array{phoneNumber: string, isRegistered: bool}, message: string}
     */
    public static function checkWhatsAppNumber(string $phoneDigits): array
    {
        $phone = WhatsAppService::formatPhoneNumber($phoneDigits);
        if (strlen($phone) < 10) {
            return [
                'success' => false,
        ...
        if (!self::isConfigured()) {
            return [
                'success' => false,
                'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
                'message' => 'Evolution API belum dikonfigurasi (EVOLUTION_API_BASE_URL / EVOLUTION_API_KEY).',
            ];
        }

        $inst = self::getStoredInstanceName();
        if ($inst === '' || !preg_match('/^[a-zA-Z0-9._-]+$/', $inst)) {
            return [
                'success' => false,
                'data' => ['phoneNumber' => $phone, 'isRegistered' => false],
        ...
        $res = self::request('POST', '/chat/whatsappNumbers/' . rawurlencode($inst), [
            'numbers' => [$phone],
        ], true);
        ...
        if (!$res['ok']) {
            ...
            if ($msg === '') {
                $msg = 'Gagal mengecek nomor lewat Evolution API (HTTP ' . (int) ($res['status'] ?? 0) . ').';
            }
        ...
        $isRegistered = self::parseWhatsAppNumbersResponse($res['data'], $phone);
    ...
    /**
     * @param mixed $data respons JSON Evolution (array daftar atau dibungkus)
     */
    private static function parseWhatsAppNumbersResponse($data, string $phone): bool
    {
        $items = [];
        if (is_array($data)) {
    ...
    public static function getAppConfig(): array
    {
        ...
        $secretTrim = trim((string) \getenv('EVOLUTION_WEBHOOK_SECRET'));
        ...
        return [
            ...
            'data' => [
                ...
                'inbound_webhook_secret_configured' => $secretTrim !== '',
            ],
            'message' => null,
        ];
    }
    ...
    /**
     * URL webhook inbound untuk didaftarkan ke Evolution (POST /webhook/set/…).
     * Menambahkan query ?secret= bila EVOLUTION_WEBHOOK_SECRET di-set di server.
     */
    public static function getInboundWebhookUrlForEvolutionRegistration(): string
    {
        $configAll = require __DIR__ . '/../../config.php';
        $pub = rtrim((string) ($configAll['api_public_url'] ?? ''), '/');
        if ($pub === '') {
        ...
        $base = $pub . '/api/public/evolution-webhook';
        $secret = trim((string) \getenv('EVOLUTION_WEBHOOK_SECRET'));
        if ($secret !== '') {
