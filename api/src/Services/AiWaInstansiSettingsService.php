...
final class AiWaInstansiSettingsService
{
    ...
    public static function canonicalPhoneDigitsFromJid(?string $jid): ?string
    {
        ...
        return self::isPlausibleIndonesianMsisdn($d) ? $d : null;
    }

    /**
     * MSISDN untuk kolom phone_normalized: dari JID @s.whatsapp.net atau dari webhook (senderPn / canonicalNumber).
     * Digit mentah @lid tidak disimpan sebagai nomor HP.
     */
    public static function resolveMsisdnForContact(?string $fromJid, ?string $nomorWebhook): ?string
    {
        $jid = $fromJid !== null ? trim($fromJid) : '';
        $fromJidPhone = self::canonicalPhoneDigitsFromJid($jid);
        if ($fromJidPhone !== null) {
            return $fromJidPhone;
        }

        $raw = trim((string) $nomorWebhook);
        if ($raw === '') {
            return null;
        }
        ...
        if (preg_match('/@lid$/i', $jid)) {
            $lidDigits = preg_replace('/\D/', '', preg_replace('/@lid$/i', '', $jid)) ?? '';
            if ($lidDigits !== '' && $digits === $lidDigits) {
                return null;
            }
        }

        $formatted = WhatsAppService::formatPhoneNumber($digits);

        return self::isPlausibleIndonesianMsisdn($formatted) ? $formatted : null;
    }

    private static function isPlausibleIndonesianMsisdn(string $digits): bool
    {
        ...
        return (bool) preg_match('/^62[1-9]\d{8,11}$/', $d);
    }

    public static function upsertInboundContact(\PDO $db, ?string $fromJid, ?string $nomorWebhook = null): void
    {
        ...
        $phone = self::resolveMsisdnForContact($jid, $nomorWebhook);
        try {
            $stmt = $db->prepare(
                'INSERT INTO ai___wa_obrolan_kontak (wa_jid, phone_normalized, first_seen_at, last_seen_at) VALUES (?, ?, NOW(), NOW()) '
                . 'ON DUPLICATE KEY UPDATE last_seen_at = NOW(), '
                . 'phone_normalized = IF(VALUES(phone_normalized) IS NOT NULL AND TRIM(VALUES(phone_normalized)) <> \'\', '
