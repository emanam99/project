...
class iPaymuService
{
     ...
     * Ref: https://documenter.getpostman.com/view/40296808/2sB3WtseBT
     *
     * @param string $paymentMethod va|cstore|qris|cod|ewallet
     * @param string $paymentChannel bca|bri|bsi|alfamart|indomaret|shopeepay|dana|...
    ...
    public function getExpiredForChannel(string $paymentMethod, string $paymentChannel, array $paymentData = []): array
    {
        ...
        if ($paymentMethod === 'ewallet') {
            // E-wallet (ShopeePay/DANA): default singkat agar deep-link tidak menggantung lama
            return ['expired' => 60, 'expiredType' => 'minutes'];
        }
    ...
    public function createPayment(array $paymentData): array
    {
        ...
        $paymentMethod = $paymentData['payment_method'] ?? 'va'; // va, cstore, qris, ewallet, cod
        ...
        if ($paymentMethod === 'va') {
            ...
            // Docs iPaymu: paymentMethod=ewallet, paymentChannel=shopeepay.
            // Channel `dana` aktif di dashboard merchant (belum selalu tercantum di docs publik).
            if ($paymentChannel === '') {
                return [
                    'success' => false,
                    'message' => 'Payment channel wajib diisi untuk e-wallet. Pilih ShopeePay atau DANA.',
                ];
            }
            $channelLower = strtolower($paymentChannel);
            $ewalletAliases = [
                'shopee' => 'shopeepay',
                'shopeepay' => 'shopeepay',
            ...
            if (isset($ewalletAliases[$channelLower])) {
                $channelLower = $ewalletAliases[$channelLower];
            }
            $validEwallet = ['shopeepay', 'dana'];
            if (!in_array($channelLower, $validEwallet, true)) {
                return [
                    'success' => false,
                    'message' => 'Channel e-wallet iPaymu tidak didukung. Gunakan "shopeepay" atau "dana". Input: "' . $paymentChannel . '".',
                ];
            }
            $requestData['paymentChannel'] = $channelLower;
            // Deep-link e-wallet membutuhkan return/cancel agar user kembali ke app
            $returnUrlCheck = trim((string) ($paymentData['return_url'] ?? $this->config['return_url'] ?? ''));
            if ($returnUrlCheck === '') {
                return [
                    'success' => false,
                    'message' => 'Return URL wajib untuk pembayaran e-wallet.',
                ];
            }
        } elseif ($paymentMethod === 'qris') {
            // Docs: paymentChannel mpm (opsional di implementasi lama — kirim jika ada)
            if ($paymentChannel !== '') {
                $requestData['paymentChannel'] = strtolower($paymentChannel);
            } else {
                $requestData['paymentChannel'] = 'mpm';
            }
            if (!($this->config['production_mode'] ?? false)) {
    ...
    private function updateTransactionFromCallback(array $callbackData): void
    {
        try {
            ...
            if ($status === 'paid') {
                ...
                $tidWa = (int) $transactionId;
                $selfWa = $this;
                DeferredHttpTask::runAfterResponse(static function () use ($selfWa, $tidWa): void {
                    $selfWa->sendWaPembayaranBerhasilByTransactionId($tidWa);
                });
     ...
     */
    /**
     * Selesaikan pembayaran sukses (insert domain + WA) — dipakai juga dari Xendit callback.
     */
    public function completePaidSettlement(int $transactionId): void
    {
        $this->updatePaymentStatus($transactionId, 'Success');
        $tidWa = $transactionId;
        $self = $this;
        DeferredHttpTask::runAfterResponse(static function () use ($self, $tidWa): void {
            $self->notifyPaymentSuccessByTransactionId($tidWa);
        });
    }

    /**
     * @internal dipanggil dari completePaidSettlement
     */
    public function notifyPaymentSuccessByTransactionId(int $transactionId): void
    {
    ...
    public function updatePaymentStatusFromTransaction(int $idPayment): void
    {
        try {
            ...
            $this->ensureCashlessTopUpInserted($idPayment);
        } catch (\Exception $e) {
    ...
    public function ensureUwabaKhususTunggakanBayarInserted(int $idPayment): void
    {
        try {
            ...
            $viaLabel = ViaPembayaranHelper::normalizeForDisplay($p['via'] ?? null);
            if ($viaLabel === '-') {
                $viaLabel = ViaPembayaranHelper::CANONICAL_IPAYMU;
            }
            ...
            if ($jenis === 'Uwaba' && $idSantri && ($tahunAjaranPayment !== null || $idReferensi !== null)) {
                ...
                $ins = $this->db->prepare("INSERT INTO uwaba___bayar (id_santri, tahun_ajaran, nominal, via, admin, id_admin, hijriyah, masehi, nomor, id_payment) VALUES (?, ?, ?, ?, 'Online', NULL, ?, CURRENT_TIMESTAMP, ?, ?)");
                $ins->execute([$idSantri, $tahunAjaran, (int) round($nominal), $viaLabel, $hijriyah, $nomor, $idPayment]);
            ...
            if ($jenis === 'Khusus' && $idReferensi && $idSantri) {
                ...
                $ins = $this->db->prepare("INSERT INTO uwaba___bayar_khusus (id_khusus, id_santri, nominal, via, id_admin, admin, hijriyah, id_payment) VALUES (?, ?, ?, ?, NULL, 'Online', ?, ?)");
                $ins->execute([$idReferensi, $idSantri, (int) round($nominal), $viaLabel, $hijriyahK, $idPayment]);
            ...
            if ($jenis === 'Tunggakan' && $idReferensi && $idSantri) {
                ...
                $ins = $this->db->prepare("INSERT INTO uwaba___bayar_tunggakan (id_tunggakan, id_santri, nominal, via, id_admin, admin, hijriyah, id_payment) VALUES (?, ?, ?, ?, NULL, 'Online', ?, ?)");
                $ins->execute([$idReferensi, $idSantri, (int) round($nominal), $viaLabel, $hijriyahT, $idPayment]);
    ...
    /**
     * Top-up wallet cashless setelah payment Cashless berhasil (iPayMu).
     */
    public function ensureCashlessTopUpInserted(int $idPayment): void
    {
        try {
            $stmt = $this->db->prepare(
                "SELECT id, status, jenis_pembayaran, id_referensi, tabel_referensi, id_santri, nominal, id_user, via
                 FROM payment WHERE id = ? LIMIT 1"
            );
            $stmt->execute([$idPayment]);
            $p = $stmt->fetch(\PDO::FETCH_ASSOC);
            ...
            $idReferensi = isset($p['id_referensi']) ? (int) $p['id_referensi'] : 0;
            ...
            $stmtPt = $this->db->prepare(
                "SELECT id FROM payment___transaction WHERE id_payment = ? AND status = 'paid' LIMIT 1"
            );
            $stmtPt->execute([$idPayment]);
            $transactionPaid = (bool) $stmtPt->fetch(\PDO::FETCH_ASSOC);
            if (!$paymentSuccess && !$transactionPaid) {
            ...
            if ($idSantri > 0) {
                if ($actorUserId <= 0) {
                    $stmtU = $this->db->prepare('SELECT id_user FROM santri WHERE id = ? LIMIT 1');
                    $stmtU->execute([$idSantri]);
                    $actorUserId = (int) ($stmtU->fetchColumn() ?: 0);
                }
                $result = $svc->topUpViaGateway($idSantri, $nominal, $actorUserId, $idPayment, $viaLabel);
                if (!($result['success'] ?? false)) {
                    error_log('iPaymuService::ensureCashlessTopUpInserted failed payment=' . $idPayment . ' msg=' . ($result['message'] ?? ''));
                }
                return;
            }

            // Top-up wallet toko: id_referensi = cashless___accounts.id (PEDAGANG)
            if ($idReferensi > 0) {
                $stmtAcc = $this->db->prepare(
                    "SELECT entity_type, entity_id FROM cashless___accounts WHERE id = ? LIMIT 1"
                );
                $stmtAcc->execute([$idReferensi]);
                $acc = $stmtAcc->fetch(\PDO::FETCH_ASSOC);
                if ($acc && strtoupper((string) ($acc['entity_type'] ?? '')) === 'PEDAGANG') {
                    $pedagangId = (int) ($acc['entity_id'] ?? 0);
                    if ($pedagangId > 0) {
                        if ($actorUserId <= 0) {
                            $stmtU = $this->db->prepare('SELECT id_users FROM cashless___pedagang WHERE id = ? LIMIT 1');
                            $stmtU->execute([$pedagangId]);
                            $actorUserId = (int) ($stmtU->fetchColumn() ?: 0);
                        }
                        $result = $svc->topUpPedagangViaGateway($pedagangId, $nominal, $actorUserId, $idPayment, $viaLabel);
                        if (!($result['success'] ?? false)) {
                            error_log('iPaymuService::ensureCashlessTopUpInserted pedagang failed payment=' . $idPayment . ' msg=' . ($result['message'] ?? ''));
                        }
                    }
                }
            }
        } catch (\Exception $e) {
            error_log('iPaymuService::ensureCashlessTopUpInserted error: ' . $e->getMessage());
        }
    }
    ...
    private function updatePaymentStatusExpired(int $transactionId): void
    {
        try {
            ...
            $selfWa = $this;
            DeferredHttpTask::runAfterResponse(static function () use ($selfWa, $idPaymentWa): void {
                $selfWa->sendPaymentExpiredWaForPaymentId($idPaymentWa);
            });
        } catch (\Exception $e) {
    ...
    private function sendPaymentExpiredWaForPaymentId(int $idPayment): void
    {
        try {
            ...
            error_log('iPaymuService::sendPaymentExpiredWaForPaymentId: ' . $e->getMessage());
        }
    }
    ...
    private function updatePaymentStatusFailed(int $transactionId): void
    {
        try {
            ...
            $selfWa = $this;
            DeferredHttpTask::runAfterResponse(static function () use ($selfWa, $idPaymentWa): void {
                $selfWa->sendPaymentFailedWaForPaymentId($idPaymentWa);
            });
        } catch (\Exception $e) {
    ...
    private function sendPaymentFailedWaForPaymentId(int $idPayment): void
    {
        try {
            $stmtPay = $this->db->prepare("SELECT id_santri, jenis_pembayaran, id_user FROM payment WHERE id = ?");
            $stmtPay->execute([$idPayment]);
            ...
            error_log('iPaymuService::sendPaymentFailedWaForPaymentId: ' . $e->getMessage());
        }
    }
    ...
    private function insertToPendaftaranTransaction(array $paymentData): void
    {
        try {
            ...
            $viaLabel = ViaPembayaranHelper::CANONICAL_IPAYMU;
            $stmtVia = $this->db->prepare('SELECT via FROM payment WHERE id = ? LIMIT 1');
            $stmtVia->execute([$idPayment]);
            $viaRow = $stmtVia->fetch(\PDO::FETCH_ASSOC);
            if ($viaRow) {
                $normalized = ViaPembayaranHelper::normalizeForDisplay($viaRow['via'] ?? null);
                if ($normalized !== '-') {
                    $viaLabel = $normalized;
                }
            }
            ...
            try {
                ...
                // Update bayar, kurang, tanggal pembayaran pertama; jangan turunkan keterangan_status bila sudah terverifikasi/aktif
                ...
                if ($idSantri) {
                    ...
                    $nominalWa = (float) $nominal;
                    DeferredHttpTask::runAfterResponse(static function () use ($idSantriWa, $nominalWa): void {
                        try {
                            $db = Database::getInstance()->getConnection();
                            $stmtSantri = $db->prepare("SELECT nama, no_telpon, no_wa_santri FROM santri WHERE id = ?");
                            $stmtSantri->execute([$idSantriWa]);
                            $santri = $stmtSantri->fetch(\PDO::FETCH_ASSOC);
                            if ($santri) {
                                $noWa = trim((string) ($santri['no_wa_santri'] ?? '')) ?: trim((string) ($santri['no_telpon'] ?? ''));
                                if ($noWa !== '') {
                                    WhatsAppService::sendPsbPembayaranBerhasil($noWa, $santri['nama'] ?? '', $nominalWa, $idSantriWa);
                                }
                            }
                        } catch (\Throwable $e) {
                            error_log('iPaymuService::insertToPendaftaranTransaction send WA (deferred): ' . $e->getMessage());
                        }
                    });
                }
