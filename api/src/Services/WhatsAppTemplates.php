...
class WhatsAppTemplates
{
    ...
    /**
     * Ack singkat setelah user kirim permintaan via WA (daftar / lupa password / lupa username / handshake serupa).
     * Dipisah sebagai pesan 1 sebelum isi balasan (link / username / dll).
     */
    ...
    /**
     * Pengirim WA ≠ nomor yang diisi di aplikasi.
     */
    public static function pesanHarusDariNomorSama(string $waTercatat, string $waPengirim): string
    {
        $t = trim($waTercatat) !== '' ? trim($waTercatat) : '-';
        $p = trim($waPengirim) !== '' ? trim($waPengirim) : '-';

        return "Pesan harus dikirim dari nomor yang sama dengan yang Anda isi di aplikasi.\n\n"
            . "wa tercatat: {$t}\n"
            . "wa pengirim: {$p}";
    }

    /**
     * Prefiks ack «Terima kasih…» + SPLIT hanya jika belum dikirim ke nomor ini di bulan kalender berjalan.
     * Satu nomor maksimal 1× per bulan (YYYY-MM).
     */
    public static function prependPermintaanSedangDiprosesAck(\PDO $db, string $noWa, string $replyBody): string
    {
        $ack = self::claimPermintaanSedangDiprosesAck($db, $noWa);
        if ($ack === null || $ack === '') {
            return $replyBody;
        }
    ...
    /**
     * Klaim slot ack bulanan. Null = sudah pernah dikirim bulan ini (jangan kirim lagi).
     */
    public static function claimPermintaanSedangDiprosesAck(\PDO $db, string $noWa): ?string
    {
        $norm = WhatsAppService::formatPhoneNumber($noWa);
        if ($norm === '') {
            return self::permintaanSedangDiprosesAck();
        }

        $ym = date('Y-m');
        try {
            if ($db->query("SHOW TABLES LIKE 'wa_permintaan_proses_ack'")->rowCount() === 0) {
                return self::permintaanSedangDiprosesAck();
            }
            $ins = $db->prepare(
                'INSERT IGNORE INTO wa_permintaan_proses_ack (no_wa, tahun_bulan, sent_at) VALUES (?, ?, NOW())'
            );
            $ins->execute([$norm, $ym]);
            if ($ins->rowCount() < 1) {
                return null;
            ...
            error_log('WhatsAppTemplates::claimPermintaanSedangDiprosesAck ' . $e->getMessage());
            return self::permintaanSedangDiprosesAck();
        }
