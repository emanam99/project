...
final class AiChatDailyLimitService
{
    ...
    /**
     * users.id dengan no_wa (08/62 sama setelah normalisasi) dan no_wa_verified_at terisi + ai_enabled.
     * Dipakai AI WA instansi agar pengurus terverifikasi dibalas sebagai akun sendiri, bukan pengunjung.
     */
    public static function resolveVerifiedUserIdByWaDigits(\PDO $db, string $nomorTujuan): ?int
    {
        ...
        try {
            ...
            $stmt = $db->prepare(
                'SELECT id FROM users WHERE id IN (' . $ph . ') AND no_wa_verified_at IS NOT NULL ORDER BY id ASC'
            );
            $stmt->execute($ids);
            $verified = [];
            while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
                $uid = (int) ($row['id'] ?? 0);
                if ($uid > 0 && AiWaInstansiSettingsService::isValidQuotaUser($db, $uid)) {
                    $verified[] = $uid;
                }
            }
            if ($verified === []) {
            ...
            if (count($verified) > 1) {
                error_log(
                    'AiChatDailyLimitService::resolveVerifiedUserIdByWaDigits: beberapa akun untuk nomor '
                    . $digits . ' — memakai id terkecil: ' . implode(',', $verified)
                );
            }

            return $verified[0];
        } catch (\Throwable $e) {
            error_log('AiChatDailyLimitService::resolveVerifiedUserIdByWaDigits ' . $e->getMessage());
