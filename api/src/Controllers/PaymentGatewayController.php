...
class PaymentGatewayController
{
    ...
    /**
     * GET /api/payment-gateway/mybeddian-provider — gateway aktif untuk myBeddian
     */
    public function getMybeddianProvider(Request $request, Response $response): Response
    {
        try {
            $provider = PaymentGatewayConfig::getMybeddianPaymentProvider();
            $productionMode = PaymentGatewayConfig::isProductionModeActive();

            return $this->jsonResponse($response, [
                'success' => true,
                'data' => [
                    'provider' => $provider,
                    'provider_label' => $provider === PaymentGatewayConfig::PROVIDER_XENDIT ? 'Xendit' : 'iPayMu',
                    'production_mode' => $productionMode,
                    'ipaymu_configured' => PaymentGatewayConfig::isProviderConfigured(
                        PaymentGatewayConfig::PROVIDER_IPAYMU,
                        $productionMode
                    ),
                    'xendit_configured' => PaymentGatewayConfig::isProviderConfigured(
                        PaymentGatewayConfig::PROVIDER_XENDIT,
                        $productionMode
                    ),
                ],
            ], 200);
        } catch (\Exception $e) {
            error_log('getMybeddianProvider error: ' . $e->getMessage());

            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal mengambil pengaturan gateway myBeddian',
            ], 500);
        }
    }

    /**
     * PUT /api/payment-gateway/mybeddian-provider — set gateway myBeddian (ipaymu|xendit)
     */
    public function putMybeddianProvider(Request $request, Response $response): Response
    {
        try {
            $input = $request->getParsedBody();
            ...
            $provider = isset($input['provider']) ? strtolower(trim((string) $input['provider'])) : '';
            if (!in_array($provider, [PaymentGatewayConfig::PROVIDER_IPAYMU, PaymentGatewayConfig::PROVIDER_XENDIT], true)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Provider harus ipaymu atau xendit',
                ], 400);
            }

            if (!PaymentGatewayConfig::setMybeddianPaymentProvider($provider)) {
                return $this->jsonResponse($response, [
                    'success' => false,
            ...
            $label = $provider === PaymentGatewayConfig::PROVIDER_XENDIT ? 'Xendit' : 'iPayMu';
            $modeLabel = $productionMode ? 'production' : 'sandbox';
            $message = 'Gateway myBeddian diatur ke ' . $label;
            if (!$configured) {
                $message .= '. Isi Secret API Key ' . $label . ' (mode ' . $modeLabel . ') agar pembayaran myBeddian berjalan.';
            }
            ...
            error_log('putMybeddianProvider error: ' . $e->getMessage());

            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal menyimpan pengaturan gateway',
            ], 500);
        }
    }
    ...
    public function getServerInfo(Request $request, Response $response): Response
    {
        try {
            ...
            $callbackUrlXendit = $baseUrl . '/api/payment-transaction/callback/xendit';
            ...
            return $this->jsonResponse($response, [
                ...
                'data' => [
                    ...
                    'callback_url_xendit' => $callbackUrlXendit,
