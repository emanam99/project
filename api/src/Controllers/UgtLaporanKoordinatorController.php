...
class UgtLaporanKoordinatorController
{
    ...
    private const MAX_FOTO_COUNT = 5;
    private const FOTO_PATH_PATTERN = '#^uploads/ugt/[^/]+$#';
    ...
    /** @return list<string> */
    private function decodeFotosFromStorage(?string $raw): array
    {
        ...
        if ($raw !== '' && $raw[0] === '[') {
            ...
            if (is_array($decoded)) {
                ...
                foreach ($decoded as $p) {
                    $p = trim((string) $p);
                    if ($p !== '' && preg_match(self::FOTO_PATH_PATTERN, $p)) {
                        $out[] = $p;
                    }
                }

                return array_slice(array_values(array_unique($out)), 0, self::MAX_FOTO_COUNT);
            }
        }
        if (preg_match(self::FOTO_PATH_PATTERN, $raw)) {
            return [$raw];
        }
     ...
     */
    private function encodeFotosForStorage(array $paths): ?string
    {
        $clean = [];
        foreach ($paths as $p) {
            $p = trim((string) $p);
        ...
        if (count($clean) > self::MAX_FOTO_COUNT) {
            $clean = array_slice($clean, 0, self::MAX_FOTO_COUNT);
        }
     ...
     * @param list<string> $paths
     */
    private function validateFotoPaths(array $paths): bool
    {
        if (count($paths) > self::MAX_FOTO_COUNT) {
            return false;
        }
        foreach ($paths as $p) {
            if (!preg_match(self::FOTO_PATH_PATTERN, trim((string) $p))) {
                return false;
            }
        }
    ...
    /** @return list<string> */
    private function parseFotosFromBody(array $data): array
    {
        if (isset($data['foto_list']) && is_array($data['foto_list'])) {
            $list = [];
            foreach ($data['foto_list'] as $p) {
                $p = trim((string) $p);
                if ($p !== '') {
                    $list[] = $p;
                }
            ...
            return array_slice(array_values(array_unique($list)), 0, self::MAX_FOTO_COUNT);
        }
        if (array_key_exists('foto', $data)) {
            $single = trim((string) $data['foto']);
     ...
     */
    private function deleteRemovedFotos(array $old, array $new): void
    {
        foreach (array_diff($old, $new) as $path) {
            $this->deleteStoredFoto($path);
        }
    }

    /** @param array<string,mixed> $row */
    private function attachFotosFields(array $row): array
    {
        $list = $this->decodeFotosFromStorage(isset($row['foto']) ? (string) $row['foto'] : null);
        $row['foto_list'] = $list;
        $row['foto'] = $list[0] ?? null;
    ...
    private function normalizeLaporanRow(array $row): array
    {
        ...
        return $this->attachFotosFields($row);
    }
    ...
    public function create(Request $request, Response $response): Response
    {
        try {
            $user = $request->getAttribute('user');
            $userArr = is_array($user) ? $user : [];
            if (!RoleHelper::tokenUgtLaporanCanTambah($this->db, $userArr, 'koordinator')) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Tidak berhak menambah laporan koordinator',
                ], 403);
            }
            ...
            $fotoPaths = $this->parseFotosFromBody($data);
            if (!$this->validateFotoPaths($fotoPaths)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Foto tidak valid atau melebihi batas ' . self::MAX_FOTO_COUNT . ' file',
                ], 400);
            }
            $foto = $this->encodeFotosForStorage($fotoPaths);
            $usulan = isset($data['usulan']) ? trim((string) $data['usulan']) : null;
            $usulan = ($usulan !== null && $usulan !== '') ? $usulan : null;
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $oldFotoList = $this->decodeFotosFromStorage(isset($existing['foto']) ? (string) $existing['foto'] : null);
            if (array_key_exists('foto_list', $data) || array_key_exists('foto', $data)) {
                $fotoPaths = $this->parseFotosFromBody($data);
                if (!$this->validateFotoPaths($fotoPaths)) {
                    return $this->jsonResponse($response, [
                        'success' => false,
                ...
                $this->deleteRemovedFotos($oldFotoList, $fotoPaths);
                $foto = $this->encodeFotosForStorage($fotoPaths);
            } else {
                $foto = $this->encodeFotosForStorage($oldFotoList);
            }
            if (array_key_exists('usulan', $data)) {
                $usulan = trim((string) $data['usulan']);
                $usulan = $usulan !== '' ? $usulan : null;
            } else {
                $usulan = isset($existing['usulan']) && $existing['usulan'] !== '' && $existing['usulan'] !== null
                    ? (string) $existing['usulan']
                    : null;
            }
    ...
    public function delete(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            foreach ($this->decodeFotosFromStorage(isset($existing['foto']) ? (string) $existing['foto'] : null) as $fotoPath) {
                $this->deleteStoredFoto($fotoPath);
            }
