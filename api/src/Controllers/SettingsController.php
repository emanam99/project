...
class SettingsController
{
    ...
    public function getEbeddienMenuFitur(Request $request, Response $response): Response
    {
        try {
            ...
            try {
                ...
                foreach ($cntStmt->fetchAll(\PDO::FETCH_ASSOC) as $cr) {
                    $countMap[(int) ($cr['role_id'] ?? 0)] = (int) ($cr['cnt'] ?? 0);
                }
            } catch (\Throwable $e) {
    ...
    /**
     * GET /api/settings/roles/{roleId}/pengurus-checklist
     * Semua pengurus + flag has_role untuk role tertentu (centang di offcanvas Fitur).
     * Yang punya role di atas; yang belum di bawah.
     */
    public function getRolePengurusChecklist(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $roleStmt = $this->db->prepare('SELECT `id`, `key`, `label` FROM `role` WHERE `id` = ? LIMIT 1');
            ...
            $sql = 'SELECT p.`id` AS pengurus_id, p.`nama`, u.`username`,
                           pr.`id` AS pengurus_role_id
                    FROM `pengurus` p
                    LEFT JOIN `users` u ON u.`id` = p.`id_user`
                    LEFT JOIN `pengurus___role` pr ON pr.`pengurus_id` = p.`id` AND pr.`role_id` = ?
                    ORDER BY (pr.`id` IS NULL) ASC, p.`nama` ASC, p.`id` ASC';
            ...
            $withRole = 0;
            foreach ($rows as $row) {
                $has = !empty($row['pengurus_role_id']);
                if ($has) {
                    $withRole++;
                }
                $items[] = [
                    'pengurus_id' => (int) ($row['pengurus_id'] ?? 0),
                    'nama' => (string) ($row['nama'] ?? ''),
                    'username' => $row['username'] !== null && $row['username'] !== ''
                        ? (string) $row['username']
                        : null,
                    'has_role' => $has,
                    'pengurus_role_id' => $has ? (int) $row['pengurus_role_id'] : null,
                ];
            }
            ...
            error_log('SettingsController::getRolePengurusChecklist ' . $e->getMessage());
    ...
    /**
     * PUT /api/settings/roles/{roleId}/pengurus-checklist
     * Body: { pengurus_id: int, has_role: bool }
     * Langsung assign/cabut role pada pengurus (tabel pengurus___role).
     */
    public function putRolePengurusChecklist(Request $request, Response $response, array $args): Response
    {
        try {
            $roleId = (int) ($args['roleId'] ?? 0);
            if ($roleId < 1) {
            ...
            $body = (array) $request->getParsedBody();
            $pengurusId = (int) ($body['pengurus_id'] ?? 0);
            $hasRole = filter_var($body['has_role'] ?? false, FILTER_VALIDATE_BOOLEAN);
            if ($pengurusId < 1) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'pengurus_id tidak valid',
                ], 400);
            }

            $roleStmt = $this->db->prepare('SELECT `id` FROM `role` WHERE `id` = ? LIMIT 1');
            $roleStmt->execute([$roleId]);
            ...
            $pStmt = $this->db->prepare('SELECT `id` FROM `pengurus` WHERE `id` = ? LIMIT 1');
            $pStmt->execute([$pengurusId]);
            if ($pStmt->fetch() === false) {
                return $this->jsonResponse($response, [
                    'success' => false,
            ...
            $find = $this->db->prepare(
                'SELECT `id` FROM `pengurus___role` WHERE `pengurus_id` = ? AND `role_id` = ? LIMIT 1'
            );
            $find->execute([$pengurusId, $roleId]);
            $existing = $find->fetch(\PDO::FETCH_ASSOC);
            $pengurusRoleId = $existing ? (int) $existing['id'] : null;

            if ($hasRole) {
                if ($pengurusRoleId === null) {
                    $currentUser = $request->getAttribute('user');
                    $idAdmin = (int) ($currentUser['user_id'] ?? $currentUser['id'] ?? 0);
                    if ($idAdmin < 1) {
                        $idAdmin = $pengurusId;
                    }
                    $waktu = (new \DateTime('now', new \DateTimeZone('Asia/Jakarta')))->format('Y-m-d H:i:s');
                    $ins = $this->db->prepare(
                        'INSERT INTO `pengurus___role` (`pengurus_id`, `role_id`, `lembaga_id`, `id_admin`, `tanggal_dibuat`)
                         VALUES (?, ?, NULL, ?, ?)'
                    );
                    $ins->execute([$pengurusId, $roleId, $idAdmin, $waktu]);
                    $pengurusRoleId = (int) $this->db->lastInsertId();
                }
            } elseif ($pengurusRoleId !== null) {
                $del = $this->db->prepare(
                    'DELETE FROM `pengurus___role` WHERE `id` = ? AND `pengurus_id` = ?'
                );
                $del->execute([$pengurusRoleId, $pengurusId]);
                $pengurusRoleId = null;
            }
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'message' => $hasRole ? 'Role ditambahkan ke pengurus' : 'Role dicabut dari pengurus',
                'data' => [
                    'pengurus_id' => $pengurusId,
                    'role_id' => $roleId,
                    'has_role' => $hasRole,
                    'pengurus_role_id' => $pengurusRoleId,
                    'pengurus_count' => $pengurusCount,
                ],
            ], 200);
        } catch (\Throwable $e) {
            error_log('SettingsController::putRolePengurusChecklist ' . $e->getMessage());

            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal mengubah role pengurus',
                'error' => null,
            ], 500);
        }
    }
    ...
    public function postEmailConfigTest(Request $request, Response $response): Response
    {
        try {
            ...
            // Tes SMTP boleh jalan walau toggle OTP belum ON — cukup kredensial di DB lengkap (OTP nyata tetap cek di EmailService::sendOtpEmail).
            $sendResult = EmailService::send(
                $recipient,
                $subject !== '' ? $subject : 'Tes koneksi SMTP eBeddien',
                $html,
                $message,
                false
            );
