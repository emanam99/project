...
class SantriBerkasControllerV2
{
     ...
     *
     * Gunakan {@see PathSafetyHelper::resolveWithinBase()} agar realpath-nya
     * dipastikan tetap di bawah uploadsBasePath (anti path traversal kalau
     * suatu saat kolom path_file di DB dikompromi).
     *
     * @return string Path absolut aman, atau string kosong jika tidak valid / tidak ada.
     */
    private function resolveFilePath(string $pathFile): string
    {
        ...
        // mustBeFile=false: caller existing memakai is_file() sendiri (mis. untuk
        // membedakan 404 vs broken record). Yang penting di sini cegah traversal.
            ...
            if (stripos($jenisBerkas, 'Bukti Pembayaran') !== false) {
                try {
                    ...
                    if ($santri) {
                        ...
                        if ($noWa !== '') {
                            $namaWa = (string) ($santri['nama'] ?? '');
                            $sidWa = (int) $idSantri;
                            DeferredHttpTask::runAfterResponse(static function () use ($noWa, $namaWa, $sidWa): void {
                                try {
                                    WhatsAppService::sendPsbPembayaranLink($noWa, $namaWa, 'open', $sidWa);
                                } catch (\Throwable $e) {
    ...
    private function checkBerkasLengkapAndSendWa(int $idSantri, Request $request): void
    {
        try {
            ...
            $santriPayload = ['id' => $idSantri, 'nis' => $santri['nis'] ?? null, 'nama' => $santri['nama'] ?? '', 'email' => $santri['email'] ?? ''];
            $listAdaCopy = $listAda;
            $listTidakAdaCopy = $listTidakAda;
            $phonesCopy = array_values($phones);
            DeferredHttpTask::runAfterResponse(static function () use ($santriPayload, $listAdaCopy, $listTidakAdaCopy, $phonesCopy): void {
                try {
                    WhatsAppService::sendPsbBerkasLengkap($santriPayload, $listAdaCopy, $listTidakAdaCopy, $phonesCopy);
                } catch (\Throwable $e) {
                    error_log('SantriBerkasControllerV2 sendPsbBerkasLengkap (deferred): ' . $e->getMessage());
                }
            });
        } catch (\Throwable $e) {
