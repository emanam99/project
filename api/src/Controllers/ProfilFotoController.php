...
class ProfilFotoController
{
    ...
    public function upload(Request $request, Response $response): Response
    {
        try {
            ...
            $validation = FileUploadValidator::validate($file, ['jpg', 'jpeg', 'png', 'webp', 'gif'], 2 * 1024 * 1024);
            ...
            $ext = $validation['extension'] === 'jpeg' ? 'jpg' : $validation['extension'];
            ...
            $postCheck = FileUploadValidator::validateMovedFile($filePath, $ext === 'jpg' ? 'jpg' : $ext);
            ...
            $stmtUser = $this->db->prepare('SELECT id_user FROM pengurus WHERE id = ? LIMIT 1');
            $stmtUser->execute([$pengurusId]);
            $idUser = (int) $stmtUser->fetchColumn();
            if ($idUser > 0) {
                $this->db->prepare('UPDATE users SET foto_profil = ? WHERE id = ?')->execute([$relativePath, $idUser]);
            }
    ...
    public function delete(Request $request, Response $response): Response
    {
        try {
            ...
            if ($path) {
                ...
                $idUser = (int) $stmtUser->fetchColumn();
                if ($idUser > 0) {
                    $this->db->prepare('UPDATE users SET foto_profil = NULL WHERE id = ?')->execute([$idUser]);
                }
            }
