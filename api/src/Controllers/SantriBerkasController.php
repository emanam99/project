...
class SantriBerkasController
{
    ...
    private function resolvePath(string $pathFile): ?string
    {
        ...
        return PathSafetyHelper::resolveWithinBase($this->uploadsPath, $pathFile, true);
    }
    ...
    public function uploadBerkas(Request $request, Response $response): Response
    {
        try {
            ...
            // Khusus foto multi-riwayat: selalu insert baru (tidak replace)
            $isMultiFoto = in_array($jenisBerkas, ['foto_juara', 'foto_cashless'], true);
            if ($isMultiFoto) {
                $existingBerkas = null; // Force insert baru
            }
    ...
    /**
     * GET /api/public/juara-foto - Metadata foto juara (tanpa path_file mentah).
     * Hanya untuk santri yang tercatat di santri___juara.
     */
    public function getPublicFotoJuara(Request $request, Response $response): Response
    {
        try {
            ...
            if (!$this->santriIsPublicJuara((int) $idSantri)) {
                return $this->jsonResponse($response, [
                    'success' => false,
            ...
            if ($all) {
                $sql = "SELECT id, nama_file, tipe_file, tanggal_dibuat
                        FROM santri___berkas 
                        WHERE id_santri = ? AND jenis_berkas = 'foto_juara' 
                        ORDER BY tanggal_dibuat DESC
                        LIMIT 20";
                $stmt = $this->db->prepare($sql);
                $stmt->execute([(int) $idSantri]);
                $berkasList = $stmt->fetchAll(\PDO::FETCH_ASSOC);
                $berkasList = array_map([$this, 'mapPublicJuaraBerkasMeta'], $berkasList);
            ...
            $sql = "SELECT id, nama_file, tipe_file 
                    FROM santri___berkas 
                    WHERE id_santri = ? AND jenis_berkas = 'foto_juara' 
                    ORDER BY tanggal_dibuat DESC 
                    LIMIT 1";
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'data' => $this->mapPublicJuaraBerkasMeta($berkas)
            ], 200);
    ...
    /**
     * GET /api/public/juara-foto/list - List metadata foto juara (tanpa path_file).
     */
    public function getPublicFotoJuaraList(Request $request, Response $response): Response
    {
        try {
            ...
            $sql = "SELECT id, nama_file, tipe_file, keterangan, tanggal_dibuat
                    FROM santri___berkas 
                    WHERE id_santri = ? AND jenis_berkas = 'foto_juara' 
                    ORDER BY tanggal_dibuat DESC
                    LIMIT 20";
    ...
    /**
     * GET /api/public/juara-foto-image - Serve gambar foto juara (berkas_id atau path terverifikasi).
     */
    public function serveFotoJuaraImage(Request $request, Response $response): Response
    {
        try {
            $queryParams = $request->getQueryParams();
            $berkasId = isset($queryParams['berkas_id']) ? (int) $queryParams['berkas_id'] : 0;
            $pathFile = $queryParams['path'] ?? null;
            $resolvedPath = null;

            if ($berkasId > 0) {
                $stmt = $this->db->prepare(
                    "SELECT path_file, id_santri FROM santri___berkas
                     WHERE id = ? AND jenis_berkas = 'foto_juara' LIMIT 1"
                );
                $stmt->execute([$berkasId]);
                $row = $stmt->fetch(\PDO::FETCH_ASSOC);
                if (!$row || !$this->santriIsPublicJuara((int) ($row['id_santri'] ?? 0))) {
                    return $this->jsonResponse($response, [
                        'success' => false,
                        'message' => 'File tidak ditemukan'
                    ], 404);
                }
                $resolvedPath = $this->resolvePath((string) ($row['path_file'] ?? ''));
            } elseif ($pathFile) {
                // Backward compat: path hanya dilayani jika berkas juara milik santri juara
                $stmt = $this->db->prepare(
                    "SELECT id_santri FROM santri___berkas
                     WHERE path_file = ? AND jenis_berkas = 'foto_juara' LIMIT 1"
                );
                $stmt->execute([(string) $pathFile]);
                $row = $stmt->fetch(\PDO::FETCH_ASSOC);
                if (!$row || !$this->santriIsPublicJuara((int) ($row['id_santri'] ?? 0))) {
                    return $this->jsonResponse($response, [
                        'success' => false,
                ...
                $resolvedPath = $this->resolvePath((string) $pathFile);
            } else {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Parameter berkas_id atau path wajib diisi'
                ], 400);
            }
            ...
            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal menyajikan foto juara.'
            ], 500);
        }
    }

    private function santriIsPublicJuara(int $idSantri): bool
    {
        if ($idSantri <= 0) {
            return false;
        }
        $stmt = $this->db->prepare('SELECT 1 FROM santri___juara WHERE id_santri = ? LIMIT 1');
     ...
     */
    private function mapPublicJuaraBerkasMeta(array $berkas): array
    {
        $id = (int) ($berkas['id'] ?? 0);

        return [
            'id' => $id,
            'nama_file' => $berkas['nama_file'] ?? null,
            'tipe_file' => $berkas['tipe_file'] ?? null,
            'keterangan' => $berkas['keterangan'] ?? null,
            'tanggal_dibuat' => $berkas['tanggal_dibuat'] ?? null,
            'image_url' => $id > 0 ? '/api/public/juara-foto-image?berkas_id=' . $id : null,
        ];
    }
