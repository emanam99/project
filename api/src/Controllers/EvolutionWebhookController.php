...
final class EvolutionWebhookController
{
    ...
    private static function webhookSecretValid(Request $request): bool
    {
        ...
        if ($expected === '') {
            // Fail-fast di production: tolak jika secret belum diset.
            // Di non-production (dev/staging) tetap longgar untuk kemudahan testing webhook.
            if (strtolower((string) \getenv('APP_ENV')) === 'production') {
                error_log('EvolutionWebhook: tolak karena EVOLUTION_WEBHOOK_SECRET kosong di production');
                return false;
            }
            return true;
        }
