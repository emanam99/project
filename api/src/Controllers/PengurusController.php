...
class PengurusController
{
    ...
    public function getList(Request $request, Response $response): Response
    {
        try {
            ...
            $joinJabatan = "LEFT JOIN pengurus___jabatan pj ON pj.pengurus_id = p.id AND {$pjAktifSql}
            ...
            if ($lembagaId !== '') {
                $whereConditions[] = "{$effectiveJabatanLembagaSql} = ?";
            ...
            $sql = "SELECT
                        p.id, p.nip, p.nama, p.id_user, COALESCE(u.no_wa, '') AS whatsapp,
                    ...
                    {$joinJabatan}
                    WHERE {$whereClause}
                    GROUP BY p.id, p.nip, p.nama, p.id_user, u.no_wa, p.dusun, p.rt, p.rw, p.desa, p.kecamatan, p.kabupaten, p.provinsi, p.kode_pos
                    ORDER BY p.nama ASC";
            ...
            foreach ($list as &$row) {
                ...
                $row['id_user'] = !empty($row['id_user']) ? (int) $row['id_user'] : null;
