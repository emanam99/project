...
class MadrasahController
{
        ...
        try {
            ...
            $sql = "
                SELECT m.*,
                    ...
                    COALESCE(
                        NULLIF(TRIM(uj.username), ''),
                        (
                            SELECT u_login.username
                            FROM users u_login
                            WHERE u_login.id_madrasah = m.id
                            ORDER BY u_login.id ASC
                            LIMIT 1
                        )
                    ) AS login_username,
                    COALESCE(
                ...
                if (strpos($e->getMessage(), 'nip') !== false || strpos($e->getMessage(), 'Unknown column') !== false) {
                    $sqlFallback = "
                        SELECT m.*,
                            ...
                            up.username AS pengasuh_nama, COALESCE(up.no_wa, '') AS pengasuh_wa,
                            uj.username AS pjgt_nama, COALESCE(uj.no_wa, '') AS pjgt_wa,
                            uj.username AS login_username,
                            m.id_pjgt AS login_user_id,
                            pk.nama AS koordinator_nama, pk.nip AS koordinator_nip, COALESCE(uk.no_wa, '') AS koordinator_wa
                        FROM madrasah m
                        LEFT JOIN alamat a ON a.id = m.id_alamat
                        LEFT JOIN users up ON up.id = m.id_pengasuh
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'data' => MadrasahTingkatanHelper::enrichList($list),
            ], 200);
        } catch (\Exception $e) {
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'data' => MadrasahTingkatanHelper::enrichRow($row),
            ], 200);
        } catch (\Exception $e) {
    ...
    public function create(Request $request, Response $response): Response
    {
        try {
            ...
            $kegiatanJam = $this->parseKegiatanJamFromRequest($data, $opt);
            ...
            $insertPairs = [
                ...
                'bendahara' => $bendahara,
                'kegiatan_pagi' => $kegiatanJam['kegiatan_pagi'],
                'kegiatan_sore' => $kegiatanJam['kegiatan_sore'],
                'kegiatan_malam' => $kegiatanJam['kegiatan_malam'],
                'kegiatan_mulai' => $kegiatanJam['kegiatan_mulai'],
                'kegiatan_sampai' => $kegiatanJam['kegiatan_sampai'],
                'kegiatan_pagi_mulai' => $kegiatanJam['kegiatan_pagi_mulai'],
                'kegiatan_pagi_sampai' => $kegiatanJam['kegiatan_pagi_sampai'],
                'kegiatan_sore_mulai' => $kegiatanJam['kegiatan_sore_mulai'],
                'kegiatan_sore_sampai' => $kegiatanJam['kegiatan_sore_sampai'],
                'kegiatan_malam_mulai' => $kegiatanJam['kegiatan_malam_mulai'],
                'kegiatan_malam_sampai' => $kegiatanJam['kegiatan_malam_sampai'],
                'tempat' => $tempat,
        ...
        try {
            ...
            $stmt = $this->db->prepare("
                UPDATE madrasah SET
                    ...
                    tingkatan = ?, kelas_tertinggi = ?, kurikulum = ?, jumlah_murid = ?, foto_path = ?, logo_path = ?,
                    kepala = ?, sekretaris = ?, bendahara = ?, kegiatan_pagi = ?, kegiatan_sore = ?, kegiatan_malam = ?, kegiatan_mulai = ?, kegiatan_sampai = ?,
                    kegiatan_pagi_mulai = ?, kegiatan_pagi_sampai = ?, kegiatan_sore_mulai = ?, kegiatan_sore_sampai = ?, kegiatan_malam_mulai = ?, kegiatan_malam_sampai = ?,
            ...
            $stmt->execute([
                ...
                $desa, $kecamatan, $kabupaten, $provinsi, $kode_pos, $id_koordinator, $sektor,
                $nama_pengasuh, $id_pengasuh, $no_pengasuh, $nama_pjgt, $id_pjgt, $no_pjgt,
                $tingkatanJson, $kelas_tertinggi, $kurikulum, $jumlah_murid, $foto_path, $logo_path,
                $kepala, $sekretaris, $bendahara,
                $kegiatanJam['kegiatan_pagi'], $kegiatanJam['kegiatan_sore'], $kegiatanJam['kegiatan_malam'],
                $kegiatanJam['kegiatan_mulai'], $kegiatanJam['kegiatan_sampai'],
                $kegiatanJam['kegiatan_pagi_mulai'], $kegiatanJam['kegiatan_pagi_sampai'],
                $kegiatanJam['kegiatan_sore_mulai'], $kegiatanJam['kegiatan_sore_sampai'],
                $kegiatanJam['kegiatan_malam_mulai'], $kegiatanJam['kegiatan_malam_sampai'],
                $tempat, $berdiri_tahun, $keterangan,
                $banin_banat, $seragam, $syahriah, $pengelola, $gedung_madrasah, $kantor, $bangku, $kamar_mandi_murid, $kamar_gt, $kamar_mandi_gt, $km_bersifat, $konsumsi, $kamar_gt_jarak, $masyarakat, $alumni, $jarak_md_lain,
                $id
            ]);
    ...
    /**
     * @param callable(string, int=): ?string $opt
     * @return array<string, int|string|null>
     */
    private function parseKegiatanJamFromRequest(array $data, callable $opt): array
    {
        ...
        $pagiMulai = $pagi ? $opt('kegiatan_pagi_mulai', 10) : null;
        $pagiSampai = $pagi ? $opt('kegiatan_pagi_sampai', 10) : null;
        $soreMulai = $sore ? $opt('kegiatan_sore_mulai', 10) : null;
        $soreSampai = $sore ? $opt('kegiatan_sore_sampai', 10) : null;
        $malamMulai = $malam ? $opt('kegiatan_malam_mulai', 10) : null;
        $malamSampai = $malam ? $opt('kegiatan_malam_sampai', 10) : null;

        $legacyMulai = $opt('kegiatan_mulai', 10);
        $legacySampai = $opt('kegiatan_sampai', 10);
