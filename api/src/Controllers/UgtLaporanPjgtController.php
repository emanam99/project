...
class UgtLaporanPjgtController
{
    ...
    /**
     * Konteks Mybeddian PJGT: token punya madrasah_id — data laporan dibatasi ke madrasah itu.
     * Hanya dipakai pada rute /api/mybeddian/…; eBeddien /api/ugt/… memakai authContext (admin/super/koordinator).
     */
    private function mybeddianPjgtMadrasahScope(Request $request): ?int
    {
    ...
    {
        if ($this->mybeddianPjgtMadrasahScope($request) !== null) {
            return null;
        }
    ...
    private function userMayAccessMadrasah(Request $request, int $madrasahId): bool
    {
        ...
        $scope = $this->mybeddianPjgtMadrasahScope($request);
        if ($scope !== null) {
            return $madrasahId === $scope;
        }
        $ctx = $this->authContext($request);
    ...
    public function getAll(Request $request, Response $response): Response
    {
        try {
            ...
            $pjgtScope = $this->mybeddianPjgtMadrasahScope($request);
            if ($pjgtScope !== null) {
                if ($idMadrasah > 0 && $idMadrasah !== $pjgtScope) {
                    return $this->jsonResponse($response, ['success' => false, 'message' => 'Akses ditolak untuk madrasah ini'], 403);
                }
                $idMadrasah = $pjgtScope;
            ...
            $params = [];
            if ($applyListKoordinator && $pjgtScope === null) {
    ...
    public function create(Request $request, Response $response): Response
    {
        try {
            ...
            $userArr = is_array($user) ? $user : [];
            $pjgtScope = $this->mybeddianPjgtMadrasahScope($request);
            if ($pjgtScope === null && !RoleHelper::tokenUgtLaporanCanTambah($this->db, $userArr, 'pjgt')) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Tidak berhak menambah laporan PJGT',
                ], 403);
            }
    ...
    /**
     * GET /api/mybeddian/v2/laporan-pjgt/tahun-ajaran-options
     * Daftar tahun ajaran hijriyah dari master (hanya baris dengan rentang dari–sampai terisi).
     */
    public function getTahunAjaranOptions(Request $request, Response $response): Response
    {
        try {
            $rows = TahunAjaranActiveHelper::fetchHijriyahRowsWithRentang($this->db);
            ...
            error_log('UgtLaporanPjgtController::getTahunAjaranOptions ' . $e->getMessage());

            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal memuat daftar tahun ajaran',
            ], 500);
        }
    }

    /**
     * GET /api/mybeddian/v2/laporan-pjgt/konteks-sekarang
     * GET /api/ugt/laporan-pjgt/konteks-sekarang
     *
     * Tahun ajaran hijriyah aktif: tanggal Masehi (parameter tanggal, default hari ini) berada di antara kolom dari–sampai.
     * Bulan Hijriyah (1–12): dari tabel psa___kalender (sama logika /api/kalender?action=today), termasuk opsi geser setelah Maghrib.
     *
     * Query opsional: tanggal=Y-m-d, waktu=H:i:s (untuk aturan maghrib kalender).
     */
    public function getKonteksSekarang(Request $request, Response $response): Response
    {
        try {
            ...
            // Bulan Hijriyah dari kalender master (geser +1 hari Masehi jika sudah lewat batas maghrib — konsisten dengan KalenderController)
            ...
            // Tahun ajaran: rentang masehi di master (fallback TA terbaru bila hari ini di luar rentang)
            ...
            return $this->jsonResponse($response, [
                ...
                'data' => [
                    ...
                    'waktu' => $waktu,
                    'id_tahun_ajaran' => $taResolved['tahun_ajaran'],
            ...
            error_log('UgtLaporanPjgtController::getKonteksSekarang ' . $e->getMessage());
    ...
    /** Sama seperti KalenderController::isAfterMaghrib */
