...
use App\Database;
use App\Helpers\ChatPengirimNamaHelper;
...
class ChatController
{
    ...
    public function saveChat(Request $request, Response $response): Response
    {
        try {
            ...
            if (is_array($user)) {
                $fromToken = RoleHelper::getPengurusIdFromPayload($user);
                if ($fromToken !== null && $fromToken > 0) {
                    $idPengurusPengirim = $fromToken;
                }
            }
    ...
    /**
     * GET /api/chat/get-all - Riwayat chat by nomor_tujuan (satu nomor = satu thread).
     * id_santri (+ phone_field) hanya untuk resolve nomor di server bila FE punya nomor ter-mask;
     * query tetap by nomor agar 2 santri berbagi nomor melihat riwayat yang sama.
     */
    public function getAll(Request $request, Response $response): Response
    {
        try {
            ...
            $idSantri = isset($queryParams['id_santri']) ? (int) $queryParams['id_santri'] : 0;
            $phoneField = trim((string) ($queryParams['phone_field'] ?? $queryParams['field'] ?? 'no_wa_santri'));
            $defaultLimit = ($nomorTujuan !== '' || $idSantri > 0) ? 30 : 100;
            ...
            // Resolve nomor dari santri — id_santri hanya kunci lookup, bukan filter riwayat
            if (($nomorTujuan === '' || \App\Helpers\NikHelper::looksMasked($nomorTujuan)) && $idSantri > 0) {
                $resolved = WhatsAppService::resolveSantriPhoneField($this->db, $idSantri, $phoneField !== '' ? $phoneField : 'no_wa_santri');
                if ($resolved) {
                    $nomorTujuan = $resolved;
                }
            }

            if ($nomorTujuan === '' || \App\Helpers\NikHelper::looksMasked($nomorTujuan)) {
                return $this->jsonResponse($response, [
                    'success' => true,
                    'data' => [],
                    'message' => $idSantri > 0 ? 'Nomor tidak ditemukan pada data santri' : null,
                ], 200);
            }
            ...
            // Variasi penyimpanan: 62xxx, 08xxx, digit mentah dari request
            $nomorLocal = (strpos($nomor62, '62') === 0 && strlen($nomor62) > 2)
                ? ('0' . substr($nomor62, 2))
                : '';
            $nomorVariants = array_values(array_unique(array_filter([
                $nomorTujuan,
                $nomor62,
                $nomorLocal,
                preg_replace('/\D/', '', $nomorTujuan) ?: '',
            ], static fn ($v) => $v !== '')));
            ...
            $placeholders = implode(',', array_fill(0, count($nomorVariants), '?'));
            $sql = "SELECT $selCols FROM whatsapp WHERE nomor_tujuan IN ($placeholders)";
            $params = $nomorVariants;
            ...
            $this->attachNamaPengirim($merged);

            return $this->jsonResponse($response, [
                'success' => true,
                'data' => $merged,
                'phone_masked' => \App\Helpers\NikHelper::maskPhone($nomor62),
            ], 200);
        } catch (\Exception $e) {
     ...
     * Body: { nomor_tujuan?, id_santri?, phone_field?, limit? } — id_santri hanya resolve nomor; sync by nomor.
    ...
    public function syncFromWa(Request $request, Response $response): Response
    {
        try {
            ...
            $idSantri = isset($input['id_santri']) ? (int) $input['id_santri'] : 0;
            $phoneField = trim((string) ($input['phone_field'] ?? $input['field'] ?? 'no_wa_santri'));
            if (($nomorTujuan === '' || \App\Helpers\NikHelper::looksMasked($nomorTujuan)) && $idSantri > 0) {
                $resolved = WhatsAppService::resolveSantriPhoneField($this->db, $idSantri, $phoneField !== '' ? $phoneField : 'no_wa_santri');
                if ($resolved) {
                    $nomorTujuan = $resolved;
            ...
            $syncedCount = 0;
            $insertSql = $idSantri > 0
                ? 'INSERT INTO whatsapp (arah, nomor_tujuan, isi_pesan, wa_message_id, tujuan, kategori, sumber, status, created_at, id_santri) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
                : 'INSERT INTO whatsapp (arah, nomor_tujuan, isi_pesan, wa_message_id, tujuan, kategori, sumber, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
    ...
    /**
     * Isi nama_pengirim pada array pesan keluar dari lookup pengurus/users.
     *
     * @param array<int, array<string, mixed>> $data
     */
    private function attachNamaPengirim(array &$data): void
    {
        $ids = [];
        foreach ($data as $m) {
            if (isset($m['id_pengurus']) && (int) $m['id_pengurus'] > 0) {
                $ids[] = (int) $m['id_pengurus'];
            }
        }
        if ($ids === []) {
            return;
        ...
        try {
            $namaById = ChatPengirimNamaHelper::resolveByStoredIds($this->db, $ids);
            foreach ($data as &$m) {
                if (isset($m['id_pengurus']) && (int) $m['id_pengurus'] > 0) {
            ...
            $this->log('ATTACH_NAMA_PENGIRIM: ' . $e->getMessage());
        }
    }
}
