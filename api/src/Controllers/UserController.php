...
class UserController
{
    ...
    public function updateProfile(Request $request, Response $response): Response
    {
        try {
            ...
            if (!PengurusScopeAccessHelper::canEditPengurusBiodata($this->db, $request, $targetPengurusId)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Akses ditolak. Hanya dapat mengedit profil sendiri atau butuh izin Pengurus · Edit untuk pengurus dalam cakupan lembaga Anda.'
                ], 403);
            }
            ...
            $fieldsPengurus = [
                ...
                'dusun', 'rt', 'rw', 'desa', 'kecamatan', 'kabupaten', 'provinsi', 'kode_pos', 'jarak',
            ...
            foreach ($fieldsPengurus as $field) {
                if (isset($input[$field])) {
                    $updateFields[] = "{$field} = ?";
                    $raw = $input[$field] === '' ? null : $input[$field];
                    if ($field === 'jarak') {
                        if ($raw === null) {
                            $params[] = null;
                            ...
                            return $this->jsonResponse($response, [
                                'success' => false,
                                'message' => 'Jarak harus berupa angka (km)',
                            ], 400);
                        } else {
                            $jv = round((float) $raw, 2);
                            if ($jv < 0) {
                                return $this->jsonResponse($response, [
                                    'success' => false,
                                    'message' => 'Jarak tidak boleh negatif',
                                ], 400);
                            }
                            $params[] = $jv;
                        }
                    } else {
                        $params[] = \is_string($raw) ? ProperCaseHelper::forBiodataField($field, $raw) : $raw;
                    }
                }
            }
            ...
            if (array_key_exists('email', $input)) {
                ...
                if ($rowU && !empty($rowU['id_user'])) {
                    ...
                    $stmtPrev = $this->db->prepare('SELECT email FROM users WHERE id = ? LIMIT 1');
                    $stmtPrev->execute([$idUser]);
                    $prevRow = $stmtPrev->fetch(\PDO::FETCH_ASSOC);
                    $prevNorm = strtolower(trim((string) ($prevRow['email'] ?? '')));
                    $nextNorm = strtolower(trim((string) ($emailVal ?? '')));
                    try {
                        if ($prevNorm !== $nextNorm) {
                            try {
                                $this->db->prepare('UPDATE users SET email = ?, email_verified_at = NULL, email_reminder_snoozed_until = NULL WHERE id = ?')->execute([$emailVal, $idUser]);
                            } catch (\Throwable $e) {
                                $this->db->prepare('UPDATE users SET email = ?, email_verified_at = NULL WHERE id = ?')->execute([$emailVal, $idUser]);
                            }
                            try {
                                $this->db->prepare('DELETE FROM user___email_verify_tokens WHERE user_id = ?')->execute([$idUser]);
                            } catch (\Throwable $e) {
                                // Tabel token belum dimigrasi — abaikan
                            }
                        } else {
    ...
    public function verifyPassword(Request $request, Response $response): Response
    {
        try {
            ...
            $targetPengurusId = (int) $userId;
            if ($targetPengurusId <= 0 || !SantriJwtAccessHelper::assertPasswordTargetAllowed($request, $targetPengurusId)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Akses ditolak. Hanya dapat memverifikasi password akun sendiri.'
                ], 403);
            }
    ...
    public function updatePassword(Request $request, Response $response): Response
    {
        try {
            ...
            $minLen = (int) ($config['security']['password_min_length'] ?? 8);
            if (strlen((string) $newPassword) < $minLen) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Password minimal ' . $minLen . ' karakter'
                ], 400);
            }
            ...
            if ($targetPengurusId <= 0 || !SantriJwtAccessHelper::assertPasswordTargetAllowed($request, $targetPengurusId)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Akses ditolak. Hanya dapat mengubah password akun sendiri.'
                ], 403);
            }
    ...
    /**
     * GET /api/user/{id} - Get user data by ID (pengurus.id).
     * Akses: profil sendiri, super_admin, atau action.pengurus.edit + cakupan lembaga.
     */
    public function getUserById(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $requestedId = (int) $id;
            if (!PengurusScopeAccessHelper::canEditPengurusBiodata($this->db, $request, $requestedId)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Akses ditolak. Hanya dapat melihat profil sendiri, atau butuh izin Pengurus · Edit untuk pengurus dalam cakupan lembaga Anda.'
                ], 403);
            }
            ...
            $stmt = $this->db->prepare("
                SELECT 
                    ...
                    u.no_wa_verified_at, u.email_verified_at
                FROM pengurus p
                LEFT JOIN users u ON u.id = p.id_user
                WHERE p.id = ?
            ");
            ...
            if ($user) {
                ...
                $viewerPengurusId = is_array($payload) ? RoleHelper::getPengurusIdFromPayload($payload) : null;
                $isSelf = $viewerPengurusId !== null && (int) $viewerPengurusId === $requestedId;
                $isSuperAdmin = is_array($payload) && RoleHelper::tokenHasAnyRoleKey($payload, ['super_admin']);
                $wantFullPii = false;
                if (is_array($payload)) {
                    $qp = $request->getQueryParams();
                    $wantFullPii = in_array(strtolower(trim((string) ($qp['full_pii'] ?? ''))), ['1', 'true', 'yes'], true);
                }
                // PII penuh: diri sendiri / super_admin / full_pii=1 (sudah lolos canEditPengurusBiodata).
                $revealPii = $isSelf || $isSuperAdmin || $wantFullPii;
                if ($wantFullPii && !$isSelf && !$isSuperAdmin) {
                    AuditLogger::log(
                        (string) ($viewerPengurusId ?? $payload['user_id'] ?? '0'),
                        'user_profil_full_pii',
                        ['target_pengurus_id' => $requestedId],
                        null,
                        true
                    );
                }
                if (!$revealPii) {
                    if (!empty($user['nik'])) {
                        $user['nik'] = NikHelper::mask((string) $user['nik']);
                    }
                    if (!empty($user['no_kk'])) {
                        $user['no_kk'] = NikHelper::mask((string) $user['no_kk']);
                    }
                    if (!empty($user['email'])) {
                        $user['email'] = NikHelper::maskEmail((string) $user['email']);
                    }
                    if (!empty($user['whatsapp'])) {
                        $user['whatsapp'] = NikHelper::maskPhone((string) $user['whatsapp']);
                    }
                    $user['pii_masked'] = true;
                } else {
                    $user['pii_masked'] = false;
                }
                // Rekening hanya untuk diri sendiri / super_admin (bukan semua yang punya pengurus.edit).
                if (!$isSelf && !$isSuperAdmin) {
                    unset($user['rekening_jatim'], $user['an_jatim']);
                }
                // Jabatan dari pengurus___jabatan (dengan lembaga: nama, kategori, deskripsi; jabatan: deskripsi)
                $jabatanStmt = $this->db->prepare("
                    SELECT 
                        j.nama AS jabatan_nama,
                        COALESCE(l.kategori, l2.kategori) AS jabatan_kategori,
