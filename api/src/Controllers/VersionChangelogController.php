...
class VersionChangelogController
{
     ...
     * Query: app (optional) = api | daftar | ebeddien | mybeddien | nailul-murod (alias legacy ebeddien: uwaba). Kosong = semua.
    ...
    public function getChangelog(Request $request, Response $response): Response
    {
        ...
        $allowedApps = ['api', 'daftar', 'uwaba', 'ebeddien', 'mybeddien', 'nailul-murod'];
        if ($app !== null && $app !== '') {
            if (!in_array($app, $allowedApps, true)) {
                $response->getBody()->write(json_encode([
                    'success' => false,
                    'message' => 'Parameter app harus salah satu: api, daftar, ebeddien, mybeddien, nailul-murod'
