...
use App\Services\DeepseekOpenAiTransport;
...
{
    /** Default chat (generateContent) — selaras frontend `DEFAULT_GEMINI_MODEL_ID`. */
    private const GEMINI_DEFAULT_CHAT_MODEL_ID = 'gemini-2.5-flash-lite';
    ...
    /**
     * Sisipkan blok [KONTEKS_NAVIGASI_UI] ke konten user terakhir (setelah merge RAG).
     *
     * @param array<int, array<string, mixed>> $normMessages
     */
    private function prependNavigationContextToLastUserTurn(array &$normMessages, array $body): void
    {
        $ctx = AiClientNavigationContextHelper::normalize($body['navigation_context'] ?? $body['ui_context'] ?? null);
        if ($ctx === null) {
            return;
        }
        $block = AiClientNavigationContextHelper::formatPromptPrefix($ctx);
        if ($block === '') {
            return;
        }
        for ($i = count($normMessages) - 1; $i >= 0; $i--) {
            if (($normMessages[$i]['role'] ?? '') === 'user') {
                $normMessages[$i]['content'] = $block . "\n\n---\n" . (string) ($normMessages[$i]['content'] ?? '');
                break;
            }
        }
    }
    ...
    /**
     * POST JSON lewat stream wrapper (tanpa cURL). Dipakai jika extension curl tidak aktif di php.ini Apache.
     *
     * @param array<int, string> $headers Baris «Nama: nilai»
     *
     * @return array{raw:string|false,http_code:int,curl_error:string,curl_errno:int}
     */
    private function postJsonViaStreamWrapper(string $url, array $headers, string $jsonPayload): array
    {
        ...
        if (!$fopenOk) {
            return [
                'raw' => false,
                'http_code' => 0,
                'curl_error' => 'cURL tidak tersedia dan allow_url_fopen=Off. Aktifkan salah satu: di php.ini Apache uncomment extension=curl, atau set allow_url_fopen=On, lalu restart Apache.',
                'curl_errno' => -3,
            ];
        }
        ...
        if ($raw === false) {
            $err = error_get_last();
            $msg = is_array($err) ? (string) ($err['message'] ?? 'HTTPS gagal') : 'file_get_contents gagal';
    ...
    /**
     * GET JSON (untuk ListModels Gemini, dll.).
     *
     * @param array<int, string> $headers Baris «Nama: nilai»
     *
     * @return array{raw:string|false,http_code:int,curl_error:string,curl_errno:int}
     */
    private function getJsonViaStreamWrapperGet(string $url, array $headers): array
    {
        $fopenOk = filter_var(ini_get('allow_url_fopen'), FILTER_VALIDATE_BOOLEAN);
        if (!$fopenOk) {
            return [
                'raw' => false,
                'http_code' => 0,
                'curl_error' => 'cURL tidak tersedia dan allow_url_fopen=Off.',
                'curl_errno' => -3,
            ];
        }
    ...
    private function curlGetJson(string $url, array $headers): array
    {
        if (!function_exists('curl_init') || !function_exists('curl_exec')) {
            return $this->getJsonViaStreamWrapperGet($url, $headers);
        }
    ...
    /** Cache hasil ListModels agar getAccount + gemini-chat ringan. */
    private static ?array $geminiListModelsCache = null;

    private static float $geminiListModelsCacheAt = 0.0;

    private const GEMINI_LIST_MODELS_TTL_SEC = 600.0;

    /**
     * Model Gemini yang mendukung generateContent (v1beta ListModels).
     *
     * @return array{0: array<int, array{id:string,label:string}>, 1: ?string} [models, errorMessage]
     */
    private function fetchGeminiModelsForChatSelect(string $apiKey): array
    {
        $apiKey = trim($apiKey);
        if ($apiKey === '') {
        ...
        do {
            ...
            $url = 'https://generativelanguage.googleapis.com/v1beta/models?' . http_build_query($q);
            $headers = [
                'Accept: application/json',
                'x-goog-api-key: ' . $apiKey,
            ];
            $res = $this->curlGetJson($url, $headers);
            if ($res['raw'] === false || $res['curl_error'] !== '') {
                return [[], 'Gagal menghubungi Google ListModels: ' . ($res['curl_error'] !== '' ? $res['curl_error'] : 'respons kosong')];
            }
            $decoded = json_decode((string) $res['raw'], true);
            if (!is_array($decoded)) {
                return [[], 'Respons ListModels bukan JSON valid.'];
            }
            if (!empty($decoded['error']) && is_array($decoded['error'])) {
                $em = isset($decoded['error']['message']) ? (string) $decoded['error']['message'] : 'ListModels error';

                return [[], $em];
            }
            if ((int) $res['http_code'] >= 400) {
                $em = isset($decoded['error']['message']) ? (string) $decoded['error']['message'] : 'ListModels HTTP ' . (string) $res['http_code'];
            ...
            foreach ($decoded['models'] ?? [] as $m) {
                ...
                if ($name === '' || !preg_match('#^models/gemini-[a-zA-Z0-9._-]+$#', $name)) {
                    continue;
                }
                $id = substr($name, strlen('models/'));
                if ($id === '' || stripos($id, 'embedding') !== false) {
                    continue;
                }
                ...
                $label = isset($m['displayName']) && trim((string) $m['displayName']) !== ''
                    ? trim((string) $m['displayName'])
                    : $id;
                $collected[$id] = ['id' => $id, 'label' => $label];
            }

            $pageToken = isset($decoded['nextPageToken']) && is_string($decoded['nextPageToken'])
                ? $decoded['nextPageToken']
                : null;
            $pages++;
        } while ($pageToken !== null && $pageToken !== '' && $pages < 30);
        ...
        $pref = self::GEMINI_DEFAULT_CHAT_MODEL_ID;
        $pin = null;
    ...
    /**
     * @return array{models: array<int, array{id:string,label:string}>, error: ?string}
     */
    private function getGeminiListModelsCached(string $apiKey): array
    {
        $now = microtime(true);
        $c = self::$geminiListModelsCache;
        if ($c !== null && ($now - self::$geminiListModelsCacheAt) < self::GEMINI_LIST_MODELS_TTL_SEC) {
            return ['models' => $c['models'], 'error' => $c['error']];
        }

        [$models, $err] = $this->fetchGeminiModelsForChatSelect($apiKey);
        self::$geminiListModelsCacheAt = $now;
        self::$geminiListModelsCache = ['models' => $models, 'error' => $err];
    ...
    private function curlPostJsonWithRetry(string $url, array $headers, array $payload): array
    {
        ...
        if ($jsonPayload === false) {
            return [
                'raw' => false,
                'http_code' => 0,
                'curl_error' => 'json_encode payload gagal',
                'curl_errno' => -2,
            ];
        }

        if (!function_exists('curl_init') || !function_exists('curl_exec')) {
            return $this->postJsonViaStreamWrapper($url, $headers, $jsonPayload);
        }
    ...
    private function fetchLastNChatExchangesAsOpenAiMessages(\PDO $db, ?int $usersId, string $sessionId, int $n): array
    {
        ...
        $lim = max(1, min(20, $n));
        try {
            if ($sessionId === self::EBEDDIEN_MAIN_SESSION_ID) {
                $sql = 'SELECT user_message, ai_response FROM ai___chat WHERE users_id = ? AND session_id IN (?, \'ebeddien-agent\') ORDER BY id DESC LIMIT '
                    . (int) $lim;
                $stmt = $db->prepare($sql);
    ...
    private function persistAiExchange(
        ...
        string $aiReply,
        string $thinking,
        string $modelUsed = 'ebeddien_assistant'
    ): void {
        ...
        $modelUsedDb = mb_substr(trim($modelUsed), 0, 100);
        if ($modelUsedDb === '') {
            $modelUsedDb = 'ebeddien_assistant';
        }
        try {
    ...
    public function getAccount(Request $request, Response $response): Response
    {
        try {
            ...
            $hasProviderCol = AiChatUserProviderPrefHelper::columnExists($db, 'ai_chat_api_provider_pref');
            $modeColSql = $hasChatModeCol ? ', ai_chat_mode_pref' : '';
            $providerColSql = $hasProviderCol ? ', ai_chat_api_provider_pref, ai_chat_gemini_model_pref' : '';
            $stmt = $db->prepare("SELECT id, email{$modeColSql}{$providerColSql} FROM users WHERE id = ? LIMIT 1");
            $stmt->execute([$usersId]);
            ...
            $agentUse = AiAgentAccessHelper::canUseAgent($db, $payload);
            $agentConfirm = AiAgentAccessHelper::canConfirmAgentWrites($db, $payload);
            $providerPref = AiChatUserProviderPrefHelper::getForUser($db, $usersId);
            $geminiApiKey = trim((string) (getenv('GEMINI_API_KEY') ?: ''));
            $geminiConfigured = $geminiApiKey !== '';
            $geminiModels = [];
            $geminiModelsError = null;
            if ($geminiConfigured) {
                $gp = $this->getGeminiListModelsCached($geminiApiKey);
                $geminiModels = $gp['models'];
                $geminiModelsError = $gp['error'];
            }
            if ($email === '') {
                return $this->json($response, [
                    'success' => true,
                    ...
                    'data' => [
                        ...
                        'ai_agent_can_use' => $agentUse,
                        'ai_agent_can_confirm_write' => $agentConfirm,
                        'gemini_configured' => $geminiConfigured,
                        'gemini_models' => $geminiModels,
                        'gemini_models_error' => $geminiModelsError,
                        'ai_chat_api_provider' => $providerPref['provider'],
                        'ai_chat_gemini_model' => $providerPref['gemini_model'],
                    ],
                ], 200);
            }
    ...
    /**
     * PUT /api/deepseek/api-provider-preference — sinkron pilihan DeepSeek/Gemini (UI + WA).
     * Body: { "provider": "deepseek"|"gemini", "gemini_model": "gemini-2.5-flash-lite" (opsional) }
     */
    public function putApiProviderPreference(Request $request, Response $response): Response
    {
        try {
            $userPayload = $request->getAttribute('user');
            if (!is_array($userPayload)) {
            ...
            $geminiModel = isset($body['gemini_model']) ? trim((string) $body['gemini_model']) : null;
            if ($geminiModel === '') {
                $geminiModel = null;
            }
            $res = AiChatUserProviderPrefHelper::saveForUser($db, $usersId, $provider, $geminiModel);
            if (!$res['ok']) {
            ...
            return $this->json($response, [
                'success' => true,
                'message' => 'Preferensi provider disimpan',
                'data' => [
                    'ai_chat_api_provider' => $pref['provider'],
                    'ai_chat_gemini_model' => $pref['gemini_model'],
                ],
            ], 200);
        } catch (\Throwable $e) {
            error_log('DeepseekController::putApiProviderPreference ' . $e->getMessage());
    ...
    public function proxyChat(Request $request, Response $response): Response
    {
        try {
            ...
            try {
                $ragged = AiChatPromptContextHelper::mergeIntoUserTurn(
                    $db,
                    $userPayload,
                    $ragged,
                    $usersIdForHistory !== null && $usersIdForHistory > 0 ? $usersIdForHistory : null,
                    $sessionStr !== '' ? $sessionStr : null
                );
            } catch (\Throwable $e) {
                error_log('DeepseekController::proxyChat keuangan context ' . $e->getMessage());
            }
    ...
    {
        return DeepseekOpenAiTransport::postChatCompletions($apiKey, $payload);
    }
    ...
    /**
     * Normalisasi pesan dari klien untuk /api/deepseek/api-chat — hanya role «user».
     * Riwayat assistant/system harus berasal dari server (ai___chat), bukan dari body klien (mitigasi prompt injection).
     *
     * @param mixed $messages
     *
     * @return array{0: bool, 1: string, 2: array<int, array{role:string,content:string}>|null}
     */
    private function normalizeDirectApiChatClientMessages($messages): array
    {
        if (!is_array($messages) || $messages === []) {
            return [false, 'messages harus array non-kosong dengan {role, content}', null];
        }
        ...
        foreach ($messages as $row) {
            ...
            $content = isset($row['content']) ? trim((string) $row['content']) : '';
            if ($role !== 'user') {
                return [false, 'Hanya pesan role «user» yang diizinkan dari klien; riwayat sistem/asisten dikelola server.', null];
            }
            if ($content === '') {
                return [false, 'content tidak boleh kosong', null];
            }
    ...
    private const GEMINI_INLINE_IMAGE_MAX_COUNT = 6;

    private const GEMINI_INLINE_IMAGE_MAX_BYTES = 6291456; // 6 MiB decoded per file

    /** @var list<string> */
    private const GEMINI_INLINE_IMAGE_MIMES = [
        'image/jpeg',
        'image/png',
        'image/webp',
        'image/gif',
    ...
    /**
     * Normalisasi pesan klien untuk Gemini: teks + opsional gambar (inline base64).
     * Hanya satu giliran user; gambar hanya pada pesan terakhir.
     *
     * @param mixed $messages
     *
     * @return array{0: bool, 1: string, 2: array<int, array<string, mixed>>|null}
     */
    private function normalizeGeminiDirectApiChatClientMessages($messages): array
    {
        if (!is_array($messages) || $messages === []) {
            return [false, 'messages harus array non-kosong dengan {role, content}', null];
        }
        ...
        foreach ($messages as $idx => $row) {
            ...
            if (!is_array($row)) {
                return [false, 'Setiap item messages harus object', null];
            }
            $role = isset($row['role']) ? strtolower(trim((string) $row['role'])) : '';
            $content = isset($row['content']) ? trim((string) $row['content']) : '';
            if ($role !== 'user') {
                return [false, 'Hanya pesan role «user» yang diizinkan dari klien.', null];
            }

            $rawImages = $row['images'] ?? [];
            if (!is_array($rawImages)) {
                return [false, 'images harus array', null];
            }
            if ($idx !== $lastIdx && $rawImages !== []) {
                return [false, 'Gambar hanya diizinkan pada pesan user terakhir.', null];
            }
            if (count($rawImages) > self::GEMINI_INLINE_IMAGE_MAX_COUNT) {
                return [false, 'Maksimal ' . (string) self::GEMINI_INLINE_IMAGE_MAX_COUNT . ' lampiran per pesan.', null];
            }

            $sanitizedImages = [];
            foreach ($rawImages as $imgRow) {
                if (!is_array($imgRow)) {
                    return [false, 'Setiap gambar harus object { mime_type, data }', null];
                }
                $mime = '';
                ...
                if (!in_array($mime, self::GEMINI_INLINE_IMAGE_MIMES, true)) {
                    return [false, 'Mime lampiran tidak didukung (gambar, pdf, word, excel, csv, txt).', null];
                }
                $b64 = '';
                ...
                if ($b64 === '') {
                    return [false, 'Data base64 gambar kosong', null];
                }
                $decoded = base64_decode($b64, true);
                if ($decoded === false) {
                    return [false, 'Data gambar bukan base64 valid', null];
                }
                $len = strlen($decoded);
                if ($len < 32) {
                    return [false, 'Berkas gambar terlalu kecil atau rusak', null];
                }
                if ($len > self::GEMINI_INLINE_IMAGE_MAX_BYTES) {
                    return [false, 'Satu lampiran terlalu besar (maks. 6 MB setelah decode).', null];
                }
                $sanitizedImages[] = ['mime_type' => $mime, 'data' => $b64];
            }

            if ($content === '' && $sanitizedImages === []) {
                return [false, 'Tulis pesan atau lampirkan setidaknya satu gambar', null];
            }
            if (mb_strlen($content) > 32000) {
                return [false, 'Satu pesan terlalu panjang (maks. 32000 karakter)', null];
            }
            ...
            if ($sanitizedImages !== []) {
                $entry['gemini_inline_images'] = $sanitizedImages;
            }
            $out[] = $entry;
            $n++;
        }
    ...
    /**
     * POST /api/deepseek/api-chat — mode utama; memanggil api.deepseek.com (format OpenAI).
     * Body: messages hanya role «user»; riwayat assistant dari ai___chat di server.
     * Atau { "prompt": "..." } (setara satu giliran user).
     * Production: temperature dari klien diabaikan kecuali API_CHAT_ALLOW_CLIENT_TEMPERATURE=true.
     */
    public function directApiChat(Request $request, Response $response): Response
    {
        try {
            ...
            [$ok, $err, $normMessages] = $this->normalizeDirectApiChatClientMessages($messages);
            ...
            $this->prependNavigationContextToLastUserTurn($normMessages, $body);

            $systemPrompt = AiTrainingRagHelper::getEbeddienAssistantSystemPrompt();
            try {
                $systemPrompt = AiChatPromptContextHelper::augmentSystemPrompt(
                    $systemPrompt,
                    $db,
                    $userPayload,
                    $lastUserOriginal,
                    $usersId !== null && $usersId > 0 ? $usersId : null,
                    $sessionKey !== '' ? $sessionKey : null,
                    'web'
                );
            } catch (\Throwable $e) {
                error_log('DeepseekController::directApiChat keuangan context ' . $e->getMessage());
            }
            ...
            $allowClientTemperature = filter_var(
                (string) (getenv('API_CHAT_ALLOW_CLIENT_TEMPERATURE') ?: 'false'),
                FILTER_VALIDATE_BOOLEAN
            );
            $isProductionEnv = strtolower((string) (getenv('APP_ENV') ?: '')) === 'production';
            if (!$isProductionEnv || $allowClientTemperature) {
            ...
            $packaged = \App\Helpers\AiAssistantReplyStyleHelper::packageForApiResponse($content);

            return $this->json($response, [
                'success' => true,
                'data' => [
                    'message' => $packaged['message'] !== '' ? $packaged['message'] : $content,
                    'reply_parts' => $packaged['reply_parts'],
    ...
    private function isValidGeminiModelId(string $model): bool
    {
        if ($model === '' || strlen($model) > 96) {
            return false;
        }

        return (bool) preg_match('/^gemini-[a-zA-Z0-9._-]+$/', $model);
    }

    /**
     * @param array<int, array{role:string,content:string}> $openAiMessages
     *
     * @return array{system:string,contents:array<int,array{role:string,parts:array<int,array<string,mixed>>}>}
     */
    private function openAiMessagesToGeminiParts(array $openAiMessages): array
    {
        $system = '';
        $contents = [];
        foreach ($openAiMessages as $m) {
            ...
            if ($role === 'user') {
                $imgs = isset($m['gemini_inline_images']) && is_array($m['gemini_inline_images']) ? $m['gemini_inline_images'] : [];
                $parts = [];
                if ($content !== '') {
                    $parts[] = ['text' => $content];
                }
                ...
                if ($content === '' && $imgs !== []) {
                    array_unshift($parts, [
                        'text' => 'Analisis gambar yang dilampirkan. Jawab ringkas dalam Bahasa Indonesia sesuai konteks asisten eBeddien.',
                    ]);
                }
     ...
     * @return array{raw:string|false,http_code:int,curl_error:string,curl_errno:int}
     */
    private function curlGeminiGenerateContent(string $apiKey, string $modelId, array $payload): array
    {
        ...
        return $this->curlPostJsonWithRetry($url, $headers, $payload);
    }

    /**
     * Peta error Gemini (field `error` JSON) ke status HTTP respons kita.
     * Jangan pakai 502 untuk overload Google — itu membingungkan (bukan gateway lokal rusak).
     *
     * @param array<string, mixed> $err
     */
    private function mapGeminiApiErrorToClientStatus(array $err): int
    {
        $code = (int) ($err['code'] ?? 0);
        $status = strtoupper((string) ($err['status'] ?? ''));

        // Selaras pola error Google / gRPC (https://cloud.google.com/apis/design/errors)
        ...
        if ($status === 'INVALID_ARGUMENT' || $status === 'FAILED_PRECONDITION' || $code === 400) {
            return 400;
        }
        if ($status === 'RESOURCE_EXHAUSTED' || $code === 429) {
            return 429;
        }
        if (in_array($status, ['ABORTED', 'ALREADY_EXISTS'], true) || $code === 409) {
            return 409;
        }
        if (in_array($status, ['UNAVAILABLE', 'DEADLINE_EXCEEDED'], true) || in_array($code, [503, 504], true)) {
            return 503;
        }
        if (in_array($status, ['INTERNAL', 'UNKNOWN', 'DATA_LOSS'], true) || in_array($code, [500, 502], true)) {
            return 503;
        }
        if ($code >= 500) {
            return 503;
    ...
    private function geminiClientStatusIsRetryable(int $clientStatus): bool
    {
        return $clientStatus === 429 || $clientStatus === 503;
    }

    /**
     * @param array<string, mixed>|null $decoded
     */
    private function isGeminiTransientRetryable(?array $decoded): bool
    {
        if (!is_array($decoded) || empty($decoded['error']) || !is_array($decoded['error'])) {
            return false;
        }
        $st = $this->mapGeminiApiErrorToClientStatus($decoded['error']);

        return $this->geminiClientStatusIsRetryable($st);
    }

    /** Status HTTP mentah dari cURL (bukan isi JSON error). */
    private function mapGeminiUpstreamRawHttpCodeToClient(int $httpCode): int
    {
        if ($httpCode === 429) {
            return 429;
        }
    ...
    /**
     * @return array{text:string,usage:array<string,int>|null,finish_reason:string}
     */
    private function parseGeminiGenerateContentJson(array $decoded): array
    {
        $text = '';
        $finish = '';
    ...
    /**
     * POST /api/deepseek/gemini-chat — Google Gemini (generateContent), limit & log sama seperti api-chat.
     * Body: messages / prompt, session_id, temperature opsional, model (id Gemini, default gemini-2.5-flash-lite).
     * Multimodal: item messages role=user boleh punya `images`: [{ "mime_type": "image/jpeg", "data": "<base64>" }, …]
     * (maks. 4 gambar, mime jpeg|png|webp|gif, decode maks. 4 MiB per gambar). Teks boleh kosong jika ada gambar.
     * Kunci: GEMINI_API_KEY di lingkungan server.
     */
    public function directGeminiApiChat(Request $request, Response $response): Response
    {
        try {
            $userPayload = $request->getAttribute('user');
            if (!is_array($userPayload)) {
            ...
            if ($apiKey === '') {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Gemini belum dikonfigurasi di server (set GEMINI_API_KEY di .env API).',
                ], 503);
            }
            ...
            $model = isset($body['model']) ? trim((string) $body['model']) : self::GEMINI_DEFAULT_CHAT_MODEL_ID;
            if ($model === '') {
                $model = self::GEMINI_DEFAULT_CHAT_MODEL_ID;
            }
            if (!$this->isValidGeminiModelId($model)) {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Model Gemini tidak valid.',
                ], 400);
            }

            $listPayload = $this->getGeminiListModelsCached($apiKey);
            $allowedIds = array_column($listPayload['models'], 'id');
            if ($listPayload['error'] === null && $allowedIds !== [] && !in_array($model, $allowedIds, true)) {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Model ini tidak tersedia untuk kunci API Gemini Anda (tidak ada di ListModels / tidak mendukung generateContent). Muat ulang halaman untuk memperbarui daftar.',
                ], 400);
            }
            ...
            [$ok, $err, $normMessages] = $this->normalizeGeminiDirectApiChatClientMessages($messages);
            if (!$ok || $normMessages === null) {
                return $this->json($response, ['success' => false, 'message' => $err], 400);
            }
            ...
            if ($usersId !== null && $usersId > 0) {
                ...
                $usageLim = $this->aiDailyUsageForLoggedInUser($db, $usersId);
                if ($usageLim['today'] >= $usageLim['limit']) {
                    return $this->json($response, [
                        'success' => false,
                        'message' => $this->buildAiLimitMessage(),
                    ], 429);
            ...
            $lastUserOriginal = '';
            if ($lastUserIdx !== null) {
                $rawUserText = trim((string) ($normMessages[$lastUserIdx]['content'] ?? ''));
                $imgCount = isset($normMessages[$lastUserIdx]['gemini_inline_images']) && is_array($normMessages[$lastUserIdx]['gemini_inline_images'])
                    ? count($normMessages[$lastUserIdx]['gemini_inline_images'])
                    : 0;
                if ($rawUserText !== '') {
                    $lastUserOriginal = $rawUserText . ($imgCount > 0 ? "\n[" . $imgCount . ' gambar dilampirkan]' : '');
                } elseif ($imgCount > 0) {
                    $lastUserOriginal = '[' . $imgCount . ' gambar dilampirkan]';
                }
                $contentForRag = $rawUserText;
                if ($contentForRag === '' && $imgCount > 0) {
                    $contentForRag = 'Pengguna mengirim ' . $imgCount . ' gambar tanpa teks tambahan. Jawab berdasarkan isi visual gambar.';
                }
                try {
                    $merged = AiTrainingRagHelper::mergeIntoPrompt($db, $contentForRag);
                    $normMessages[$lastUserIdx]['content'] = $merged;
                } catch (\Throwable $e) {
                    error_log('DeepseekController::directGeminiApiChat RAG ' . $e->getMessage());
                }
            }
            ...
            try {
                ...
                error_log('DeepseekController::directGeminiApiChat context ' . $e->getMessage());
            }
            ...
            $gemParts = $this->openAiMessagesToGeminiParts($normMessages);
            if ($gemParts['contents'] === []) {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Konten obrolan kosong setelah normalisasi.',
                ], 400);
            }
            ...
            if ($gemParts['system'] !== '') {
                $gemPayload['systemInstruction'] = [
                    'parts' => [['text' => $gemParts['system']]],
                ];
            }
            ...
            if (!$isProductionEnv || $allowClientTemperature) {
                if (is_numeric($temperature)) {
                    $t = (float) $temperature;
                    if ($t >= 0 && $t <= 2) {
                        $gemPayload['generationConfig']['temperature'] = $t;
                    }
                }
            }
            ...
            for ($gemAttempt = 1; $gemAttempt <= 2; $gemAttempt++) {
                $curlResult = $this->curlGeminiGenerateContent($apiKey, $model, $gemPayload);
                $raw = $curlResult['raw'];
                ...
                if ($raw === false || $curlResult['curl_error'] !== '') {
                    $payloadErr = [
                        'success' => false,
                        'message' => 'Gagal menghubungi layanan Gemini.',
                    ];
                    if (!$isProduction) {
                        $payloadErr['debug'] = [
                            'curl_errno' => $curlResult['curl_errno'],
                ...
                if (!empty($decoded['error']) && is_array($decoded['error'])) {
                    if ($this->isGeminiTransientRetryable($decoded) && $gemAttempt < 2) {
                        usleep(1500000);

                        continue;
                    }
                    $errBlob = $decoded['error'];
                    $errMsg = isset($errBlob['message']) ? (string) $errBlob['message'] : 'Gemini mengembalikan error.';
                    $clientStatus = $this->mapGeminiApiErrorToClientStatus($errBlob);

                    return $this->json($response, [
                        'success' => false,
                        'message' => $errMsg,
                        'provider_error' => $errBlob,
                        'retryable' => $this->geminiClientStatusIsRetryable($clientStatus),
                    ], $clientStatus);
                }

                if ($httpCode >= 400) {
                    $clientStatus = $this->mapGeminiUpstreamRawHttpCodeToClient($httpCode);
                    if ($this->geminiClientStatusIsRetryable($clientStatus) && $gemAttempt < 2) {
                        usleep(1500000);

                        continue;
                    }
                    $errMsg = isset($decoded['error']['message']) ? (string) $decoded['error']['message'] : 'Gemini mengembalikan error HTTP.';
            ...
            if (is_string($blockReason) && $blockReason !== '') {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Permintaan diblokir kebijakan Gemini: ' . $blockReason,
                ], 400);
            }

            $parsed = $this->parseGeminiGenerateContentJson($decoded);
            $content = $parsed['text'];
            $finish = $parsed['finish_reason'];
            if ($content === '' && ($finish === 'SAFETY' || $finish === 'RECITATION')) {
                return $this->json($response, [
                    'success' => false,
                    'message' => 'Gemini tidak menghasilkan teks (alasan: ' . $finish . ').',
                ], 400);
            }
            ...
            try {
                ...
                $this->persistAiExchange(
                    ...
                    $userEmail,
                    $sessionKey,
                    $lastUserText !== '' ? $lastUserText : '(gemini-chat)',
                    $content,
                    '',
                    'gemini:' . $model
                );
                ...
                $sqlState = $e instanceof \PDOException ? $e->errorInfo[0] ?? '' : '';
                error_log(
                    'DeepseekController::directGeminiApiChat persist ' . $e->getMessage()
                    . ($sqlState !== '' ? ' [SQLSTATE=' . $sqlState . ']' : '')
                );
            }
            ...
            error_log('DeepseekController::directGeminiApiChat ' . $e->getMessage());
    ...
    public function chatHistory(Request $request, Response $response): Response
    {
        try {
            ...
            if ($sessionFilter !== '') {
                // Utas utama: sertakan session legacy agen ('ebeddien-agent') agar riwayat lama tetap muncul setelah reload.
                if ($sessionFilter === self::EBEDDIEN_MAIN_SESSION_ID) {
                    $stmt = $db->prepare(
                        'SELECT id, user_message, ai_response, `timestamp` AS row_at FROM ai___chat WHERE users_id = ? '
                        . 'AND session_id IN (?, \'ebeddien-agent\') ORDER BY id DESC LIMIT '
                        . (int) $limit
                    );
                    $stmt->execute([$usersId, self::EBEDDIEN_MAIN_SESSION_ID]);
                } else {
                    $stmt = $db->prepare(
