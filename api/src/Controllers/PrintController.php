...
class PrintController
{
    ...
    /**
     * Satu blok PSB (registrasi + item + transaksi) untuk print / QR publik.
     *
     * @return array{biodata: array<string, mixed>, registrasi: array<string, mixed>, tunggakan: array<int, array<string, mixed>>, pembayaran: array<int, array<string, mixed>>}
     */
    private function buildPendaftaranPrintSegment(array $registrasi, array $biodataTemplate): array
    {
     ...
     * Route dilindungi AuthMiddleware (lihat routes/02_auth_v2_profil.php).
     * Role 'santri' hanya boleh print dirinya sendiri (ownership check di bawah).
     */
    public function getPrintData(Request $request, Response $response): Response
    {
        try {
            $registrasiPayload = null;
            $pendaftaran_multi = false;
            $pendaftaran_blocks = null;
            ...
            // Ownership check: jika user login berperan 'santri', batasi ke dirinya sendiri.
            $authUser = $request->getAttribute('user');
            if (is_array($authUser)) {
                $roleKey = (string) ($authUser['role_key'] ?? $authUser['role'] ?? '');
                if ($roleKey === 'santri') {
                    $ownSantriId = $authUser['id_santri'] ?? ($authUser['santri_id'] ?? null);
                    if ($ownSantriId === null || (int) $ownSantriId !== (int) $idSantri) {
                        return $this->jsonResponse($response, [
                            'success' => false,
                            'message' => 'Akses ditolak: hanya boleh print data sendiri.',
                        ], 403);
                    }
                }
            }
            ...
            $stmt = $this->db->prepare("SELECT s.*, 
                ...
                COALESCE(rd.kel, '') AS kel_diniyah,
                COALESCE(ld.nama, '') AS diniyah_lembaga_nama,
                COALESCE(lt.tingkatan, '') AS lttq,
            ...
            if ($pageMode === 'pendaftaran') {
                $sqlBase = "SELECT r.id, r.id_santri, r.wajib, r.bayar, r.kurang, r.id_admin, 
                ...
                if ($tahunHijriyah && $tahunHijriyah !== '' && $tahunMasehi && $tahunMasehi !== '') {
                    $stmt = $this->db->prepare($sqlBase . ' AND r.tahun_hijriyah = ? AND r.tahun_masehi = ? ORDER BY r.tanggal_dibuat DESC, r.id DESC LIMIT 1');
                    $stmt->execute([$idSantri, $tahunHijriyah, $tahunMasehi]);
                    $registrasiRows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
                } elseif ($tahunHijriyah && $tahunHijriyah !== '') {
                    $stmt = $this->db->prepare($sqlBase . ' AND r.tahun_hijriyah = ? ORDER BY r.tanggal_dibuat DESC, r.id DESC LIMIT 1');
                    $stmt->execute([$idSantri, $tahunHijriyah]);
                    $registrasiRows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
                } elseif ($tahunMasehi && $tahunMasehi !== '') {
                    $stmt = $this->db->prepare($sqlBase . ' AND r.tahun_masehi = ? ORDER BY r.tanggal_dibuat DESC, r.id DESC LIMIT 1');
                    $stmt->execute([$idSantri, $tahunMasehi]);
                    $registrasiRows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
                } else {
                    $stmt = $this->db->prepare($sqlBase . ' ORDER BY r.tahun_hijriyah DESC, r.id DESC');
                    $stmt->execute([$idSantri]);
                    $registrasiRows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
                }

                if ($registrasiRows === false || count($registrasiRows) === 0) {
                    return $this->jsonResponse($response, [
                        'success' => false,
                ...
                if (count($registrasiRows) > 1) {
                    $pendaftaran_multi = true;
                    $pendaftaran_blocks = [];
                    foreach ($registrasiRows as $regRow) {
                        $pendaftaran_blocks[] = $this->buildPendaftaranPrintSegment($regRow, $biodata);
                    }
                    $tunggakan = [];
                    $pembayaran = [];
                    $registrasiPayload = null;
                } else {
                    $segment = $this->buildPendaftaranPrintSegment($registrasiRows[0], $biodata);
                    $biodata = $segment['biodata'];
                    $registrasiPayload = $segment['registrasi'];
                    $tunggakan = $segment['tunggakan'];
                    $pembayaran = $segment['pembayaran'];
                }
            } elseif ($pageMode === 'uwaba') {
            ...
            return $this->jsonResponse($response, [
                ...
                'registrasi' => $registrasiPayload,
                'pendaftaran_multi' => $pendaftaran_multi,
                'pendaftaran_blocks' => $pendaftaran_blocks,
            ], 200);
