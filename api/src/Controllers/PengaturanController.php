...
class PengaturanController
{
    ...
    /** Key yang boleh dibaca tanpa JWT staf (form daftar / publik). */
    private const PUBLIC_READ_KEYS = [
        'tahun_hijriyah',
        'tahun_masehi',
    ];
    ...
    private function isPublicPengaturanKey(string $key): bool
    {
        if (in_array($key, self::PUBLIC_READ_KEYS, true)) {
            return true;
        }

        return str_starts_with($key, 'gelombang_');
    }

    /** Staf eBeddien (JWT pengurus + fitur / super_admin) boleh baca semua key. */
    private function canReadAllPengaturan(Request $request): bool
    {
        ...
        return RoleHelper::tokenHasAnyRoleKey($user, ['super_admin'])
            || RoleHelper::tokenUnionHasAnyEbeddienFiturAssignment($this->db, $user);
    }
     ...
     */
    private function formatPengaturanRow(array $row): array
    {
        return [
            'id' => (int) $row['id'],
            'key' => $row['key'],
    ...
    public function getAll(Request $request, Response $response): Response
    {
        try {
            ...
            $fullAccess = $this->canReadAllPengaturan($request);
            ...
            if ($results) {
                foreach ($results as $row) {
                    $key = (string) ($row['key'] ?? '');
                    if (!$fullAccess && !$this->isPublicPengaturanKey($key)) {
                        continue;
                    }
                    $formattedData[] = $this->formatPengaturanRow($row);
                }
            }
    ...
    public function getByKey(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            if (!$this->canReadAllPengaturan($request) && !$this->isPublicPengaturanKey((string) $key)) {
                return $this->jsonResponse($response, [
                    'success' => false,
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'data' => $this->formatPengaturanRow($result)
            ], 200);
