...
class SantriJuaraController
{
    ...
    /**
     * GET /api/public/juara - Daftar juara untuk display publik (tanpa NIS / id_santri).
     * Wajib filter tahun_ajaran (cegah dump seluruh tabel).
     */
    public function getPublicJuara(Request $request, Response $response): Response
    {
        try {
            $queryParams = $request->getQueryParams();
            $tahunAjaran = isset($queryParams['tahun_ajaran']) ? trim((string) $queryParams['tahun_ajaran']) : '';
            if ($tahunAjaran === '') {
            ...
            $sql = "
                ...
                WHERE sj.tahun_ajaran = ?
                ORDER BY sj.tanggal_dibuat DESC
                LIMIT 300
            ";
