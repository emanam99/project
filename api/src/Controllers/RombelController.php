...
{
    private const FITUR_ROMBEL_SEMUA_DI_LEMBAGA = 'action.rombel.filter.semua_rombel_lembaga';

    private const FITUR_ROMBEL_BERTUGAS = 'action.rombel.rombel_bertugas';
    ...
    /**
     * Hanya rombel tempat pengurus = wali kelas aktif atau guru FAN pada wali aktif.
     *
     * @param array<string, mixed> $user
     */
    private function mustApplyRombelBertugasFilter(array $user): bool
    {
        if ($this->isSuperAdminToken($user)) {
            return false;
        }
        if (RoleHelper::tokenHasEbeddienFiturCode($this->db, $user, self::FITUR_ROMBEL_SEMUA_DI_LEMBAGA)) {
            return false;
        }

        return RoleHelper::tokenHasEbeddienFiturCode($this->db, $user, self::FITUR_ROMBEL_BERTUGAS);
    }

    /**
     * Filter opsional id_pengurus_ampu hanya jika punya aksi «semua rombel di lembaga».
     * Mode rombel bertugas: pakai id pengurus dari token (abaikan query).
     *
     * @param array<string, mixed> $user
     * @param array<string, string> $params
     */
    private function resolveAmpuPengurusId(Request $request, array $params, array $user): ?int
    {
        if ($this->mustApplyRombelBertugasFilter($user)) {
        ...
        $raw = $params['id_pengurus_ampu'] ?? $params['idPengurusAmpu'] ?? null;
        $n = isset($raw) && $raw !== '' ? (int) $raw : 0;

        return $n > 0 ? $n : null;
    }

    private function pengurusAmpuRombelId(int $pengurusId, int $rombelId): bool
    {
        try {
            $stmt = $this->db->prepare('
                SELECT 1 FROM lembaga___wali_kelas w
                WHERE w.id_kelas = ? AND w.status = \'aktif\' AND (
                    w.id_pengurus = ?
                    OR EXISTS (
                        SELECT 1 FROM lembaga___wali_kelas_guru_fan g
                        WHERE g.id_wali_kelas = w.id AND g.id_pengurus = ?
                    )
                )
                LIMIT 1
            ');
            $stmt->execute([$rombelId, $pengurusId, $pengurusId]);
    ...
    /** @param array<string, mixed> $user */
    private function denyRombelMasterMutationsForBertugas(Request $request, Response $response): ?Response
    {
        $user = $this->userPayload($request);
        if ($this->mustApplyRombelBertugasFilter($user)) {
            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Akses rombel bertugas: tidak dapat mengubah master rombel',
            ], 403);
        }
    ...
    public function getAll(Request $request, Response $response): Response
    {
        try {
            ...
            $user = $this->userPayload($request);
            $ampuPid = $this->resolveAmpuPengurusId($request, $params, $user);
            if ($this->mustApplyRombelBertugasFilter($user) && ($ampuPid === null || $ampuPid <= 0)) {
                $where .= ' AND 1=0';
            } elseif ($ampuPid !== null && $ampuPid > 0) {
                $where .= " AND r.id IN (
                    SELECT w.id_kelas FROM lembaga___wali_kelas w
                    WHERE w.status = 'aktif' AND (
                        w.id_pengurus = ?
                        OR EXISTS (
                            SELECT 1 FROM lembaga___wali_kelas_guru_fan g
                            WHERE g.id_wali_kelas = w.id AND g.id_pengurus = ?
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            if ($this->mustApplyRombelBertugasFilter($user)) {
                $pid = RoleHelper::getPengurusIdFromPayload($user);
                if ($pid === null || $pid <= 0 || !$this->pengurusAmpuRombelId((int) $pid, $id)) {
                    return $this->jsonResponse($response, [
                        'success' => false,
                        'message' => 'Rombel tidak ditemukan'
                    ], 404);
    ...
    {
        try {
            $deny = $this->denyRombelMasterMutationsForBertugas($request, $response);
            if ($deny !== null) {
                return $deny;
            }
