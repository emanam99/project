...
class WatzapController
{
    ...
    {
        $expected = trim((string) getenv('WATZAP_WEBHOOK_SECRET'));
        if ($expected === '') {
            $expected = trim((string) getenv('EVOLUTION_WEBHOOK_SECRET'));
        }
        if ($expected === '') {
            if (strtolower((string) getenv('APP_ENV')) === 'production') {
                error_log('WatZap webhook: tolak karena WATZAP_WEBHOOK_SECRET kosong di production');
