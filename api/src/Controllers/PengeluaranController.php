...
class PengeluaranController
{
    ...
    /** Kolom id_admin / FK pengurus: jangan pakai users.id mentah dari token. */
    private function resolvePengurusActorId(Request $request): ?int
    {
        ...
        $raw = isset($user['user_id']) ? (int) $user['user_id'] : (int) ($user['id'] ?? 0);

        return $this->resolveActorPengurusId($raw > 0 ? $raw : null);
    }

    private function denyUnlessPengurusActor(Request $request, Response $response): ?Response
    {
        $pid = $this->resolvePengurusActorId($request);
        if ($pid !== null && $pid > 0) {
            return null;
        }
        ...
        return $this->jsonResponse($response, [
            'success' => false,
            'message' => 'Akun Anda belum terhubung ke data pengurus. Hubungi admin untuk menghubungkan akun login dengan NIP pengurus.',
        ], 400);
    }
    ...
    /**
     * Lewati filter lembaga pada baca pengeluaran bila ?semua_lembaga=1 dan user punya menu Aktivitas / Aktivitas TA.
     */
    private function requestBypassPengeluaranLembagaScopeForAktivitas(Request $request): bool
    {
        $qp = $request->getQueryParams();
        $raw = $qp['semua_lembaga'] ?? '';
        if (!in_array(strtolower(trim((string) $raw)), ['1', 'true', 'yes'], true)) {
            return false;
        }
        $user = $this->userFromRequest($request);
        foreach (['menu.aktivitas', 'menu.aktivitas_tahun_ajaran'] as $code) {
            if (RoleHelper::tokenHasEbeddienFiturCode($this->db, $user, $code)) {
                return true;
            }
        }
    ...
    {
        if ($this->requestBypassPengeluaranLembagaScopeForAktivitas($request)) {
            return;
        }
        $user = $this->userFromRequest($request);
    ...
    public function createRencana(Request $request, Response $response): Response
    {
        try {
            ...
            $denyP = $this->denyUnlessPengurusActor($request, $response);
            if ($denyP !== null) {
                return $denyP;
            }
            $idAdmin = $this->resolvePengurusActorId($request);
            $keterangan = $data['keterangan'] ?? '';
            $kategori = $data['kategori'] ?? null;
            $lembaga = $data['lembaga'] ?? null;
            $sumberUang = $data['sumber_uang'] ?? 'Cash';
            $hijriyah = $data['hijriyah'] ?? null;
            $tahunAjaran = TahunAjaranActiveHelper::resolveTahunAjaranForTransaction($this->db, $hijriyah);
            $details = $data['details'] ?? [];
            $kirimNotifDraft = $this->resolveKirimNotifikasiDraft($data, $userArr);
            ...
            try {
                ...
                foreach ($details as $detail) {
                    ...
                    $stmtDetail->execute([$idRencana, $item, $harga, $jumlah, $nominal, $idAdmin]);
                }
                ...
                if ($status === 'draft' && $kirimNotifDraft) {
                    ...
                    $this->schedulePendingRencanaWaFromRequestData($data, (int) $idRencana, $idAdmin !== null ? (int) $idAdmin : null, true);
                }
    ...
    public function getRencanaDetail(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            // Get rencana: admin_nama = pembuat; last_edit_admin_nama = pengedit terakhir dari detail
            // (versi > 1, atau item baru/baris yang id_admin ≠ pembuat rencana), urut id detail terbaru
            $sqlRencana = "SELECT r.*, 
                          ...
                          (SELECT pg.nama FROM pengeluaran___rencana_detail rd
                           ...
                           WHERE rd.id_pengeluaran_rencana = r.id
                             AND (
                               rd.versi > 1
                               OR (
                                 r.id_admin IS NOT NULL
                                 AND rd.id_admin IS NOT NULL
                                 AND rd.id_admin <> r.id_admin
                               )
                             )
    ...
    public function updateRencana(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            try {
                ...
                $hijriyah = $data['hijriyah'] ?? null;
                $tahunAjaran = array_key_exists('tahun_ajaran', $data) ? ($data['tahun_ajaran'] ?? null) : null;
                if ($hijriyah !== null) {
                    $resolvedTa = TahunAjaranActiveHelper::resolveTahunAjaranForTransaction($this->db, $hijriyah);
                    if ($resolvedTa !== null) {
                ...
                if ($newRencana && (($newRencana['ket'] ?? '') === 'draft') && $kirimNotifDraft) {
                    $this->scheduleNotifOnDraftSaved(
                        (int) $idRencana,
                ...
                } elseif ($newRencana && in_array((string) ($newRencana['ket'] ?? ''), ['pending', 'di edit'], true)) {
                    $this->schedulePendingRencanaWaFromRequestData($data, (int) $idRencana, $idAdmin !== null ? (int) $idAdmin : null, false);
                }
    ...
    /**
     * Queue notif WA rencana pending/edit dari body create/update (recipients).
     * Response HTTP dikembalikan dulu; kirim WA di background.
     *
     * @param array<string, mixed> $data
     */
    private function schedulePendingRencanaWaFromRequestData(
        array $data,
        int $rencanaId,
        ?int $idPengirim,
        bool $isCreate
        ...
        if ($rencanaId < 1 || $recipients === []) {
            return;
        }

        $row = $this->fetchRencanaRowForPendingWa($rencanaId);
        if ($row === null) {
            return;
        }
        if (!$isCreate) {
            $editorNama = $this->resolvePengurusNamaForPush($idPengirim);
            if ($editorNama !== '—') {
                $row['last_edit_admin_nama'] = $editorNama;
            }
        }
        $total = $this->sumRencanaDetailNominalNonRejected($rencanaId);
        $jk = (int) ($row['jumlah_komentar'] ?? 0);
        $jv = (int) ($row['jumlah_viewer'] ?? 0);
        $msg = RencanaPengeluaranWaHelper::buildPendingMessage($row, $total, $isCreate, $jk, $jv);
        $this->deliverRencanaWaRecipients($msg, $recipients, $idPengirim, $rencanaId);
    }

    /** @return array<string, mixed>|null */
    private function fetchRencanaRowForPendingWa(int $idRencana): ?array
    {
        $sql = "SELECT r.*, p.nama as admin_nama,
            (SELECT COUNT(*) FROM pengeluaran___komentar k WHERE k.id_rencana = r.id) as jumlah_komentar,
            (SELECT COUNT(*) FROM pengeluaran___viewer v WHERE v.id_rencana = r.id) as jumlah_viewer
            FROM pengeluaran___rencana r
            LEFT JOIN pengurus p ON r.id_admin = p.id
            WHERE r.id = ? LIMIT 1";
        $stmt = $this->db->prepare($sql);
     ...
     * Public agar AiAgentToolExecutor dapat memicu alur yang sama setelah draft dibuat lewat agen.
     */
    public function scheduleNotifOnDraftSaved(
        int $rencanaId,
        ?string $lembagaRaw,
        ?int $actorId,
        string $keterangan,
     ...
     */
    private function resolveKirimNotifikasiDraft(array $data, array $userArr): bool
    {
        if (!array_key_exists('kirim_notifikasi_draft', $data)) {
            return true;
        }
        $raw = $data['kirim_notifikasi_draft'];
        if (is_bool($raw)) {
            $want = $raw;
        } else {
        ...
        if ($want === false && !RoleHelper::tokenMayToggleDraftNotifOnSave($this->db, $userArr)) {
            return true;
        }
    ...
    private function runDraftSavedNotifDelivery(
        ...
        $rowMeta = [];
        try {
            $stmtMeta = $this->db->prepare('SELECT kategori, sumber_uang FROM pengeluaran___rencana WHERE id = ? LIMIT 1');
        ...
        $waMessage = RencanaPengeluaranWaHelper::buildDraftSavedMessage(
            ...
            $idrDraft,
            [
                'kategori' => (string) ($rowMeta['kategori'] ?? ''),
                'sumber_uang' => (string) ($rowMeta['sumber_uang'] ?? ''),
            ]
        );
    ...
    public function approveRencana(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $catatan = TextSanitizer::cleanTextOrNull(isset($input['catatan']) ? (string) $input['catatan'] : null);
            ...
            try {
                // Update status rencana + catatan opsional
                $sqlUpdate = "UPDATE pengeluaran___rencana SET ket = 'di approve', catatan = ? WHERE id = ?";
                $stmtUpdate = $this->db->prepare($sqlUpdate);
                $stmtUpdate->execute([$catatan, $idRencana]);
    ...
    public function rejectRencana(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $sqlUpdate = "UPDATE pengeluaran___rencana SET ket = 'ditolak', catatan = ? WHERE id = ?";
            $stmtUpdate = $this->db->prepare($sqlUpdate);
            $stmtUpdate->execute([$catatan, $idRencana]);
            $rencana['catatan'] = $catatan;
    ...
    public function uploadFile(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $fileType = (string) $file->getClientMediaType();
            ...
            if (!is_dir($uploadDir) && !@mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Gagal membuat folder upload. Periksa izin folder atau konfigurasi path upload.',
                ], 507);
            }
            ...
            try {
                ...
                error_log('PengeluaranController moveTo: ' . $e->getMessage());
                return $this->jsonResponse($response, [
                    'success' => false,
            ...
            $finalFileSize = ($diskSize !== false) ? (int) $diskSize : (int) $file->getSize();

            if ($extension === 'pdf') {
                $finalFileSize = $this->maybeShrinkUploadedRencanaPdf($filePath, $finalFileSize);
            }
    ...
    private function compressPdf(string $filePath): ?string
    {
        try {
            ...
            // Tanpa Ghostscript: biarkan PDF asli (fallback regex di PHP rawan error pada file besar).
            return null;
        } catch (\Throwable $e) {
    ...
    private function isPathInsideUploads(string $path): bool
    {
        ...
        $normConfigured = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $this->uploadsPath), DIRECTORY_SEPARATOR);
    ...
    /**
     * Coba kompresi PDF (Ghostscript). Gagal → lewati; tidak menghapus asli sebelum penggantian aman.
     */
    private function maybeShrinkUploadedRencanaPdf(string $filePath, int $originalSize): int
    {
        ...
        try {
            ...
            error_log('maybeShrinkUploadedRencanaPdf: ' . $e->getMessage());
