...
use App\Helpers\PengurusScopeAccessHelper;
...
class ManageUsersController
{
    ...
    private const FITUR_PENGURUS_EDIT = 'action.pengurus.edit';

    /** @return null|list<string> */
    private function resolvePengurusListLembagaScopeIds(Request $request): ?array
    {
        return PengurusScopeAccessHelper::resolvePengurusListLembagaScopeIds($this->db, $request);
    }

    private function pengurusTargetVisibleInScope(Request $request, string $targetPengurusId): bool
    {
        return PengurusScopeAccessHelper::pengurusTargetVisibleInScope($this->db, $request, $targetPengurusId);
    }
    ...
    public function getAllUsers(Request $request, Response $response): Response
    {
        try {
            ...
            $effectiveJabatanLembagaSql = "COALESCE(NULLIF(TRIM(pj.lembaga_id), ''), j.lembaga_id)";
            $pjAktifSql = "(pj.status = 'aktif' OR pj.status = 'active' OR pj.status IS NULL OR TRIM(COALESCE(pj.status, '')) = '')";
            $pj2AktifSql = "(pj2.status = 'aktif' OR pj2.status = 'active' OR pj2.status IS NULL OR TRIM(COALESCE(pj2.status, '')) = '')";
            $joinJabatan = "LEFT JOIN pengurus___jabatan pj ON p.id = pj.pengurus_id AND {$pjAktifSql}
                    LEFT JOIN jabatan j ON pj.jabatan_id = j.id
                    LEFT JOIN lembaga l ON l.id = {$effectiveJabatanLembagaSql}";
            if ($jabatanLembagaFilter !== '') {
                $joinJabatan = "INNER JOIN pengurus___jabatan pj ON p.id = pj.pengurus_id AND {$pjAktifSql}
                        INNER JOIN jabatan j ON pj.jabatan_id = j.id AND {$effectiveJabatanLembagaSql} = ?
                        LEFT JOIN lembaga l ON l.id = {$effectiveJabatanLembagaSql}";
            ...
            if ($lembagaKategoriFilter !== '') {
                if (\is_array($scopeIds) && $scopeIds !== []) {
                    ...
                    $whereConditions[] = "EXISTS (SELECT 1 FROM pengurus___jabatan pj2 INNER JOIN jabatan j2 ON pj2.jabatan_id = j2.id LEFT JOIN lembaga l2 ON l2.id = COALESCE(NULLIF(TRIM(pj2.lembaga_id), ''), j2.lembaga_id) WHERE pj2.pengurus_id = p.id AND {$pj2AktifSql} AND l2.kategori = ? AND l2.id IN ({$phKat}))";
                    $params[] = $lembagaKategoriFilter;
                    foreach ($scopeIds as $sid) {
                        $params[] = $sid;
                    }
                } else {
                    $whereConditions[] = "EXISTS (SELECT 1 FROM pengurus___jabatan pj2 INNER JOIN jabatan j2 ON pj2.jabatan_id = j2.id LEFT JOIN lembaga l2 ON l2.id = COALESCE(NULLIF(TRIM(pj2.lembaga_id), ''), j2.lembaga_id) WHERE pj2.pengurus_id = p.id AND {$pj2AktifSql} AND l2.kategori = ?)";
                    $params[] = $lembagaKategoriFilter;
                }
            }
            ...
            if (\is_array($scopeIds) && $scopeIds !== []) {
                ...
                $wScope = PengurusScopeAccessHelper::buildPengurusLembagaScopeWhere((int) $viewer, $scopeIds);
            ...
            if ($jabatanIdFilter > 0) {
                $whereConditions[] = "EXISTS (SELECT 1 FROM pengurus___jabatan pj2 WHERE pj2.pengurus_id = p.id AND pj2.jabatan_id = ? AND {$pj2AktifSql})";
            ...
            $sql = "SELECT 
                        ...
                        p.dusun, p.rt, p.rw, p.desa, p.kecamatan, p.kabupaten, p.provinsi, p.kode_pos, p.jarak,
                        p.status,
                        p.sejak, p.nyabang, p.hijriyah, p.masehi,
                        p.tanggal_dibuat, p.tanggal_update, p.rekening_jatim,
                        u.email, u.no_wa AS whatsapp, COALESCE(u.no_wa, '') AS no_telpon, u.username,
                        ...
                        GROUP_CONCAT(DISTINCT CONCAT(COALESCE(NULLIF(TRIM(pj.lembaga_id), ''), j.lembaga_id, ''), ':', j.id, ':', j.nama) SEPARATOR '||') as jabatan_data
                    FROM pengurus p 
                    {$joinUsers}
                    {$joinRoleFilter}
                    {$joinJabatan}
                    ...
                    GROUP BY p.id, p.nip, p.grup, p.id_user, p.gelar_awal, p.nama, p.gelar_akhir, p.nik, p.no_kk, p.kategori, p.status_pengurus,
                        p.gender, p.tempat_lahir, p.tanggal_lahir, p.pendidikan_terakhir, p.sekolah, p.tahun_lulus,
                        p.s1, p.s2, p.s3, p.tmt, p.bidang_studi, p.jurusan_title, p.status_nikah, p.pekerjaan,
                        p.niy, p.nidn, p.nuptk, p.npk, p.dusun, p.rt, p.rw, p.desa, p.kecamatan, p.kabupaten, p.provinsi, p.kode_pos, p.jarak,
                        p.status, p.sejak, p.nyabang, p.hijriyah, p.masehi, p.tanggal_dibuat, p.tanggal_update, p.rekening_jatim,
                        u.email, u.no_wa, u.username
                    ORDER BY p.nama ASC 
                    LIMIT ? OFFSET ?";
     ...
     * Body: { rows: [{ id, ...field }] } — hanya kunci yang dikirim yang di-update (selaras editor Excel).
     */
    public function bulkUpdatePengurusFromExcel(Request $request, Response $response): Response
    {
        try {
            ...
            $allowedPengurus = [
                'nama', 'grup', 'gelar_awal', 'gelar_akhir', 'nik', 'no_kk', 'kategori', 'status_pengurus',
                'gender', 'tempat_lahir', 'tanggal_lahir', 'pendidikan_terakhir', 'sekolah', 'tahun_lulus',
                's1', 's2', 's3', 'tmt', 'bidang_studi', 'jurusan_title', 'status_nikah', 'pekerjaan',
                'niy', 'nidn', 'nuptk', 'npk', 'dusun', 'rt', 'rw', 'desa', 'kecamatan', 'kabupaten', 'provinsi', 'kode_pos', 'jarak',
                'sejak', 'nyabang', 'hijriyah', 'masehi', 'rekening_jatim', 'an_jatim', 'status',
            ];
            ...
            $actorPengurusId = RoleHelper::getPengurusIdFromPayload($reqUserArr);
            ...
            foreach ($rows as $idx => $row) {
                ...
                $idRaw = $row['id'] ?? null;
                if ($idRaw === null || $idRaw === '') {
                    $errors[] = 'Baris ' . ($idx + 1) . ': id pengurus wajib';
                    $skipped++;
                    continue;
                }
                $userIdResolved = PengurusHelper::resolveIdByNip($this->db, $idRaw);
                if ($userIdResolved === null) {
                    $errors[] = 'Baris ' . ($idx + 1) . ': pengurus tidak ditemukan';
                    $skipped++;
                    continue;
                }
                $pid = (string) $userIdResolved;

                if (!$this->pengurusTargetVisibleInScope($request, $pid)) {
                    $errors[] = 'Baris ' . ($idx + 1) . ': tidak ada akses ke pengurus ini';
                    $skipped++;
                    continue;
                }

                if (\array_key_exists('nama', $row) && trim((string) $row['nama']) === '') {
                    $errors[] = 'Baris ' . ($idx + 1) . ': nama tidak boleh kosong';
                    $skipped++;
                    continue;
                }

                $stmtOld = $this->db->prepare('SELECT p.*, u.email, u.no_wa AS whatsapp FROM pengurus p LEFT JOIN users u ON u.id = p.id_user WHERE p.id = ? LIMIT 1');
                $stmtOld->execute([$userIdResolved]);
                ...
                $setP = [];
                $paramsP = [];
                foreach ($allowedPengurus as $field) {
                    if (!\array_key_exists($field, $row)) {
                        continue;
                    }
                    $val = $row[$field];
                    if ($field === 'nama') {
                        $val = TextSanitizer::cleanText((string) $val);
                        if ($val === '') {
                            $errors[] = 'Baris ' . ($idx + 1) . ': nama tidak boleh kosong';
                            continue 2;
                        }
                        $val = ProperCaseHelper::forBiodataField('nama', $val) ?? '';
                    } elseif ($field === 'status') {
                        $allowedStatuses = ['active', 'inactive', 'pending', 'aktif', 'tidak aktif'];
                        $statusLower = strtolower(trim((string) $val));
                        if (!\in_array($statusLower, array_map('strtolower', $allowedStatuses), true)) {
                            $errors[] = 'Baris ' . ($idx + 1) . ': status tidak valid';
                            continue 2;
                        }
                        if ($statusLower === 'aktif') {
                            $val = 'active';
                        } elseif ($statusLower === 'tidak aktif') {
                            $val = 'inactive';
                        } else {
                            $val = $statusLower;
                        }
                        ...
                        if ($val === '' || $val === null) {
                            ...
                            $errors[] = 'Baris ' . ($idx + 1) . ': jarak harus angka';
                            continue 2;
                        } else {
                            $val = round((float) $val, 2);
                            if ($val < 0) {
                                $errors[] = 'Baris ' . ($idx + 1) . ': jarak tidak boleh negatif';
                                continue 2;
                            }
                        }
                    } else {
                ...
                $didUser = false;
                $idUser = !empty($oldRow['id_user']) ? (int) $oldRow['id_user'] : null;
                if ($idUser !== null && (\array_key_exists('email', $row) || \array_key_exists('whatsapp', $row))) {
                    $uParts = [];
                    $uParams = [];
                    if (\array_key_exists('email', $row)) {
                        $uParts[] = 'email = ?';
                        $uParams[] = TextSanitizer::cleanText($row['email'] ?? '') ?: null;
                    }
                    if (\array_key_exists('whatsapp', $row)) {
                        $uParts[] = 'no_wa = ?';
                        $uParams[] = TextSanitizer::cleanText($row['whatsapp'] ?? '') ?: null;
                    }
                    if ($uParts !== []) {
                        $uParams[] = $idUser;
                        try {
                            $this->db->prepare('UPDATE users SET ' . implode(', ', $uParts) . ' WHERE id = ?')->execute($uParams);
                            $didUser = true;
                ...
                $isPengurusUpdated = false;
                if ($setP !== []) {
                    $paramsP[] = $userIdResolved;
                    $sql = 'UPDATE pengurus SET ' . implode(', ', $setP) . ' WHERE id = ?';
                    $stmt = $this->db->prepare($sql);
                    $stmt->execute($paramsP);
                    $isPengurusUpdated = $stmt->rowCount() > 0;
                }

                if ($didUser || $isPengurusUpdated) {
                    $updated++;
                    if ($isPengurusUpdated && $actorPengurusId !== null) {
                        $stmtNew = $this->db->prepare('SELECT id, nip, nama, status, grup FROM pengurus WHERE id = ?');
                        $stmtNew->execute([$userIdResolved]);
            ...
            error_log('Bulk update pengurus excel error: ' . $e->getMessage());
            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal menyimpan perubahan massal pengurus',
            ], 500);
        }
    }
    ...
    public function createUser(Request $request, Response $response): Response
    {
        try {
            ...
            $nama = ProperCaseHelper::forBiodataField('nama', (string) $nama) ?? '';
    ...
    public function updateUser(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $updateFields[] = "nama = ?";
            $params[] = ProperCaseHelper::forBiodataField('nama', TextSanitizer::cleanText($data['nama'] ?? '')) ?? '';
    ...
    public function getUserById(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $jabatanSql = "SELECT 
                        ...
                        COALESCE(l.kategori, '') as jabatan_kategori,
                        pj.lembaga_id,
                        pj.tanggal_mulai,
                        pj.tanggal_selesai,
                    ...
                    LEFT JOIN lembaga l ON l.id = COALESCE(NULLIF(TRIM(pj.lembaga_id), ''), j.lembaga_id)
    ...
    public function addUserJabatan(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            // Insert jabatan
            $insertSql = "INSERT INTO pengurus___jabatan (pengurus_id, jabatan_id, lembaga_id, tanggal_mulai, tanggal_selesai, status, mengajar, id_admin) 
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $mengajarJabatan = \App\Helpers\PengurusBooleanHelper::normalizeToTinyInt($data['mengajar'] ?? 0);
            $insertStmt->execute([
                $userId,
                $data['jabatan_id'],
    ...
    /**
     * PUT /api/manage-users/{id}/jabatan/{pengurusJabatanId} - Update jabatan (status, tanggal, mengajar)
     * Body: status (aktif|nonaktif, opsional), mengajar (0|1, opsional), tanggal_mulai/selesai (opsional).
     */
    public function updateUserJabatanStatus(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            if (array_key_exists('mengajar', $body)) {
                $sets[] = 'mengajar = ?';
                $params[] = \App\Helpers\PengurusBooleanHelper::normalizeToTinyInt($body['mengajar']);
            }
     ...
     * Query: type=santri|pengurus|toko|pjgt|all, search=, limit=, page=, role_id=, lembaga_id= (dari role), jabatan_lembaga_id= (lembaga dari tabel jabatan).
     * Return: users dengan is_santri, is_pengurus, is_toko, is_pjgt, pengurus_id, santri_id, nama (dari pengurus/santri/username).
     */
    public function getAllUsersV2(Request $request, Response $response): Response
    {
        try {
            $queryParams = $request->getQueryParams();
            $typeFilter = $queryParams['type'] ?? 'all'; // santri | pengurus | toko | pjgt | all
            ...
            $joinSantriAgg = "
                LEFT JOIN (
                    SELECT id_user, COUNT(*) AS santri_count, MIN(id) AS santri_first_id
                    FROM santri
                    GROUP BY id_user
                ) santri_sc ON santri_sc.id_user = u.id
                LEFT JOIN santri s ON s.id = santri_sc.santri_first_id
            ";
            $joinTokoAgg = "
                LEFT JOIN (
                    SELECT id_users AS id_user, COUNT(*) AS toko_count
                    FROM cashless___pedagang
                    WHERE id_users IS NOT NULL
                    GROUP BY id_users
                ) toko_sc ON toko_sc.id_user = u.id
            ";
            $joinPjgt = " LEFT JOIN madrasah pjgt_m ON pjgt_m.id_pjgt = u.id ";
            ...
            // Filter tipe identitas Mybeddian / pengurus
            if ($typeFilter === 'santri') {
                $whereConditions[] = 'santri_sc.id_user IS NOT NULL';
            } elseif ($typeFilter === 'pengurus') {
                $whereConditions[] = 'p.id IS NOT NULL';
            } elseif ($typeFilter === 'toko') {
                $whereConditions[] = 'toko_sc.id_user IS NOT NULL';
            } elseif ($typeFilter === 'pjgt') {
                $whereConditions[] = 'pjgt_m.id IS NOT NULL';
            }

            if ($search !== '') {
                $whereConditions[] = "(u.username LIKE ? OR u.email LIKE ? OR u.no_wa LIKE ? OR p.nama LIKE ? OR s.nama LIKE ? OR EXISTS (SELECT 1 FROM santri s2 WHERE s2.id_user = u.id AND (s2.nama LIKE ? OR CAST(s2.nis AS CHAR) LIKE ?)))";
                $searchParam = '%' . $search . '%';
                $params[] = $searchParam;
                $params[] = $searchParam;
                $params[] = $searchParam;
            ...
            // Subquery / join: users LEFT JOIN pengurus, agregat santri; optional INNER JOIN role/jabatan untuk filter
            ...
            $sql = "SELECT
                    ...
                    COALESCE(santri_sc.santri_count, 0) AS santri_count,
                    s.id AS santri_id,
                    s.nis AS santri_nis,
                    s.nama AS santri_nama,
                    COALESCE(toko_sc.toko_count, 0) AS toko_count,
                    pjgt_m.id AS pjgt_madrasah_id,
                    pjgt_m.nama AS pjgt_madrasah_nama
                FROM users u
                LEFT JOIN pengurus p ON p.id_user = u.id
                {$joinSantriAgg}
                {$joinTokoAgg}
            ...
            foreach ($rows as $row) {
                $users[] = [
                    ...
                    'is_santri' => ((int) ($row['santri_count'] ?? 0)) > 0 || !empty($row['santri_id']),
                    'is_pengurus' => !empty($row['pengurus_id']),
                    'is_toko' => ((int) ($row['toko_count'] ?? 0)) > 0,
                    'is_pjgt' => !empty($row['pjgt_madrasah_id']),
                    'toko_count' => (int) ($row['toko_count'] ?? 0),
                    'pjgt_madrasah_id' => !empty($row['pjgt_madrasah_id']) ? (int) $row['pjgt_madrasah_id'] : null,
                    'pjgt_madrasah_nama' => $row['pjgt_madrasah_nama'] ?? null,
                    'pengurus_id' => $row['pengurus_id'] ? (int) $row['pengurus_id'] : null,
                    'santri_id' => $row['santri_id'] ? (int) $row['santri_id'] : null,
                    'santri_count' => (int) ($row['santri_count'] ?? 0),
     ...
     * Query: search=, limit= (default 30), editing_user_id= (opsional: hanya santri tanpa akun atau sudah tertaut ke user itu).
     * Return: id, nis, nama, id_user (jika sudah terhubung akun).
     */
    public function getSantriOptions(Request $request, Response $response): Response
    {
        try {
            ...
            $editingUserId = isset($queryParams['editing_user_id']) ? (int) $queryParams['editing_user_id'] : 0;

            $where = ['1=1'];
            $params = [];
            if ($editingUserId > 0) {
                $where[] = '(s.id_user IS NULL OR s.id_user = ?)';
                $params[] = $editingUserId;
            }
            if ($search !== '') {
                $where[] = '(CAST(s.nis AS CHAR) LIKE ? OR s.nama LIKE ?)';
                $searchParam = '%' . $search . '%';
                $params[] = $searchParam;
                $params[] = $searchParam;
            }
     ...
     * Jika santri_id null: lepaskan semua santri yang terhubung ke user ini.
     * Untuk cabut satu santri saja: DELETE /api/v2/manage-users/{id}/mybeddian-santri/{santriId}.
     * Super_admin only.
     */
    ...
    /**
     * DELETE /api/v2/manage-users/{id}/mybeddian-santri/{santriId} — lepas satu tautan santri dari user.
     */
    public function removeOneMybeddianSantri(Request $request, Response $response, array $args): Response
    {
        try {
            $userId = (int) ($args['id'] ?? 0);
            $santriId = (int) ($args['santriId'] ?? 0);
            if ($userId <= 0 || $santriId <= 0) {
            ...
            $up->execute([$santriId, $userId]);
            if ($up->rowCount() === 0) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'Santri tidak ditemukan atau tidak tertaut ke user ini'], 404);
            }
            return $this->jsonResponse($response, ['success' => true, 'message' => 'Tautan santri berhasil dilepas'], 200);
        } catch (\Exception $e) {
            error_log('ManageUsersController::removeOneMybeddianSantri ' . $e->getMessage());
            return $this->jsonResponse($response, ['success' => false, 'message' => 'Terjadi kesalahan'], 500);
        }
    }

    /**
     * GET /api/v2/manage-users/pengurus-options — daftar pengurus untuk tautkan ke akun (login eBeddien).
     * Query: search=, limit=, editing_user_id= (hanya id_user kosong atau sudah tertaut ke user itu).
     */
    public function getPengurusOptions(Request $request, Response $response): Response
    {
        try {
            $queryParams = $request->getQueryParams();
            ...
            $params = [];
            if ($editingUserId > 0) {
                $where[] = '(p.id_user IS NULL OR p.id_user = ?)';
                $params[] = $editingUserId;
            } else {
                $where[] = 'p.id_user IS NULL';
            }
            if ($search !== '') {
                $where[] = '(CAST(p.nip AS CHAR) LIKE ? OR p.nama LIKE ? OR CAST(p.id AS CHAR) LIKE ?)';
                $searchParam = '%' . $search . '%';
                $params[] = $searchParam;
                $params[] = $searchParam;
                $params[] = $searchParam;
            ...
            $sql = 'SELECT p.id, p.nip, p.nama, p.status, p.id_user
                FROM pengurus p
                WHERE ' . implode(' AND ', $where) . '
                ORDER BY p.nama ASC
                LIMIT ?';
            ...
            error_log('ManageUsersController::getPengurusOptions ' . $e->getMessage());
            return $this->jsonResponse($response, [
                'success' => false,
    ...
    /**
     * PUT /api/v2/manage-users/{id}/pengurus-link — tautkan / lepas pengurus ke akun users.id (1:1 via pengurus.id_user).
     * Body: { pengurus_id: number | null }. null = lepas tautan pengurus dari user ini.
     */
    public function setPengurusLink(Request $request, Response $response, array $args): Response
    {
        try {
            $userId = (int) ($args['id'] ?? 0);
            if ($userId <= 0) {
            ...
            $data = $request->getParsedBody() ?? [];
            $pengurusIdRaw = array_key_exists('pengurus_id', $data) ? $data['pengurus_id'] : null;
            $pengurusId = $pengurusIdRaw === null || $pengurusIdRaw === '' ? null : (int) $pengurusIdRaw;
            ...
            if ($pengurusId === null) {
                $upd = $this->db->prepare('UPDATE pengurus SET id_user = NULL WHERE id_user = ?');
                $upd->execute([$userId]);
                $affected = $upd->rowCount();
                ...
                return $this->jsonResponse($response, [
                    'success' => true,
                    'message' => $affected > 0
                        ? 'Tautan pengurus dilepas. User tidak lagi terhubung ke data pengurus.'
                        : 'User belum tertaut ke pengurus.',
                    'data' => ['pengurus' => null],
                ], 200);
            }

            if ($pengurusId <= 0) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'ID pengurus tidak valid'], 400);
            }

            $stmt = $this->db->prepare('SELECT id, nip, nama, status, id_user FROM pengurus WHERE id = ?');
            ...
            $existingOwner = !empty($pengurus['id_user']) ? (int) $pengurus['id_user'] : 0;
            if ($existingOwner > 0 && $existingOwner !== $userId) {
                return $this->jsonResponse($response, [
                    'success' => false,
                    'message' => 'Pengurus ini sudah tertaut ke akun user lain (ID ' . $existingOwner . ').',
                ], 409);
            }

            // Satu user hanya boleh satu pengurus: lepas tautan lama dulu
            $this->db->prepare('UPDATE pengurus SET id_user = NULL WHERE id_user = ? AND id <> ?')
                ->execute([$userId, $pengurusId]);
            $this->db->prepare('UPDATE pengurus SET id_user = ? WHERE id = ?')
                ->execute([$userId, $pengurusId]);

            $pengurusOut = [
                'id' => (int) $pengurus['id'],
                'nip' => $pengurus['nip'] ?? null,
                'nama' => (string) ($pengurus['nama'] ?? ''),
                'status' => $pengurus['status'] ?? null,
            ];

            return $this->jsonResponse($response, [
                'success' => true,
                'message' => 'Pengurus berhasil ditautkan. User dapat memakai role/jabatan pengurus untuk login eBeddien.',
                'data' => ['pengurus' => $pengurusOut],
            ], 200);
        } catch (\Exception $e) {
            error_log('ManageUsersController::setPengurusLink ' . $e->getMessage());
            return $this->jsonResponse($response, [
                'success' => false,
                'message' => 'Gagal menautkan pengurus',
            ], 500);
        }
    }

    /**
     * GET /api/v2/users/{id}/detail-readonly — ringkasan akun (mode baca).
     * Akses lebih luas dari manage-users v2 (pengurus / santri / UGT / manage users).
     */
    public function getUserDetailReadonly(Request $request, Response $response, array $args): Response
    {
        return $this->getUserByIdV2($request, $response, $args);
    }
    ...
    public function getUserByIdV2(Request $request, Response $response, array $args): Response
    {
        try {
            ...
            $stmt = $this->db->prepare("SELECT u.id, u.username, u.no_wa, u.email, u.role AS user_role, u.email_verified_at, u.no_wa_verified_at, u.tanggal_dibuat, u.tanggal_update,
                COALESCE(u.access_ebeddien, 1) AS access_ebeddien,
                COALESCE(u.access_mybeddian_santri, 1) AS access_mybeddian_santri,
                COALESCE(u.access_mybeddian_toko, 1) AS access_mybeddian_toko,
                COALESCE(u.access_mybeddian_pjgt, 1) AS access_mybeddian_pjgt
            ...
            $user['id'] = (int) $user['id'];
            $user['access_ebeddien'] = (int) ($user['access_ebeddien'] ?? 1);
            $user['access_mybeddian_santri'] = (int) ($user['access_mybeddian_santri'] ?? 1);
            $user['access_mybeddian_toko'] = (int) ($user['access_mybeddian_toko'] ?? 1);
            $user['access_mybeddian_pjgt'] = (int) ($user['access_mybeddian_pjgt'] ?? 1);
            $user['pengurus'] = null;
            $user['santri'] = null;
            $user['santri_list'] = [];
            $user['pjgt_madrasah'] = null;
            ...
            $santriStmt = $this->db->prepare('SELECT id, nis, nama FROM santri WHERE id_user = ? ORDER BY id ASC');
            $santriStmt->execute([$userId]);
            $santriList = [];
            while ($row = $santriStmt->fetch(\PDO::FETCH_ASSOC)) {
                $santriList[] = [
                    'id' => (int) $row['id'],
                    'nis' => $row['nis'] ?? null,
                    'nama' => $row['nama'] ?? '',
            ...
            $user['santri_list'] = $santriList;
            if (count($santriList) > 0) {
                $first = $santriList[0];
                $user['santri'] = $first;
                $user['santri_id'] = $first['id'];
                $user['santri_nis'] = $first['nis'] ?? null;
            }
            ...
            try {
                $pjStmt = $this->db->prepare('SELECT id, nama, identitas FROM madrasah WHERE id_pjgt = ? LIMIT 1');
                $pjStmt->execute([$userId]);
                $pjRow = $pjStmt->fetch(\PDO::FETCH_ASSOC);
                if ($pjRow) {
                    $user['pjgt_madrasah'] = [
                        'id' => (int) $pjRow['id'],
                        'nama' => $pjRow['nama'] ?? '',
                        'identitas' => $pjRow['identitas'] ?? null,
                    ];
                }
            } catch (\Throwable $e) {
            }
    ...
    /**
     * PUT /api/v2/manage-users/{id}/portal-access — flag akses eBeddien & Mybeddian (santri/toko/PJGT). Body: access_ebeddien, access_mybeddian_santri, access_mybeddian_toko, access_mybeddian_pjgt (0|1).
     */
    public function updateUserPortalAccessV2(Request $request, Response $response, array $args): Response
    {
        try {
            $userId = (int) ($args['id'] ?? 0);
            if ($userId <= 0) {
            ...
            $fields = [];
            $params = [];
            if (array_key_exists('access_ebeddien', $body)) {
                $fields[] = 'access_ebeddien = ?';
                $params[] = $norm($body['access_ebeddien']);
            }
            if (array_key_exists('access_mybeddian_santri', $body)) {
                $fields[] = 'access_mybeddian_santri = ?';
                $params[] = $norm($body['access_mybeddian_santri']);
            }
            if (array_key_exists('access_mybeddian_toko', $body)) {
                $fields[] = 'access_mybeddian_toko = ?';
                $params[] = $norm($body['access_mybeddian_toko']);
            }
            if (array_key_exists('access_mybeddian_pjgt', $body)) {
                $fields[] = 'access_mybeddian_pjgt = ?';
                $params[] = $norm($body['access_mybeddian_pjgt']);
            }
            if ($fields === []) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'Tidak ada field yang diubah'], 400);
            }
            $params[] = $userId;
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'message' => 'Pengaturan akses aplikasi diperbarui',
            ], 200);
        } catch (\Exception $e) {
            error_log('ManageUsersController::updateUserPortalAccessV2 ' . $e->getMessage());
            return $this->jsonResponse($response, [
                'success' => false,
        ...
        try {
            ...
            DeferredHttpTask::runAfterResponse(static function () use ($tokenIdWa, $noWaD, $msgWa, $logCtxWa): void {
                try {
                    ...
                    error_log('ManageUsersController::sendResetPasswordLink deferred WA: ' . $e->getMessage());
                }
            });
            return $this->jsonResponse($response, ['success' => true, 'message' => 'Link reset password sedang dikirim ke WhatsApp user.', 'notifications' => ['wa' => 'queued']], 200);
        } catch (\Exception $e) {
    ...
    /**
     * GET /api/v2/manage-users/madrasah-pjgt-options — madrasah yang belum punya PJGT (id_pjgt IS NULL). Query: search, limit.
     */
    public function getMadrasahPjgtOptions(Request $request, Response $response): Response
    {
        try {
            $params = $request->getQueryParams();
            $search = isset($params['search']) ? trim((string) $params['search']) : '';
            $limit = isset($params['limit']) ? min(100, max(1, (int) $params['limit'])) : 40;

            $sql = 'SELECT id, nama, identitas FROM madrasah WHERE id_pjgt IS NULL';
            $bind = [];
            if ($search !== '') {
                $sql .= ' AND (nama LIKE ? OR identitas LIKE ?)';
                $q = '%' . $search . '%';
                $bind[] = $q;
                $bind[] = $q;
            }
            ...
            error_log('ManageUsersController::getMadrasahPjgtOptions ' . $e->getMessage());
            return $this->jsonResponse($response, ['success' => false, 'message' => 'Terjadi kesalahan'], 500);
        }
    }

    /**
     * POST /api/v2/manage-users/{id}/pjgt — tautkan users.id sebagai PJGT ke madrasah (madrasah_id). id_pjgt harus masih kosong.
     */
    public function linkUserPjgt(Request $request, Response $response, array $args): Response
    {
        try {
            $userId = (int) ($args['id'] ?? 0);
            $body = $request->getParsedBody();
            $body = is_array($body) ? $body : [];
            $madrasahId = isset($body['madrasah_id']) ? (int) $body['madrasah_id'] : 0;
            if ($userId <= 0 || $madrasahId <= 0) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'ID user dan madrasah wajib diisi'], 400);
            }
            ...
            $stmt = $this->db->prepare('SELECT id, id_pjgt, nama, identitas FROM madrasah WHERE id = ?');
            $stmt->execute([$madrasahId]);
            $m = $stmt->fetch(\PDO::FETCH_ASSOC);
            if (!$m) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'Madrasah tidak ditemukan'], 404);
            }
            if (!empty($m['id_pjgt'])) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'Madrasah ini sudah memiliki akun PJGT'], 400);
            }

            $this->db->prepare('UPDATE madrasah SET id_pjgt = NULL WHERE id_pjgt = ?')->execute([$userId]);

            $up = $this->db->prepare('UPDATE madrasah SET id_pjgt = ? WHERE id = ? AND id_pjgt IS NULL');
            $up->execute([$userId, $madrasahId]);
            if ($up->rowCount() === 0) {
                return $this->jsonResponse($response, ['success' => false, 'message' => 'Gagal menetapkan PJGT (slot sudah terisi?)'], 409);
            }
            ...
            return $this->jsonResponse($response, [
                'success' => true,
                'message' => 'User berhasil dihubungkan sebagai PJGT madrasah',
                'data' => [
                    'pjgt_madrasah' => [
                        'id' => (int) $m['id'],
                        'nama' => (string) ($m['nama'] ?? ''),
            ...
            error_log('ManageUsersController::linkUserPjgt ' . $e->getMessage());
            return $this->jsonResponse($response, ['success' => false, 'message' => 'Terjadi kesalahan'], 500);
        }
    }

    /**
     * DELETE /api/v2/manage-users/{id}/pjgt — lepas peran PJGT user (id_pjgt di madrasah + id_madrasah di users).
     */
    public function unlinkUserPjgt(Request $request, Response $response, array $args): Response
    {
        try {
            $userId = (int) ($args['id'] ?? 0);
            if ($userId <= 0) {
            ...
            $this->db->prepare('UPDATE madrasah SET id_pjgt = NULL WHERE id_pjgt = ?')->execute([$userId]);
            try {
                $this->db->prepare('UPDATE users SET id_madrasah = NULL WHERE id = ?')->execute([$userId]);
            } catch (\Throwable $e) {
            }

            return $this->jsonResponse($response, ['success' => true, 'message' => 'Hubungan PJGT berhasil dilepas'], 200);
        } catch (\Exception $e) {
            error_log('ManageUsersController::unlinkUserPjgt ' . $e->getMessage());
            return $this->jsonResponse($response, ['success' => false, 'message' => 'Terjadi kesalahan'], 500);
        }
    }
