...
function Add-DropTableBeforeCreate {
    param([string]$SqlContent)

  # phpMyAdmin dump tanpa DROP TABLE; jika API jalan saat import, middleware bisa
  # membuat rate_limits lebih dulu → CREATE TABLE gagal (1050 already exists).
    $nl = [Environment]::NewLine
    return [regex]::Replace($SqlContent, '(?m)^CREATE TABLE `([^`]+)`', {
        param($m)
        $tableName = $m.Groups[1].Value
        "DROP TABLE IF EXISTS ``$tableName``;${nl}CREATE TABLE ``$tableName``"
    })
}
...
function Invoke-LocalImport {
    ...
    Write-Host "[local] Tip: hentikan Apache/API selama import agar tidak ada tabel dibuat otomatis (mis. rate_limits)." -ForegroundColor DarkYellow
...
Write-Host "Menyiapkan db.sql (DROP TABLE sebelum CREATE)..." -ForegroundColor DarkGray
$preparedSqlPath = Join-Path ([System.IO.Path]::GetTempPath()) ("db-import-prepared-" + [guid]::NewGuid().ToString("N") + ".sql")
try {
    $rawSql = Get-Content -Path $resolvedSqlPath -Raw -Encoding UTF8
    $preparedSql = Add-DropTableBeforeCreate -SqlContent $rawSql
    [System.IO.File]::WriteAllText($preparedSqlPath, $preparedSql, [System.Text.UTF8Encoding]::new($false))

    switch ($resolvedTarget) {
        "local" {
            Invoke-LocalImport -ResolvedSqlPath $preparedSqlPath -ResolvedMysqlPath $MysqlPath -ResolvedApiEnvPath $resolvedApiEnvPath -ShouldRecreateDb $resolvedRecreateDb
        }
        "staging" {
            Invoke-StagingImport -ResolvedSqlPath $preparedSqlPath -ResolvedSshUser $SshUser -ResolvedSshHost $SshHost -ResolvedSshPort $SshPort -ResolvedRemoteApiPath $RemoteApiPath -ShouldRecreateDb $resolvedRecreateDb
        }
        default {
    ...
    if (Test-Path $preparedSqlPath) {
        Remove-Item $preparedSqlPath -Force -ErrorAction SilentlyContinue
    }
}
