...
export default function RiwayatKhusus() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      pembayaranAPI.getRincian(idSantri, 'khusus'),
      pembayaranAPI.getHistory(idSantri, 'khusus'),
    ])
      .then(([r1, r2]) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-3 sm:py-4 pb-6">
      <Link
        ...
        className="inline-flex items-center gap-1.5 text-xs text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300 hover:underline mb-3 transition-colors"
      >
      ...
      <h1 className="text-lg font-semibold text-gray-900 dark:text-white mb-3 tracking-tight">Khusus</h1>

      {loading ? (
        <div className="flex justify-center py-8">
          <div className="animate-spin rounded-full h-6 w-6 border-2 border-teal-500 dark:border-teal-400 border-t-transparent" />
        </div>
      ) : (
        <>
          <section className="mb-4 pt-3 border-t border-gray-200 dark:border-gray-600">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="rounded-lg bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-600 p-2.5 text-center shadow-sm">
                <p className="text-[10px] text-green-600 dark:text-green-400 mb-0.5">Bayar</p>
                <p className="text-xs font-bold text-green-700 dark:text-green-300 tabular-nums">{formatCurrency(total.bayar)}</p>
              </div>
              <div className="rounded-lg bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-600 p-2.5 text-center shadow-sm">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs">
                      ...
                      {showBayar && (
                        <button
                          ...
                          className="ml-auto inline-flex items-center gap-1.5 px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white text-xs font-medium rounded-lg"
                        >
          ...
          <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-600">
            <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
              Pembayaran Khusus mencakup pembayaran yang hanya dilalui santri 1 kali selama nyantri.
            </p>
            <ul className="mt-1.5 text-xs text-gray-600 dark:text-gray-400 list-disc list-inside space-y-0.5">
              <li>UJBA</li>
              <li>Guru Tugas</li>
              <li>KKN</li>
              <li>PLP / PPL</li>
          ...
          <BayarOffcanvas
            ...
            title="Bayar Khusus (iPayMu)"
            jenisPembayaran="Khusus"
            idSantri={idSantri}
            idReferensi={bayarFor?.id ?? null}
            tabelReferensi="uwaba___khusus"
            wajib={bayarFor?.wajib ?? 0}
            kurang={bayarFor?.kurang ?? 0}
            onSuccess={() => { setBayarFor(null); Promise.all([pembayaranAPI.getRincian(idSantri, 'khusus'), pembayaranAPI.getHistory(idSantri, 'khusus')]).then(([r1, r2]) => { if (r1?.success && r1?.data) { setRincian(r1.data.rincian || []); setTotal(r1.data.total || { total: 0, bayar: 0, kurang: 0 }) } if (r2?.success && r2?.data) setHistory(uniqueHistoryById(Array.isArray(r2.data) ? r2.data : [])) }) }}
          />
        </>
      )}
    </div>
