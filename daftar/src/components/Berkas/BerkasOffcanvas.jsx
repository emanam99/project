  ...
  useEffect(() => {
    if (isOpen && idSantri) {
      ...
      if (existingBerkas) {
        ...
        setJenisBerkas(defaultJenisBerkas || 'Ijazah SD Sederajat')
        setKeterangan('')
      }
  ...
  const handleFileSelect = async (e) => {
    ...
    if (isImage && file.size > maxSizeBytes) {
      try {
        ...
        navigate('/editor', { state: { file: compressed } })
      } catch (err) {
              ...
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={!!existingBerkas}
                    className="w-full px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 disabled:opacity-60"
                  >
                    {jenisBerkasOptions.map((opt) => (
                      <option key={opt} value={opt}>{opt}</option>
                    ))}
                  </select>
                  {existingBerkas && (
                    <p className="text-xs text-gray-400 mt-1 truncate">Mengganti: {existingBerkas.nama_file}</p>
                  )}
                </div>
                  ...
                  {selectedFile && (
                    <div className={`mt-3 p-3 rounded-xl flex items-center gap-3 ${fileReady ? 'bg-teal-50 dark:bg-teal-900/20 border border-teal-100 dark:border-teal-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800'}`}>
                      ...
                      {previewUrl && (
                        <button
                          type="button"
                          onClick={() => navigate('/editor', { state: { file: selectedFile } })}
