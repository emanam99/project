...
import { formatPaymentApiError } from '../utils/ipaymuPayment'
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (originalRequest?.url?.includes('/auth/login') ||
        originalRequest?.url?.includes('/auth/daftar-wa-') ||
        originalRequest?.url?.includes('/alumni/login-nik')) {
      return Promise.reject(error)
    }
...
export const authAPI = {
  /** Login NIK; jika NIK sudah ada di DB, kirim tanggal_lahir (Y-m-d) + nama_ibu. */
  login: async (nik, tanggalLahir = null, namaIbu = null) => {
    const body = { nik }
    if (tanggalLahir) body.tanggal_lahir = tanggalLahir
    if (namaIbu) body.nama_ibu = namaIbu
    const response = await api.post('/auth/login-nik', body)
    return response.data
  },

  /** Legacy: siapkan token + URL wa.me (link lama / fallback). */
  prepareDaftarWa: async (nik, noWa) => {
    const response = await api.post('/auth/daftar-wa-prepare', { nik, no_wa: noWa })
    return response.data
  },

  /** Tukar token sekali pakai (dari link WA) menjadi JWT sesi daftar. */
  consumeDaftarWaToken: async (token) => {
    const response = await api.post('/auth/daftar-wa-consume', { token })
    return response.data
  },
...
export const pendaftaranAPI = {
  searchByNik: async (nik) => {
    // Pakai instance `api` agar interceptor (Authorization Bearer + CSRF) ikut.
    // Backend: tanpa token → respons hanya { exists }. Dengan token valid → ada id/nis/nama.
    // Jangan swallow error jadi exists:false — caller (Login) harus bisa bedakan gagal cek vs NIK baru.
    const response = await api.get(`/pendaftaran/check-nik?nik=${encodeURIComponent(nik)}`)
    return response.data
  },
...
export const paymentTransactionAPI = {
  ...
  createTransaction: async (data) => {
    try {
      ...
      return {
        success: false,
        message: formatPaymentApiError(body, 'Gagal membuat transaksi pembayaran'),
  ...
  cancelTransaction: async (transactionId) => {
    try {
      ...
      return {
        success: false,
        message: formatPaymentApiError(body, 'Gagal membatalkan transaksi'),
        ...(body && typeof body === 'object' ? body : {}),
      }
    }
  },
  ...
  updateTransaction: async (transactionId, data) => {
    try {
      ...
      return {
        success: false,
        message: formatPaymentApiError(body, 'Gagal mengubah transaksi'),
        ...(body && typeof body === 'object' ? body : {}),
      }
    }
  },
