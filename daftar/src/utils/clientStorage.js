...
/** Sidik jari NIK untuk perbandingan sesi — tidak menyimpan NIK mentah di localStorage. */
export function nikFingerprint(nik) {
...
/** Simpan user_data tanpa NIK; NIK hanya di sessionStorage (tab). */
export function persistDaftarUserData(user) {
  if (!user || typeof user !== 'object') return
  const nik = user.nik != null && String(user.nik).trim() !== '' ? String(user.nik).trim() : null
  if (nik && typeof sessionStorage !== 'undefined') {
    try {
  ...
  delete forDisk.nik
  try {
    localStorage.setItem('user_data', JSON.stringify(forDisk))
  } catch (_) { /* ignore */ }
}
...
export function ensureStorageNik(currentNik) {
  ...
  const fp = nikFingerprint(nik)
  const stored = localStorage.getItem(STORAGE_NIK_KEY)
  if (stored != null && stored !== fp) {
    // Nilai lama bisa berupa NIK mentah; anggap sama jika fingerprint cocok
    const sameAsLegacy = /^\d{16}$/.test(stored) && nikFingerprint(stored) === fp
    if (!sameAsLegacy) {
      clearDaftarDataOnly()
    }
  }
  try {
    localStorage.setItem(STORAGE_NIK_KEY, fp)
  } catch (e) { /* ignore */ }
  try {
    if (typeof sessionStorage !== 'undefined') {
      sessionStorage.setItem('daftar_login_nik', nik)
