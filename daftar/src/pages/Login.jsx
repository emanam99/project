...
import { APP_LOGO_URL, getGambarUrl } from '../config/images'
...
/** Nomor admin bantuan pendaftaran (sama dengan Dashboard / FAQ). */
const HELP_WA_NUMBER = '6285123123399'

function buildHelpWaUrl(nik, tanggalLahir, namaIbu) {
  const lines = [
    'Butuh bantuan untuk pendaftaran santri.',
    `NIK: ${nik || '-'}`,
  ]
  if (tanggalLahir) lines.push(`Tanggal lahir: ${tanggalLahir}`)
  if (namaIbu) lines.push(`Nama ibu: ${namaIbu}`)
  return `https://wa.me/${HELP_WA_NUMBER}?text=${encodeURIComponent(lines.join('\n'))}`
}
...
function Login() {
  ...
  const consumingWaRef = useRef(false)
  ...
  // Link lama dari balasan WA: /login?wa_token=...
  useEffect(() => {
    const waToken = (searchParams.get('wa_token') || '').trim().toLowerCase()
    if (!waToken || waToken.length !== 64 || consumingWaRef.current) return
    consumingWaRef.current = true
    let cancelled = false
    ;(async () => {
      setLoading(true)
      setError('')
      try {
        const response = await authAPI.consumeDaftarWaToken(waToken)
        if (cancelled) return
        if (!response.success || !response.data?.token) {
          setError(response.message || 'Token WhatsApp tidak valid')
          return
        }
        const user = response.data.user
        const nikVal = user?.nik || ''
        try { sessionStorage.setItem('daftar_login_nik', nikVal) } catch { /* ignore */ }
        const userToSet = user
          ? { ...user, nik: user.nik || nikVal, nis: normalizeNisForStorage(user.nis ?? user.NIS) }
          : user
        ...
        if (cancelled) return
        setError(err?.response?.data?.message || 'Gagal masuk lewat link WhatsApp')
      } finally {
        if (!cancelled) setLoading(false)
        consumingWaRef.current = false
      }
    })()
    return () => { cancelled = true }
  }, [searchParams, setAuth, setSearchParams, navigate])
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        if (response.success && response.data && typeof response.data.exists === 'boolean') {
          ...
          // Jangan anggap "baru" jika cek gagal — server tetap sumber kebenaran
          setNikExists(null)
          checkedNikRef.current = null
          setError(response.message || 'Gagal mengecek NIK. Coba lagi.')
        }
      } catch (err) {
        console.error('Error checking NIK:', err)
        setNikExists(null)
        checkedNikRef.current = null
        setError(err?.response?.data?.message || 'Gagal mengecek NIK. Coba lagi.')
      } finally {
  ...
  const applyLoginSuccess = (response) => {
    const user = response.data.user
    const nikVal = user?.nik || nik
    try { sessionStorage.setItem('daftar_login_nik', nikVal) } catch { /* ignore */ }
    const userToSet = user
      ? { ...user, nik: user.nik || nikVal, nis: normalizeNisForStorage(user.nis ?? user.NIS) }
      : user
  ...
  const isVerifyHelpCode = (code) =>
    code === 'ttl_mismatch' ||
    code === 'verify_mismatch' ||
    code === 'need_help'

  const handleLoginFailureCode = (code, message) => {
    if (code === 'ttl_required' || code === 'ibu_required' || isVerifyHelpCode(code)) {
      setNikExists(true)
    }
    if (isVerifyHelpCode(code)) {
      setShowHelpWa(true)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (nikExists === null) {
      setError('Sedang mengecek NIK atau cek gagal. Tunggu / coba lagi.')
      return
    }
    if (nikExists === true && !tanggalLahir) {
      setError('Tanggal lahir wajib diisi karena data NIK sudah ada')
      return
    }
    if (nikExists === true && !namaIbu.trim()) {
      setError('Nama ibu wajib diisi karena data NIK sudah ada')
      return
    }
    setError('')
    setShowHelpWa(false)
    setLoading(true)
    try {
      // Kirim TTL/ibu bila sudah diisi (aman jika NIK ternyata sudah ada di server)
      ...
      if (!response.success || !response.data?.token) {
        handleLoginFailureCode(response.code || '', response.message)
        return
      }
      applyLoginSuccess(response)
    } catch (err) {
      ...
      if (!err.response) {
        ...
        setError(msg || 'Terlalu banyak percobaan. Silakan tunggu sebentar.')
      } else if (code) {
        handleLoginFailureCode(code, msg)
      } else {
  ...
  const openHelpWa = () => {
    window.open(buildHelpWaUrl(nik, tanggalLahir, namaIbu.trim()), '_blank', 'noopener,noreferrer')
  }
  ...
  const getButtonText = () => {
    ...
    if (nik.length === 16) {
      ...
      if (nikExists === true) return 'Verifikasi & masuk'
      if (nikExists === false) return 'Masuk / Daftar'
    }
    return 'Masuk'
  }
  ...
  const canSubmit =
    ...
    nik.length === 16 &&
    nikExists !== null &&
    (nikExists === false || (nikExists === true && !!tanggalLahir && !!namaIbu.trim()))
        ...
        >
          ...
          >
            ...
            <div className="hidden md:block text-center mb-8">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">Masukkan NIK. Jika data sudah ada, verifikasi tanggal lahir dan nama ibu</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 md:space-y-5" style={{ perspective: '600px' }}>
                ...
                <>
                    ...
                    >
                      <div className="relative">
                        ...
                        <input
                          ...
                          onFocus={() => setFocusedInput('ttl')}
                          onBlur={() => setFocusedInput(null)}
                          className="w-full pl-12 pr-4 py-3 border-0 rounded-none bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-0 text-base"
                          required
                        />
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-center text-gray-400 dark:text-gray-500">
                      Data NIK sudah ada. Verifikasi tanggal lahir dan nama ibu.
                    </p>
                  </motion.div>
                </>
              )}

              {nikExists === false && nik.length === 16 && !checkingNik && (
                <motion.p
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-xs text-center text-green-600 dark:text-green-400"
                >
                  NIK belum terdaftar — lanjut isi formulir pendaftaran.
                </motion.p>
              )}
              ...
              <motion.button
                ...
                whileHover={{ scale: !canSubmit ? 1 : 1.01 }}
                whileTap={{ scale: 0.99 }}
              >
                {loading || checkingNik ? (
              ...
              {showHelpWa ? (
                <motion.div
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="space-y-2 rounded-xl border border-teal-200 dark:border-teal-800/60 bg-teal-50/80 dark:bg-teal-900/20 p-4"
                >
                  <p className="text-sm text-teal-900 dark:text-teal-100 text-center">
                    Admin akan memperbaiki data secara manual. Kirim pesan bantuan lewat WhatsApp.
                  </p>
                  <button
                    type="button"
                    onClick={openHelpWa}
                    className="w-full py-3 rounded-xl font-semibold text-white bg-[#25D366] hover:bg-[#1ebe57] transition-colors"
                  >
                    WhatsApp: butuh bantuan pendaftaran
                  </button>
                </motion.div>
              ) : null}
        ...
        {showInfoModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-4">
                <img src={getGambarUrl('/info/nik.jpg')} alt="Info NIK" className="w-full h-auto rounded-xl shadow-sm" />
