...
import FormulirPrintOffcanvas from '../components/FormulirPrintOffcanvas'
import { canPrintFormulirByFormal, canPrintFormulirByPayment } from '../utils/formulirFormalPrint'
...
function Dashboard() {
  ...
  const [daftarFormal, setDaftarFormal] = useState(() => {
    try {
      return localStorage.getItem('daftar_formal') || ''
    } catch {
      return ''
    }
  })
  const [showFormulirPrint, setShowFormulirPrint] = useState(false)
  const [registrasiBayar, setRegistrasiBayar] = useState(0)

  const showTombolPrintFormulir =
    canPrintFormulirByFormal(daftarFormal) &&
    canPrintFormulirByPayment(registrasiBayar) &&
    user?.id &&
        ...
        if (user?.id) {
          ...
          try {
            ...
            if (registrasiResult.success && registrasiResult.data) {
              ...
              if (registrasi.daftar_formal) {
                setDaftarFormal(String(registrasi.daftar_formal).trim())
              }
              const bayar = Number(registrasi.bayar || 0)
              setRegistrasiBayar(bayar)
  ...
  return (
    <div className="h-full overflow-y-auto bg-white dark:bg-gray-900 rounded-lg shadow-sm p-3 md:p-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2 gap-3">
            ...
            {showTombolPrintFormulir ? (
              <button
                type="button"
                onClick={() => setShowFormulirPrint(true)}
                className="px-3 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors text-sm font-semibold inline-flex items-center gap-2 shadow-sm"
      ...
      <FormulirPrintOffcanvas
        isOpen={showFormulirPrint}
        onClose={() => setShowFormulirPrint(false)}
        idSantri={user?.id}
