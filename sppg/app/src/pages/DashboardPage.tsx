...
function TrendBadge({ pct }: { pct: number | null }) {
  if (pct == null) return <span className="text-[11px] text-muted">—</span>
  const up = pct > 0
  const flat = pct === 0
  const color = flat ? 'text-muted' : up ? 'text-[var(--ok-ink)]' : 'text-[var(--danger)]'
  const bg = flat ? 'bg-surface-soft' : up ? 'bg-[var(--ok-bg)]' : 'bg-[var(--danger-bg)]'
  const arrow = flat ? '→' : up ? '▲' : '▼'
  return (
    <span className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[11px] font-semibold ${color} ${bg}`}>
...
export default function DashboardPage() {
  ...
  const peakDay = useMemo(() => {
    ...
    return data.daily.reduce((best, cur) => (cur.total > best.total ? cur : best), data.daily[0])
  }, [data])
  ...
  const kpis = [
    {
      label: 'Hari ini',
      value: formatRp(data.total_hari_ini),
      hint: `${data.catatan_hari_ini} catatan`,
      trend: data.pct_vs_kemarin,
      trendLabel: 'vs kemarin',
      accent: true,
    },
    {
      label: 'Kemarin',
      value: formatRp(data.total_kemarin),
      hint: 'Acuan harian',
    ...
    {
      label: 'Bulan ini',
      value: formatRp(data.total_bulan_ini),
      hint: `Rata ${formatRp(data.rata_harian_bulan)}/hari`,
      trend: data.pct_vs_bulan_lalu,
      trendLabel: 'vs bulan lalu',
      accent: false,
    },
    {
      label: 'Total semua',
      value: formatRp(data.total_semua),
      hint: `${data.jumlah_catatan} catatan · ${data.jumlah_item} item`,
      trend: null as number | null,
      trendLabel: '',
      accent: false,
    },
  ...
  return (
    <div className="space-y-3.5 min-w-0 max-w-full overflow-x-hidden">
      <div className="flex flex-wrap items-center justify-end gap-2">
        <Link to="/belanja?hari=1" className="ui-btn-ghost">
          Lihat hari ini
        </Link>
        {canManage && (
          <Link to="/belanja/baru" className="ui-btn-primary">
            + Catat belanja
          </Link>
        )}
      </div>
        ...
        {kpis.map((c) => (
          ...
          >
            <div className="flex items-start justify-between gap-1.5 min-w-0">
              ...
              {c.trend != null && <TrendBadge pct={c.trend} />}
            </div>
            ...
            {[...data.daily_delta].reverse().map((row) => (
              <li key={row.tanggal} className="flex items-center justify-between gap-2 py-1 border-b border-line last:border-0 min-w-0">
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-muted tabular-nums">{formatRp(row.total)}</div>
                </div>
                    ...
                    <>
                      <div
                        className={`text-[12px] font-semibold tabular-nums ${
                          row.delta > 0 ? 'text-[var(--ok-ink)]' : row.delta < 0 ? 'text-[var(--danger)]' : 'text-muted'
                        }`}
                      >
                        {row.delta > 0 ? '+' : ''}
                        {formatRp(row.delta)}
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-2.5">
        <section className="ui-card p-3">
          <h2 className="ui-section-title mb-2.5">Per kategori · 30 hari</h2>
          {data.by_kategori.length === 0 ? (
            <p className="text-[13px] text-muted">Belum ada data.</p>
          ) : (
            <HorizonBars rows={data.by_kategori} />
          )}
        </section>

        <section className="ui-card p-3">
          <h2 className="ui-section-title mb-2.5">Per rekening · 30 hari</h2>
          {data.by_rekening.length === 0 ? (
            <p className="text-[13px] text-muted">Belum ada data.</p>
          ) : (
            <HorizonBars rows={data.by_rekening} />
          )}
        </section>
      ...
      <section className="ui-card p-3">
        <div className="flex items-center justify-between gap-2 mb-2">
          ...
          <Link to="/belanja" className="text-[12px] font-semibold text-[var(--accent)] hover:underline">
            Semua →
          </Link>
        </div>
        {data.recent.length === 0 ? (
          <p className="text-[13px] text-muted">Belum ada catatan belanja.</p>
        ) : (
          <ul className="divide-y divide-[var(--line)]">
            {data.recent.map((row) => (
              <li key={row.id}>
                <Link to={`/belanja/${row.id}`} className="flex items-center justify-between gap-2 py-2 hover:bg-surface-soft/60 rounded-md px-1 -mx-1 transition">
                  <div className="min-w-0">
