  ...
  const applyNamaSuggestion = (key: string, nama: string) => {
    ...
    setItems((prev) =>
      prev.map((it) => {
        ...
        // Isi satuan/harga dari riwayat jika masih default / kosong
    ...
    if (res.success && res.data?.belanja?.id) {
      ...
      if (pendingFiles.length > 0) {
        ...
        if (fail > 0) {
          ...
          navigate(`/belanja/${id}`, { replace: true })
          return
        }
      }
      setLoading(false)
  ...
  return (
    <div className="space-y-3.5 max-w-3xl">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <Link to="/belanja" className="text-[12px] text-muted hover:underline">
          ← Kembali
        </Link>
      ...
      <form onSubmit={(e) => void handleSubmit(e)} className="ui-card p-3 space-y-3.5">
        ...
        <div className="space-y-2.5">
          <h2 className="font-semibold text-ink text-[13px]">Item belanja</h2>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Mis. Belanja mingguan pasar"
            rows={3}
          />
        </div>
