...
export default function UsersPage() {
  ...
  const roleOptions = (current: string) => {
    const opts = [
      ...
      { value: 'admin_maker', label: 'admin_maker' },
      { value: 'admin_approve', label: 'admin_approve' },
    ]
      ...
      <form onSubmit={(e) => void handleCreate(e)} className="ui-card p-3 grid sm:grid-cols-3 gap-2">
        ...
        <div>
          ...
          <select className="ui-input" value={role} onChange={(e) => setRole(e.target.value)}>
            ...
            <option value="admin_maker">admin_maker</option>
            <option value="admin_approve">admin_approve</option>
      ...
      <section className="ui-card p-3 space-y-2">
        ...
        <ul className="space-y-1.5 text-[12px] text-muted leading-snug">
          ...
          <li>
            ...
            {' — '}
            bisa melihat dashboard, belanja, dan rekening (tanpa menambah/mengubah/menghapus).
          </li>
          <li>
            <span className="font-semibold text-ink">admin</span>
            {' — '}
            kelola belanja &amp; rekening; tidak bisa ubah status BNI (Belum/Maker/Approved).
          </li>
          <li>
            <span className="font-semibold text-ink">admin_maker</span>
            {' — '}
            seperti admin, plus ubah status Belum → Maker. Catatan Maker/Approved terkunci.
          </li>
          <li>
            <span className="font-semibold text-ink">admin_approve</span>
            {' — '}
            seperti admin, plus ubah status Maker → Approved. Catatan Maker/Approved terkunci.
          </li>
          <li>
            <span className="font-semibold text-ink">super_admin</span>
            {' — '}
            kelola pengguna; maju status BNI; pada catatan terkunci bisa hapus list / ubah
            keterangan &amp; kategori saja (item tetap terkunci).
          </li>
        </ul>
      </section>
    </div>
