...
import { santriAPI, chatAPI, pendaftaranAPI, uwabaAPI, santriBiodataAPI } from '../../services/api'
...
import { parseKelompok } from '../../pages/Lttq/lttqKelompokUtils'

const LTTQ_TIDAK_MENGAJI = 'Tidak Mengaji'
...
function BiodataBox({ onSantriChange, onOpenSearch, externalSantriId }) {
  ...
  const [tidakMengajiLttqChecked, setTidakMengajiLttqChecked] = useState(true)
    ...
    if (prevExternalSantriIdRef.current !== externalSantriId) {
      ...
      if (externalSantriId) {
        ...
        if (externalSantriId !== santriId) {
          ...
          setHasChanges(false)
          // Jangan onSantriChange(null): di UWABA parent memakai externalSantriId sebagai sumber NIS;
          // null memicu setCurrentSantri(null) dan race dengan URL/offcanvas → render berkedip.
          if (onSantriChange) {
            if (getPage() === 'uwaba') {
              onSantriChange({
                nis: String(externalSantriId),
                id: String(externalSantriId),
                status_santri: '',
                kategori: '',
                diniyah: '',
                formal: '',
  ...
  useEffect(() => {
    santriBiodataAPI.getLttqTingkatanOptions({ lembaga_id: 'LTTQ', status: 'aktif', limit: 500 })
      .then((res) => {
        setLttqTingkatanOptions(res?.success && Array.isArray(res.data) ? res.data : [])
      })
      .catch(() => setLttqTingkatanOptions([]))
  }, [])

  /** Agar kalkulator UWABA mengenali «Tidak Mengaji» saat checkbox aktif (kolom DB hanya id_lttq_tingkatan). */
  useEffect(() => {
    if (!biodata || !tidakMengajiLttqChecked) return
    if (
      String(biodata.lttq ?? '').trim() !== LTTQ_TIDAK_MENGAJI &&
      (biodata.id_lttq_tingkatan ?? '') === ''
    ) {
      setBiodata((prev) =>
        prev
          ? {
              ...prev,
              lttq: LTTQ_TIDAK_MENGAJI,
              id_lttq_tingkatan: '',
              kelas_lttq: '',
              kel_lttq: ''
            }
  ...
  }, [tidakMengajiLttqChecked, biodata?.id, biodata?.id_lttq_tingkatan, biodata?.lttq])

  const lttqProgramOptions = useMemo(() => {
    const map = new Map()
    lttqTingkatanOptions.forEach((t) => {
      const tk = String(t.tingkatan || '').trim()
      if (!tk || tk === LTTQ_TIDAK_MENGAJI) return
  ...
  const lttqProgramSelected = useMemo(() => {
    if (!biodata || tidakMengajiLttqChecked) return ''
    const fromField = String(biodata.lttq ?? '').trim()
    if (fromField && fromField !== LTTQ_TIDAK_MENGAJI) return fromField
    const row = lttqTingkatanOptions.find((t) => String(t.id) === String(biodata.id_lttq_tingkatan ?? ''))
    return row ? String(row.tingkatan || '').trim() : ''
  }, [biodata, tidakMengajiLttqChecked, lttqTingkatanOptions])

  const lttqKelasOptions = useMemo(() => {
    if (!lttqProgramSelected) return []
    const set = new Set()
    lttqTingkatanOptions
      .filter((t) => String(t.tingkatan || '').trim() === lttqProgramSelected)
      .forEach((t) => {
  ...
  const lttqKelasSelected = useMemo(() => {
    if (!biodata || tidakMengajiLttqChecked) return ''
    return String(biodata.kelas_lttq ?? '').trim()
  }, [biodata, tidakMengajiLttqChecked])

  const lttqKelRowOptions = useMemo(() => {
    if (!lttqProgramSelected || !lttqKelasSelected) return []
    return lttqTingkatanOptions
      .filter((t) => {
        if (String(t.tingkatan || '').trim() !== lttqProgramSelected) return false
        const { kelas } = parseKelompok(t.kelompok)
        return String(kelas).trim() === lttqKelasSelected
      })
      .map((t) => {
        ...
        return {
          id: t.id,
          kel: kelLabel,
          label: kelompok ? `${kelLabel} (${kelompok})` : kelLabel
        }
      })
      .sort((a, b) => a.label.localeCompare(b.label))
  }, [lttqTingkatanOptions, lttqProgramSelected, lttqKelasSelected])

  const handleLttqProgramChange = (value) => {
    setBiodata((prev) => {
      if (!prev) return prev
      return {
        ...prev,
  ...
  const handleLttqKelasChange = (value) => {
    setBiodata((prev) => {
      if (!prev) return prev
      return {
        ...prev,
  ...
  const handleLttqKelChange = (e) => {
    const idVal = e.target.value === '' ? '' : e.target.value
    setBiodata((prev) => {
      if (!prev) return prev
      const row = lttqTingkatanOptions.find((t) => String(t.id) === String(idVal))
      if (!row) {
        return {
          ...prev,
          id_lttq_tingkatan: '',
  ...
  const handleTidakMengajiLttqChange = (checked) => {
    setTidakMengajiLttqChecked(checked)
    if (checked) {
      setBiodata((prev) => {
        if (!prev) return prev
        return {
      ...
      setBiodata((prev) => {
        ...
        return {
          ...prev,
          lttq: prev.lttq === LTTQ_TIDAK_MENGAJI ? '' : prev.lttq,
          id_lttq_tingkatan: '',
          kelas_lttq: '',
          kel_lttq: ''
        }
  ...
  // Load opsi master status + lembaga
  useEffect(() => {
    if (!biodata) return
    let cancelled = false
    Promise.all([
  ...
  useEffect(() => {
    if (!biodata) return
    if (biodata.kategori && kategoriOptions.length > 0 && !kategoriOptions.includes(biodata.kategori)) {
      handleFieldChange('kategori', '')
      handleFieldChange('id_daerah', '')
      handleFieldChange('id_kamar', '')
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kategoriOptions, biodata?.id])
  ...
  useEffect(() => {
    ...
    const lttqFilled =
      (biodata.id_lttq_tingkatan ?? '') !== '' ||
      (String(biodata.lttq ?? '').trim() !== '' && String(biodata.lttq).trim() !== LTTQ_TIDAK_MENGAJI)
    const lttqTidakMengajiOnly =
      String(biodata.lttq ?? '').trim() === LTTQ_TIDAK_MENGAJI && (biodata.id_lttq_tingkatan ?? '') === ''
    if (biodata.id !== prevBiodataIdRef.current) {
      ...
      setTidakMengajiLttqChecked(!lttqFilled || lttqTidakMengajiOnly)
    } else {
      if (diniyahFilled) setTidakSekolahDiniyahChecked(false)
      if (formalFilled) setTidakSekolahFormalChecked(false)
      if (lttqFilled && !lttqTidakMengajiOnly) setTidakMengajiLttqChecked(false)
    }
  }, [
    biodata?.id,
    biodata?.id_diniyah,
    biodata?.lembaga_diniyah,
    biodata?.diniyah,
    biodata?.id_formal,
  ...
  // Save biodata:
  // - Saat "Tidak Sekolah" dicentang, kirim null agar id_diniyah/id_formal benar-benar dibersihkan di backend.
  // - Untuk field relasi lain, nilai kosong tetap dihapus dari payload agar tidak menimpa tak sengaja.
  const handleSave = async () => {
    ...
    if (tidakSekolahDiniyahChecked) {
      payload.id_diniyah = null
      payload.nim_diniyah = ''
    } else if (payload.id_diniyah === '' || payload.id_diniyah == null) {
      delete payload.id_diniyah
    }
    if (tidakSekolahFormalChecked) {
      payload.id_formal = null
      payload.nim_formal = ''
    } else if (payload.id_formal === '' || payload.id_formal == null) {
      delete payload.id_formal
    }
    ...
    delete payload.lttq
    delete payload.kelas_lttq
    delete payload.kel_lttq
    if (tidakMengajiLttqChecked) {
      payload.id_lttq_tingkatan = null
    } else if (payload.id_lttq_tingkatan === '' || payload.id_lttq_tingkatan == null) {
      payload.id_lttq_tingkatan = null
    } else {
      payload.id_lttq_tingkatan = Number(payload.id_lttq_tingkatan)
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md h-full flex flex-col overflow-hidden relative">
      ...
      <div className="flex-shrink-0 bg-gray-200 dark:bg-gray-700/50 p-2 border-b-2 border-gray-300 dark:border-gray-600">
        <div className="flex gap-1.5 items-center justify-between">
          <div className="flex gap-1.5 items-center">
            <span className="text-teal-600 dark:text-teal-400 font-semibold whitespace-nowrap text-sm">
              NIS
            </span>
            <input
              type="text"
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-6" style={{ 
        ...
        {biodata && !loading && (
          <form ref={formRef}>
          <div key={`bio-form-${biodata.id}`} className="space-y-4">
          {/* Nama */}
          <div>
          ...
          {/* LTTQ: Tidak Mengaji atau Tingkatan → Kelas → Kel */}
          <div className="mt-2">
            <label className="inline-flex items-center gap-2 cursor-pointer mb-1">
              <input
                type="checkbox"
                checked={tidakMengajiLttqChecked}
                onChange={(e) => handleTidakMengajiLttqChange(e.target.checked)}
                className="w-4 h-4 text-teal-600 border-gray-300 dark:border-gray-600 rounded focus:ring-teal-500"
              />
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Tidak Mengaji (LTTQ)</span>
            </label>
            <AnimatePresence initial={false}>
              {!tidakMengajiLttqChecked && (
                <motion.div
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
                  exit={{ opacity: 0, height: 0 }}
                ...
                >
                  <div className="flex gap-4 flex-wrap items-end">
                    <div className="flex-1 min-w-0">
                      <label className={getLabelClassName('lttq')}>Tingkatan</label>
                      <select
                        value={lttqProgramSelected}
                        onChange={(e) => handleLttqProgramChange(e.target.value)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <select
                        value={lttqKelasSelected}
                        onChange={(e) => handleLttqKelasChange(e.target.value)}
                        onFocus={() => setFocusedField('kelas_lttq')}
                        onBlur={() => setFocusedField(null)}
                        disabled={!lttqProgramSelected}
                        className="w-full p-2 border-b-2 border-gray-300 dark:border-gray-600 focus:border-teal-500 focus:outline-none bg-transparent text-gray-900 dark:text-gray-100 disabled:opacity-50"
                      >
                        <option value="">Pilih Kelas</option>
                    ...
                    <div className="flex-1 min-w-0">
                      <label className={getLabelClassName('id_lttq_tingkatan')}>Kel</label>
                      <select
                        value={
                          biodata.id_lttq_tingkatan != null && biodata.id_lttq_tingkatan !== ''
                            ? String(biodata.id_lttq_tingkatan)
                            : ''
                        }
                        onChange={handleLttqKelChange}
                        onFocus={() => setFocusedField('id_lttq_tingkatan')}
                        onBlur={() => setFocusedField(null)}
                        disabled={!lttqProgramSelected || !lttqKelasSelected}
                        className="w-full p-2 border-b-2 border-gray-300 dark:border-gray-600 focus:border-teal-500 focus:outline-none bg-transparent text-gray-900 dark:text-gray-100 disabled:opacity-50"
                      >
                        <option value="">Pilih Kel</option>
                        {lttqKelRowOptions.map((r) => (
