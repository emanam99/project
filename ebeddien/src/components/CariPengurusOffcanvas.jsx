 ...
 * - onlyWithoutUser: bool — hanya tampilkan pengurus tanpa akun (id_user kosong)
 * - allowUserId: number (opsional) — bersama onlyWithoutUser, tetap tampilkan yang sudah tertaut ke user ini
 */
...
/** Di atas offcanvas form madrasah (100021), GT (100019), Cari Madrasah (100021). */
export const CARI_PENGURUS_Z_BACKDROP = 100030
export const CARI_PENGURUS_Z_PANEL = 100031

export default function CariPengurusOffcanvas({
  isOpen,
  onClose,
  onSelect,
  title = 'Cari Pengurus',
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      // Filter onlyWithoutUser butuh id_user dari server — jangan pakai cache lama tanpa field itu
      if (!onlyWithoutUser) {
  ...
  }, [isOpen, cacheKey, loadListFromServer, onlyWithoutUser])
  ...
  const filteredList = useMemo(() => {
    let list = [...pengurusList]
    if (onlyWithoutUser) {
      const allowId = allowUserId != null && allowUserId !== '' ? Number(allowUserId) : null
      list = list.filter((p) => {
        const uid = p.id_user != null && p.id_user !== '' ? Number(p.id_user) : null
        if (uid == null || Number.isNaN(uid) || uid <= 0) return true
        return allowId != null && uid === allowId
      })
    }
    ...
    return list
  }, [pengurusList, searchQuery, filters, onlyWithoutUser, allowUserId])
