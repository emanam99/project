...
function DetailOffcanvas({
  ...
  canEdit = false,
  canApprove = false,
  /** Jika tidak diisi, mengikuti canApprove (perilaku lama) */
  canReject = undefined,
  /** Pengeluaran: tombol edit header. undefined = true (perilaku lama) */
  ...
  const showPengeluaranDelete =
    type === 'pengeluaran' &&
    (canDeletePengeluaran !== undefined ? canDeletePengeluaran : isSuperAdmin)
  const showPengeluaranEdit =
    type === 'pengeluaran' && (canEditPengeluaran !== undefined ? canEditPengeluaran : true)
  const effectiveCanReject = canReject !== undefined ? canReject : canApprove
      ...
      <AnimatePresence>
        ...
        {isOpen && (
          ...
          >
                      ...
                      <div className="space-y-6 lg:overflow-y-auto lg:pr-2 lg:h-full">
                        ...
                        <div>
                          ...
                          <div className="rounded-xl border border-gray-200/90 dark:border-gray-600 bg-gray-50/80 dark:bg-gray-800/60 p-4 space-y-3">
                            ...
                            {detailData.ket === 'di approve' && detailData.catatan?.trim() && (
                              <div className="flex justify-between gap-3">
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400 shrink-0">Catatan</span>
                                <span className="text-sm text-gray-900 dark:text-gray-100 text-right whitespace-pre-wrap">
                                  {detailData.catatan}
                                </span>
                              </div>
                            )}
                            {detailData.ket === 'ditolak' && detailData.catatan?.trim() && (
                              <div className="flex justify-between gap-3">
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400 shrink-0">Alasan</span>
                                <span className="text-sm text-gray-900 dark:text-gray-100 text-right whitespace-pre-wrap">
                                  {detailData.catatan}
                                </span>
                              </div>
              ...
              {
                type === 'rencana' && (canEdit || canApprove || effectiveCanReject) && (
