...
import { getSlimApiUrl, getAuthHeaders, waAPI, chatAPI } from '../../services/api'
...
function PendaftaranPrintOffcanvas({
  isOpen,
  onClose,
  santriId,
  tahunHijriyah = '',
  tahunMasehi = '',
  forcePrintSections = null,
  /** Naikkan layer saat dibuka di atas offcanvas kanan (mis. Tes Masuk z-[9999]). */
  stackOnTop = false,
}) {
  const [printSections, setPrintSections] = useState(loadPrintSectionsFromStorage)
  const effectivePrintSections = forcePrintSections ?? printSections
  const {
    printKwitansi,
    printBiodataForm,
    printRaporTes,
  ...
  useEffect(() => {
    if (isOpen && santriId && /^\d{7}$/.test(santriId)) {
      ...
      if (h) params.set('tahun_hijriyah', h)
      if (m) params.set('tahun_masehi', m)
  ...
  }, [isOpen, santriId, user, tahunHijriyah, tahunMasehi])

  useEffect(() => {
    if (!forcePrintSections) {
      savePrintSectionsToStorage(printSections)
    }
  }, [printSections, forcePrintSections])
...
Barakallahu fiikum.`
  ...
  const backdropZ = stackOnTop ? 'z-[10002]' : 'z-40'
  const panelZ = stackOnTop ? 'z-[10003]' : 'z-50'
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className={`no-print fixed inset-0 bg-black bg-opacity-50 ${backdropZ}`}
          />
          ...
          <motion.div
            ...
            className={`print-offcanvas-wrapper fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 rounded-t-2xl shadow-[0_-10px_25px_-5px_rgba(0,0,0,0.1)] ${panelZ} overflow-hidden flex flex-col`}
