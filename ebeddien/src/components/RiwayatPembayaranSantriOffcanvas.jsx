    ...
    const run = async () => {
      try {
        await Promise.all([
          ...
          paymentAPI.getPublicUwabaTahunList(idSantri).then((res) => {
          ...
          paymentAPI.getPublicRincian(idSantri, 'khusus').then((r) => {
            if (r?.success && r?.data?.total) {
          ...
          paymentAPI.getPublicRincian(idSantri, 'tunggakan').then((r) => {
            if (r?.success && r?.data?.total) {
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 bg-black/50 z-[10256]"
            onClick={onClose}
            aria-hidden="true"
          />
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-white dark:bg-gray-800 shadow-xl z-[10257] flex flex-col"
