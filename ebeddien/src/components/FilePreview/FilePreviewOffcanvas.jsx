...
import { isImageMime, isPdfMime, preparePreviewFromBlob, ensureTypedBlob } from '../../utils/filePreviewMedia'
...
function FilePreviewOffcanvas({ file, onClose, onDownload, onReplace, formatFileSize }) {
  ...
  const showPdf = previewFileUrl && isPdfMime(previewMime, file?.nama_file)
  const showImage = previewFileUrl && isImageMime(previewMime, file?.nama_file)
  ...
  const handleDownload = async () => {
    if (file && onDownload) {
      try {
        const blob = await onDownload(file.id, file.nama_file)
        const { blob: typed } = ensureTypedBlob(blob, file.tipe_file, file.nama_file)
        ...
        >
            ...
            <div className="flex-1 overflow-hidden bg-gray-100 dark:bg-gray-900">
              {loadingPreview ? (
                ...
                <div className="h-full w-full overflow-auto flex flex-col min-h-0">
                  {showImage ? (
                    <img
                      src={previewFileUrl}
                      alt={file.nama_file}
                    ...
                    <>
                      ...
                      />
                      <p className="shrink-0 py-2 text-center text-xs text-gray-500 dark:text-gray-400">
