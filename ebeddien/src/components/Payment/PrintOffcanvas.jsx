...
function PrintOffcanvas({ isOpen, onClose, santriId, mode = 'tunggakan' }) {
  ...
  useEffect(() => {
    if (isOpen && santriId && /^\d{7}$/.test(santriId)) {
      ...
      if (mode === 'uwaba') {
        ...
        // Tambahkan tahun_ajaran jika tersedia (penting untuk load data yang benar sesuai tahun hijriyah)
        if (tahunAjaran) {
  ...
  const loadPrintDataForWA = async () => {
    try {
      ...
      // Tambahkan tahun_ajaran jika tersedia dan mode adalah uwaba (penting untuk load data yang benar sesuai tahun hijriyah)
