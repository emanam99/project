import { UGT_LAPORAN_TAB_ACCORDIONS, groupUgtLaporanFiturChildren } from '../config/ugtLaporanFiturCodes'

/** Accordion aksi menu Laporan UGT (tab Koordinator / GT / PJGT + tambah per tab). */
export default function UgtLaporanFiturTabAccordions({
  children,
  openKeys,
  onToggleKey,
  renderRow,
  ...
  const buckets = groupUgtLaporanFiturChildren(children)
  const sections = UGT_LAPORAN_TAB_ACCORDIONS.map((def) => ({
    ...def,
        ...
        <div className="border-t border-gray-100 dark:border-gray-700/50">
          <div className="px-3 py-2 pl-12 bg-amber-50/50 dark:bg-amber-900/15">
            <p className="text-xs font-medium text-amber-900 dark:text-amber-200">
              Aksi umum — berlaku tab Koordinator, GT, dan PJGT
            </p>
            <p className="text-[11px] text-amber-800/90 dark:text-amber-300/90 mt-0.5 leading-snug">
              Mis. «Filter semua koordinator»: lihat laporan semua koordinator (bukan hanya diri sendiri).
            </p>
          </div>
