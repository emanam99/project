...
function Header() {
  ...
  const headerGroups = useMemo(() => {
    ...
    if (dataReady) {
      ...
      const flat = catalogMenusToNavFlat(filterOutNestedMenuChildren(filtered))
      if (flat.length > 0) {
  ...
  const isPengurusExcelEditorRoute = normalizedPathname === '/pengurus/excel-editor'
  const isExcelEditorRoute = isSantriExcelEditorRoute || isPengurusExcelEditorRoute
  const excelEditorEventPrefix = isPengurusExcelEditorRoute ? 'excel-pengurus' : 'excel-santri'
  const { open: openTemplateOffcanvas } = useWhatsAppTemplate()
  const { canTemplateWa } = useUmumFiturAccess()
  ...
  /** Session fetch foto: naikkan saat ganti user / refresh agar respons lama tidak menimpa yang baru. */
  const headerPhotoSessionRef = useRef(0)
  ...
  const loadHeaderPhoto = useCallback(() => {
    const session = ++headerPhotoSessionRef.current
    ...
    profilAPI
      .getUser(uid)
      .then((res) => {
        if (session !== headerPhotoSessionRef.current) return
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      headerPhotoSessionRef.current += 1
      if (headerPhotoRef.current) {
        URL.revokeObjectURL(headerPhotoRef.current)
        headerPhotoRef.current = null
      }
    }
  }, [user?.id, loadHeaderPhoto])

  // Interval polling: dinaikkan per audit Mei 2026 agar tidak boros request
  // ke API saat banyak tab staff terbuka. Hook usePollingScheduler tetap pause
  // saat tab hidden + back-off saat offline.
  const CHAT_UNREAD_POLL_MS = 90000
  const PAYMENT_POLL_MS = 90000
  const SALDO_POLL_MS = 90000
  const PENDAPATAN_POLL_MS = 90000
  const TANGGAL_POLL_MS = 300000
  const CHAT_AI_USAGE_POLL_MS = 90000
  ...
  useEffect(() => {
    ...
    window.addEventListener('excel-pengurus-changed-count', onChangedCount)
    return () => {
      window.removeEventListener('excel-santri-changed-count', onChangedCount)
      window.removeEventListener('excel-pengurus-changed-count', onChangedCount)
    }
  }, [])
  ...
  const pageTitle = useMemo(() => {
    if (isExcelEditorRoute) return 'Excel Editor'
    ...
    return last
      ...
      .filter(Boolean)
      .join(' ')
  }, [isExcelEditorRoute, matchedNavState?.label, catalogPathLabel, location.pathname])
  ...
  const renderViaBreakdown = (viaData) => {
      ...
      <div className="mb-4">
        ...
        <div className="space-y-1.5">
          <div>
            ...
            {renderViaBreakdown(paymentSummaryData.rincianVia.uwaba)}
          </div>
          <div>
            <div className="flex justify-between items-center">
            ...
            {renderViaBreakdown(paymentSummaryData.rincianVia.tunggakan)}
          </div>
          <div>
            <div className="flex justify-between items-center">
            ...
            {renderViaBreakdown(paymentSummaryData.rincianVia.khusus)}
          </div>
          <div>
            <div className="flex justify-between items-center">
            ...
            {renderViaBreakdown(paymentSummaryData.rincianVia.pendaftaran)}
          </div>
  ...
  const handleLogout = async () => {
    try {
      ...
      // abaikan: logout server best-effort, tetap clear state lokal
    }
    logout()
    // Hard redirect: hapus sisa state React (foto header, query cache) di laptop bersama.
  ...
  return (
    <header className={`${isStaging ? 'bg-red-600 dark:bg-red-800' : 'bg-primary-600 dark:bg-primary-800'} text-white p-3 rounded-lg mb-2 shadow-lg flex items-start md:items-center justify-between relative gap-3 mx-2 sm:mx-3 mt-2`}>
      ...
      <div className="flex-1 min-w-0 relative">
        ...
        {!isExcelEditorRoute && isKeuanganGroup ? (
        ...
        ) : !isExcelEditorRoute && showAktivitasAsDefault ? (
          isChatAiRoute ? (
      ...
      <div className="relative flex items-center gap-3 flex-shrink-0" ref={paymentRef}>
        {isExcelEditorRoute ? (
          <div className="flex items-center gap-1.5 md:gap-2">
            <button
              type="button"
              onClick={() => window.dispatchEvent(new CustomEvent(`${excelEditorEventPrefix}-columns-toggle`))}
            ...
            <button
              type="button"
              onClick={() => window.dispatchEvent(new CustomEvent(`${excelEditorEventPrefix}-save-request`))}
        ...
        {canInstall && (
          <button
            ...
            className="inline-flex h-9 sm:h-10 items-center justify-center gap-1.5 px-2.5 sm:px-3 rounded-xl bg-white/15 hover:bg-white/25 text-white text-xs font-semibold border border-white/20 shadow-sm transition-colors"
            title="Install aplikasi eBeddien"
          ...
          <AnimatePresence>
            {showUserDropdown && (
              ...
              >
                ...
                {/* Theme Toggle */}
                <div className="w-full text-left px-4 py-2 flex items-center gap-3">
                  <svg className="w-5 h-5 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                  </svg>
                  <span className="flex-1 text-sm">Tema</span>
                  <HeaderThemeSwitch theme={theme} onToggle={toggleTheme} className="scale-90 origin-right" />
                </div>
