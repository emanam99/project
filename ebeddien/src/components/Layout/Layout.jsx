...
import { UserDetailOffcanvasProvider } from '../../contexts/UserDetailOffcanvasContext'
import WhatsAppTemplateOffcanvas from '../WhatsAppTemplateOffcanvas'
import PasskeyPromptModal from '../Auth/PasskeyPromptModal'
import EmailReminderModal from '../Auth/EmailReminderModal'

const headerTransition = { type: 'tween', duration: 0.35, ease: [0.4, 0, 0.2, 1] }

/** Jangan mulai opacity 0 — spinner/lazy page di dalam main ikut tidak terlihat */
const pageVariants = {
  initial: {
    opacity: 1,
    y: 8
...
function Layout() {
  ...
  const { isOpen: chatAiOffcanvasOpen, isPinned: chatAiOffcanvasPinned } = useChatAiOffcanvas()
  const { isOpen: chatOffcanvasOpen, isPinned: chatOffcanvasPinned } = useChatOffcanvas()
  ...
  const chatAiReserveRight = chatAiOffcanvasOpen && chatAiOffcanvasPinned && layoutLg
  const chatReserveRight = chatOffcanvasOpen && chatOffcanvasPinned && layoutLg
  const offcanvasPinnedReserveRight = chatAiReserveRight || chatReserveRight
  ...
  const syncActiveFromMaster = useTahunAjaranStore((s) => s.syncActiveFromMaster)
  ...
  useEffect(() => {
    Promise.all([
      ...
      syncActiveFromMaster(hijriyah, masehi)
    }).catch(() => { /* tetap pakai fallback dari store */ })
  }, [setOptions, setOptionsMasehi, syncActiveFromMaster])

  return (
    <WhatsAppTemplateContext.Provider value={templateContextValue}>
    <UserDetailOffcanvasProvider>
    ...
    <div className="flex h-screen max-h-screen min-h-0 relative overflow-hidden w-full">
      ...
      {/* Main Content Area — geser kiri saat panel Chat / Chat AI dipin (desktop lg) */}
      <div
        className={`flex flex-col flex-1 min-h-0 overflow-hidden relative z-10 w-0 transition-[padding] duration-200 ease-out ${
          offcanvasPinnedReserveRight ? 'lg:pr-96' : ''
        }`}
      >
        ...
        <main
          ...
          } ${chatMobileThreadOpen ? 'pb-0' : 'pb-0 max-sm:pb-16'}`}
        >
          {/* relative + absolute inset-0: mode sync overlay halaman lama/baru, bukan stack vertikal */}
          <div className="relative flex-1 min-h-0 w-full overflow-hidden">
            <AnimatePresence mode="sync" initial={false}>
              <motion.div
                ...
                className="absolute inset-0 flex flex-col min-h-0 w-full overflow-hidden"
              >
                <Outlet />
              </motion.div>
            </AnimatePresence>
