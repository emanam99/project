...
/**
 * Toggle tema: checked = terang, unchecked = gelap (selaras animasi docs/togle-dark.md).
 */
export default function HeaderThemeSwitch({
  theme,
  onToggle,
  className = '',
  'aria-label': ariaLabel = 'Ganti tema gelap atau terang'
}) {
  const isLight = theme === 'light'

  return (
    <div className={`header-theme-switch ${className}`.trim()}>
      <label className="switch">
        ...
        <span className="slider">
          ...
          <svg viewBox="0 0 16 16" className="cloud_1 cloud" aria-hidden>
            <path transform="matrix(.77976 0 0 .78395-299.99-418.63)" fill="#fff" d={CLOUD_PATH} />
          </svg>
        </span>
      </label>
    </div>
