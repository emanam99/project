...
import UsernameLinkButton from '../UserDetailOffcanvas/UsernameLinkButton'
...
function DetailPengurusOffcanvas({
  isOpen,
  onClose,
  pengurusId,
  lembagaList = [],
  ...
  onOpenEdit,
  onCloseEdit,
  canEditPengurus = true,
}) {
  const { showNotification } = useNotification()
  const { user } = useAuthStore()
  const isSuperAdmin = userHasSuperAdminAccess(user)
  const currentPengurusId = user?.user_id ?? user?.id_pengurus ?? user?.id
  const isOwnProfile =
    currentPengurusId != null &&
    pengurusId != null &&
    String(currentPengurusId) === String(pengurusId)
  const showEditButton = isOwnProfile || canEditPengurus
  ...
  const [jabatanMengajarSavingId, setJabatanMengajarSavingId] = useState(null)
  ...
  const openEditOffcanvas = () => {
    if (!pengurusId || !showEditButton) return
    onOpenEdit?.()
    setEditFormLoading(true)
    profilAPI.getUser(pengurusId, { fullPii: true }).then((res) => {
  ...
  const handleSaveWhatsapp = async (newNumber) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const resProfil = await profilAPI.getUser(pengurusId, { fullPii: true })
  ...
  const openEditJabatan = (j) => {
    setNewJabatan({
      ...
      mengajar: isBooleanTruthy(j.mengajar)
    })
  ...
  const openTambahJabatan = () => {
    setNewJabatan({ jabatan_id: '', lembaga_id: '', tanggal_mulai: '', tanggal_selesai: '', status: 'aktif', mengajar: false })
    setEditingJabatanId(null)
    setShowAddJabatanModal(true)
  }
  ...
  const handleSaveJabatan = async () => {
    if (editingJabatanId != null) {
      try {
        const res = await manageUsersAPI.updateJabatanStatus(pengurusId, editingJabatanId, {
          ...
          mengajar: newJabatan.mengajar ? 1 : 0
        })
        if (res.success) {
          const again = await manageUsersAPI.getById(pengurusId)
    ...
    try {
      const res = await manageUsersAPI.addUserJabatan(pengurusId, {
        ...newJabatan,
        mengajar: newJabatan.mengajar ? 1 : 0
      })
      if (res.success) {
  ...
  const handleJabatanMengajarToggle = async (j) => {
    if (!pengurusId || jabatanMengajarSavingId !== null) return
    const idKey = Number(j.pengurus_jabatan_id)
    if (!idKey) return
    const prev = isBooleanTruthy(j.mengajar)
    const next = !prev
    setJabatanMengajarSavingId(idKey)
    try {
      const res = await manageUsersAPI.updateJabatanStatus(pengurusId, idKey, { mengajar: next ? 1 : 0 })
      if (res.success) {
        const nextJabatan = userJabatan.map((row) =>
          Number(row.pengurus_jabatan_id) === idKey ? { ...row, mengajar: next ? 1 : 0 } : row
        )
        setUserJabatan(nextJabatan)
        showNotification(`Mengajar: ${next ? 'Ya' : 'Tidak'}`, 'success')
        onPengurusPatch?.(pengurusId, { jabatan: nextJabatan })
      } else {
        showNotification(res.message || 'Gagal mengubah status mengajar', 'error')
      }
    } catch (err) {
      showNotification(err?.response?.data?.message || err?.message || 'Gagal mengubah status mengajar', 'error')
    } finally {
      setJabatanMengajarSavingId(null)
    }
  }
        ...
        <div className="flex-shrink-0 px-5 pt-5 pb-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {pengurusId != null && detail && showEditButton && (
                <button
                  type="button"
                  ...
                  {userJabatan.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                      {userJabatan.map((j, idx) => {
                        ...
                        const isMengajar = isBooleanTruthy(j.mengajar)
                        const savingStatus = Number(jabatanStatusSavingId) === Number(j.pengurus_jabatan_id)
                        const savingMengajar = Number(jabatanMengajarSavingId) === Number(j.pengurus_jabatan_id)
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                              <div className="flex flex-col items-center gap-0.5">
                                <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">Aktif</span>
                                <span className={`text-xs font-medium px-2 py-0.5 rounded-md ${isJabatanAktif ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-200 text-gray-600 dark:bg-gray-600 dark:text-gray-300'}`}>
                                  {savingStatus ? '...' : (isJabatanAktif ? 'Ya' : 'Tidak')}
                                </span>
                                <button
                                  type="button"
                                  role="switch"
                                  aria-checked={isJabatanAktif}
                                  aria-label="Status aktif jabatan"
                              ...
                              <div className="flex flex-col items-center gap-0.5">
                                <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">Mengajar</span>
                                <span className={`text-xs font-medium px-2 py-0.5 rounded-md ${isMengajar ? 'bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300' : 'bg-gray-200 text-gray-600 dark:bg-gray-600 dark:text-gray-300'}`}>
                                  {savingMengajar ? '...' : (isMengajar ? 'Ya' : 'Tidak')}
                                </span>
                                <button
                                  type="button"
                                  role="switch"
                                  aria-checked={isMengajar}
                                  aria-label="Status mengajar jabatan"
                                  disabled={savingMengajar}
                                  onClick={() => handleJabatanMengajarToggle(j)}
                                  className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-1 disabled:opacity-50 ${isMengajar ? 'bg-teal-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                                >
                                  <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow transition duration-200 ${isMengajar ? 'translate-x-4' : 'translate-x-0.5'}`} />
                                </button>
                              </div>
                              {isSuperAdmin && (
                                <button
                ...
                <div className="p-4 space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      {detail.username && detail.id_user ? (
                        <UsernameLinkButton
                          userId={detail.id_user}
                          username={detail.username}
                          stackBaseZIndex={220}
                          emptyLabel="-"
                          className="text-sm font-medium text-teal-700 dark:text-teal-300 hover:underline break-all text-left"
                        />
                      ) : (
          ...
          <>
            ...
            >
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-4 pb-4">
                    ...
                    {availableJabatan
                      ...
                      .map((j, i) => (
                        <option key={j.id != null && j.id !== '' ? j.id : `jabatan-opt-${i}`} value={j.id}>
                          {j.nama} {(j.lembaga_kategori || j.kategori) ? `(${j.lembaga_kategori || j.kategori})` : ''}
                        </option>
                      ))}
                  </select>
                </div>
                ...
                <div className="flex items-center justify-between gap-4 rounded-xl border border-gray-200 dark:border-gray-600 px-3 py-2.5 bg-gray-50/80 dark:bg-gray-700/30">
                  <div>
                    <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Mengajar</p>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400">Mengajar di lembaga/jabatan ini</p>
                  </div>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={!!newJabatan.mengajar}
                    onClick={() => setNewJabatan((prev) => ({ ...prev, mengajar: !prev.mengajar }))}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-1 ${newJabatan.mengajar ? 'bg-teal-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                  >
                    <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow transition duration-200 ${newJabatan.mengajar ? 'translate-x-5' : 'translate-x-0.5'}`} />
                  </button>
                </div>
              </div>
