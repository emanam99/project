...
import RolePengurusChecklistOffcanvas from './RolePengurusChecklistOffcanvas'
...
export default function FiturMenuRoleOffcanvas({
  ...
  /** Opsional: sync pengurus_count ke parent setelah toggle di checklist */
  onRolePengurusCountChange = null,
}) {
  ...
  /** roleId → jumlah pengurus (bisa di-update dari offcanvas checklist) */
  const [pengurusCountByRole, setPengurusCountByRole] = useState({})
  ...
  // Tutup checklist hanya saat offcanvas ditutup / ganti fitur — jangan saat roles.pengurus_count sync.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const counts = {}
    roles.forEach((r) => {
      counts[r.id] = Number(r.pengurus_count) || 0
    })
    setPengurusCountByRole(counts)
  ...
  const handlePengurusCountChange = (roleId, count) => {
    setPengurusCountByRole((prev) => ({ ...prev, [roleId]: count }))
    onRolePengurusCountChange?.(roleId, count)
  }
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
              ...
              <div className="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <ul className="p-3 space-y-1 max-h-[min(52vh,28rem)] overflow-y-auto">
                  {roles.length === 0 ? (
                    ...
                    roles.map((r) => {
                      ...
                      const count = pengurusCountByRole[r.id] ?? Number(r.pengurus_count) ?? 0
                      return (
                        <li key={r.id}>
                          <div className="flex items-center gap-2 p-2.5 rounded-xl bg-gray-50 dark:bg-gray-700/40 border border-gray-100 dark:border-gray-600/50 hover:border-indigo-200 dark:hover:border-indigo-800 transition-colors">
                            ...
                            <button
                              ...
                              className="shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
                ...
                <p className="px-4 pb-3 text-[11px] text-gray-500 dark:text-gray-400">
                  Centang = role boleh akses fitur ini. Ikon user + angka = daftar pengurus role (centang langsung assign/cabut).
                </p>
              </div>
            </div>
