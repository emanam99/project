...
export function usePengeluaranFiturAccess() {
  ...
  return useMemo(() => {
    ...
    /**
     * Boleh memilih tidak kirim notifikasi saat simpan/update draft (body kirim_notifikasi_draft).
     * Selaras RoleHelper::tokenMayToggleDraftNotifOnSave — tanpa bypass legacy.
     */
    const rencanaToggleDraftNotifOnSave =
      isSuper ||
      (apiHasPengeluaran &&
        (codes.includes(C.draftNotifLembagaSesuaiRole) ||
          codes.includes(C.notifSemuaLembaga) ||
          codes.includes(C.notifLembagaSesuaiRole)))
