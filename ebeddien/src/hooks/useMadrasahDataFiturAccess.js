...
export function useMadrasahDataFiturAccess() {
  ...
  return useMemo(() => {
    ...
    /** Tinjau pengajuan edit PJGT — legacy: akses menu data madrasah tanpa aksi granular. */
    let canPengajuanEdit = false
    if (isSuper) {
      canPengajuanEdit = true
    } else if (apiHasMadrasahActions) {
      canPengajuanEdit = codes.includes(UGT_MADRASAH_ACTION_CODES.pengajuanEdit)
    } else {
      canPengajuanEdit = hasDataMadrasahAccess
    }
