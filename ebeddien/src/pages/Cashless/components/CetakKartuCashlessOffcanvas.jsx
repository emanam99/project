                ...
                >
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <div className="min-w-0 flex-1">
                        {data.entity_type === 'SANTRI' ? (
                          <>
                            {data.entity_label && (
                              <p className="text-xs font-semibold leading-tight line-clamp-2" title={data.entity_label}>
                                {data.entity_label}
                              </p>
                            )}
                            {data.entity_nis ? (
                              <div className="mt-1.5">
                                <p className="text-[8px] font-semibold uppercase tracking-wider text-gray-500">NIS</p>
                                <p className="font-mono text-sm font-bold break-all leading-tight mt-0.5">{data.entity_nis}</p>
                              </div>
                            ) : (
                              <p className="text-[9px] text-gray-400 mt-1 italic">NIS belum diisi</p>
                            )}
                          </>
                        ) : (
                          <>
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider">No. Akun</p>
                      ...
                      <div className="flex-shrink-0 w-[4.75rem] h-[4.75rem] p-0.5 bg-white rounded">
                        <QRCodeSVG value={qrValue} size={72} level="M" includeMargin={false} className="w-full h-full" />
                      </div>
                    </div>
                    {data.card_uid && (
