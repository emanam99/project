...
import DetailTokoOffcanvas from './components/DetailTokoOffcanvas'
import DetailTokoPanel from './components/DetailTokoPanel'
...
/** Bar pencarian + Menu (Tambah, List/Grid, Refresh) */
const SearchAndMenuSection = memo(({ searchInput, onSearchInputChange, onSearchInputFocus, onSearchInputBlur, onTambahClick, viewMode, onViewModeChange, onRefresh }) => {
      ...
      <div className="relative pb-2 px-4 pt-3">
        <div className="relative">
          <input
            ...
            placeholder="Cari nama, kode toko, atau penanggung jawab..."
          />
                ...
                {menuOpen && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full px-3 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 border-t border-gray-200 dark:border-gray-600 mt-1"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function DataToko() {
  ...
  const selectToko = useCallback((t) => {
    const id = t?.id ?? null
    setSelectedTokoId(id)
    if (id && !isDesktopLayout()) setMobileDetailOpen(true)
  }, [])
  const openEditFromDetail = (toko) => {
    setEditingToko(toko)
    setShowTambahOffcanvas(true)
  }
    ...
    <div className="h-full min-h-0 flex flex-col overflow-hidden">
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-hidden px-4 md:px-6 pt-4 md:pt-6 pb-24 lg:pb-6 gap-4">
        ...
        <div className="flex-1 min-w-0 flex flex-col min-h-0">
          {error && (
            <div className="flex-shrink-0 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-3">
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-900 sticky top-0 z-10">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Penanggung Jawab</th>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      {list.map((t, index) => {
                        const isSelected = selectedTokoId === t.id
                        return (
                          <tr
                            key={t.id}
                            onClick={() => selectToko(t)}
                          ...
                          >
                            ...
                            <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400">{t.penanggung_jawab_nama || '—'}</td>
                  ...
                  {list.map((t, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        onClick={() => selectToko(t)}
                        className={`bg-white dark:bg-gray-800 rounded-lg shadow-md border overflow-hidden grid grid-rows-[3fr_1fr] aspect-[3/4] min-h-0 cursor-pointer transition-colors ${
                          isSelected
                            ? 'border-teal-500 ring-2 ring-teal-400/40 dark:border-teal-400'
                      ...
                      >
                        ...
                        <div className="p-2 sm:p-3 flex flex-col justify-center min-h-0 overflow-hidden gap-0.5">
                          ...
                          <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-500 truncate" title={t.penanggung_jawab_nama || ''}>
                            PJ: {t.penanggung_jawab_nama || '—'}
                          </p>
        ...
        <div className="hidden lg:flex lg:w-80 xl:w-96 shrink-0 flex-col min-h-0">
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-700">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Detail Toko</h3>
            </div>
            <DetailTokoPanel
      ...
      <DetailTokoOffcanvas
        isOpen={mobileDetailOpen && Boolean(selectedTokoId)}
