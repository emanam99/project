...
function labelAlamatMaster(a) {
  const ring = [a.desa, a.kecamatan, a.kabupaten].filter(Boolean).join(', ')
  const base = ring ? `${ring} (#${a.id})` : a.dusun || a.rt || a.rw ? `${a.dusun || a.rt || a.rw} (#${a.id})` : `Alamat #${a.id}`
  const hasGps =
    a.latitude != null &&
    a.longitude != null &&
  ...
  const r = Number.isFinite(rm) && rm > 0 ? rm : 100
...
export default function AbsenLokasiOffcanvas({
  ...
  const alamatFiltered = useMemo(() => (Array.isArray(alamatList) ? alamatList : []), [alamatList])

  useEffect(() => {
    if (!isOpen) return
    const sel = String(form.id_absen_alamat ?? '').trim()
    if (!sel) return
    const ok = alamatFiltered.some((a) => String(a.id) === sel)
    if (!ok) setForm((f) => ({ ...f, id_absen_alamat: '' }))
  }, [isOpen, alamatFiltered, form.id_absen_alamat, setForm])

  const onPilihAlamatMaster = useCallback(
    (rawId) => {
      const idStr = String(rawId ?? '').trim()
      if (!idStr) {
        setForm((f) => ({ ...f, id_absen_alamat: '' }))
        return
      }
      const row = alamatFiltered.find((a) => String(a.id) === idStr)
      if (!row) {
        setForm((f) => ({ ...f, id_absen_alamat: '' }))
        return
      }
      setForm((f) => ({
        ...f,
        id_absen_alamat: idStr,
        dusun: row.dusun != null ? String(row.dusun) : '',
        rt: row.rt != null ? String(row.rt) : '',
        rw: row.rw != null ? String(row.rw) : '',
        desa: row.desa != null ? String(row.desa) : '',
        kecamatan: row.kecamatan != null ? String(row.kecamatan) : '',
        kabupaten: row.kabupaten != null ? String(row.kabupaten) : '',
        provinsi: row.provinsi != null ? String(row.provinsi) : ''
      }))
    },
    [alamatFiltered, setForm]
  )
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              <div className="rounded-lg border border-dashed border-gray-300 dark:border-gray-600 bg-gray-50/80 dark:bg-gray-900/30 p-3 space-y-2">
                ...
                {alamatList.length > 0 && (
                  <div className="pb-1 border-b border-gray-200/80 dark:border-gray-600/80 mb-2">
                    <label className="block text-[11px] font-medium text-gray-600 dark:text-gray-300 mb-0.5">
                      Alamat master (boleh kosong)
                    </label>
                    <select
                      value={form.id_absen_alamat != null && form.id_absen_alamat !== '' ? String(form.id_absen_alamat) : ''}
                      disabled={disabledForm}
                      onChange={(e) => onPilihAlamatMaster(e.target.value)}
                      className="w-full text-sm border rounded-lg px-2 py-1.5 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 disabled:opacity-60"
                    >
                      <option value="">— Isi manual di bawah / gabung saat simpan —</option>
                      {alamatFiltered.map((a) => (
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 leading-snug">
                      Satu alamat bisa dipakai beberapa titik. Master «semua lembaga» bisa dipakai titik lembaga manapun.
                      Kosongkan lalu isi manual bila perlu alamat khusus titik ini saja.
                    </p>
                  </div>
                )}
