...
export default function EditSantriOffcanvas({ isOpen, onClose, santri, onSaved, stackBaseZIndex = null }) {
      ...
      .then((res) => {
        if (res?.success && res?.data) {
          ...
          setFormData({
            ...
            provinsi: d.provinsi || '',
            id_lttq_tingkatan: d.id_lttq_tingkatan != null ? String(d.id_lttq_tingkatan) : '',
            id_kamar: d.id_kamar ?? '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        ayah: biodataPersonName(formData.ayah) || null,
        ...
        ibu: biodataPersonName(formData.ibu) || null,
        ...
        wali: biodataPersonName(formData.wali) || null,
        ...
        id_lttq_tingkatan: formData.id_lttq_tingkatan === '' ? null : Number(formData.id_lttq_tingkatan),
  ...
  const editBackdropStyle = zb != null ? { zIndex: zb } : undefined
  const editPanelStyle = zb != null ? { zIndex: zb + 1 } : undefined
              ...
              <>
                ...
                <div className="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  <motion.div className="grid grid-cols-1 gap-3">
                    <motion.div>
                      <label className={labelClass}>Tingkatan LTTQ</label>
                      <select
                        value={formData.id_lttq_tingkatan}
                        onChange={(e) => handleChange('id_lttq_tingkatan', e.target.value)}
                        className={inputClass}
                      >
                        <option value="">— Tidak ada / kosong —</option>
                        {lttqTingkatanOptions.map((t) => (
                          <option key={t.id} value={t.id}>
                            {t.tingkatan || '–'}
                            {t.kelompok ? ` · ${t.kelompok}` : ''}
                          </option>
                        ))}
                      </select>
                    </motion.div>
