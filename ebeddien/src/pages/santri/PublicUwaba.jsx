  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        // Load data rincian UWABA
        // Untuk uwaba, tahun_ajaran diperlukan (format hijriyah seperti "1446-1447")
        const tahunAjaranParam = tahunAjaran || localStorage.getItem('tahun_ajaran') || new Date().getFullYear().toString()
        const rincianResponse = await fetch(`${apiBaseUrl}/public/pembayaran/uwaba?id_santri=${idSantri}&tahun_ajaran=${encodeURIComponent(tahunAjaranParam)}`)
        ...
        if (rincianData.success) {
          ...
          // Jika endpoint belum ada, set empty
          setRincian([])
        ...
        <div className="mb-6">
          ...
          {rincian.length === 0 ? (
            <div className="text-gray-500 dark:text-gray-400 text-center py-8 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              Tidak ada data pembayaran UWABA
            </div>
          ) : (
