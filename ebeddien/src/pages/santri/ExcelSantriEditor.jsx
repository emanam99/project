...
import { extractRowsFromWorkbook, valuesEqualForExcelField } from '../../utils/excelEditorDiff'
...
/** Kolom tambahan mode Excel Guru Tugas; jangan splice ke SHEET_COLUMNS (urutan persist user). */
const UGT_GT_EXCEL_COLUMNS = [
  { key: 'id_tugasan', label: 'ID Tugasan', readonly: true },
  { key: 'tahun_ajaran', label: 'Tahun Ajaran (Tugas)' },
  { key: 'id_madrasah', label: 'ID Madrasah (Tugas)' },
  { key: 'nama_madrasah_ugt', label: 'Nama Madrasah (Tugas)', readonly: true },
  { key: 'is_aktif', label: 'Aktif (1/0)' },
]
...
const NUMERIC_FIELD_KEYS = new Set(['id_kamar', 'id_diniyah', 'id_formal', 'id_madrasah', 'id_tugasan', 'is_aktif'])
...
const getDefaultVisibleColumns = () => {
  const acc = SHEET_COLUMNS.reduce((a, col) => {
    a[col.key] = true
    return a
  }, {})
  UGT_GT_EXCEL_COLUMNS.forEach((col) => {
    acc[col.key] = true
  })
  return acc
}
...
const readUgtGuruTugasExcelFlag = (search) => {
  const q = new URLSearchParams(search || '')
  return q.get('ugt_guru_tugas') === '1'
}

export default function ExcelSantriEditor() {
  const { showNotification } = useNotification()
  const location = useLocation()
  const isUgtGuruTugasExcel = useMemo(
    () => readUgtGuruTugasExcelFlag(location.search),
    [location.search]
  )
  ...
  useLayoutEffect(() => {
    const ugt = readUgtGuruTugasExcelFlag(location.search)
    setColumnOrder((prev) => {
      const ugKeys = new Set(UGT_GT_EXCEL_COLUMNS.map((c) => c.key))
      if (ugt) {
        const keys = new Set(prev.map((c) => c.key))
        const toAdd = UGT_GT_EXCEL_COLUMNS.filter((c) => !keys.has(c.key)).map((c) => ({ ...c }))
        return toAdd.length ? [...prev, ...toAdd] : prev
      }
      const next = prev.filter((c) => !ugKeys.has(c.key))
  ...
  const syncCurrentWorkbookRowsToAllRows = useCallback(() => {
    ...
    const currentRows = extractRowsFromWorkbook(sourceWorkbook, columnOrderRef.current)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const isUgt = query.get('ugt_guru_tugas') === '1'
        if (isUgt) {
          params.ugt_guru_tugas = '1'
          const taUgt = query.get('tahun_ajaran_ugt')
          if (taUgt != null && String(taUgt).trim() !== '') params.tahun_ajaran_ugt = String(taUgt).trim()
        }
        ...
        if (!cancelled && res?.success) {
          ...
          let cols = columnOrderRef.current
          if (isUgt) {
            const keys = new Set(cols.map((c) => c.key))
            const extra = UGT_GT_EXCEL_COLUMNS.filter((c) => !keys.has(c.key)).map((c) => ({ ...c }))
          ...
          baselineRowsByIdRef.current = new Map(
            dataRows.map((row) => {
              ...
              const key = isUgt && row.id_tugasan != null && String(row.id_tugasan).trim() !== ''
                ? `t:${String(row.id_tugasan).trim()}`
                : `s:${sid}`
              return [key, row]
            })
          )
  ...
  const excelRowStorageKey = useCallback((row) => {
    const sid = String(row?.id || '').trim()
    if (isUgtGuruTugasExcel && row?.id_tugasan != null && String(row.id_tugasan).trim() !== '') {
      return `t:${String(row.id_tugasan).trim()}`
    }
    return sid ? `s:${sid}` : ''
  }, [isUgtGuruTugasExcel])
  ...
  const buildPendingChangesFromRows = useCallback((rowsRaw) => {
    ...
    rows.forEach((row) => {
      ...
      if (!id) return
      const storageKey = excelRowStorageKey(row)
      const beforeRow = (storageKey && baseline.get(storageKey)) || baseline.get(`s:${id}`) || {}
      columnOrderRef.current.forEach((col) => {
        ...
        const payloadKey = storageKey || `s:${id}`
        if (!payloadMap.has(payloadKey)) {
          const payload = { id }
          if (isUgtGuruTugasExcel && row.id_tugasan != null && String(row.id_tugasan).trim() !== '') {
            payload.id_tugasan = Number(row.id_tugasan)
          }
          payloadMap.set(payloadKey, payload)
        }
        payloadMap.get(payloadKey)[col.key] = normalizeForApiValue(col.key, row[col.key])
      })
    })
  ...
  }, [excelRowStorageKey, isUgtGuruTugasExcel])

  const buildPendingChanges = useCallback((sourceWorkbook) => {
    const rowsRaw = extractRowsFromWorkbook(sourceWorkbook, columnOrderRef.current)
  ...
  const handleConfirmSave = async () => {
    try {
      ...
      const query = new URLSearchParams(location.search || '')
      const bulkOpts = {}
      if (query.get('ugt_guru_tugas') === '1') {
        bulkOpts.ugt_guru_tugas = true
        const ta = query.get('tahun_ajaran_ugt')
        if (ta != null && String(ta).trim() !== '') bulkOpts.tahun_ajaran_ugt = String(ta).trim()
      }
      const res = await santriAPI.bulkUpdateFromExcel(pendingPayloadRows, bulkOpts)
      if (res?.success) {
        syncCurrentWorkbookRowsToAllRows()
        const latestWorkbook = workbookRef.current?.getAllSheets?.()
        const sourceWorkbook = Array.isArray(latestWorkbook) && latestWorkbook.length > 0
        ...
        const ugtAfterSave = query.get('ugt_guru_tugas') === '1'
        baselineRowsByIdRef.current = new Map(
          freshRows.map((r) => {
            const sid = String(r.id)
            const key = ugtAfterSave && r.id_tugasan != null && String(r.id_tugasan).trim() !== ''
              ? `t:${String(r.id_tugasan).trim()}`
              : `s:${sid}`
            return [key, r]
          })
        )
  ...
  }, [columnFilters, isUgtGuruTugasExcel, syncCurrentWorkbookRowsToAllRows, visibleColumns])

  const handleMoveColumn = (index, direction) => {
    const target = direction === 'up' ? index - 1 : index + 1
    if (target < 0 || target >= columnOrderDraft.length) return

    const nextColumns = [...columnOrderDraft]
