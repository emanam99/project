import { useActiveHijriyahTahunAjaran, useActiveMasehiTahunAjaran } from '../../hooks/useActiveTahunAjaran'
...
import { useRombelScopeAccess } from '../../hooks/useRombelScopeAccess'
import { LEMBAGA_FILTER_ACTION_CODES, ROMBEL_CATATAN_ACTION_CODES } from '../../config/lembagaFilterFiturCodes'
...
function formatRombelAktifLabel(prefix, lembaga, kelas, kel) {
  const l = String(lembaga ?? '').trim()
  const k = String(kelas ?? '').trim()
  const group = String(kel ?? '').trim()
  if (!l && !k && !group) return ''
  return `${prefix}: ${l || 'Rombel'}${k ? ` - ${k}` : ''}${group ? ` (${group})` : ''}`
}

function formatKamarAktifLabel(santri) {
  const daerah = String(santri?.daerah ?? '').trim()
  const kamar = String(santri?.kamar ?? '').trim()
  const daerahKamar = String(santri?.daerah_kamar ?? '').trim()
  if (daerahKamar) return `Kamar aktif: ${daerahKamar}`
  if (daerah && kamar) return `Kamar aktif: ${daerah}.${kamar}`
  if (daerah || kamar) return `Kamar aktif: ${daerah || kamar}`
  return ''
}

/** Keterangan sumber catatan untuk konteks Pengaturan → Rombel + kondisi aktif santri. */
function buildCatatanKeteranganRombel(rombel, santri = null) {
  if (!rombel && !santri) return 'Rombel'
  ...
  const parts = [`Rombel · ${detail}`]
  const diniyah = formatRombelAktifLabel(
    'Diniyah aktif',
    santri?.diniyah_lembaga_nama ?? santri?.diniyah,
    santri?.kelas_diniyah,
    santri?.kel_diniyah
  )
  const formal = formatRombelAktifLabel(
    'Formal aktif',
    santri?.formal_lembaga_nama ?? santri?.formal,
    santri?.kelas_formal,
    santri?.kel_formal
  )
  const kamar = formatKamarAktifLabel(santri)
  if (diniyah) parts.push(diniyah)
  if (formal) parts.push(formal)
  if (kamar) parts.push(kamar)
  return parts.join(' | ')
}
...
function Rombel() {
  ...
  const {
    ...
    optionsMasehi: tahunAjaranMasehiOptions
  } = useTahunAjaranStore()
  const tahunAjaranHijriyah = useActiveHijriyahTahunAjaran()
  const tahunAjaranMasehiStore = useActiveMasehiTahunAjaran()

  /** Form wali kelas: satu dropdown — Hijriyah + Masehi (sama sumber dengan Layout / Header). */
  const waliKelasTahunAjaranGroups = useMemo(() => {
    const hij = Array.isArray(tahunAjaranOptions) ? tahunAjaranOptions : []
    const mas = Array.isArray(tahunAjaranMasehiOptions) ? tahunAjaranMasehiOptions : []
    return { hij, mas }
  }, [tahunAjaranOptions, tahunAjaranMasehiOptions])
  ...
  const rombelScope = useRombelScopeAccess()
  const rombelFormReadOnly = rombelScope.applyBertugasFilter && !isSuperAdmin
  ...
  /** Data pengurus (per lembaga) + santri (per rombel) siap sebelum select wali kelas diaktifkan */
  const [waliSelectorsReady, setWaliSelectorsReady] = useState(false)
  const [waliSelectorsLoading, setWaliSelectorsLoading] = useState(false)
  /** Satu baris = satu select guru FAN (id pengurus sebagai string) */
  const [guruFanIds, setGuruFanIds] = useState([])
  ...
  const [rowCatatanJenisBaru, setRowCatatanJenisBaru] = useState('putih')
  const [rowCatatanFilterJenis, setRowCatatanFilterJenis] = useState('')
  ...
  const [rowCatatanDeletingId, setRowCatatanDeletingId] = useState(null)
  ...
  /** Filter daftar rombel per pengurus (wali / guru FAN) — hanya jika punya aksi «semua rombel di lembaga» */
  const [filterPengurusAmpu, setFilterPengurusAmpu] = useState('')
  const [pengurusFilterOptions, setPengurusFilterOptions] = useState([])
  ...
  const canHapusCatatanSantri = useMemo(() => {
    const codes = Array.isArray(fiturMenuCodes) ? fiturMenuCodes : []
    return isSuperAdmin || codes.includes(ROMBEL_CATATAN_ACTION_CODES.hapus)
  }, [fiturMenuCodes, isSuperAdmin])

  /** Disimpan ke API sebagai `keterangan` pada catatan santri (asal rombel + kondisi aktif santri). */
  const catatanKeteranganAsal = useMemo(
    () => buildCatatanKeteranganRombel(santriOffcanvasRombel, santriRowSheetSantri),
    [santriOffcanvasRombel, santriRowSheetSantri]
  )
  ...
  useEffect(() => {
    ...
    if (filterStatus && !validStatus.has(filterStatus)) {
      setFilterStatus(validStatus.has('aktif') ? 'aktif' : '')
    }
  }, [filterStatus, statusOptions])

  const selectedLembagaIdForPengurus = useMemo(() => {
    if (!filterLembaga || !lembagaMaster?.length) return ''
    const row = lembagaMaster.find((l) => canonicalLembagaNama(l.nama) === canonicalLembagaNama(filterLembaga))
    return row?.id != null ? String(row.id) : ''
  }, [filterLembaga, lembagaMaster, canonicalLembagaNama])

  useEffect(() => {
    setFilterPengurusAmpu('')
  }, [filterLembaga])

  useEffect(() => {
    if (!rombelScope.canFilterPengurusAmpu || !selectedLembagaIdForPengurus) {
      setPengurusFilterOptions([])
      return undefined
    }
    let cancelled = false
    pengurusAPI.getList({ lembaga_id: selectedLembagaIdForPengurus })
      .then((res) => {
        if (cancelled) return
        setPengurusFilterOptions(Array.isArray(res?.data) ? res.data : [])
      })
      .catch(() => {
        if (!cancelled) setPengurusFilterOptions([])
      })
    return () => {
      cancelled = true
    }
  }, [rombelScope.canFilterPengurusAmpu, selectedLembagaIdForPengurus])
  ...
  }, [lembagaAccess.allowedLembagaIds, rombelScope.canFilterPengurusAmpu, filterPengurusAmpu])
  ...
  }, [page, limit, filterLembaga, filterStatus, filterKelas, searchQuery, lembagaAccess.allowedLembagaIds, rombelScope.canFilterPengurusAmpu, filterPengurusAmpu])
  ...
  const handleOpenOffcanvas = (rombel = null) => {
    if (!rombel && rombelScope.applyBertugasFilter) {
      showNotification('Tambah rombel tidak tersedia untuk akses rombel bertugas', 'error')
      return
    }
    if (rombel) {
  ...
  useEffect(() => {
    if (!waliOffcanvasOpen || editingRombel?.id == null) {
      setWaliSelectorsReady(false)
      setWaliSelectorsLoading(false)
      return undefined
    }
    let cancelled = false
    setWaliSelectorsReady(false)
    setWaliSelectorsLoading(true)
    setPengurusList([])
    setSantriList([])
    const rid = editingRombel.id
    const lembagaId = String(editingRombel.lembaga_id ?? '').trim()
    ;(async () => {
      try {
        const prParams = lembagaId ? { lembaga_id: lembagaId } : {}
        const [pr, st] = await Promise.all([pengurusAPI.getList(prParams), santriAPI.getByRombelId(rid)])
        if (cancelled) return
        if (pr?.data) setPengurusList(Array.isArray(pr.data) ? pr.data : [])
        else setPengurusList([])
        if (st?.success && Array.isArray(st.data)) setSantriList(st.data)
        else setSantriList([])
      } catch (_) {
        if (!cancelled) {
          setPengurusList([])
  ...
  }, [waliOffcanvasOpen, editingRombel?.id, editingRombel?.lembaga_id])

  const waliSelectsDisabled = waliSelectorsLoading || !waliSelectorsReady

  useEffect(() => {
    const w = String(waliFormData.id_pengurus || '').trim()
    if (!w) return
    setGuruFanIds((prev) => prev.filter((g) => String(g) !== w))
  }, [waliFormData.id_pengurus])
  ...
  const handleEditWaliOffcanvas = (wk) => {
    ...
    setEditingWali(wk)
    const fan = Array.isArray(wk.guru_fan) ? wk.guru_fan : []
    setGuruFanIds(fan.map((g) => (g?.id_pengurus != null ? String(g.id_pengurus) : '')).filter(Boolean))
    setWaliFormData({
  ...
  /** Diniyah / formal dari kategori master lembaga rombel (bukan struktural). */
  const rombelLembagaJenis = useMemo(() => {
    const id = santriOffcanvasRombel?.lembaga_id
    if (id == null || id === '') return null
    const row = lembagaMaster.find((l) => String(l.id) === String(id))
    const k = String(row?.kategori || '').trim().toLowerCase()
    if (k === 'diniyah') return 'diniyah'
    if (k === 'formal') return 'formal'
    return null
  }, [santriOffcanvasRombel?.lembaga_id, lembagaMaster])

  const resetSantriRowSheet = useCallback(() => {
    setSantriRowSheet(null)
    setSantriRowSheetSantri(null)
    setRowCatatanText('')
    setRowCatatanJenisBaru('putih')
    setRowCatatanFilterJenis('')
    setRowCatatanList([])
    setRowCatatanLoading(false)
    setRowCatatanSubmitting(false)
    setRowCatatanDeletingId(null)
  }, [])
  ...
  const closeSantriTambahSearch = useOffcanvasBackClose(santriTambahSearchOpen, () => setSantriTambahSearchOpen(false))
  ...
  const loadRowCatatan = useCallback(async (idSantri) => {
    ...
    try {
      const res = await tarbiyahDomisiliSantriAPI.getCatatan(idSantri, {
        jenis_catatan: rowCatatanFilterJenis === '' ? undefined : rowCatatanFilterJenis
      })
  ...
  const handleRowCatatanSimpan = async () => {
    ...
    try {
      const res = await tarbiyahDomisiliSantriAPI.postCatatan({
        ...
        jenis_catatan: rowCatatanJenisBaru === 'hitam' ? 'hitam' : 'putih'
      })
      if (res?.success) {
        setRowCatatanText('')
        setRowCatatanJenisBaru('putih')
  ...
  const handleRowCatatanHapus = async (catatan) => {
    const idCatatan = catatan?.id
    const idS = santriRowSheetSantri?.id
    if (!idCatatan || !idS || rowCatatanDeletingId != null) return
    const ok = typeof window === 'undefined' ? true : window.confirm('Hapus catatan santri ini?')
    if (!ok) return
    setRowCatatanDeletingId(idCatatan)
    try {
      const res = await tarbiyahDomisiliSantriAPI.deleteCatatan(idCatatan)
      if (res?.success) {
        showNotification('Catatan dihapus.', 'success')
        await loadRowCatatan(idS)
      } else {
        showNotification(res?.message || 'Gagal hapus catatan', 'error')
      }
    } catch (err) {
      showNotification(err?.response?.data?.message || err?.message || 'Gagal hapus catatan', 'error')
    } finally {
      setRowCatatanDeletingId(null)
    }
  }
  ...
  const handleOpenTambahSantriSearch = useCallback(() => {
    setSantriTambahSearchOpen(true)
  }, [])

  const handleTambahSantriPicked = useCallback(
    async (santri) => {
      const rombel = santriOffcanvasRombel
      const jenis = rombelLembagaJenis
      if (!santri?.id || !rombel?.id || !jenis) {
        if (santri?.id && rombel?.id && !jenis) {
          showNotification(
            'Kategori lembaga rombel ini bukan Diniyah atau Formal — tidak bisa menugaskan rombel. Periksa data lembaga di Pengaturan → Lembaga.',
            'warning'
          )
        }
        return
      }
      const rid = String(rombel.id)
      if (jenis === 'diniyah' && String(santri.id_diniyah ?? '') === rid) {
        showNotification('Santri ini sudah terdaftar di rombel diniyah ini.', 'info')
        return
      }
      if (jenis === 'formal' && String(santri.id_formal ?? '') === rid) {
        showNotification('Santri ini sudah terdaftar di rombel formal ini.', 'info')
        return
      }
      const taH = (tahunAjaranHijriyah || '').trim()
      const taM = (tahunAjaranMasehiStore || '').trim()
      const payload = {}
      if (jenis === 'diniyah') {
        payload.id_diniyah = rombel.id
        if (taH) payload.tahun_ajaran_diniyah = taH
      } else {
        payload.id_formal = rombel.id
        if (taM) payload.tahun_ajaran_formal = taM
      }
      const oldId = jenis === 'diniyah' ? santri.id_diniyah : santri.id_formal
      setSantriTambahSubmitting(true)
      try {
        const res = await santriAPI.update(santri.id, payload)
        if (res?.success) {
          showNotification('Santri berhasil ditambahkan ke rombel ini', 'success')
          if (oldId != null && String(oldId) !== '' && String(oldId) !== rid) {
            updateRombelJumlahSantri(Number(oldId), -1)
          }
          updateRombelJumlahSantri(rombel.id, 1)
          await refreshSantriOffcanvasList()
        } else {
          showNotification(res?.message || 'Gagal menambahkan santri ke rombel', 'error')
        }
      } catch (err) {
        console.error('Error tambah santri ke rombel:', err)
        showNotification('Gagal menambahkan santri ke rombel', 'error')
      } finally {
    ...
    [
      ...
      rombelLembagaJenis,
      tahunAjaranHijriyah,
      tahunAjaranMasehiStore,
      showNotification,
      refreshSantriOffcanvasList,
      updateRombelJumlahSantri
    ]
  )

  const handleOpenDeleteRombelModal = () => {
    if (rombelFormReadOnly) {
      showNotification('Penghapusan rombel tidak tersedia untuk akses rombel bertugas', 'error')
      return
    }
  ...
  const handleSubmitWali = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        id_pengurus_fan: [...new Set(guruFanIds.map((x) => parseInt(String(x), 10)).filter((n) => !Number.isNaN(n) && n > 0))]
      }
      if (editingWali?.id) {
      ...
      const message = err?.response?.data?.message || err?.message || 'Terjadi kesalahan saat menyimpan'
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (rombelFormReadOnly) {
      showNotification('Data master rombel hanya baca untuk peran Anda. Kelola wali kelas / guru FAN di bawah.', 'error')
      return
    }
    ...
    <div className="h-full overflow-hidden" style={{ minHeight: 0 }}>
      <div className="h-full overflow-y-auto page-content-scroll" style={{ minHeight: 0 }}>
        <div className="container mx-auto px-4 py-6 max-w-7xl">
          ...
          {rombelScope.applyBertugasFilter && (
            <div className="rounded-lg border border-teal-200 bg-teal-50/80 dark:border-teal-800 dark:bg-teal-900/20 px-3 py-2 mb-4 text-sm text-teal-900 dark:text-teal-100">
              Anda memiliki akses <strong>rombel bertugas</strong>: hanya rombel tempat Anda wali kelas aktif atau guru FAN. Master rombel (tambah/ubah/hapus) tidak tersedia.
            </div>
          )}
          ...
          <div className="sticky top-0 z-10 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-4">
                ...
                >
                  <div className="px-4 py-2">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {rombelScope.canFilterPengurusAmpu ? (
                        <select
                          value={filterPengurusAmpu}
                          onChange={(e) => { setFilterPengurusAmpu(e.target.value); setPage(1) }}
                          disabled={!selectedLembagaIdForPengurus}
                          className="border rounded p-1 h-7 min-w-0 text-xs bg-white dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600 focus:ring-1 focus:ring-teal-400 max-w-[200px] disabled:opacity-50"
                          title={!selectedLembagaIdForPengurus ? 'Pilih lembaga di filter terlebih dahulu' : 'Filter rombel per pengurus (wali / guru FAN)'}
                        >
                          <option value="">Semua pengurus</option>
                          {pengurusFilterOptions.map((p) => (
                            <option key={p.id} value={String(p.id)}>{p.nama || `ID ${p.id}`}</option>
                          ))}
                        </select>
                      ) : null}
                    </div>
            ...
            {(isSuperAdmin || !rombelScope.applyBertugasFilter) && (
              <button
                type="button"
          ...
          {rombelList.length === 0 && !loading && (
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                {searchQuery || filterLembaga || filterStatus || filterKelas || filterPengurusAmpu
              ...
              >
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between flex-shrink-0">
                  <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">
                    {rombelFormReadOnly && editingRombel ? 'Rombel (lihat)' : editingRombel ? 'Edit Rombel' : 'Tambah Rombel'}
                  </h3>
                  <button
                    type="button"
                    onClick={() => { if (panelHistoryCountRef.current > 0) window.history.back() }}
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {rombelFormReadOnly && (
                      <p className="text-sm text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-900/25 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">
                        Data kelas ini hanya dapat dilihat. Anda dapat mengelola bagian <strong>Wali kelas</strong> di bawah.
                      </p>
                    )}
                    <div>
                      ...
                      <select
                        ...
                        required
                        disabled={rombelFormReadOnly}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200 disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="min-w-0 flex-1 pt-0.5">
                                  ...
                                  {Array.isArray(wk.guru_fan) && wk.guru_fan.length > 0 ? (
                                    <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">
                                      Guru FAN: {wk.guru_fan.map((g) => g?.nama || g?.id_pengurus).filter(Boolean).join(', ')}
                                    </p>
                                  ) : null}
                                  {isAktif && (
                  ...
                  <div className="p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between gap-2 flex-shrink-0">
                    <div>
                      {editingRombel?.id && isSuperAdmin && Number(editingRombel.jumlah_santri ?? 0) === 0 && !rombelFormReadOnly && (
                        <button
                          type="button"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="submit"
                        disabled={saving || rombelFormReadOnly}
                  ...
                  <>
                    ...
                    >
                      ...
                      <form onSubmit={handleSubmitWali} className="flex-1 flex flex-col min-h-0">
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                          {waliSelectorsLoading ? (
                            <p className="text-sm text-teal-700 dark:text-teal-300">Memuat pengurus dan santri rombel…</p>
                          ) : null}
                          <div>
                            ...
                            <select
                              ...
                              disabled={waliSelectsDisabled}
                              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200 disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                              <option value="">-- Pilih Tahun Ajaran --</option>
                              {waliKelasTahunAjaranGroups.hij.length > 0 ? (
                                <optgroup label="Hijriyah">
                                  {waliKelasTahunAjaranGroups.hij.map((o) => (
                                    <option key={`ta-h-${o.value}`} value={o.value}>{o.label}</option>
                                  ))}
                                </optgroup>
                              ) : null}
                              {waliKelasTahunAjaranGroups.mas.length > 0 ? (
                                <optgroup label="Masehi">
                                  {waliKelasTahunAjaranGroups.mas.map((o) => (
                                    <option key={`ta-m-${o.value}`} value={o.value}>{o.label}</option>
                                  ))}
                                </optgroup>
                              ) : null}
                            </select>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              disabled={waliSelectsDisabled}
                              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-gray-200 disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Guru FAN (pengurus lembaga)</label>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Opsional; beberapa pengurus pendamping. Hanya daftar pengurus di lembaga rombel ini.</p>
                            <div className="space-y-2">
                              {guruFanIds.map((gid, idx) => {
                                const waliPid = String(waliFormData.id_pengurus || '').trim()
                                const opts = pengurusList.filter((p) => {
                                  const pid = String(p.id)
                                  if (waliPid && pid === waliPid) return false
                                  return !guruFanIds.some((g, j) => j !== idx && g && g === pid)
                                })
                                return (
                                  <div key={`fan-${idx}`} className="flex gap-2 items-center">
                                    <select
                                      value={gid}
                                      onChange={(e) => {
                                        const v = e.target.value
                                        setGuruFanIds((prev) => prev.map((x, i) => (i === idx ? v : x)))
                                      }}
                                      disabled={waliSelectsDisabled}
                                      className="flex-1 min-w-0 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-gray-200 disabled:opacity-60 disabled:cursor-not-allowed"
                                    >
                                      <option value="">-- Pilih pengurus --</option>
                                      {opts.map((p) => (
                                        <option key={p.id} value={p.id}>{p.nama || `ID ${p.id}`}</option>
                                    ...
                                    <button
                                      type="button"
                                      disabled={waliSelectsDisabled}
                                      onClick={() => setGuruFanIds((prev) => prev.filter((_, i) => i !== idx))}
                                      className="shrink-0 px-2 py-2 text-sm rounded-lg border border-red-300 text-red-700 hover:bg-red-50 dark:border-red-700 dark:text-red-300 dark:hover:bg-red-900/20 disabled:opacity-50"
                                    >
                                      Hapus
                                    </button>
                                  </div>
                            ...
                            <button
                              type="button"
                              disabled={waliSelectsDisabled}
                              onClick={() => setGuruFanIds((prev) => [...prev, ''])}
                              className="mt-2 w-full sm:w-auto px-3 py-2 text-sm rounded-lg border border-teal-600 text-teal-700 hover:bg-teal-50 dark:border-teal-500 dark:text-teal-300 dark:hover:bg-teal-900/20 disabled:opacity-50"
                            >
                              + Tambah guru FAN
                            </button>
                          </div>
                        ...
                        <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2 flex-shrink-0">
                          ...
                          <button
                            type="submit"
                            disabled={savingWali || waliSelectorsLoading || !waliSelectorsReady}
                ...
                <div className="flex-shrink-0 border-b border-gray-200 p-4 dark:border-gray-700">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <button
                        type="button"
                        onClick={handleOpenTambahSantriSearch}
                        disabled={santriTambahSubmitting || !rombelLembagaJenis}
                        title={
                          !rombelLembagaJenis
                            ? 'Lembaga rombel ini bukan kategori Diniyah/Formal — tidak bisa menugaskan rombel dari sini.'
                            : 'Tambah santri ke rombel ini (cari santri)'
                        }
                          ...
                          return (
                            ...
                            >
                              <motion.span
                                ...
                                initial={false}
                                animate={{
                                  width: santriSelectMode ? 22 : 0,
                                  opacity: santriSelectMode ? 1 : 0,
                                  marginRight: santriSelectMode ? 8 : 0
                                }}
                              ...
                              >
                                <input
                                  ...
                                  className="h-4 w-4 shrink-0 rounded border-gray-300 text-teal-600 focus:ring-teal-500 dark:border-gray-500 dark:bg-gray-700 dark:focus:ring-teal-600"
                                  aria-label={`Pilih ${s.nama || s.nis || 'santri'}`}
                                  tabIndex={santriSelectMode ? 0 : -1}
                                  style={{ pointerEvents: santriSelectMode ? 'auto' : 'none' }}
                                />
                              </motion.span>
                                  ...
                                  (moveLoadingId === s.id ? (
                                    ...
                                    >
                                      <button
                                        ...
                                        className="rounded-lg p-2 text-gray-500 hover:bg-gray-200 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-gray-100"
                          ...
                          {santriRowSheet === 'catatan' && (
                            <div className="space-y-3 pb-2">
                              <div>
                                ...
                                <SantriCatatanJenisToggle
                                  id="rombel-santri-row-catatan-jenis"
                                  value={rowCatatanJenisBaru === 'hitam' ? 'hitam' : 'putih'}
                                  onChange={setRowCatatanJenisBaru}
                                  disabled={rowCatatanSubmitting}
                                />
                                <textarea
                              ...
                              <div>
                                ...
                                <SantriCatatanJenisFilter
                                  id="rombel-santri-catatan-filter"
                                  value={rowCatatanFilterJenis}
                                  onChange={setRowCatatanFilterJenis}
                                />
                                {rowCatatanLoading ? (
                                    ...
                                    {rowCatatanList.map((c) => {
                                      ...
                                      >
                                        <div className="flex items-start gap-2">
                                          ...
                                          {canHapusCatatanSantri ? (
                                            <button
                                              type="button"
                                              disabled={rowCatatanDeletingId === c.id}
                                              onClick={() => void handleRowCatatanHapus(c)}
                                              className="shrink-0 rounded-md px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 disabled:opacity-50 dark:text-red-300 dark:hover:bg-red-900/30"
                                              title="Hapus catatan"
                                            >
                                              {rowCatatanDeletingId === c.id ? '...' : 'Hapus'}
                                            </button>
                                          ) : null}
                                        </div>
                                        {c.keterangan ? (
      ...
      {createPortal(
        <SearchOffcanvas
          isOpen={Boolean(santriTambahSearchOpen && santriOffcanvasOpen && santriOffcanvasRombel?.id)}
          onClose={closeSantriTambahSearch}
          onSelectSantriRecord={handleTambahSantriPicked}
          zIndex={10200}
        />,
        document.body
      )}
