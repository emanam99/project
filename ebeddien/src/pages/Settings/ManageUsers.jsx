...
const UserListItem = memo(({ user, index, onClick, getStatusBadgeColor, getStatusDisplayName }) => {
  ...
  const isToko = user.is_toko === true
  const isPjgt = user.is_pjgt === true
  const hasIdentity = isPengurus || isSantri || isToko || isPjgt
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0 pr-4">
          ...
          {/* Badge identitas — satu akun bisa punya beberapa peran */}
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            ...
            {isSantri && (
              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                Santri
                {user.santri_count > 1 ? ` (${user.santri_count})` : ''}
              </span>
            )}
            {isToko && (
              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-teal-100 text-teal-800 dark:bg-teal-900/30 dark:text-teal-400">
                Toko
                {user.toko_count > 1 ? ` (${user.toko_count})` : ''}
              </span>
            )}
            {isPjgt && (
              <span
                className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-400 max-w-[12rem] truncate"
                title={user.pjgt_madrasah_nama || 'PJGT'}
              >
                PJGT{user.pjgt_madrasah_nama ? `: ${user.pjgt_madrasah_nama}` : ''}
              </span>
            )}
            {!hasIdentity && (
              <span className="text-xs text-gray-400 dark:text-gray-500 italic">Belum terhubung identitas</span>
            )}
          </div>
        </div>
...
function ManageUsers() {
  ...
  const [typeFilter, setTypeFilter] = useState('all') // all | pengurus | santri | toko | pjgt
  ...
  useEffect(() => {
    const raw = location.state?.openEditUserId
    if (raw == null || raw === '') return
    setEditUserId(String(raw))
  ...
  const loadAllUsers = useCallback(async (opts = {}) => {
  ...
  const handleUserClick = useCallback((userId) => {
    setEditUserId(userId != null ? String(userId) : null)
  }, [])
  ...
  const handleExportUsers = useCallback(() => {
    try {
      ...
      const excelData = filteredUsers.map(user => ({
        ...
        'No. WA': user.no_wa || '',
        'Tipe': [
          user.is_pengurus && 'Pengurus',
          user.is_santri && 'Santri',
          user.is_toko && 'Toko',
          user.is_pjgt && 'PJGT',
        ].filter(Boolean).join(', ') || '-',
        'Status': user.status || '',
  ...
  return (
    ...
    <EditUserOffcanvas
      ...
      onUserSaved={() => loadAllUsers({ silent: true })}
    />
    </Fragment>
  )
}
