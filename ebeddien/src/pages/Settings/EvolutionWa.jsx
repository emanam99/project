...
// Polling backoff untuk cek status koneksi QR Evolution: 2s → 5s → 10s.
// Auto-stop setelah 5 menit untuk menghindari beban server kalau halaman ditinggal.
const QR_POLL_STEPS_MS = [2000, 5000, 10000]
const QR_POLL_MAX_MS = 5 * 60 * 1000
...
export default function EvolutionWa() {
  ...
  const [webhookInstanceInput, setWebhookInstanceInput] = useState('')
  const [webhookCustomUrl, setWebhookCustomUrl] = useState('')
  const [webhookExtraConnection, setWebhookExtraConnection] = useState(false)
  const [webhookLoading, setWebhookLoading] = useState(null)
  const [webhookLastResponse, setWebhookLastResponse] = useState(null)

  const loadConfig = useCallback(async () => {
    const res = await evolutionApiAPI.getConfig()
    if (res?.success && res?.data) {
      setConfig(res.data)
      const n = res.data.instance_name || ''
      setInstanceName(n)
      setWebhookInstanceInput((prev) => (prev.trim() === '' ? n : prev))
    } else {
      setConfig(null)
    }
  }, [])
  ...
  const handleWebhookRead = async () => {
    const name = (webhookInstanceInput.trim() || instanceName.trim())
    if (!name) {
      showNotification('Isi nama instance untuk webhook', 'warning')
      return
    }
    setWebhookLoading('read')
    setWebhookLastResponse(null)
    try {
      const res = await evolutionApiAPI.getInstanceWebhook(name)
      setWebhookLastResponse(res)
      if (res?.success) {
        showNotification('Konfigurasi webhook di Evolution berhasil dibaca', 'success')
      } else {
        showNotification(res?.message || 'Gagal membaca webhook', 'error')
      }
    } catch (err) {
      const res = err?.response?.data
      setWebhookLastResponse(res || { success: false, message: err?.message })
      showNotification(res?.message || err?.message || 'Gagal', 'error')
    } finally {
      setWebhookLoading(null)
    }
  }

  const handleWebhookApply = async () => {
    const name = (webhookInstanceInput.trim() || instanceName.trim())
    if (!name) {
      showNotification('Isi nama instance untuk webhook', 'warning')
      return
    }
    const custom = webhookCustomUrl.trim()
    const events = ['MESSAGES_UPSERT']
    if (webhookExtraConnection) {
      events.push('CONNECTION_UPDATE')
    }
    setWebhookLoading('set')
    setWebhookLastResponse(null)
    try {
      const body = {
        events,
      ...
      if (custom !== '') {
        body.use_app_inbound_url = false
        body.url = custom
      }
      const res = await evolutionApiAPI.setInstanceWebhook(name, body)
      setWebhookLastResponse(res)
      if (res?.success) {
        showNotification(res?.message || 'Webhook diterapkan di Evolution', 'success')
      } else {
        showNotification(res?.message || 'Gagal set webhook', 'error')
      }
    } catch (err) {
      const res = err?.response?.data
      setWebhookLastResponse(res || { success: false, message: err?.message })
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return
      if (Date.now() - startedAt > QR_POLL_MAX_MS) return
      if (typeof document !== 'undefined' && document.hidden) {
        timeoutId = setTimeout(tick, QR_POLL_STEPS_MS[0])
        return
      }
      await check()
      const delay = QR_POLL_STEPS_MS[Math.min(stepIdx, QR_POLL_STEPS_MS.length - 1)]
      stepIdx += 1
        ...
        <div className="max-w-2xl mx-auto p-4 pb-8">
          ...
          {config?.configured && (
            <div className="mb-4 rounded-lg border border-blue-200 dark:border-blue-900/50 bg-blue-50 dark:bg-blue-950/30 px-3 py-2 text-sm text-blue-900 dark:text-blue-100 space-y-2">
              ...
              {config.inbound_webhook_secret_configured ? (
                <p className="text-xs text-blue-800/90 dark:text-blue-200/90">
                  Secret aktif di server: tombol «Terapkan webhook ke Evolution» di bawah akan mendaftarkan URL lengkap termasuk query <code className="font-mono">?secret=…</code> ke instance (nilai secret tidak ditampilkan di browser).
                </p>
              ) : null}
            </div>
          )}
          ...
          <div className="space-y-6">
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setWebhookInstanceInput(String(name))
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                <h2 className="text-base font-semibold text-gray-800 dark:text-gray-200">Set webhook di Evolution</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Mendaftarkan URL inbound eBeddien ke instance Evolution lewat API{' '}
                  <code className="text-xs font-mono">POST /webhook/set/&#123;instance&#125;</code>. Default event:{' '}
                  <code className="text-xs font-mono">MESSAGES_UPSERT</code> (pesan masuk). Butuh{' '}
                  <code className="text-xs font-mono">API_PUBLIC_URL</code> di backend kecuali Anda pakai URL kustom. Pesan webhook dari Evolution hanya diproses backend jika nama instance di payload sama dengan nama default yang disimpan di blok Instance default.
                </p>
              </div>
              <div className="p-4 space-y-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama instance Evolution</label>
                  <input
                    type="text"
                    value={webhookInstanceInput}
                    onChange={(e) => setWebhookInstanceInput(e.target.value)}
                    placeholder={instanceName || 'sama dengan nama default di atas'}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm font-mono"
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Kosongkan field ini untuk memakai nama instance dari blok Instance default di atas.
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">URL webhook kustom (opsional)</label>
                  <input
                    type="url"
                    value={webhookCustomUrl}
                    onChange={(e) => setWebhookCustomUrl(e.target.value)}
                    placeholder="https://… (kosong = pakai API_PUBLIC_URL + path evolution-webhook)"
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm font-mono"
                  />
                </div>
                <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={webhookExtraConnection}
                    onChange={(e) => setWebhookExtraConnection(e.target.checked)}
                    className="rounded border-gray-300 dark:border-gray-600"
                  />
                  Juga langganan event <code className="text-xs font-mono">CONNECTION_UPDATE</code> (status koneksi)
                </label>
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={handleWebhookApply}
                    disabled={webhookLoading !== null || !config?.configured}
                  ...
                  >
                    {webhookLoading === 'set' ? 'Menerapkan…' : 'Terapkan webhook ke Evolution'}
                  </button>
                  <button
                    type="button"
                    onClick={handleWebhookRead}
                    disabled={webhookLoading !== null || !config?.configured}
                  ...
                  >
                    {webhookLoading === 'read' ? 'Memuat…' : 'Baca webhook dari Evolution'}
                  </button>
                </div>
                {webhookLastResponse != null && (
                  <pre className="text-xs bg-gray-50 dark:bg-gray-900 rounded-lg p-3 overflow-x-auto max-h-56 border border-gray-200 dark:border-gray-600">
                    {JSON.stringify(webhookLastResponse, null, 2)}
                  </pre>
                )}
              </div>
            </div>
