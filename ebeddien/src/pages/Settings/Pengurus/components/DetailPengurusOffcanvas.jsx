...
/**
 * Rata kanan, selebar panel detail (`max-w-md`). Pakai `w-full max-w-md` (bukan `100vw`) agar tidak overflow scrollbar → tombol tidak terpotong di tepi kanan.
 * `min-h-0 overflow-hidden` + area scroll `min-h-0` agar footer (Simpan/Tambah) tetap terlihat di dalam flex column.
 */
...
function DetailPengurusOffcanvas({ isOpen, onClose, pengurusId, lembagaList = [], onPengurusPatch, showEditPanel = false, onOpenEdit, onCloseEdit }) {
  ...
  /** Daftar role untuk tambah/hapus (terfilter role___boleh_assign_role bila dibatasi). */
  ...
  /** Tombol kelola role: super_admin selalu; selain itu mengikuti matriks / daftar boleh assign. */
