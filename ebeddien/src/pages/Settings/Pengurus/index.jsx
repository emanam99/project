...
import {
  filterPengurusList,
  normalizePengurusStatus,
  matchPengurusByStatus,
  matchPengurusByKategori,
  matchPengurusByLembaga,
  matchPengurusByJabatan,
  buildPengurusListFilterQueryString,
  sortPengurusList,
  buildJabatanUrutanLookup,
  PENGURUS_SORT_NAMA,
  PENGURUS_SORT_JABATAN_URUTAN,
  PENGURUS_SORT_STORAGE_KEY,
} from './pengurusListFilterUtils'
              ...
              <AnimatePresence>
                {menuOpen && (
                  ...
                  >
                    ...
                    {canExcel && (
                      <button
                        onClick={() => runAndClose(onExcelClick)}
                        className="w-full px-3 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="border-t border-gray-100 dark:border-gray-600 my-1" />
                    <div className="px-3 py-1.5 text-xs font-medium text-gray-500 dark:text-gray-400">Urutkan</div>
                    <button
                      onClick={() => { onSortChange?.(PENGURUS_SORT_NAMA); setMenuOpen(false) }}
                      className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2 ${sortBy === PENGURUS_SORT_NAMA ? 'text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                    >
                      {sortBy === PENGURUS_SORT_NAMA && (
                        <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                    ...
                    <button
                      onClick={() => { onSortChange?.(PENGURUS_SORT_JABATAN_URUTAN); setMenuOpen(false) }}
                      className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2 ${sortBy === PENGURUS_SORT_JABATAN_URUTAN ? 'text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                    >
                      {sortBy === PENGURUS_SORT_JABATAN_URUTAN && (
                        <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                      )}
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0 pr-4">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex flex-wrap items-center gap-x-1">
            <span>NIP: {pengurus.nip ?? pengurus.id}</span>
            {pengurus.username ? (
              <>
                <span>·</span>
                <UsernameLinkButton
                  userId={pengurus.id_user}
            ...
            {pengurus.email ? <span>· {pengurus.email}</span> : null}
            {pengurus.whatsapp ? <span>· {pengurus.whatsapp}</span> : null}
          </p>
...
function Pengurus() {
  ...
  const [jabatanMasterList, setJabatanMasterList] = useState([])
  ...
  const [sortBy, setSortBy] = useState(() => {
    try {
      const saved = localStorage.getItem(PENGURUS_SORT_STORAGE_KEY)
      if (saved === PENGURUS_SORT_JABATAN_URUTAN) return PENGURUS_SORT_JABATAN_URUTAN
    } catch (e) {}
    return PENGURUS_SORT_NAMA
  })
  const handleSortChange = useCallback((mode) => {
    setSortBy(mode)
    try {
      localStorage.setItem(PENGURUS_SORT_STORAGE_KEY, mode)
    } catch (e) {}
  }, [])
  ...
  useEffect(() => {
    const loadMeta = async () => {
      try {
        const [lembagaResponse, jabatanResponse] = await Promise.all([
          api.get('/lembaga'),
          jabatanAPI.getList({ status: 'aktif' }),
        ])
        if (lembagaResponse.data?.success) {
          setLembagaList(lembagaResponse.data.data || [])
        }
        if (jabatanResponse?.success && Array.isArray(jabatanResponse.data)) {
          setJabatanMasterList(jabatanResponse.data)
        }
      } catch (err) {
        console.error('Error loading lembaga/jabatan:', err)
      }
    }
    loadMeta()
  }, [])
  ...
  const listFilters = useMemo(
    () => ({
      status: statusFilter,
      kategori_lembaga: kategoriLembagaFilter,
      lembaga: jabatanLembagaFilter,
      jabatan: jabatanFilter,
      q: searchQuery,
    }),
    [statusFilter, kategoriLembagaFilter, jabatanLembagaFilter, jabatanFilter, searchQuery]
  )

  const filteredPengurus = useMemo(
    () => filterPengurusList(allPengurus, listFilters),
    [allPengurus, listFilters]
  )

  const jabatanUrutanLookup = useMemo(
    () => buildJabatanUrutanLookup(jabatanMasterList),
    [jabatanMasterList]
  )

  const sortedPengurus = useMemo(
    () =>
      sortPengurusList(filteredPengurus, sortBy, {
        lookup: jabatanUrutanLookup,
        scopeFilters: {
          lembaga: jabatanLembagaFilter,
          jabatan: jabatanFilter,
        },
      }),
    [filteredPengurus, sortBy, jabatanUrutanLookup, jabatanLembagaFilter, jabatanFilter]
  )

  useEffect(() => {
    setCurrentPage(1)
  }, [statusFilter, kategoriLembagaFilter, jabatanLembagaFilter, jabatanFilter, searchQuery, sortBy])
  ...
  const { statusOptions, kategoriOptions, lembagaOptions, jabatanOptions } = useMemo(() => {
    ...
    const dataForStatus = base.filter(
      (p) =>
        matchPengurusByKategori(p, kategoriLembagaFilter) &&
        matchPengurusByLembaga(p, jabatanLembagaFilter) &&
        matchPengurusByJabatan(p, jabatanFilter)
    )
    const statusCounts = {}
    dataForStatus.forEach((p) => {
      const n = normalizePengurusStatus(p.status) || '(tanpa status)'
    ...
    const dataForKategori = base.filter(
      (p) =>
        matchPengurusByStatus(p, statusFilter) &&
        matchPengurusByLembaga(p, jabatanLembagaFilter) &&
        matchPengurusByJabatan(p, jabatanFilter)
    )
  ...
  const totalPengurus = sortedPengurus.length
  const totalPages = Math.ceil(totalPengurus / itemsPerPage)
  const paginatedPengurus = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage
    return sortedPengurus.slice(startIndex, startIndex + itemsPerPage)
  }, [sortedPengurus, currentPage, itemsPerPage])
...
const handleDownloadTemplate = useCallback(() => {
  ...
  const handleOpenExcelEditor = useCallback(() => {
    const qs = buildPengurusListFilterQueryString(listFilters)
    navigate(qs ? `/pengurus/excel-editor?${qs}` : '/pengurus/excel-editor')
  }, [navigate, listFilters])
    ...
    <div className="h-full overflow-hidden" style={{ minHeight: 0 }}>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
              {paginatedPengurus.length === 0 ? (
                <div className="p-8 text-center text-gray-500 dark:text-gray-400">
                  {sortedPengurus.length === 0 && allPengurus.length > 0
      ...
      {createPortal(
        <DetailPengurusOffcanvas
          ...
          canEditPengurus={pengurusFitur.editPengurus}
        />,
        document.body
      )}
    </div>
