...
function normalizeMenuPathForBerandaFilter(p) {
  const s = p == null ? '/' : String(p).replace(/\/$/, '') || '/'
  return s
}
...
const menuIconsByPath = {
  ...
  '/ugt/guru-tugas': <svg className={iconClass} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>,
  ...
  '/kalender/jadwal-sholat': <svg className={iconClass} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
...
export default function Beranda() {
  ...
  useEffect(() => {
    ...
    profilAPI.getUser(expectedId).then((res) => {
      if (cancelled || useAuthStore.getState().user?.id !== expectedId) return
      if (!res?.success || !res.user?.foto_profil) {
  ...
  const { allowedMenus, menuListLoading } = useMemo(() => {
    ...
    const filtered = menus.filter((item) => {
      ...
      const pathNorm = normalizeMenuPathForBerandaFilter(item.path)
      if (BERANDA_EXCLUDE_HEADER_DUPLICATE_PATHS.has(pathNorm)) return false
      return true
    })
    ...
    >
      ...
      >
        <div className="flex justify-center sm:justify-start gap-2 mb-3">
          ...
          >
            {/* Sparkles — ikon khas AI (ChatGPT/Gemini/Claude style) */}
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
              ...
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 3v3M3.5 4.5h3M5.5 17v3M4 18.5h3" />
            </svg>
          </button>
          <button
            type="button"
          ...
          {canInstall && (
            <button
              ...
              className="flex h-11 items-center justify-center gap-2 rounded-xl border border-teal-200/80 dark:border-teal-700/60 bg-teal-600 text-white shadow-sm hover:shadow-md hover:bg-teal-700 transition-all px-3 text-xs font-semibold"
            >
