      ...
      >
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800/90 sm:p-5">
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            atau broadcast). Pengguna dengan <strong className="text-gray-700 dark:text-gray-200">no_wa terverifikasi</strong> di eBeddien
            (profil, format 08 atau 62) otomatis dibalas sebagai akun mereka — limit &amp; riwayat selaras tab Obrolan. Aktifkan &quot;terima
            semua pengirim&quot; agar nomor lain (belum terverifikasi) juga mendapat balasan sebagai pengunjung. Nomor tersimpan terpisah dari
            JID.
          </p>
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800/90 sm:p-5">
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            JID = identitas chat di WhatsApp. Nomor diisi dari <strong className="text-gray-700 dark:text-gray-200">@s.whatsapp.net</strong>{' '}
            atau dari nomor kanonik yang dikirim server WA (mis. <code className="text-[11px]">senderPn</code> /{' '}
            <code className="text-[11px]">canonicalNumber</code> pada chat <code className="text-[11px]">@lid</code>). Baris lama tanpa nomor terisi
            setelah kontak mengirim pesan lagi (deploy server WA + API terbaru).
          </p>
                  ...
                  contacts.map((c) => (
                    <tr key={c.wa_jid} className="border-t border-gray-100 dark:border-gray-700">
                      ...
                      <td className="px-2 py-2 font-mono text-gray-700 dark:text-gray-300">
                        {c.phone_normalized ? (
                          c.phone_normalized
                        ) : /@lid$/i.test(String(c.wa_jid || '')) ? (
                          <span className="text-gray-400 dark:text-gray-500" title="Nomor belum terdeteksi — kirim pesan lagi dari kontak ini">
                            belum diketahui
                          </span>
                        ) : (
                          '—'
