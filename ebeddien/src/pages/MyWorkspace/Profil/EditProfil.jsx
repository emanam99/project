...
const initialFormData = {
  ...
  email: '', whatsapp: '', no_wa_verified_at: '', email_verified_at: '',
...
export default function EditProfil() {
    ...
    profilAPI.getUser(user.id).then((res) => {
      if (res.success && res.user) {
        ...
        setFormData({
          ...
          email: u.email || '', whatsapp: u.whatsapp || '', no_wa_verified_at: u.no_wa_verified_at || '', email_verified_at: u.email_verified_at || '',
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      const { no_wa_verified_at, email_verified_at, ...rest } = formData
  ...
  const handleKirimLinkVerifikasiEmail = async () => {
    ...
    if (!em) {
      showNotification('Isi dan simpan email di profil terlebih dulu.', 'warning')
      return
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em)) {
    ...
    try {
      ...
      if (res?.success) {
        showNotification(res?.message || 'Link verifikasi dikirim.', 'success')
      } else {
        showNotification(res?.message || 'Gagal mengirim link', 'error')
      }
    } catch (err) {
      showNotification(err?.response?.data?.message || 'Gagal mengirim link', 'error')
    } finally {
                ...
                {formData.email_verified_at ? (
                  <>
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                      {new Date(formData.email_verified_at).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}
                    </span>
                  </>
                ) : (formData.email || '').trim() !== '' ? (
                  <>
                    ...
                    >
                      {sendingEmailVerify ? 'Mengirim…' : 'Kirim link verifikasi ke email ini'}
                    </button>
                  </>
                ) : null}
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">Simpan perubahan email di profil dulu bila baru mengganti alamat, lalu minta link verifikasi.</p>
            </div>
            <div className="sm:col-span-2">
