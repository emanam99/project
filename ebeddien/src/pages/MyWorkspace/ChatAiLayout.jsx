...
export default function ChatAiLayout() {
  ...
  /** Strip tab selalu tampil (Obrolan + Kemampuan); tab pelatihan dll. mengikuti hak akses. */
  ...
  return (
    <ChatAiChromeProvider showSectionTabs={showTabStrip}>
                ...
                {showTabStrip ? (
                  <div className="shrink-0 z-10 border-b border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800 sm:rounded-t-lg">
                    ...
                    >
                      ...
                      <NavLink to="/chat-ai/kemampuan" className={tabNavClass}>
                        Kemampuan
                      </NavLink>
                      {showChatAiSectionTabs && pageTrainingBank ? (
                        <NavLink to="/chat-ai/training" className={tabNavClass}>
                          Bank Q&amp;A
                        </NavLink>
                      ) : null}
                      {showChatAiSectionTabs && pageTrainingChat ? (
                        <NavLink to="/chat-ai/training-chat" className={tabNavClass}>
                          Training Chat
                        </NavLink>
                      ) : null}
                      {showChatAiSectionTabs && pageDashboard ? (
                        <NavLink to="/chat-ai/dashboard" className={tabNavClass}>
                          Dashboard
                        </NavLink>
                      ) : null}
                      {showChatAiSectionTabs && pageRiwayat ? (
                        <NavLink to="/chat-ai/riwayat" className={tabNavClass}>
                          Riwayat
                        </NavLink>
                      ) : null}
                      {showChatAiSectionTabs && pagePengaturan ? (
                        <NavLink to="/chat-ai/pengaturan" className={tabNavClass}>
                          Pengaturan
                        </NavLink>
                      ) : null}
                      {showChatAiSectionTabs && uiUserAiSettings ? (
