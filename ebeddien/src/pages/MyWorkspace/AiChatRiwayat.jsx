...
function extractStoredImageContext(raw) {
  const t = String(raw || '')
  const marker = '[KONTEKS_GAMBAR_TERSIMPAN]'
  const i = t.indexOf(marker)
  if (i < 0) return { prompt: t.trim(), memory: '' }
  const prompt = t.slice(0, i).trim()
  const memory = t.slice(i + marker.length).trim()
...
const ChatLogListItem = memo(({ row, index, onClick }) => {
  const extracted = extractStoredImageContext(row.user_message)
  const q = previewText(extracted.prompt || row.user_message, 140)
  const cat = (row.category || '').trim() || '—'
  const hasImageMemory = extracted.memory !== ''
    ...
    >
      <div className="flex w-full gap-3 border-b border-gray-100 p-4 dark:border-gray-700/80 sm:gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {hasImageMemory ? (
            <p className="mb-1 inline-flex items-center rounded-md bg-amber-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">
              Konteks gambar tersimpan
            </p>
          ) : null}
              ...
              >
                ...
                <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
                  <div>
                    ...
                    />
                    {extractStoredImageContext(selected.user_message || '').memory ? (
                      <div className="mt-2 rounded-xl border border-amber-200 bg-amber-50/70 px-3 py-2 dark:border-amber-800/50 dark:bg-amber-900/20">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-[11px] font-semibold uppercase tracking-wide text-amber-800 dark:text-amber-300">
                            Konteks gambar tersimpan (audit)
                          </p>
                          <button
                            type="button"
                            onClick={async () => {
                              const memory = extractStoredImageContext(selected.user_message || '').memory
                              try {
                                if (!navigator?.clipboard?.writeText) {
                                  throw new Error('Clipboard API tidak tersedia')
                                }
                                await navigator.clipboard.writeText(memory)
                                showNotification('Konteks gambar berhasil disalin.', 'success')
                              } catch {
                                showNotification('Gagal menyalin konteks gambar.', 'error')
                              }
                            }}
                            className="rounded-md border border-amber-300 bg-white px-2 py-1 text-[10px] font-semibold text-amber-800 hover:bg-amber-100 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-200 dark:hover:bg-amber-900/40"
                          >
                            Salin konteks gambar
                          </button>
                        </div>
                        <pre className="mt-1 whitespace-pre-wrap break-words text-[11px] leading-relaxed text-amber-900 dark:text-amber-200">
                          {extractStoredImageContext(selected.user_message || '').memory}
                        </pre>
                      </div>
                    ) : null}
