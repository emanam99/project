...
function ImportTunggakan() {
  ...
  const validateRow = (row, index) => {
    ...
    // Validasi Keterangan 1 (required, bebas)
  ...
  const downloadTemplate = () => {
    const templateData = [
      {
        'NIS': '1234567',
        'Keterangan 1': 'Contoh keterangan tunggakan',
    ...
    const wsData = [
      ['KETERANGAN:'],
      ['Kolom Keterangan 1 dapat diisi bebas (tidak ada batasan)'],
      [''],
      ['Format Tahun Ajaran:'],
    ...
    XLSX.writeFile(wb, 'template_import_tunggakan.xlsx')
  }
    ...
    try {
      ...
      for (const item of validData) {
        try {
          ...
          const response = await paymentAPI.insertTunggakanKhusus(dataToSend, 'tunggakan')
          if (response.success) {
            successCount++
          } else {
            errorCount++
      ...
      <div className="h-full overflow-y-auto" style={{ minHeight: 0 }}>
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          >
            ...
            <div className="mb-6">
              <button
                onClick={() => navigate('/pembayaran/tunggakan')}
                className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 mb-4"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
                </svg>
                Kembali ke Data Tunggakan
              </button>
              <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Import Data Tunggakan dari Excel</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Upload file Excel untuk menambah data tunggakan
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                Format kolom: NIS (7 digit), Keterangan 1 (bebas), Keterangan 2 (opsional), Tahun Ajaran (1446-1447 atau 2025-2026), Lembaga, Total (angka)
              </p>
            </div>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <input
                              ...
                              className={`w-40 px-2 py-1 border rounded text-xs dark:bg-gray-700 dark:text-gray-200 ${
...
export default ImportTunggakan
