...
function PrintUwaba({ santriId, inOffcanvas = false, tahunAjaran: tahunAjaranProp }) {
  const [searchParams] = useSearchParams()
  const { user } = useAuthStore()
  const tahunAjaranAktif = useActiveHijriyahTahunAjaran()
  const tahunAjaran = useMemo(() => {
    const fromProp = tahunAjaranProp != null ? String(tahunAjaranProp).trim() : ''
    if (fromProp) return fromProp
    const fromUrl = searchParams.get('tahun_ajaran')
    if (fromUrl) return String(fromUrl).trim()
    return tahunAjaranAktif || ''
  }, [tahunAjaranProp, searchParams, tahunAjaranAktif])
  ...
  // Link QR → myBeddien riwayat UWABA + nis/id (login & validasi santri)
  const getCurrentUrlWithParams = () => {
    const b = uwabaData?.biodata || {}
    return getMybeddienKwitansiQrUrl('uwaba', {
      nis: b.nis,
      id: b.id || santriId || searchParams.get('id'),
    })
  }
  ...
  useEffect(() => {
    ...
    loadData()
  }, [santriId, searchParams, tahunAjaran])
    ...
    <>
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px'}}>
        ...
        <div className="qr-code-container">
          <a href={getCurrentUrlWithParams()} target="_blank" rel="noopener noreferrer" className="qr-code-link" title="Klik untuk membuka riwayat UWABA di myBeddien">
