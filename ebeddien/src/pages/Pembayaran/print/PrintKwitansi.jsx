...
function PrintKwitansi({ santriId, inOffcanvas = false, mode = 'tunggakan' }) {
  ...
  // Link QR → myBeddien + nis/id untuk validasi akun login
  const getCurrentUrlWithParams = () => {
    const biodata = kwitansiData?.biodata || {}
    const identity = {
      nis: biodata.nis,
      id: biodata.id || santriId || searchParams.get('id'),
    }
    if (currentMode === 'pendaftaran') return getMybeddienKwitansiQrUrl('pendaftaran', identity)
    if (currentMode === 'khusus') return getMybeddienKwitansiQrUrl('khusus', identity)
    return getMybeddienKwitansiQrUrl('tunggakan', identity)
  }
    ...
    <>
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px'}}>
        ...
        <div className="qr-code-container" style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px'}}>
          <a href={getCurrentUrlWithParams()} target="_blank" rel="noopener noreferrer" className="qr-code-link" title={currentMode === 'pendaftaran' ? 'Klik untuk membuka riwayat PSB di myBeddien' : currentMode ? `Klik untuk membuka riwayat ${currentMode === 'khusus' ? 'Khusus' : 'Tunggakan'} di myBeddien` : 'Klik untuk membuka myBeddien'}>
