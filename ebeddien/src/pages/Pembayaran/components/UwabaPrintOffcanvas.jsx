...
function UwabaPrintOffcanvas({ isOpen, onClose, santriId, tahunAjaran: tahunAjaranProp }) {
  ...
  const tahunAjaran = (tahunAjaranProp && String(tahunAjaranProp).trim()) || activeTahunAjaran
  ...
  useEffect(() => {
    if (isOpen && santriId && /^\d{7}$/.test(santriId)) {
      // Generate URL untuk PrintUwaba.jsx (React component)
      // Gunakan route /print-uwaba dengan query parameter id
      const baseUrl = window.location.origin
      let url = `${baseUrl}/print-uwaba?id=${santriId}&page=uwaba`
      // Tambahkan tahun_ajaran jika tersedia (penting untuk load data yang benar)
      if (tahunAjaran) {
  ...
  }, [isOpen, santriId, user, tahunAjaran])
  ...
  const loadUwabaDataForWA = async () => {
    try {
      ...
      let url = `${apiUrl}/print?id_santri=${encodeURIComponent(santriId)}&page=uwaba`
      ...
      console.error('Error loading uwaba data for WA:', e)
    }
  }
  ...
  const handleSendWA = async () => {
    if (!uwabaData || !uwabaData.biodata) {
    ...
    const biodata = uwabaData.biodata
    const namaSantri = biodata.nama || biodata.id || 'Santri'

    // Link ke myBeddien riwayat UWABA + identitas santri (login & validasi)
    const urlRiwayat = getMybeddienKwitansiQrUrl('uwaba', {
      nis: biodata?.nis,
      id: biodata?.id || santriId,
    })
    ...
    const byId = {}
    ;(uwabaData.tunggakan || []).forEach(t => { byId[t.id_bulan] = t })
    ...
    const totalBayar = uwabaData.pembayaran ? uwabaData.pembayaran.reduce((sum, p) => {
      return sum + Number(p.nominal || 0)
    }, 0) : 0
    
    // Hitung kurang menggunakan logika yang SAMA PERSIS dengan PrintUwaba
        ...
        <>
          ...
          >
            ...
            <div className="no-print flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
              <h2 className="text-xl font-semibold text-teal-600 dark:text-teal-400">Print Riwayat Uwaba</h2>
              <button
                onClick={onClose}
            ...
            {/* PrintUwaba Component Container */}
            <div className="flex-1 overflow-auto" style={{ position: 'relative' }}>
              {santriId ? (
                <div style={{ height: '100%', overflow: 'auto', position: 'relative' }}>
                  <PrintUwaba santriId={santriId} inOffcanvas={true} tahunAjaran={tahunAjaran} />
                </div>
              ) : (
...
export default UwabaPrintOffcanvas
