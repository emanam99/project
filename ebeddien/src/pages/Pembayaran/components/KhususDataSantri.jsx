...
const getKhususManageRowKey = (r) =>
  Number(r?.id_khusus) > 0 ? `k${r.id_khusus}` : `belum_${r?.id}`
...
function KhususDataSantri() {
  ...
  const loadData = async (opts = {}) => {
    ...
    const snapKey = manageDataSnapshotKey('khusus', snapParts)

    const revParams = { dataset: 'khusus' }
    ...
    try {
      ...
      if (!softRefresh && revisionForStore) {
        try {
          ...
          if (cached && cached.revision === revisionForStore && Array.isArray(cached.rows)) {
            const rows = dedupeManageDataRows(cached.rows, getKhususManageRowKey)
      ...
      const parallelResult = await fetchManageDataParallelOffsets({
        ...
        getRowKey: getKhususManageRowKey,
      })

      if (!parallelResult.ok) {
        setError(parallelResult.message || 'Gagal memuat data khusus')
        setStreamProgress((p) => ({ ...p, active: false }))
      } else {
        const rows = dedupeManageDataRows(parallelResult.rows, getKhususManageRowKey)
        setDataSantri(rows)
  ...
  const dataSantriUnique = useMemo(
    () => dedupeManageDataRows(dataSantri, getKhususManageRowKey),
    [dataSantri],
  )
  ...
  }, [dataSantriUnique, searchTerm, statusFilter, kategoriFilter, lembagaFilter, keterangan1Filter, keterangan2Filter, diniyahFilter, kelasDiniyahFilter, kelDiniyahFilter, formalFilter, kelasFormalFilter, kelFormalFilter, lttqFilter, kelasLttqFilter, kelLttqFilter, saudaraFilter, genderFilter, ketFilter])
      ...
      <div className="px-2 py-3 sm:p-6 lg:p-8">
        ...
        >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
            ...
            <ManageWaBulkProgress page="khusus" onTerminal={() => loadData({ softRefresh: true })} />
