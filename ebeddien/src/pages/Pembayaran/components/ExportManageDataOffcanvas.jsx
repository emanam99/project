...
export default function ExportManageDataOffcanvas({
  isOpen,
  onClose,
  data = [],
  tahunAjaran,
  ...
  const handleExport = () => {
    ...
    const rows = data.map((santri) => {
      const countStatus = getCountStatus ? getCountStatus(santri.count) : { label: '' }
      const ketPembayaran = getKetPembayaran ? getKetPembayaran(santri) : { label: '' }
      const base = {
        ...
        wajib_sebulan: formatCurrencyForExcel(santri.wajib_sebulan),
        ...
        count: santri.count ?? '',
        status_count: countStatus.label ?? '',
    ...
    try {
      ...
      const filename = `Data_Santri_${tahunAjaran || 'All'}_${new Date().toISOString().split('T')[0]}.xlsx`
  ...
  return (
    <AnimatePresence>
      <motion.div
        key="export-manage-data-backdrop"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
          ...
          <ul className="space-y-2">
            {EXPORT_COLUMNS.map(({ key, label }) => (
              <li key={key} className="flex items-center gap-2">
                ...
                />
                <label htmlFor={`export-md-${key}`} className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                  {label}
                </label>
              </li>
            ))}
