...
import { uwabaAPI, paymentAPI, santriAPI } from '../../../services/api'
import { useAuthStore } from '../../../store/authStore'
import { getTanggalFromAPI } from '../../../utils/hijriDate'
import DeletePaymentModal from './DeletePaymentModal'
import IpaymuStaffCheckout from './IpaymuStaffCheckout'
...
function UnifiedPaymentOffcanvas({ 
  ...
  totalBayar,
  kurang,
  /** Tahun ajaran hijriyah (UWABA); jika kosong, fallback tahun aktif header */
  tahunAjaran: tahunAjaranProp,
  /** Riwayat dari parent (UWABA) — selaras filter tahun ajaran halaman */
  paymentHistory: externalPaymentHistory,
  ...
  const useExternalHistory = isUwaba && Array.isArray(externalPaymentHistory)
  const paymentHistory = useExternalHistory ? externalPaymentHistory : localPaymentHistory
  ...
  // Fetch payment history (tunggakan/khusus, atau UWABA tanpa riwayat dari parent)
  const fetchPaymentHistory = useCallback(async () => {
    const seq = ++fetchSeqRef.current

    if (isUwaba) {
      if (!santriId || !/^\d{7}$/.test(santriId) || !tahunAjaran) return
      setLoading(true)
      setLocalPaymentHistory([])
      try {
        const result = await uwabaAPI.getPaymentHistory(santriId, tahunAjaran)
        if (seq !== fetchSeqRef.current) return
        if (result.success && result.data) {
          setLocalPaymentHistory(result.data)
        } else {
          setLocalPaymentHistory([])
        }
      } catch (err) {
  ...
  }, [isUwaba, santriId, tahunAjaran, item?.id, mode])

  // Kosongkan riwayat lokal saat konteks UWABA berubah (hindari flash data tahun lama)
  useEffect(() => {
    if (useExternalHistory) return
    if (isUwaba) {
      setLocalPaymentHistory([])
    }
  }, [useExternalHistory, isUwaba, santriId, tahunAjaran])

  // Load history saat offcanvas dibuka
  useEffect(() => {
    if (isOpen) {
      if (!useExternalHistory) {
        if (isUwaba && santriId && tahunAjaran) {
          fetchPaymentHistory()
  ...
  }, [isOpen, isUwaba, useExternalHistory, santriId, tahunAjaran, item?.id, mode, fetchPaymentHistory])
  ...
  const openIpaymuCheckout = async () => {
    ...
    try {
      if (isUwaba && santriId) {
        ...
        if (result.success && result.data) santriData = result.data
    ...
    if (!santriData) {
      showNotification('Data santri tidak ditemukan. Pastikan ID santri valid.', 'error')
      return
    }
    setIpaymuSantriContact({
      name: santriData.nama || 'Pembayar',
      phone: (santriData.no_telpon || santriData.no_wa_santri || santriData.no_hp || santriData.no_telp || '').toString(),
      email: (santriData.email || '').toString()
    })
  ...
  const ipaymuCreateExtras = useMemo(() => {
    ...
    if (isUwaba && tahunAjaran) extras.tahun_ajaran = tahunAjaran
    return extras
  }, [isUwaba, mode, tahunAjaran, item, santriId])
            ...
            <div className="md:grid md:grid-cols-2 overflow-hidden" style={{ height: 'calc(100vh - 8rem)', maxHeight: '90vh' }}>
              ...
              <div className="p-6 flex flex-col overflow-hidden" style={{ height: '100%', minHeight: 0 }}>
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0" style={{ flexShrink: 0 }}>
                  ...
                  {totals.kurang > 0 && !showForm && !showIpaymuCheckout && (
                    <div className="space-y-2">
                  ...
                  {showIpaymuCheckout && totals.kurang > 0 && (
                    <IpaymuStaffCheckout
                      key={`ipaymu-${ipaymuSantriContact.phone}-${ipaymuSantriContact.email}-${totals.kurang}`}
                      ...
                      onCreated={() => {
                        setTimeout(() => {
                          if (!useExternalHistory) fetchPaymentHistory()
                          if (onPaymentSuccess) onPaymentSuccess()
                        }, 1500)
                      }}
