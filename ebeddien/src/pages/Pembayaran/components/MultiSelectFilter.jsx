 ...
 * @param {{ value: string, count: number, label?: string }[]} options - Opsi dengan count
 * @param {{ label: string, options: { value: string, count: number, label?: string }[] }[]|null} groups - Opsi berkelompok (mis. lembaga per kategori)
...
function MultiSelectFilter({
  ...
  useEffect(() => {
    ...
    masterCheckRef.current.indeterminate =
      selected.length > 0 && selected.length < flatOptions.length
  ...
  const displayText =
    selected.length === 0
      ...
      : selected.length === 1
        ? (() => {
            const o = flatOptions.find(x => x.value === selected[0])
            return o ? (o.label ?? o.value) : selected[0]
          })()
            ...
            >
              <div className="p-2">
                  ...
                  <>
                    ...
                    >
                      <input
                        ref={masterCheckRef}
                        type="checkbox"
                        checked={selected.length === flatOptions.length && flatOptions.length > 0}
                        onChange={e => {
                          e.stopPropagation()
                          if (e.target.checked) {
                      ...
                      <span className="flex-1">
                        {selected.length === flatOptions.length && flatOptions.length > 0
                          ? 'Hapus semua centang'
                          : 'Centang semua'}
                      </span>
                    </label>
                ...
                <div className="space-y-1">
                  {groups?.length
                    ? groups.map((grp) => (
                        <div key={grp.label} className="pt-1 first:pt-0">
                          <p className="px-1.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 sticky top-0 bg-white dark:bg-gray-700">
                            {grp.label}
                          </p>
                          {grp.options.map((item) => {
                            const isChecked = selected.includes(item.value)
                            const optLabel = item.label ?? item.value
                            return (
                              <label
