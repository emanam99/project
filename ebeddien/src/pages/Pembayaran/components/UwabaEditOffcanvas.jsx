...
function UwabaEditOffcanvas({ isOpen, onClose, bulanIndex, bulanData, prices, onSave, santriId, biodata }) {
  ...
  useEffect(() => {
    if (isOpen && bulanData && !wajibLockedFromList) {
      ...
      // Prioritaskan nilai tersimpan per-bulan (jsonData), baru fallback ke biodata aktif.
      // Ini mencegah pilihan formal/diniyah "kembali lagi" setelah save.
      const dStr = jsonData.diniyah || biodata?.diniyah || ''
      const fStr = jsonData.formal || biodata?.formal || ''
      const lembagaDId = matchLembagaId(lembagaDiniyahOptions, dStr)
      const lembagaFId = matchLembagaId(lembagaFormalOptions, fStr)
      setFormData(prev => ({
        ...prev,
        status_santri: jsonData.status_santri || biodata?.status_santri || biodata?.status || 'Mukim',
        kategori: jsonData.kategori || biodata?.kategori || biodata?.kategori_santri || '',
        ...
        diniyah: dStr,
        formal: fStr,
        lttq: jsonData.lttq || biodata?.lttq || '',
        saudara: jsonData.saudara || jsonData.saudara_di_pesantren || biodata?.saudara || biodata?.saudara_di_pesantren || 'Tidak Ada',
        wajib: bulanData.wajib || 0
      }))
    }
  }, [isOpen, bulanData, lembagaDiniyahOptions, lembagaFormalOptions, wajibLockedFromList, biodata])
  ...
  useEffect(() => {
    ...
    let hargaDasar = 0
    if (formData.status_santri && prices.status_santri?.[formData.status_santri]?.wajib != null) {
      hargaDasar = prices.status_santri[formData.status_santri].wajib || 0
    } else if (formData.status_santri && formData.kategori && prices.status_santri?.[formData.status_santri]?.[formData.kategori]) {
    ...
    if (formData.status_santri) setFormData(prev => ({ ...prev, wajib }))
  ...
  const applyWajibListItem = useCallback(
    (item) => {
      ...
      if (!jd || typeof jd !== 'object') {
        ...
        if (ok) showNotification('Nominal diterapkan ke rincian. Simpan (ikon hijau) untuk ke server.', 'success')
        else showNotification('Nominal tidak diterapkan: lengkapi Status dan Kategori terlebih dahulu.', 'warning')
        return
      }
