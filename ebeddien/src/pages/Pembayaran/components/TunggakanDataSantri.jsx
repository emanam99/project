...
import {
  buildTahunAjaranFilterOptions,
} from './manageDataTahunAjaranFilter'
import BulkEditTunggakanKhususOffcanvas from './BulkEditTunggakanKhususOffcanvas'
import BulkCreateTunggakanOffcanvas from './BulkCreateTunggakanOffcanvas'
import ExportTunggakanOffcanvas from './ExportTunggakanOffcanvas'
import ManageDataStreamProgress, { MANAGE_DATA_CHUNK_SIZE } from './ManageDataStreamProgress'
import { dedupeManageDataRows, fetchManageDataParallelOffsets } from './manageDataParallelFetch'
import {
...
const getTunggakanManageRowKey = (r) =>
  Number(r?.id_tunggakan) > 0 ? `t${r.id_tunggakan}` : `belum_${r?.id}`

function TunggakanDataSantri() {
  ...
  const [showBelumAdaKewajiban, setShowBelumAdaKewajiban] = useState(false)
  ...
  const loadData = async (opts = {}) => {
    ...
    const snapParts = showBelumAdaKewajiban
      ? { belum_ada_kewajiban: true, snapshot_v: 2 }
      : { show_all: true, snapshot_v: 2 }
    const snapKey = manageDataSnapshotKey('tunggakan', snapParts)

    const revParams = { dataset: 'tunggakan' }
    if (showBelumAdaKewajiban) revParams.belum_ada_kewajiban = 'true'
    else revParams.show_all = 'true'

    try {
      let revisionForStore = null
      try {
        const revRes = await dashboardAPI.getManageDataRevision(revParams, { signal: ac.signal })
      ...
      if (!softRefresh && revisionForStore) {
        try {
          ...
          if (cached && cached.revision === revisionForStore && Array.isArray(cached.rows)) {
            const rows = dedupeManageDataRows(cached.rows, getTunggakanManageRowKey)
      ...
      const parallelResult = await fetchManageDataParallelOffsets({
        ...
        getRowKey: getTunggakanManageRowKey,
      })

      if (!parallelResult.ok) {
        setError(parallelResult.message || 'Gagal memuat data tunggakan')
        setStreamProgress((p) => ({ ...p, active: false }))
      } else {
        const rows = dedupeManageDataRows(parallelResult.rows, getTunggakanManageRowKey)
        setDataSantri(rows)
        setStreamProgress({ active: false, loaded: rows.length, total: parallelResult.total ?? rows.length })
        if (revisionForStore != null) {
          putManageDataSnapshot(snapKey, {
            revision: revisionForStore,
            rows,
            total: parallelResult.total ?? rows.length,
          })
        }
      }
    } catch (err) {
      ...
      console.error('Error loading data tunggakan:', err)
  ...
  const getRowKey = (santri) => (santri.id_tunggakan > 0 ? santri.id_tunggakan : `belum_${santri.id}`)
  ...
  const dataSantriUnique = useMemo(
    () => dedupeManageDataRows(dataSantri, getTunggakanManageRowKey),
    [dataSantri],
  )

  // Get unique values for filters
  const uniqueStatuses = useMemo(() => {
    const statuses = [...new Set(dataSantriUnique.map(s => s.status).filter(Boolean))]
    return statuses.sort()
  }, [dataSantriUnique])

  const uniqueDiniyah = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.diniyah).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueKelasDiniyah = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.kelas_diniyah).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueKelDiniyah = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.kel_diniyah).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueFormal = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.formal).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueKelasFormal = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.kelas_formal).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueKelFormal = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.kel_formal).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueLttq = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.lttq).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueKelasLttq = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.kelas_lttq).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueKelLttq = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.kel_lttq).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  const uniqueSaudara = useMemo(() => {
    const values = [...new Set(dataSantriUnique.map(s => s.saudara_di_pesantren).filter(Boolean))]
    return values.sort()
  }, [dataSantriUnique])

  // Filter data step by step untuk mendapatkan unique values yang dinamis
  const getFilteredDataForOptions = useMemo(() => {
    let filtered = dataSantriUnique
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(santri =>
        ...
        (santri.gender && String(santri.gender).toLowerCase().includes(term)) ||
  ...
  const dynamicUniqueStatuses = useMemo(() => {
    ...
    const counts = values.map(val => ({ value: val, count: filtered.filter(s => s.status === val).length }))
    return counts.sort((a, b) => a.value.localeCompare(b.value))
  }, [getFilteredDataForOptions, kategoriFilter, tahunAjaranFilter, lembagaFilter, keterangan1Filter, keterangan2Filter, genderFilter, ketFilter, diniyahFilter, kelasDiniyahFilter, kelDiniyahFilter, formalFilter, kelasFormalFilter, kelFormalFilter, lttqFilter, kelasLttqFilter, kelLttqFilter, saudaraFilter])
  ...
  const dynamicUniqueGender = useMemo(() => {
    ...
    const counts = values.map(val => ({ value: val, count: filtered.filter(s => (s.gender || '') === val).length }))
    return counts.sort((a, b) => (a.value || '').localeCompare(b.value || ''))
  }, [getFilteredDataForOptions, statusFilter, kategoriFilter, tahunAjaranFilter, lembagaFilter, keterangan1Filter, keterangan2Filter, ketFilter, diniyahFilter, kelasDiniyahFilter, kelDiniyahFilter, formalFilter, kelasFormalFilter, kelFormalFilter, lttqFilter, kelasLttqFilter, kelLttqFilter, saudaraFilter])
  ...
  const dynamicUniqueKet = useMemo(() => {
    ...
    if (kelLttqFilter) filtered = filtered.filter(s => s.kel_lttq === kelLttqFilter)
    if (saudaraFilter) filtered = filtered.filter(s => s.saudara_di_pesantren === saudaraFilter)
    const ketValues = ['Lunas', 'Kurang', 'Belum']
    const counts = ketValues.map(val => ({ value: val, count: filtered.filter(s => getKetStatus(s) === val).length }))
    return counts
  }, [getFilteredDataForOptions, statusFilter, kategoriFilter, tahunAjaranFilter, lembagaFilter, keterangan1Filter, keterangan2Filter, genderFilter, diniyahFilter, kelasDiniyahFilter, kelDiniyahFilter, formalFilter, kelasFormalFilter, kelFormalFilter, lttqFilter, kelasLttqFilter, kelLttqFilter, saudaraFilter])
  ...
  const dynamicUniqueTahunAjaran = useMemo(() => {
    return buildTahunAjaranFilterOptions(dataSantriUnique, {
      excludeTahunAjaranFilter: true,
      applyOtherFilters: (rows) => {
        let filtered = rows
        if (searchTerm) {
          const term = searchTerm.toLowerCase()
          filtered = filtered.filter((santri) =>
  ...
  }, [dataSantriUnique, searchTerm, statusFilter, kategoriFilter, lembagaFilter, keterangan1Filter, keterangan2Filter, genderFilter, ketFilter, diniyahFilter, kelasDiniyahFilter, kelDiniyahFilter, formalFilter, kelasFormalFilter, kelFormalFilter, lttqFilter, kelasLttqFilter, kelLttqFilter, saudaraFilter])
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    return filtered
  }, [dataSantriUnique, statusFilter, kategoriFilter, tahunAjaranFilter, lembagaFilter, keterangan1Filter, keterangan2Filter, genderFilter, ketFilter, diniyahFilter, kelasDiniyahFilter, kelDiniyahFilter, formalFilter, kelasFormalFilter, kelFormalFilter, lttqFilter, kelasLttqFilter, kelLttqFilter, saudaraFilter, searchTerm, sortConfig])
      ...
      <div className="px-2 py-3 sm:p-6 lg:p-8">
        ...
        >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
            ...
            <ManageWaBulkProgress page="tunggakan" onTerminal={() => loadData({ softRefresh: true })} />
              ...
              {isFilterOpen && (
                ...
                >
                  <div className="px-4 py-2">
                    <label className="flex items-center gap-2 mb-2 text-xs text-gray-700 dark:text-gray-300">
                      <input
                        type="checkbox"
                        checked={showBelumAdaKewajiban}
                        onChange={(e) => {
                          setShowBelumAdaKewajiban(e.target.checked)
                          if (e.target.checked) {
                            setTahunAjaranFilter('')
                            setLembagaFilter('')
                            setKeterangan1Filter('')
                      ...
                      />
                      Tampilkan yang belum ada kewajiban
                    </label>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <select
                        ...
                        className="border rounded p-1 h-7 min-w-0 text-xs bg-white dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600 focus:ring-1 focus:ring-teal-400 disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="px-2 py-1 h-7 text-xs rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 focus:ring-1 focus:ring-teal-400"
                      >
                        Reset Filter
                      </button>
                    </div>
          ...
          >
            ...
            <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2 flex-wrap">
              <button
                onClick={() => navigate('/pembayaran/import-tunggakan')}
              ...
              {selectedItems.size > 0 && (
                <>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs bg-orange-600 hover:bg-orange-700 text-white rounded transition-colors"
                    title="Buat tunggakan massal untuk santri yang dipilih"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button
                    ...
                    title="Antrian kirim WhatsApp (jeda acak di server)"
                  >
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {paginatedData.length === 0 ? (
                    <tr>
                      <td colSpan="30" className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm">
                            <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                              ket === 'Lunas' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300' :
                              ket === 'Kurang' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300' :
...
export default TunggakanDataSantri
