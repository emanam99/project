...
export default function ExportKhususOffcanvas({
  isOpen,
  onClose,
  data = [],
  getKetStatus
  ...
  const handleExport = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Data Khusus')
      const filename = `Data_Khusus_${new Date().toISOString().split('T')[0]}.xlsx`
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="flex gap-2 mb-4">
            <button
              ...
              className="text-xs px-2 py-1 rounded bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-purple-800/50"
            >
              Centang semua
            </button>
            <button
          ...
          <ul className="space-y-2">
            {EXPORT_COLUMNS.map(({ key, label }) => (
              <li key={key} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id={`export-khusus-${key}`}
                  checked={!!selected[key]}
                  onChange={() => handleToggle(key)}
                  className="rounded border-gray-300 dark:border-gray-600 text-purple-600 focus:ring-purple-500"
                />
                <label htmlFor={`export-khusus-${key}`} className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                  {label}
                </label>
              </li>
            ))}
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex flex-col gap-2">
          <button
            ...
            className="w-full inline-flex justify-center items-center gap-2 px-4 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
