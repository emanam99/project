...
function Pembayaran() {
  ...
  // Sinkronkan state dari URL hanya saat ada ?nis= di URL.
  // Jangan kosongkan currentSantri hanya karena URL tanpa nis: bisa terjadi sesaat saat debounce URL
  // atau transisi pilih santri dari offcanvas, dan memicu loop/flicker dengan BiodataBox.
  useEffect(() => {
  ...
  const handleSelectSantriFromSearch = useCallback((nisOrId) => {
    setIsSearchOpen(false)
    setCurrentSantri({ nis: nisOrId })
  }, [])

  const handleSantriChange = useCallback((santriData) => {
    ...
    if (santriData && isUwaba) {
      ...
      setBiodata({
        status_santri: santriData.status_santri || santriData.status || '',
        kategori: santriData.kategori || santriData.kategori_santri || '',
        ...
        lembaga_formal: santriData.lembaga_formal || '',
        formal_nama: santriData.formal || santriData.nama_formal || '',
