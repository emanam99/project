...
import { dedupeDomisiliSantriById } from '../../services/domisiliIndexedDb'
import { syncDomisiliAfterSantriKamarChange } from '../../services/domisiliCacheSync'
...
/** Master pelanggaran — nilai ENUM DB selaras API */
const PELANGGARAN_KATEGORI_UI = [
...
function labelKategoriPelanggaran(k) {
  const row = PELANGGARAN_KATEGORI_UI.find((x) => x.value === k)
  return row ? row.label : (k != null ? String(k) : '–')
}

function formatRombelSantriLabel(lembaga, kelas, kel) {
  const parts = [lembaga, kelas, kel].map((v) => String(v ?? '').trim()).filter(Boolean)
  return parts.length ? parts.join(' · ') : '–'
}

function formatDomisiliSantriLabel(s) {
...
function SantriSheetKonteksInfo({ santri }) {
  if (!santri) return null
  const alamat = buildAlamatGabungan(santri) || '–'
  return (
    <div className="mb-3 rounded-lg border border-gray-100 bg-gray-50/80 px-3 py-2 text-xs dark:border-gray-600 dark:bg-gray-700/40">
      <p className="font-semibold text-gray-900 dark:text-gray-100">
        {santri.nis ?? '–'} · {santri.nama || '–'}
      </p>
      <dl className="mt-2 space-y-1 text-gray-600 dark:text-gray-400">
        <div className="flex gap-2">
          <dt className="w-14 shrink-0 font-medium text-gray-500 dark:text-gray-500">Diniyah</dt>
          <dd className="min-w-0">{formatRombelSantriLabel(santri.diniyah, santri.kelas_diniyah, santri.kel_diniyah)}</dd>
        </div>
        <div className="flex gap-2">
          <dt className="w-14 shrink-0 font-medium text-gray-500 dark:text-gray-500">Formal</dt>
          <dd className="min-w-0">{formatRombelSantriLabel(santri.formal, santri.kelas_formal, santri.kel_formal)}</dd>
        </div>
        <div className="flex gap-2">
          <dt className="w-14 shrink-0 font-medium text-gray-500 dark:text-gray-500">Domisili</dt>
          <dd className="min-w-0">{formatDomisiliSantriLabel(santri)}</dd>
        </div>
        <div className="flex gap-2">
          <dt className="w-14 shrink-0 font-medium text-gray-500 dark:text-gray-500">Alamat</dt>
          <dd className="min-w-0 whitespace-pre-wrap break-words">{alamat}</dd>
        </div>
      </dl>
    </div>
  )
...
export function SantriPerKamarOffcanvas({
  ...
  const kamarAktifUntukTambah =
    variant === 'kamar' && kamar?.id != null && normalizeStatus(kamar.status) === 'aktif'
  /** Satu entri per id santri — sumber rows bisa berisi duplikat dari cache lama. */
  const rowsStable = useMemo(() => dedupeDomisiliSantriById(rows), [rows])
  ...
  const [catatanJenisBaru, setCatatanJenisBaru] = useState('putih')
  const [catatanFilterJenis, setCatatanFilterJenis] = useState('')
  const [catatanList, setCatatanList] = useState([])
  const [catatanLoading, setCatatanLoading] = useState(false)
  /** Sheet pelanggaran: 0 = pilih kategori, 1 = pilih jenis, 2 = catatan + simpan */
  const [pelanggaranStep, setPelanggaranStep] = useState(0)
  const [pelanggaranKategori, setPelanggaranKategori] = useState('')
  const [pelanggaranItemId, setPelanggaranItemId] = useState(null)
  const [pelanggaranItemNama, setPelanggaranItemNama] = useState('')
  const [pelanggaranCatatanText, setPelanggaranCatatanText] = useState('')
  const [pelanggaranMasterList, setPelanggaranMasterList] = useState([])
  const [pelanggaranMasterLoading, setPelanggaranMasterLoading] = useState(false)
  const [pelanggaranList, setPelanggaranList] = useState([])
  const [pelanggaranLoading, setPelanggaranLoading] = useState(false)
  ...
  const resetSheets = useCallback(() => {
    ...
    setCatatanText('')
    setCatatanJenisBaru('putih')
    setCatatanFilterJenis('')
    setCatatanList([])
    setCatatanLoading(false)
    setPelanggaranStep(0)
    setPelanggaranKategori('')
    setPelanggaranItemId(null)
    setPelanggaranItemNama('')
    setPelanggaranCatatanText('')
    setPelanggaranMasterList([])
    setPelanggaranMasterLoading(false)
    setPelanggaranList([])
  ...
  const loadCatatan = useCallback(async (idSantri) => {
    ...
    try {
      const res = await tarbiyahDomisiliSantriAPI.getCatatan(idSantri, {
        jenis_catatan: catatanFilterJenis === '' ? undefined : catatanFilterJenis
      })
  ...
  }, [sheetMode, actionSantri?.id, loadCatatan, catatanFilterJenis])

  useEffect(() => {
    if (sheetMode !== 'pelanggaran' || actionSantri?.id == null) return
    let cancelled = false
    ;(async () => {
      setPelanggaranMasterLoading(true)
      try {
        const res = await tarbiyahDomisiliSantriAPI.getPelanggaranMaster()
        if (!cancelled && res?.success && Array.isArray(res.data)) setPelanggaranMasterList(res.data)
        else if (!cancelled) setPelanggaranMasterList([])
      } catch {
        if (!cancelled) setPelanggaranMasterList([])
      } finally {
        if (!cancelled) setPelanggaranMasterLoading(false)
      }
    })()
    return () => {
      cancelled = true
    }
  }, [sheetMode, actionSantri?.id])

  const loadPelanggaranRiwayat = useCallback(async (idSantri) => {
    if (!idSantri) return
    setPelanggaranLoading(true)
    try {
      const res = await tarbiyahDomisiliSantriAPI.getPelanggaranSantri(idSantri)
      if (res?.success && Array.isArray(res.data)) setPelanggaranList(res.data)
      else setPelanggaranList([])
    } catch {
      setPelanggaranList([])
    } finally {
      setPelanggaranLoading(false)
  ...
  useEffect(() => {
    if (sheetMode === 'pelanggaran' && actionSantri?.id != null) {
      loadPelanggaranRiwayat(actionSantri.id)
    }
  }, [sheetMode, actionSantri?.id, loadPelanggaranRiwayat])

  const masterPelanggaranFiltered = useMemo(() => {
    if (!pelanggaranKategori) return []
    return pelanggaranMasterList.filter((m) => String(m.kategori) === pelanggaranKategori)
  }, [pelanggaranMasterList, pelanggaranKategori])
  ...
  const displayRows = useMemo(() => {
    if (filterStatusSantri === '') return rowsStable
    const want = filterStatusSantri.trim().toLowerCase()
    return rowsStable.filter((s) => String(s.status_santri ?? '').trim().toLowerCase() === want)
  }, [rowsStable, filterStatusSantri])
  ...
  const handleTambahSantriPicked = useCallback(
    async (santri) => {
      if (!kamar?.id || !santri?.id) return
      if (String(santri.id_kamar ?? '') === String(kamar.id)) {
        showNotification('Santri ini sudah terdaftar di kamar ini.', 'info')
        return
      }
      ...
      try {
        ...
        if (res?.success) {
          showNotification('Santri berhasil ditambahkan ke kamar ini.', 'success')
          await syncDomisiliAfterSantriKamarChange({
            idSantri: santri.id,
            idKamar: kamar.id,
            kamarRow: kamar
          })
          ...
          showNotification(res?.message || 'Gagal menetapkan kamar santri', 'error')
        }
      } catch (err) {
        showNotification(err?.message || 'Gagal menetapkan kamar santri', 'error')
      } finally {
        setSantriTambahSubmitting(false)
      }
    },
    [kamar?.id, showNotification, onSantriListChanged]
  )
  ...
  const handlePindahKamar = async () => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        await syncDomisiliAfterSantriKamarChange({
          idSantri: actionSantri.id,
          idKamar: pindahTargetKamar.id,
          kamarRow: pindahTargetKamar
        })
        if (typeof onSantriListChanged === 'function') await onSantriListChanged()
        closeSheet()
        onClose()
  ...
  const handleSimpanCatatan = async () => {
    ...
    try {
      const res = await tarbiyahDomisiliSantriAPI.postCatatan({
        ...
        jenis_catatan: catatanJenisBaru === 'hitam' ? 'hitam' : 'putih'
      })
      if (res?.success) {
        setCatatanText('')
        setCatatanJenisBaru('putih')
  ...
  const handleSimpanPelanggaran = async () => {
    if (!actionSantri?.id || !pelanggaranItemId) return
    setSubmitting(true)
    try {
      const res = await tarbiyahDomisiliSantriAPI.postPelanggaran({
        id_santri: actionSantri.id,
        id_pelanggaran: pelanggaranItemId,
        catatan: pelanggaranCatatanText.trim()
      })
      if (res?.success) {
        setPelanggaranCatatanText('')
        setPelanggaranItemId(null)
        setPelanggaranItemNama('')
        setPelanggaranStep(1)
        showNotification('Pelanggaran dicatat.', 'success')
        await loadPelanggaranRiwayat(actionSantri.id)
      } else {
        showNotification(res?.message || 'Gagal simpan pelanggaran', 'error')
      }
    } catch (err) {
      showNotification(err?.message || 'Gagal simpan pelanggaran', 'error')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      : bulkSheet === 'pindah'
        ...
        : sheetMode === 'menu'
          ...
          : sheetMode === 'pindah'
            ...
            : sheetMode === 'catatan'
              ? 'Catatan'
              : sheetMode === 'pelanggaran'
                ? pelanggaranStep === 0
                  ? 'Pelanggaran — pilih kategori'
                  : pelanggaranStep === 1
                    ? `Pelanggaran — ${labelKategoriPelanggaran(pelanggaranKategori)}`
                    : 'Pelanggaran — catatan'
                : ''
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 px-4 py-2 text-sm dark:border-gray-700">
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                {filterStatusSantri !== '' && displayRows.length !== rowsStable.length && (
                  <span className="text-xs text-gray-500 dark:text-gray-400">(dari {rowsStable.length})</span>
                )}
              </div>
              <div className="flex shrink-0 items-center gap-2">
                ...
                {kamarAktifUntukTambah && (
                  <button
                    type="button"
                    onClick={handleOpenTambahSantriSearch}
                    disabled={santriTambahSubmitting}
                    title="Tambah santri ke kamar ini (cari santri)"
                    className="inline-flex items-center gap-1.5 rounded-lg border border-teal-200 bg-teal-50 px-2.5 py-1.5 text-xs font-semibold text-teal-900 hover:bg-teal-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-teal-700 dark:bg-teal-900/35 dark:text-teal-100 dark:hover:bg-teal-900/50"
                  >
                    <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                ...
                <>
                  ...
                  >
                      ...
                      {showSheetBack && (
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (sheetMode === 'pelanggaran') {
                              if (pelanggaranStep === 2) {
                                setPelanggaranStep(1)
                                setPelanggaranCatatanText('')
                              } else if (pelanggaranStep === 1) {
                                setPelanggaranStep(0)
                                setPelanggaranKategori('')
                                setPelanggaranItemId(null)
                                setPelanggaranItemNama('')
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
                      {(sheetMode === 'pelanggaran' || sheetMode === 'catatan') && actionSantri ? (
                        <SantriSheetKonteksInfo santri={actionSantri} />
                      ) : (
                        ...
                        <div className="flex flex-col gap-1 pb-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setPelanggaranItemNama('')
                              setPelanggaranCatatanText('')
                              setSheetMode('pelanggaran')
                            }}
                            className="flex w-full items-center gap-3 rounded-xl border border-gray-100 bg-gray-50/80 px-3 py-3 text-left text-sm font-medium text-gray-900 hover:border-rose-200 hover:bg-rose-50/50 dark:border-gray-600 dark:bg-gray-700/40 dark:text-gray-100 dark:hover:border-rose-700"
                          >
                            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-200">
                              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                      ...
                      {sheetMode === 'pelanggaran' && (
                        <div className="space-y-3 pb-2">
                          {pelanggaranStep === 0 && (
                            <>
                              <p className="text-xs text-gray-500 dark:text-gray-400">Pilih kategori pelanggaran</p>
                              <ul className="space-y-1">
                                {PELANGGARAN_KATEGORI_UI.map((kat) => (
                                  <li key={kat.value}>
                                    <button
                                      type="button"
                                      onClick={() => {
                                        setPelanggaranKategori(kat.value)
                                        setPelanggaranStep(1)
                                      }}
                                      className="flex w-full items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-left text-sm hover:border-rose-300 dark:border-gray-600 dark:bg-gray-700/50 dark:hover:border-rose-600"
                                    >
                                      <span className="font-medium text-gray-900 dark:text-gray-100">{kat.label}</span>
                                      <svg className="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                                      </svg>
                                    </button>
                          ...
                          {pelanggaranStep === 1 && (
                            <>
                              <p className="text-xs text-gray-500 dark:text-gray-400">
                                Pilih jenis ({labelKategoriPelanggaran(pelanggaranKategori)})
                              </p>
                              {pelanggaranMasterLoading ? (
                                <p className="text-sm text-gray-500">Memuat daftar…</p>
                              ) : masterPelanggaranFiltered.length === 0 ? (
                                <p className="text-sm text-gray-500">Tidak ada jenis pelanggaran untuk kategori ini.</p>
                              ) : (
                                <ul className="max-h-44 space-y-1 overflow-y-auto">
                                  {masterPelanggaranFiltered.map((row) => (
                                    <li key={row.id}>
                                      <button
                                        type="button"
                                        onClick={() => {
                                          setPelanggaranItemId(row.id)
                                          setPelanggaranItemNama(row.nama || '')
                                          setPelanggaranStep(2)
                                        }}
                                        className="flex w-full items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2 text-left text-sm hover:border-rose-300 dark:border-gray-600 dark:bg-gray-700/50 dark:hover:border-rose-600"
                                      >
                          ...
                          {pelanggaranStep === 2 && (
                            <div>
                              <p className="mb-1 text-xs text-gray-600 dark:text-gray-400">
                                <span className="font-medium">Jenis:</span> {pelanggaranItemNama || '–'}
                              </p>
                              <label
                                htmlFor="santri-domisili-pelanggaran-catatan"
                                className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400"
                              >
                                Catatan (opsional)
                              </label>
                              <textarea
                                id="santri-domisili-pelanggaran-catatan"
                                value={pelanggaranCatatanText}
                                onChange={(e) => setPelanggaranCatatanText(e.target.value)}
                                rows={3}
                              ...
                              <button
                                type="button"
                                disabled={submitting || !pelanggaranItemId}
                                onClick={() => void handleSimpanPelanggaran()}
                          ...
                          <div>
                            ...
                            {pelanggaranLoading ? (
                              ...
                              <p className="text-sm text-gray-500">Belum ada pelanggaran tercatat.</p>
                            ) : (
                              <ul className="max-h-40 space-y-2 overflow-y-auto text-sm">
                                {pelanggaranList.map((row) => (
                                  ...
                                  >
                                    ...
                                    {row.catatan ? (
                                      <p className="mt-1 whitespace-pre-wrap text-gray-800 dark:text-gray-100">{row.catatan}</p>
                                    ) : null}
                                    <p className="mt-1 text-xs text-gray-500">
                                      {row.pengurus_nama ? `${row.pengurus_nama} · ` : ''}
                                      {row.tanggal_dibuat
                                        ? new Date(row.tanggal_dibuat).toLocaleString('id-ID', {
                                            dateStyle: 'short',
                                            timeStyle: 'short'
                                          })
                                        : ''}
                      ...
                      {sheetMode === 'catatan' && (
                        <div className="space-y-3 pb-2">
                          <div>
                            ...
                            <SantriCatatanJenisToggle
                              id="domisili-santri-catatan-jenis"
                              value={catatanJenisBaru === 'hitam' ? 'hitam' : 'putih'}
                              onChange={setCatatanJenisBaru}
                              disabled={submitting}
                            />
                            <textarea
                          ...
                          <div>
                            <p className="mb-1 text-xs font-medium text-gray-600 dark:text-gray-400">Riwayat</p>
                            <SantriCatatanJenisFilter
                              id="domisili-santri-catatan-filter"
                              value={catatanFilterJenis}
                              onChange={setCatatanFilterJenis}
                            />
                            {catatanLoading ? (
                              <p className="text-sm text-gray-500">Memuat…</p>
      ...
      {variant === 'kamar' &&
        createPortal(
          <SearchOffcanvas
            isOpen={Boolean(santriTambahSearchOpen && open && kamar?.id)}
