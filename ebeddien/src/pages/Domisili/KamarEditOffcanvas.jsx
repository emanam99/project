...
/** Urutan tampilan kategori (selaras Kamar / SantriPerKamarOffcanvas). */
...
export function KamarEditOffcanvas({
  ...
  /** Filter daerah: wajib pilih kategori dulu (tambah & ubah). */
  ...
  useEffect(() => {
    if (!open) return
    if (editingKamar) {
      const drow = daerahList.find((d) => String(d.id) === String(editingKamar.id_daerah))
      const kat =
        drow != null
          ? String(drow.kategori ?? '').trim()
          : String(editingKamar.daerah_kategori ?? '').trim()
      ...
      if (defaultDaerahId) {
        const drow = daerahList.find((d) => String(d.id) === String(defaultDaerahId))
        setKategoriFilter(drow != null ? String(drow.kategori ?? '').trim() : '')
      } else {
        setKategoriFilter('')
      }
      setFormData({
  ...
  }, [open, editingKamar, defaultDaerahId, daerahList])
  ...
  const kategoriOptions = useMemo(() => {
    ...
    const ordered = KATEGORI_ORDER.filter((k) => set.has(k))
    const extras = [...set].filter((k) => !KATEGORI_ORDER.includes(k)).sort((a, b) => a.localeCompare(b, 'id'))
    let result = [...ordered, ...extras]
    if (kategoriFilter && !result.includes(kategoriFilter)) {
      result = [kategoriFilter, ...result]
    }
    return result
  }, [daerahList, kategoriFilter])

  const daerahFiltered = useMemo(() => {
    if (!kategoriFilter) return []
    return daerahList
      .filter((d) => String(d.kategori ?? '').trim() === kategoriFilter)
      .slice()
      .sort((a, b) =>
        String(a.daerah ?? '').localeCompare(String(b.daerah ?? ''), 'id', { sensitivity: 'base', numeric: true })
      )
  }, [daerahList, kategoriFilter])
  ...
  const handleKategoriChange = (value) => {
    ...
    setFormData((prev) => {
      const cur = daerahList.find((d) => String(d.id) === String(prev.id_daerah))
      if (cur && String(cur.kategori ?? '').trim() !== value) {
        return { ...prev, id_daerah: '' }
      }
      return prev
    })
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!kategoriFilter) {
      showNotification('Kategori wajib dipilih dulu', 'error')
      return
    }
    if (!formData.id_daerah) {
      showNotification('Daerah wajib dipilih', 'error')
      return
    }
            ...
            <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
              <div className="flex-1 space-y-4 overflow-y-auto p-4">
                <div>
                  ...
                  <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">Pilih kategori dulu, lalu pilih daerah.</p>
                  {kategoriOptions.length === 0 ? (
                    <p className="text-sm text-amber-700 dark:text-amber-300">Belum ada data daerah dengan kategori.</p>
                  ) : (
                    <ul className="space-y-1">
                      {kategoriOptions.map((kat) => {
                        const selected = kategoriFilter === kat
                        return (
                          <li key={kat}>
                            <button
                              type="button"
                              ...
                              className={`flex w-full items-center justify-between rounded-lg border px-3 py-2.5 text-left text-sm transition-colors ${
                                selected
                                  ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-400/50 dark:border-teal-500 dark:bg-teal-900/30 dark:ring-teal-500/40'
                            ...
                            >
                              ...
                              {selected ? (
                                <svg className="h-4 w-4 shrink-0 text-teal-600 dark:text-teal-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-transparent focus:ring-2 focus:ring-teal-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                  >
                    <option value="">{kategoriFilter ? 'Pilih daerah' : 'Pilih kategori terlebih dahulu'}</option>
                    {daerahFiltered.map((d) => (
                      <option key={d.id} value={d.id}>
                        {d.daerah || `Daerah #${d.id}`}
                      </option>
                    ))}
                  </select>
                </div>
