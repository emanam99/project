...
import { manageUsersAPI, madrasahAPI } from '../../services/api'
import api from '../../services/api'
import MadrasahKoordinatorOffcanvas from './components/MadrasahKoordinatorOffcanvas'
...
const KoordinatorListItem = memo(({
  pengurus,
  index,
  onClick,
  onMadrasahBadgeClick,
  madrasahCount = 0,
  getStatusBadgeColor,
...
function Koordinator() {
  ...
  const [madrasahOffcanvasKoordinator, setMadrasahOffcanvasKoordinator] = useState(null)
  ...
  const loadMadrasah = useCallback(async () => {
    try {
      const res = await madrasahAPI.getAll()
      if (res?.success && Array.isArray(res.data)) {
        setAllMadrasah(res.data)
      } else {
        setAllMadrasah([])
      }
    } catch (err) {
  ...
  const madrasahCountByKoordinatorId = useMemo(() => {
    const map = new Map()
    for (const m of allMadrasah) {
  ...
  const getMadrasahCountForPengurus = useCallback(
    (p) => {
      if (!p) return 0
      const byId = madrasahCountByKoordinatorId.get(String(p.id)) ?? 0
      if (byId > 0) return byId
      if (p.nip != null && p.nip !== '') {
        return madrasahCountByKoordinatorId.get(String(p.nip)) ?? 0
      }
      return 0
    },
    [madrasahCountByKoordinatorId]
  )
  ...
  const handleMadrasahBadgeClick = useCallback((pengurus) => {
    setMadrasahOffcanvasKoordinator(pengurus)
  }, [])

  const handleMadrasahAssignmentChange = useCallback(() => {
    loadMadrasah()
  }, [loadMadrasah])
  ...
  return (
    <div className="h-full overflow-hidden" style={{ minHeight: 0 }}>
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                    {paginatedKoordinator.map((pengurus, index) => (
                      <KoordinatorListItem
                        ...
                        onMadrasahBadgeClick={handleMadrasahBadgeClick}
                        madrasahCount={getMadrasahCountForPengurus(pengurus)}
      ...
      {createPortal(
        <MadrasahKoordinatorOffcanvas
          isOpen={madrasahOffcanvasKoordinator != null}
          onClose={() => setMadrasahOffcanvasKoordinator(null)}
          koordinator={madrasahOffcanvasKoordinator}
          allMadrasahList={allMadrasah}
          allKoordinatorList={allKoordinator}
          onMadrasahAssignmentChange={handleMadrasahAssignmentChange}
        />,
        document.body
      )}
    </div>
