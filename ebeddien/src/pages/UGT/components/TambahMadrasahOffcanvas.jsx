...
import { KEGIATAN_WAKTU_SLOTS, kegiatanJamFieldsFromMadrasah } from '../kegiatanWaktuConfig'
import KegiatanBelajarFields from './KegiatanBelajarFields'
import TingkatanFields from './TingkatanFields'
import { tingkatanSlugsFromMadrasah } from '../tingkatanConfig'
...
function madrasahToForm(m) {
  ...
  return {
    ...
    ...kegiatanJamFieldsFromMadrasah(m),
    tempat: m.tempat ?? '',
...
const TambahMadrasahOffcanvas = forwardRef(function TambahMadrasahOffcanvas({ isOpen, onClose, onSuccess, onOpenCariKoordinator, initialData, koordinatorLocked = false, currentUserId, currentUserNip }, ref) {
  ...
  const loginUsername = (
    initialData?.login_username
    || initialData?.pjgt_nama
    || ''
  ).trim()
  const loginUserId = initialData?.login_user_id ?? initialData?.id_pjgt ?? null
  ...
  const handleKegiatanFlagChange = (flagKey, checked) => {
    setForm((prev) => {
      const next = { ...prev, [flagKey]: checked }
      if (!checked) {
        const slot = KEGIATAN_WAKTU_SLOTS.find((s) => s.flag === flagKey)
        if (slot) {
          next[slot.mulai] = ''
          next[slot.sampai] = ''
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        kegiatan_pagi_mulai: form.kegiatan_pagi ? (form.kegiatan_pagi_mulai || null) : null,
        kegiatan_pagi_sampai: form.kegiatan_pagi ? (form.kegiatan_pagi_sampai || null) : null,
        kegiatan_sore_mulai: form.kegiatan_sore ? (form.kegiatan_sore_mulai || null) : null,
        kegiatan_sore_sampai: form.kegiatan_sore ? (form.kegiatan_sore_sampai || null) : null,
        kegiatan_malam_mulai: form.kegiatan_malam ? (form.kegiatan_malam_mulai || null) : null,
        kegiatan_malam_sampai: form.kegiatan_malam ? (form.kegiatan_malam_sampai || null) : null,
        ...
        tingkatan: form.tingkatan?.length ? form.tingkatan : [],
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 bg-black/50 z-[100020]"
            onClick={onClose}
            aria-hidden="true"
          />
          <motion.div
            ...
            exit={{ x: '100%' }}
            transition={{ type: 'tween', duration: 0.25 }}
            className="fixed top-0 right-0 bottom-0 w-full max-w-lg bg-white dark:bg-gray-800 shadow-xl z-[100021] flex flex-col"
          >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {isEdit && (
              <div className="rounded-lg border border-teal-200 dark:border-teal-800 bg-teal-50/80 dark:bg-teal-900/20 px-3 py-2.5">
                <p className="text-xs font-medium text-teal-800 dark:text-teal-300 mb-0.5">Username login</p>
                <UsernameLinkButton
                  userId={loginUserId}
                  username={loginUsername}
                  stackBaseZIndex={100050}
                  emptyLabel="Belum ada akun login"
                  className="text-sm font-semibold text-teal-800 dark:text-teal-200 hover:underline break-all text-left"
                />
              </div>
            )}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Identitas madrasah</label>
              <input
                type="text"
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-3">
              ...
              {form.id_pjgt && (
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 flex flex-wrap items-center gap-1">
                  <span>Username:</span>
                  <UsernameLinkButton
                    userId={loginUserId}
                    username={loginUsername}
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-3">
              ...
              <KegiatanBelajarFields form={form} onFlagChange={handleKegiatanFlagChange} onJamChange={handleChange} />
              <div className="mt-2">
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-3">
              ...
              <TingkatanFields
                selected={form.tingkatan}
                onChange={(slugs) => handleChange('tingkatan', slugs)}
              />
              <div className="mt-2">
