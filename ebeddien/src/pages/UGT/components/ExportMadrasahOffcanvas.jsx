...
} from '../exportMadrasahConfig'
import { tingkatanLabelsFromSlugs, tingkatanSlugsFromMadrasah } from '../tingkatanConfig'
import { useNotification } from '../../../contexts/NotificationContext'

/** Normalisasi item dari API (JOIN pakai pengasuh_nama, pjgt_nama) ke key yang dipakai export */
function normalizeItem(item) {
  return {
    ...item,
    nama_pengasuh: item.nama_pengasuh ?? item.pengasuh_nama,
    no_pengasuh: item.no_pengasuh ?? item.pengasuh_wa,
    nama_pjgt: item.nama_pjgt ?? item.pjgt_nama,
    no_pjgt: item.no_pjgt ?? item.pjgt_wa
  }
}

export default function ExportMadrasahOffcanvas({ isOpen, onClose, filteredData = [] }) {
  ...
  const handleExport = () => {
    ...
    const rows = filteredData.map((item) => {
      ...
      activeColumns.forEach(({ key, label }) => {
        let v = base[key]
        if (key === 'tingkatan') {
          v = tingkatanLabelsFromSlugs(tingkatanSlugsFromMadrasah(base))
        }
        out[label] = v === null || v === undefined || v === '' ? '' : (typeof v === 'number' ? v : String(v))
      })
      return out
    })

    try {
      const ws = XLSX.utils.json_to_sheet(rows.length ? rows : [activeColumns.reduce((o, { label }) => ({ ...o, [label]: '' }), {})])
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, 'Madrasah')
      const filename = `madrasah_export_${new Date().toISOString().split('T')[0]}.xlsx`
          ...
          <ul className="space-y-2">
            {EXPORT_COLUMNS.map(({ key, label }) => (
              <li key={key} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id={`export-madrasah-${key}`}
                  checked={!!selected[key]}
                  onChange={() => handleToggle(key)}
                  className="rounded border-gray-300 dark:border-gray-600 text-teal-600 focus:ring-teal-500"
                />
                <label htmlFor={`export-madrasah-${key}`} className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                  {label}
                </label>
              </li>
            ))}
