...
import { madrasahAPI, ugtLaporanPjgtAPI } from '../../services/api'
...
const appendPjgtSearchParts = (row) => [row.ubudiyah]

export default function LaporanPJGTPage() {
  ...
  const fetchAll = useCallback(() => ugtLaporanPjgtAPI.getAll({}), [])
  ...
  }, [editParam, baruParam, scopeRows, loadingScope, setSearchParams, canTambahPjgt])

  const isOffcanvasOpen = Boolean((baruParam === '1' && canTambahPjgt) || (editParam && editingRow))
  ...
  const emptyHintTambah = canTambahPjgt ? ' Klik "Tambah laporan" untuk mengisi.' : ''
  const tableEmptyMessage = loadingScope && scopeRows.length === 0
    ? 'Memuat...'
    : scopeRows.length === 0
      ? `Belum ada laporan PJGT.${emptyHintTambah}`
      : hasActiveFilters
                ...
                <tbody>
                  {displayRows.map((row) => (
                    <tr
                      ...
                      aria-label={`${canTambahPjgt ? 'Buka ubah' : 'Buka detail'} laporan PJGT: ${row.madrasah_nama || ''} — ${row.santri_nama || ''}`}
