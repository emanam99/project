...
import { madrasahAPI, pengurusAPI, madrasahEditPengajuanAPI } from '../../services/api'
import { useNotification } from '../../contexts/NotificationContext'
import { useAuthStore } from '../../store/authStore'
import { useMadrasahDataFiturAccess } from '../../hooks/useMadrasahDataFiturAccess'
import { userCanTambahGtTugasan, userCanHapusGtTugasan } from '../../utils/ugtGuruTugasanAccess'
...
import TambahGuruTugasMadrasahOffcanvas from './components/TambahGuruTugasMadrasahOffcanvas'
import MadrasahEditPengajuanOffcanvas from './components/MadrasahEditPengajuanOffcanvas'
...
function DataMadrasah() {
  ...
  const { koordinatorFilterLocked, hasDataMadrasahAccess, canPengajuanEdit } = useMadrasahDataFiturAccess()
  const canTambahGtMadrasah = useMemo(
    () => userCanTambahGtTugasan(fiturMenuCodes, user, hasDataMadrasahAccess),
    [fiturMenuCodes, user, hasDataMadrasahAccess]
  )
  const canHapusGtMadrasah = useMemo(
    () => userCanHapusGtTugasan(fiturMenuCodes, user, hasDataMadrasahAccess),
    [fiturMenuCodes, user, hasDataMadrasahAccess]
  )
  const canKelolaGtOffcanvas = canTambahGtMadrasah || canHapusGtMadrasah
  ...
  const [showGtMadrasahOffcanvas, setShowGtMadrasahOffcanvas] = useState(false)
  const [gtMadrasahTarget, setGtMadrasahTarget] = useState(null)
  const [viewMode, setViewMode] = useState('grid')
  const [selectedItems, setSelectedItems] = useState(() => new Set())
  const [editingMadrasah, setEditingMadrasah] = useState(null)
  const [pengajuanByMadrasahId, setPengajuanByMadrasahId] = useState(() => new Map())
  const [showPengajuanOffcanvas, setShowPengajuanOffcanvas] = useState(false)
  const [activePengajuanId, setActivePengajuanId] = useState(null)
  ...
  const openTambahGuruTugas = useCallback(
    (m, e) => {
      e?.stopPropagation?.()
      if (!m?.id) return
      // Tutup offcanvas edit/detail madrasah agar tidak bertumpuk di bawah GT (z-index) — klik «X» harus menutup lapisan atas.
      ...
      setGtMadrasahTarget(m)
      setShowGtMadrasahOffcanvas(true)
    },
    [setSearchParams]
  )

  const closeGtMadrasahOffcanvas = useCallback(() => {
    setShowGtMadrasahOffcanvas(false)
    setGtMadrasahTarget(null)
  }, [])
  ...
  const loadPengajuanMenunggu = useCallback(async () => {
    if (!canPengajuanEdit) {
      setPengajuanByMadrasahId(new Map())
      return
    }
    try {
      const res = await madrasahEditPengajuanAPI.getAll({ status: 'menunggu' })
      const map = new Map()
      if (res?.success && Array.isArray(res.data)) {
        for (const row of res.data) {
          const mid = Number(row.id_madrasah)
          if (mid > 0 && !map.has(mid)) map.set(mid, row)
        }
      }
      setPengajuanByMadrasahId(map)
    } catch {
      setPengajuanByMadrasahId(new Map())
    }
  }, [canPengajuanEdit])

  const openPengajuanEdit = useCallback((m, e) => {
    e?.stopPropagation?.()
    const row = pengajuanByMadrasahId.get(Number(m?.id))
    if (!row?.id) return
    setShowTambahOffcanvas(false)
    setEditingMadrasah(null)
    setActivePengajuanId(row.id)
    setShowPengajuanOffcanvas(true)
  }, [pengajuanByMadrasahId])

  const refreshMadrasahAndPengajuan = useCallback(() => {
    loadMadrasah()
    loadPengajuanMenunggu()
  }, [loadPengajuanMenunggu])
    ...
    if (q) {
      result = result.filter((m) => {
        const fields = [
          ...
          m.kategori,
          m.status,
          m.nama_pengasuh ?? m.pengasuh_nama,
          m.nama_pjgt ?? m.pjgt_nama,
          m.identitas
        ]
  ...
  return (
    <div className="h-full overflow-hidden flex flex-col">
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 dark:bg-gray-900">
                    <tr>
                      ...
                      {canKelolaGtOffcanvas && (
                        <th className="px-2 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-11" title="Kelola Guru Tugas">
                          <span className="sr-only">Guru tugas</span>
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {paginatedList.map((m, index) => (
                      <tr
                        key={m.id}
                        onClick={(e) => {
                          if (e.target.type === 'checkbox' || e.target.closest?.('[data-gt-plus]') || e.target.closest?.('[data-pengajuan-edit]')) return
                          openEdit(m)
                        }}
                      ...
                      >
                        ...
                        {canKelolaGtOffcanvas && (
                          <td className="px-2 py-2 text-center align-middle" onClick={(e) => e.stopPropagation()}>
                            <button
                              type="button"
                              data-gt-plus
                              onClick={(e) => openTambahGuruTugas(m, e)}
                              className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-teal-500/60 text-teal-600 dark:text-teal-400 bg-white dark:bg-gray-800 hover:bg-teal-50 dark:hover:bg-teal-900/30 text-sm font-semibold"
                              title="Kelola Guru Tugas di madrasah ini"
                              aria-label="Kelola Guru Tugas"
                            >
                              +
                            </button>
                          </td>
                        ...
                        <td className="px-3 py-2 font-medium text-gray-900 dark:text-gray-100">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span>{m.nama || '-'}</span>
                            {canPengajuanEdit && pengajuanByMadrasahId.has(Number(m.id)) ? (
                              <button
                                type="button"
                                data-pengajuan-edit
                                onClick={(e) => openPengajuanEdit(m, e)}
                                className="inline-flex items-center rounded-full bg-amber-500/90 px-2 py-0.5 text-[10px] font-semibold text-white hover:bg-amber-600"
                                title="Tinjau pengajuan edit PJGT"
                              >
                                Pengajuan edit
                              </button>
                            ) : null}
              ...
              <AnimatePresence>
                {paginatedList.map((m, index) => (
                  <motion.div
                    ...
                    onClick={(e) => {
                      if (e.target.closest('[data-checkbox]') || e.target.closest('[data-gt-plus]') || e.target.closest('[data-pengajuan-edit]')) return
                      openEdit(m)
                    }}
                  ...
                  >
                    <div className="absolute left-2 top-2 z-20 flex flex-col items-start gap-1.5 max-w-[min(100%,calc(100%-3rem))] pointer-events-none">
                      {canPengajuanEdit && pengajuanByMadrasahId.has(Number(m.id)) && (
                        <button
                          type="button"
                          data-pengajuan-edit
                          className="pointer-events-auto inline-flex items-center rounded-full bg-amber-500 px-2 py-0.5 text-[10px] font-semibold text-white shadow-sm hover:bg-amber-600"
                          onClick={(e) => openPengajuanEdit(m, e)}
                          title="Tinjau pengajuan edit PJGT"
                        >
                          Pengajuan edit
                      ...
                      {canKelolaGtOffcanvas && (
                        ...
                        >
                          <button
                            type="button"
                            onClick={(e) => openTambahGuruTugas(m, e)}
                            className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-teal-500/60 text-teal-600 dark:text-teal-400 bg-white/95 dark:bg-gray-800/95 hover:bg-teal-50 dark:hover:bg-teal-900/40 text-sm font-semibold shadow-sm"
                            title="Kelola Guru Tugas"
                            aria-label="Kelola Guru Tugas"
                          >
                            +
                          </button>
                      ...
                      {m.status && (
                        <span
                          className={`pointer-events-none px-1.5 py-0.5 text-[10px] font-medium rounded shadow truncate max-w-full ${getStatusBadgeClass(m.status)}`}
                          title={m.status}
                        >
                          {m.status}
                        </span>
      ...
      <TambahMadrasahOffcanvas
        ...
        onSuccess={refreshMadrasahAndPengajuan}
      ...
      />
      <MadrasahEditPengajuanOffcanvas
        isOpen={showPengajuanOffcanvas}
        onClose={() => {
          setShowPengajuanOffcanvas(false)
          setActivePengajuanId(null)
        }}
        pengajuanId={activePengajuanId}
        onSuccess={refreshMadrasahAndPengajuan}
      />
      ...
      <TambahGuruTugasMadrasahOffcanvas
        isOpen={showGtMadrasahOffcanvas}
        onClose={closeGtMadrasahOffcanvas}
        madrasah={gtMadrasahTarget}
        canTambahTugasan={canTambahGtMadrasah}
        canHapusTugasan={canHapusGtMadrasah}
      />
