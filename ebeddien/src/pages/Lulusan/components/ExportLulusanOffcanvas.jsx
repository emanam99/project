...
} from '../exportLulusanConfig'
import { useNotification } from '../../../contexts/NotificationContext'

export default function ExportLulusanOffcanvas({ isOpen, onClose, filteredData = [] }) {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const filename = `Data_Lulusan_${new Date().toISOString().split('T')[0]}.xlsx`
          ...
          <ul className="space-y-2">
            {EXPORT_COLUMNS.map(({ key, label }) => (
              <li key={key} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id={`export-lulusan-${key}`}
                  checked={!!selected[key]}
                  onChange={() => handleToggle(key)}
                  className="rounded border-gray-300 dark:border-gray-600 text-teal-600 focus:ring-teal-500"
                />
                <label htmlFor={`export-lulusan-${key}`} className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                  {label}
                </label>
              </li>
            ))}
