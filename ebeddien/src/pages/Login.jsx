...
import { useAuthStore, EBEDDIEN_PASSKEY_PROMPT_FLAG } from '../store/authStore'
...
export function LoginFormCard() {
  ...
  /** Bersihkan sesi akun sebelumnya + hard redirect agar profil laptop bersama tidak nyangkut. */
  const finishLoginRedirect = async (token, user, refreshToken, {
    usernameForStore,
    ...
    if (usernameForStore) setStoredLoginUsername(usernameForStore)
    if (showPasskeyPrompt) {
      try {
        sessionStorage.setItem(EBEDDIEN_PASSKEY_PROMPT_FLAG, '1')
      } catch (_) { /* noop */ }
    }
    const nextPath = await resolvePostLoginRedirect(redirectUrl)
    window.location.assign(nextPath || '/beranda')
  }
    ...
    try {
      ...
      if (response.success) {
        ...
        await finishLoginRedirect(
          ...
          { usernameForStore: u, redirectUrl: response.data.redirect_url }
        )
        return
      } else {
    ...
    try {
      ...
      if (response.success) {
        ...
        await finishLoginRedirect(
          ...
          response.data.refresh_token ?? null,
          {
            usernameForStore: username.trim(),
            showPasskeyPrompt: !!response.data?.show_passkey_prompt,
