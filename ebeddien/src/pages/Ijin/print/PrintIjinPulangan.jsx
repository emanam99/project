...
function PrintIjinPulangan({ santriId, inOffcanvas = false }) {
  ...
  /** Data URL — untuk cetak/offcanvas gambar TTD ikut ter-render */
  const [ttdRaImgSrc, setTtdRaImgSrc] = useState(() => getGambarUrl('/ttd/ttd-ra-ali.png'))
  ...
  useEffect(() => {
    let cancelled = false
    const url = getGambarUrl('/ttd/ttd-ra-ali.png')
    let sameOrigin = true
    try {
    ...
    fetch(url, sameOrigin ? { credentials: 'same-origin' } : { mode: 'cors', credentials: 'omit' })
      .then(r => {
        if (!r.ok) throw new Error('ttd fetch')
        return r.blob()
      })
      .then(
        blob =>
      ...
      .then(dataUrl => {
        if (!cancelled && typeof dataUrl === 'string') setTtdRaImgSrc(dataUrl)
      })
      .catch(() => {
        /* tetap pakai URL dari state awal */
      })
    return () => {
      cancelled = true
    }
          ...
          <div className="ttd-section">
            ...
            <div className="ttd-right">
              ...
              <div className="ttd-image-container">
                <img src={ttdRaImgSrc} alt="" className="ttd-image" loading="eager" decoding="sync" />
              </div>
