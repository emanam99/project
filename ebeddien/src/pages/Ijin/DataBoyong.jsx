...
import { boyongAPI, kalenderAPI } from '../../services/api'
...
function DataBoyong() {
  ...
  const [santriResolving, setSantriResolving] = useState(false)
  const nisResolveAbortRef = useRef(0)
  ...
  useEffect(() => {
    if (showForm && form.id_santri && fullSantriList.length > 0) {
      const s = fullSantriList.find((x) => x.id === form.id_santri)
      if (s) setSelectedSantriDetail(s)
    }
  }, [showForm, form.id_santri, fullSantriList])
  ...
  const applySantriToForm = useCallback((santri, { preserveDiniyahFormal = false } = {}) => {
    if (!santri?.id) return
    const nisDisplay = String(santri.nis ?? santri.id).padStart(7, '0')
    setForm((f) => ({
      ...f,
      id_santri: santri.id,
      nisInput: nisDisplay,
      selectedSantriNama: santri.nama || '-',
      ...(preserveDiniyahFormal
        ? {}
        : {
            diniyah:
              f.diniyah ||
  ...
  const lookupSantriByNis = useCallback(
    (nisOrId) => {
      const raw = nisOrId ? String(nisOrId).trim().replace(/\D/g, '') : ''
      if (!raw) return null
      const padded = raw.length <= 7 ? raw.padStart(7, '0') : raw
      return (
        fullSantriList.find(
          (x) =>
            String(x.nis || '').padStart(7, '0') === padded ||
            String(x.id ?? '').padStart(7, '0') === padded
        ) || null
      )
    },
    [fullSantriList]
  ...
  const resolveSantriByNis = useCallback(
    async (nisOrId) => {
      const s = String(nisOrId ?? '')
        .trim()
        .replace(/\D/g, '')
        .slice(0, 7)
      if (!/^\d{7}$/.test(s)) return

      const local = lookupSantriByNis(s)
      if (local) applySantriToForm(local, { preserveDiniyahFormal: true })

      const token = ++nisResolveAbortRef.current
      setSantriResolving(true)
      try {
        const r = await loadSantriBiodataWithCache(s, {
          onCache: (data) => {
            if (token !== nisResolveAbortRef.current) return
            applySantriToForm(data, { preserveDiniyahFormal: !!local })
          }
        })
        if (token !== nisResolveAbortRef.current) return
        if (r.success && r.data) {
          applySantriToForm(r.data, { preserveDiniyahFormal: !!local })
        } else if (!local) {
          setForm((f) => ({ ...f, id_santri: '', selectedSantriNama: '' }))
          setSelectedSantriDetail(null)
        }
      } finally {
        if (token === nisResolveAbortRef.current) setSantriResolving(false)
      }
    },
    [applySantriToForm, lookupSantriByNis]
  )

  const handleSelectSantriFromSearch = useCallback(
    (santri) => {
      applySantriToForm(santri)
      setIsSearchOpen(false)
      const nis = String(santri.nis ?? santri.id ?? '').padStart(7, '0')
      if (/^\d{7}$/.test(nis)) {
        void resolveSantriByNis(nis)
      }
    },
    [applySantriToForm, resolveSantriByNis]
  )

  const handleNisBlur = () => {
    void resolveSantriByNis(form.nisInput)
  }
  ...
  const openEdit = (row) => {
    setEditingId(row.id)
    const detail = fullSantriList.find((s) => s.id === row.id_santri) || null
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      {/* Form Offcanvas — portal ke body: sibling nav pakai z-[100], kolom main z-10; fixed di dalam halaman tidak bisa menang atas nav */}
      {createPortal(
        <AnimatePresence>
          {showForm && (
            <>
              ...
              <motion.div
                ...
                className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-white dark:bg-gray-800 shadow-xl z-[103] overflow-y-auto pb-[env(safe-area-inset-bottom,0px)]"
              >
                  ...
                  <div className="flex gap-1.5 items-center justify-between">
                    <div className="flex gap-1.5 items-center">
                      ...
                      <input
                        ...
                        value={form.nisInput}
                        onChange={(e) => {
                          const nisInput = e.target.value.replace(/\D/g, '').slice(0, 7)
                          setForm((f) => ({
                            ...f,
                            nisInput,
                            id_santri: '',
                          ...
                          setSelectedSantriDetail(null)
                          if (/^\d{7}$/.test(nisInput)) {
                            void resolveSantriByNis(nisInput)
                          }
                        }}
                        ...
                        className={`w-20 min-w-[4.5rem] max-w-[5rem] p-1.5 text-sm border-2 rounded-lg focus:outline-none focus:ring-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-center ${
                          form.nisInput && !form.id_santri && form.nisInput.length === 7 && !santriResolving
      ...
      {createPortal(
        <SearchOffcanvas
          ...
          onSelectSantriRecord={handleSelectSantriFromSearch}
          zIndex={130}
        />,
        document.body
      )}
      ...
      {/* Delete confirm — portal ke body agar di atas bottom nav */}
      {createPortal(
        <AnimatePresence>
          {deleteConfirm && (
            <>
              ...
              <motion.div
                ...
                className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-white dark:bg-gray-800 rounded-xl shadow-xl z-[111] p-4"
              >
