...
function BulkEditOffcanvas({ isOpen, onClose, selectedSantriList, allDataSantri, onSuccess }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    pendaftaranAPI.getKamarOptions(idDaerahForKamar).then((res) => {
      if (cancelled) return
      const list = (res?.success && Array.isArray(res.data)) ? res.data : []
      setKamarOptions(list)
    }).catch(() => { if (!cancelled) setKamarOptions([]) })
    return () => { cancelled = true }
  }, [idDaerahForKamar, isOpen])

  const isRombelField = selectedField === 'diniyah' || selectedField === 'formal'
  const isKamarField = selectedField === 'kamar'
  ...
  const availableValues = useMemo(() => {
    ...
    return values.sort()
  }, [selectedField, allDataSantri, isRombelField, isKamarField])
  ...
  const handleSubmit = async (e) => {
    ...
    if ((fieldName === 'id_diniyah' || fieldName === 'id_formal' || fieldName === 'id_kamar') && valueToUpdate != null) {
    ...
    try {
      for (let i = 0; i < selectedSantriList.length; i++) {
        ...
        if (fieldName === 'status_santri') {
          const kat = String(santri.kategori ?? '').trim()
          if (kat) updateData.kategori = kat
        }
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    {isKamarField ? (
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              setIdDaerahForKamar(e.target.value)
                              setSelectedValue('')
                            }}
                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                          ...
                          >
                            ...
                            {(daerahOptions || []).map((d) => (
                              <option key={d.id} value={d.id}>{d.daerah || d.nama || d.id}</option>
                            ))}
                          </select>
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Kamar</label>
                          <select
                            value={selectedValue}
                            onChange={(e) => setSelectedValue(e.target.value)}
                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                            disabled={loading || !idDaerahForKamar}
                            required={isKamarField}
                          >
                      ...
                      <select
                        value={selectedValue}
                        onChange={(e) => setSelectedValue(e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-teal-500"
                      ...
                      >
                        ...
                        {(rombelOptions || []).map((r) => (
                          <option key={r.id} value={r.id}>
                            {[r.lembaga_nama || r.nama, r.kelas, r.kel].filter(Boolean).join(' · ') || r.id}
                          </option>
                        ))}
                      </select>
                    ) : (
                      <>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          />
                          <label htmlFor="bulk-edit-manual-input" className="text-sm text-gray-700 dark:text-gray-300 select-none">
                            Input manual
                          </label>
                        </div>
                        {useManualInput ? (
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={loading || !selectedField || (isKamarField ? !selectedValue : selectedValue === undefined)}
