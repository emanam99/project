...
function PrintDataTable({ data, selectedColumns, filters, fontSize = 9 }) {
  ...
  const generateFilterUrl = () => {
    ...
    if (filters.ijinMinFilter) params.set('ijin_min', filters.ijinMinFilter)
    if (filters.ijinMaxFilter) params.set('ijin_max', filters.ijinMaxFilter)
  ...
  const getFilterText = () => {
    ...
    if (filters.ijinMinFilter || filters.ijinMaxFilter) {
      activeFilters.push(
        `Ijin (TA): ${filters.ijinMinFilter || '…'}–${filters.ijinMaxFilter || '…'}`
      )
    }
  ...
  const getCellValue = (santri, key) => {
    switch (key) {
      ...
      case 'ket':
        ...
        return 'belum'
      case 'ijin_count':
        return String(santri.ijin_count ?? 0)
      default:
        return santri[key] || '-'
    }
  }
