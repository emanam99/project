...
import {
  PRINT_IJIN_PAPER_OPTIONS,
  getPrintIjinPaperSpec,
  normalizePrintIjinPaperSize,
  readPrintIjinPaperSize,
  writePrintIjinPaperSize,
} from '../print/printIjinPaper'
...
function PrintIjinOffcanvas({ isOpen, onClose, santriId, ijinId }) {
  ...
  const [paperSize, setPaperSize] = useState(() => readPrintIjinPaperSize())
  ...
  useEffect(() => {
    if (isOpen && santriId) {
      ...
      let url = `${baseUrl}/print-ijin?id_santri=${encodeURIComponent(id)}`
      if (ijinId) {
        url += `&ijin_id=${ijinId}`
      }
      if (tahunAjaran) {
      ...
      url += `&paper=${normalizePrintIjinPaperSize(paperSize)}`
      url += `&_t=${Date.now()}`
      setPrintUrl(url)
    }
  }, [isOpen, santriId, ijinId, tahunAjaran, paperSize])

  useEffect(() => {
    if (!isOpen) return
    setPageMarginMm({ ...DEFAULT_PRINT_IJIN_MARGIN_MM })
    setPaperSize(readPrintIjinPaperSize())
  ...
  // @page size mengikuti pilihan kertas (A4 / F4 landscape)
  useEffect(() => {
    if (!isOpen) return undefined
    const spec = getPrintIjinPaperSpec(paperSize)
    const style = document.createElement('style')
    style.id = 'print-ijin-paper-size'
    style.textContent = `
      @page {
    ...
    return () => {
      style.remove()
      const leftover = document.getElementById('print-ijin-paper-size')
      if (leftover) leftover.remove()
    }
  }, [isOpen, paperSize])

  const handlePaperChange = (next) => {
    const size = normalizePrintIjinPaperSize(next)
    setPaperSize(size)
    writePrintIjinPaperSize(size)
  }
              ...
              {ribbonExpanded ? (
                  ...
                  <div className="bg-white dark:bg-gray-800 border-t border-gray-300 dark:border-gray-600 px-2 sm:px-3 py-2 min-h-[88px] overflow-x-auto">
                    <div className="flex flex-wrap gap-x-4 gap-y-2 items-end">
                      <div className="flex flex-col gap-0.5">
                        ...
                        >
                          {PRINT_IJIN_PAPER_OPTIONS.map((opt) => {
                            const active = paperSize === opt.id
                            return (
                              <button
                                key={opt.id}
                                type="button"
                                onClick={() => handlePaperChange(opt.id)}
                                aria-pressed={active}
                                title={`${opt.label} landscape (${opt.hint})`}
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-2 leading-snug">
                      Kertas {paperSpec.label} landscape ({paperSpec.hint}). Margin per salinan (kiri &amp; kanan):
                      atas {marginSafe.top} · bawah {marginSafe.bottom} · kiri {marginSafe.left} · kanan{' '}
                      {marginSafe.right} mm. Saat print, pilih ukuran kertas yang sama di dialog printer.
                    </p>
                  </div>
                </>
              ) : (
                <div className="flex items-center justify-between gap-2 px-2 py-1.5">
                  <p className="text-xs text-gray-600 dark:text-gray-400 truncate">
                    {paperSpec.label} · margin {marginSafe.top}/{marginSafe.bottom}/{marginSafe.left}/
                    {marginSafe.right} mm
                  </p>
                  <button
                    type="button"
