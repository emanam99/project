...
export const useRencanaModals = (loadAllRencana, onCloseOffcanvas, sendNotifications, allRencana) => {
  ...
  const [showNotifAdminAccordion, setShowNotifAdminAccordion] = useState(false)
  ...
  const openConfirmModal = useCallback((action, id) => {
    ...
    setConfirmId(id)
    setCatatanKeputusan('')
    setShowNotifAdminAccordion(false)
  ...
  const handleConfirmAction = useCallback(async (rencanaDetail, confirmSelectedAdmins, confirmListAdmins) => {
    ...
    try {
      ...
      const catatanTrim = catatanKeputusan.trim()
      const bodyExtra = catatanTrim ? { catatan: catatanTrim } : {}
      const bodyWithRecipients = recipients.length ? { recipients, ...bodyExtra } : { ...bodyExtra }

      let response
      if (confirmAction === 'approve') {
        response = await pengeluaranAPI.approveRencana(confirmId, bodyWithRecipients)
        if (response.success) {
        ...
        response = await pengeluaranAPI.rejectRencana(confirmId, bodyWithRecipients)
        if (response.success) {
  ...
  }, [confirmId, confirmAction, catatanKeputusan, allRencana, loadAllRencana, onCloseOffcanvas, sendNotifications, showNotification, closeConfirmModal])
  ...
  const handleConfirmApproveReject = useCallback(async (rencanaDetail, confirmRencanaSelectedAdmins, rencanaListAdmins) => {
    ...
    try {
      ...
      if (pendingAction === 'approve') {
        response = await pengeluaranAPI.approveRencana(rencanaDetail.id, bodyWithRecipients)
        if (response.success) {
          showNotification('Rencana berhasil di-approve', 'success')
          const wa = response.data?.wa_notif
          if (wa && !wa.queued && !wa.async) {
        ...
        response = await pengeluaranAPI.rejectRencana(rencanaDetail.id, bodyWithRecipients)
        if (response.success) {
          showNotification('Rencana berhasil ditolak', 'success')
          const wa = response.data?.wa_notif
          if (wa && !wa.queued && !wa.async) {
  ...
  }, [pendingAction, catatanKeputusan, loadAllRencana, onCloseOffcanvas, sendNotifications, showNotification, closeApproveRejectModal])
  ...
  return {
    ...
    showPreviewPesan,
    setShowPreviewPesan,
    showNotifAdminAccordion,
    setShowNotifAdminAccordion,
    catatanKeputusan,
    setCatatanKeputusan,
    openConfirmModal,
    closeConfirmModal,
