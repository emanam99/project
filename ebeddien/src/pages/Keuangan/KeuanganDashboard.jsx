...
function KeuanganDashboard() {
  ...
  const tahunInitializedRef = useRef(false)
  ...
  const isViewingActiveTahun = Boolean(
    activeTahunAjaran && tahunAjaran && tahunAjaran === activeTahunAjaran
  )
  ...
  }, [tahunAjaran, hijriyahMasterRows])
  ...
  const loadData = async () => {
    try {
      ...
      const pemasukanTA = filterTransaksiByTahunAjaranRentang(pemasukanData, hijriyahMasterRows, tahunAjaran)
      const pengeluaranTA = filterTransaksiByTahunAjaranRentang(pengeluaranData, hijriyahMasterRows, tahunAjaran)

      const rentangTa = getHijriyahRowByTahunAjaran(hijriyahMasterRows, tahunAjaran)
      const saldoAwal = rentangTa
        ? computeSaldoAwalBeforeRentang(pemasukanData, pengeluaranData, rentangTa.dari)
        : 0
        ...
        <ScrollAnimation>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              ...
              {tahunAjaran && (
                <div ref={tahunPickerRef} className="relative mt-1 inline-block">
                  <button
                    type="button"
                    onClick={() => setTahunPickerOpen((open) => !open)}
                    className="inline-flex items-center gap-1 text-xs font-medium text-teal-700 dark:text-teal-300 rounded-md px-1.5 py-0.5 -ml-1.5 hover:bg-teal-50 dark:hover:bg-teal-900/30 transition-colors"
                    aria-expanded={tahunPickerOpen}
                    aria-haspopup="listbox"
                    aria-label="Pilih tahun ajaran hijriyah"
                  >
                    <span>
                      {isViewingActiveTahun ? 'Tahun Ajaran Aktif' : 'Tahun Ajaran'}:{' '}
                  ...
                  {tahunPickerOpen && (
                    <div
                      ...
                      className="absolute left-0 top-full z-30 mt-1 min-w-[9rem] max-h-56 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-lg py-1"
                    >
                  ...
                  {!isViewingActiveTahun && activeTahunAjaran ? (
                    <p className="mt-0.5 text-[10px] text-amber-700 dark:text-amber-300">
                      Menampilkan data tahun ajaran sebelumnya. Aktif: {activeTahunAjaran}
                    </p>
                  ) : null}
                </div>
              )}
