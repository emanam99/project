...
import { pengeluaranAPI, userAPI, lembagaAPI, tahunAjaranAPI } from '../../services/api'
import { useAuthStore } from '../../store/authStore'
import { usePengeluaranFiturAccess } from '../../hooks/usePengeluaranFiturAccess'
import { useTahunAjaranStore } from '../../store/tahunAjaranStore'
import { resolveActiveHijriyahTahunAjaranFromRows } from '../../utils/tahunAjaranActive'
...
import { ensureTypedBlob, isImageMime, isPdfMime, preparePreviewFromBlob } from '../../utils/filePreviewMedia'
...
function EditRencana({
  ...
  /** Hanya dipakai saat simpan draft + punya akses toggle (selaras API kirim_notifikasi_draft). */
  const [kirimNotifikasiDraft, setKirimNotifikasiDraft] = useState(true)
  const fileInputRef = useRef(null)
  const userTouchedTahunAjaranRef = useRef(false)
  ...
  const showDraftNotifToggle = useMemo(
    () =>
      pengeluaranFitur.rencanaToggleDraftNotifOnSave &&
      allowSaveAsDraft &&
      (isCreateMode || rencana?.ket === 'draft'),
    [
      pengeluaranFitur.rencanaToggleDraftNotifOnSave,
      allowSaveAsDraft,
      isCreateMode,
  ...
  const resolveDefaultTahunAjaranHijriyah = useCallback(async () => {
    let rows = hijriyahMasterRows
    if (!rows?.length) {
      try {
        const res = await tahunAjaranAPI.getAll({ kategori: 'hijriyah' })
        rows = res?.success && Array.isArray(res.data) ? res.data : []
      } catch {
        rows = []
      }
    }
    return resolveActiveHijriyahTahunAjaranFromRows(rows) || ''
  }, [hijriyahMasterRows])

  const loadHijriyahAndTahunAjaran = async () => {
    try {
      const { hijriyah } = await getTanggalFromAPI()
      const tahunAjaranDefault = await resolveDefaultTahunAjaranHijriyah()
      setFormData((prev) => ({
        ...prev,
        hijriyah: hijriyah || '',
        tahun_ajaran: tahunAjaranDefault
      }))
    } catch (error) {
  ...
  useEffect(() => {
    if (!isCreateMode) {
      userTouchedTahunAjaranRef.current = false
      return
    }
    userTouchedTahunAjaranRef.current = false
  }, [isCreateMode, id])
  ...
  useEffect(() => {
    if (!isCreateMode || userTouchedTahunAjaranRef.current) return
    let cancelled = false
    ;(async () => {
      const ta = await resolveDefaultTahunAjaranHijriyah()
      if (cancelled || !ta) return
      setFormData((prev) => (prev.tahun_ajaran === ta ? prev : { ...prev, tahun_ajaran: ta }))
    })()
    return () => {
      cancelled = true
    }
  }, [isCreateMode, hijriyahMasterRows, resolveDefaultTahunAjaranHijriyah])
  ...
  /** Siapkan daftar penerima WA untuk body create/update — backend yang mengirim (async). */
  const buildWaRecipientsForSave = () => {
    ...
    return listAdmins
      .filter((admin) => adminIds.includes(admin.id) && admin.whatsapp)
    ...
    try {
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          tahun_ajaran: data.tahun_ajaran || (await resolveDefaultTahunAjaranHijriyah()) || '',
  ...
  const handleDownloadFile = async (fileId, fileName) => {
    try {
      ...
      const { blob: typed } = ensureTypedBlob(blob, fileMeta?.tipe_file, fileName)
  ...
  const handlePreviewFile = async (file) => {
    try {
      ...
      const { url, mime } = await preparePreviewFromBlob(blob, file.tipe_file, file.nama_file)
      setPreviewFileUrl(url)
      setPreviewMime(mime)
    } catch (err) {
      console.error('Error loading file for preview:', err)
      showNotification(err?.message || 'Gagal memuat file untuk preview', 'error')
      setPreviewFile(null)
      setPreviewFileUrl(null)
      setPreviewMime('')
    } finally {
  ...
  const handleSubmit = async (e, isDraft = false) => {
    ...
    try {
      setSaving(true)
      const currentUserId = user?.id_pengurus ?? user?.id ?? user?.user_id
      
      const waRecipients = !isDraft ? buildWaRecipientsForSave() : []
      const payload = {
        ...
        ...(isDraft && pengeluaranFitur.rencanaToggleDraftNotifOnSave
          ? { kirim_notifikasi_draft: kirimNotifikasiDraft }
          : {}),
        ...(waRecipients.length > 0 ? { recipients: waRecipients } : {}),
      ...
      <div className="flex-1 overflow-y-auto" style={{ minHeight: 0 }}>
        <div className="p-4 sm:p-6 lg:p-8 pb-24 sm:pb-6 lg:pb-8">
          ...
          >
              ...
              <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      userTouchedTahunAjaranRef.current = true
                      setFormData((prev) => ({ ...prev, tahun_ajaran: e.target.value }))
                    }}
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200"
                    placeholder="Otomatis dari TA hijriyah aktif"
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Default: tahun ajaran hijriyah yang rentangnya mencakup hari ini (master Pengaturan → Tahun Ajaran)
                  </p>
                </div>
              </div>
            ...
            {showDraftNotifToggle ? (
              <label className="mt-6 flex items-start gap-3 cursor-pointer select-none rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/80 px-4 py-3">
                <input
                  type="checkbox"
                  className="mt-0.5 h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500 dark:border-gray-600 dark:bg-gray-700"
                  checked={kirimNotifikasiDraft}
                  onChange={(e) => setKirimNotifikasiDraft(e.target.checked)}
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  <span className="font-medium text-gray-900 dark:text-gray-100">
                    Kirim notifikasi saat simpan draft
                  </span>
                  <span className="block text-xs text-gray-500 dark:text-gray-400 mt-1">
                    WhatsApp/Push ke penerima sesuai kebijakan lembaga. Matikan jika ingin menyimpan draft tanpa mengganggu admin.
                  </span>
                </span>
              </label>
            ) : null}
                ...
                <div className="flex-1 overflow-hidden bg-gray-100 dark:bg-gray-900">
                  {loadingPreview ? (
                    ...
                    <div className="h-full w-full overflow-auto">
                      {isImageMime(previewMime, previewFile.nama_file) ? (
                        <img
                          src={previewFileUrl}
                          ...
                          style={{ maxHeight: '100%' }}
                        />
                      ) : isPdfMime(previewMime, previewFile.nama_file) ? (
                        <>
                          <iframe
                            src={previewFileUrl}
