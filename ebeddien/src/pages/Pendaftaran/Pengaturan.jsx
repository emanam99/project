...
import { cachePsbDefaultTahunAjaran } from '../../utils/pendaftaranTahunAjaranDefault'
...
function Pengaturan() {
  ...
  const loadPengaturan = async () => {
    ...
    try {
      ...
      cachePsbDefaultTahunAjaran(settings.tahun_hijriyah, settings.tahun_masehi)
  ...
  // Sort kategori: Tahun Ajaran dulu, lalu Gelombang, lalu Pendaftaran, lalu lainnya
          ...
          {sortedKategori.length === 0 ? (
            ...
            <div className="space-y-6">
              {sortedKategori.map((kategori) => {
                // Skip Pendaftaran (di-render terpisah di bawah catatan)
                if (kategori === 'Pendaftaran' || kategori === 'Payment Gateway') {
                  return null
                }
              ...
              >
                <div className="rounded-2xl bg-sky-50/80 dark:bg-sky-950/30 p-4 sm:p-5 border border-sky-200/80 dark:border-sky-800/50 shadow-sm">
                  ...
                  <ul className="text-xs sm:text-sm text-sky-700 dark:text-sky-400 space-y-2 list-none">
                    ...
                    <li className="flex items-start gap-2">
                      <span className="text-sky-500 mt-0.5 flex-shrink-0">•</span>
                      <span>Payment Gateway dipindah ke menu Pengaturan → Payment Gateway</span>
                    </li>
                  </ul>
                </div>
              </motion.div>
              
              {/* Pendaftaran (Email Verifikasi Demo dll) - tanpa judul */}
