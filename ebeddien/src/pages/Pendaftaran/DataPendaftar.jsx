...
const PENDAFTAR_DETAIL_OFFCANVAS_STATE = Object.freeze({ ebOffcanvas: 'data_pendaftar_detail' })

function formatTanggalLahirPendaftar(raw) {
  const s = String(raw || '').trim()
  if (!s || s === '0000-00-00' || s === '-') return ''
  try {
...
function formatTempatTanggalLahirPendaftar(p) {
  if (!p) return '-'
  const tempat = String(p.tempat_lahir || '').trim()
  const tgl = formatTanggalLahirPendaftar(p.tanggal_lahir)
  const parts = [tempat, tgl].filter(Boolean)
  return parts.length ? parts.join(', ') : '-'
}

function rowPendaftarIsLunas(p) {
  const w = p.wajib != null ? Number(p.wajib) : 0
  const b = p.bayar != null ? Number(p.bayar) : 0
  return w > 0 && b >= w
}

function rowPendaftarIsBelumLunas(p) {
  const w = p.wajib != null ? Number(p.wajib) : 0
  const b = p.bayar != null ? Number(p.bayar) : 0
  return w > 0 && b < w
}
...
/** Di luar body DataPendaftar agar identitas komponen stabil (bukan nested component per render). */
function PendaftarSortIcon({ columnKey, sortKey, direction }) {
...
function DataPendaftar() {
  ...
  /** Filter cepat tabel: '' | 'lunas' | 'belum_lunas' (hanya yang punya tagihan wajib > 0) */
  const [paymentQuickFilter, setPaymentQuickFilter] = useState('')
  ...
  const [riwayatChatMeta, setRiwayatChatMeta] = useState({ nomor: '', idSantri: '', namaSantri: '', phoneField: 'no_wa_santri' })
  ...
  useEffect(() => {
    ...
    setWaStatusTelpon(null)
    setWaStatusWaSantri(null)
    const idSantri = selectedPendaftar.id || selectedPendaftar.id_santri
    const noTelponRaw = (selectedPendaftar.no_telpon || '').trim()
    const noWaRaw = (selectedPendaftar.no_wa_santri || '').trim()
    const noTelponDigits = noTelponRaw.replace(/\D/g, '')
    const noWaDigits = noWaRaw.replace(/\D/g, '')
    const telponMasked = noTelponRaw.includes('*')
    const waMasked = noWaRaw.includes('*')
    if (telponMasked ? idSantri : noTelponDigits.length >= 10) {
      checkPhoneNumberTelpon(selectedPendaftar.no_telpon, { no_telpon: selectedPendaftar.no_telpon }, { id_santri: idSantri })
    }
    if (waMasked ? idSantri : noWaDigits.length >= 10) {
      checkPhoneNumberWaSantri(selectedPendaftar.no_wa_santri, { no_wa_santri: selectedPendaftar.no_wa_santri }, { id_santri: idSantri })
    }
  ...
  useEffect(() => {
    ...
    if (found?.id != null) {
      setKamarAktif(String(found.id))
      return
    }
    const opt = kelOptionsAktif.find((item) => {
      const k = item != null && typeof item === 'object' ? String(item.kel ?? '') : String(item ?? '')
      return k === String(kelAktif)
    })
    const optId = opt != null && typeof opt === 'object' && opt.id != null ? String(opt.id) : ''
    setKamarAktif(optId)
  }, [showAktifOffcanvas, aktifMode, lembagaAktif, kelasAktif, kelAktif, rombelMasterAktif, kelOptionsAktif])
  ...
  useEffect(() => {
    if (!tableActionsMenuOpen) return
    const onDoc = (e) => {
      const t = e.target
      if (!(t instanceof Node)) return
      if (tableActionsMenuRef.current?.contains(t)) return
  ...
  const resetPendaftarFilters = useCallback(() => {
    ...
    setStatusMuridFilter('')
    setPaymentQuickFilter('')
    setSearchQuery('')
    setCurrentPage(1)
  }, [])
  ...
  useEffect(() => {
    ...
    if (paymentQuickFilter === 'lunas') {
      filtered = filtered.filter(rowPendaftarIsLunas)
    } else if (paymentQuickFilter === 'belum_lunas') {
      filtered = filtered.filter(rowPendaftarIsBelumLunas)
    }
  ...
  }, [searchQuery, pendaftarList, statusPendaftarFilter, formalFilter, diniyahFilter, keteranganStatusFilter, gelombangFilter, statusSantriFilter, statusMuridFilter, sortConfig, isSuperAdmin, applyScopedLembagaToRows, paymentQuickFilter, hasFullAccess])

  const handleQuickExcel = useCallback(async (mode) => {
    ...
    exportPendaftarRowsToXlsx(
      rows,
      {
        ...
        filenameTag: mode || 'sesuai_filter'
      },
      showNotification
    )
  }, [selectedItems, filteredList, tahunAjaran, tahunAjaranMasehi, showNotification])
  ...
  const closeDetailOffcanvasWithBack = useOffcanvasBackClose(
    isDetailOffcanvasOpen && Boolean(selectedPendaftar),
    handleCloseDetailOffcanvas,
    { state: PENDAFTAR_DETAIL_OFFCANVAS_STATE }
  )

  const handleEditClick = () => {
    if (!selectedPendaftar?.id) return
    const h = String(tahunAjaran || '').trim()
    const m = String(tahunAjaranMasehi || '').trim()
    if (!h || !m) {
      showNotification('Pilih tahun ajaran hijriyah dan masehi terlebih dahulu.', 'warning')
      return
    }
    ...
    params.set('nis', nisForUrl)
    params.set('tahun_hijriyah', h)
    params.set('tahun_masehi', m)
    params.set('tab', 'biodata')
    params.set('scroll', 'nama')
    // Jangan pakai closeDetailOffcanvasWithBack() — history.back() membatalkan navigate (tetap di Data Pendaftar).
    setIsDetailOffcanvasOpen(false)
    setSelectedPendaftar(null)
    navigate(`/pendaftaran?${params.toString()}`, { replace: true })
  }
      ...
      <div className="h-full overflow-y-auto page-content-scroll" style={{ minHeight: 0 }}>
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          >
            ...
            />
            {/* Search & Filter — sticky seperti page Data Santri */}
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-2 pt-3 mt-2 border-t border-gray-200 dark:border-gray-600">
                          <button
                            ...
                            title="Segarkan dari server"
                          >
                            <svg className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                            </svg>
                            Segarkan dari server
                          </button>
                          <button
                            type="button"
                            onClick={resetPendaftarFilters}
            ...
            <p className="mb-3 text-xs text-gray-600 dark:text-gray-400">
              Ringkasan pembayaran, breakdown lembaga, dan cek duplikasi:{' '}
              <Link to="/pendaftaran/analisis" className="font-medium text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300">
                buka halaman Analisis
              </Link>
              .
            </p>

            {/* List Pendaftar — ikut scroll halaman (satu kolom seperti Data Santri); scroll horizontal hanya pada area tabel */}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
              <div className="px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div ref={tableActionsMenuRef} className="relative">
                    <button
                      type="button"
                      onClick={() => {
                        setTableActionsMenuOpen((o) => !o)
                      }}
                      className={`inline-flex items-center gap-2 rounded-xl border px-3.5 py-2 text-sm font-medium transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-gray-900 ${
                        tableActionsMenuOpen
                          ? 'border-teal-300/80 bg-teal-50/90 text-teal-900 shadow-md ring-1 ring-teal-500/15 dark:border-teal-700/60 dark:bg-teal-950/35 dark:text-teal-100 dark:ring-teal-400/20'
                          : 'border-gray-200/90 bg-gradient-to-b from-white to-gray-50/90 text-gray-700 shadow-sm ring-1 ring-black/[0.04] hover:border-gray-300 hover:to-gray-50 hover:shadow dark:border-gray-600 dark:from-gray-800 dark:to-gray-800/95 dark:text-gray-200 dark:ring-white/[0.06] dark:hover:border-gray-500 dark:hover:to-gray-800'
                      }`}
                      aria-expanded={tableActionsMenuOpen}
                      aria-haspopup="true"
                      title="Menu"
                    >
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gray-900/[0.06] text-gray-600 dark:bg-white/10 dark:text-gray-300">
                      ...
                      <svg
                        className={`h-4 w-4 shrink-0 text-gray-500 transition-transform duration-200 dark:text-gray-400 ${tableActionsMenuOpen ? 'rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                        aria-hidden
                    ...
                    {tableActionsMenuOpen ? (
                      <div
                        className="absolute left-0 top-full z-50 mt-2 w-[min(100vw-2rem,17.5rem)] max-h-[min(75vh,440px)] overflow-y-auto toolbar-scrollbar rounded-2xl border border-gray-200/90 bg-white/95 py-2 shadow-xl shadow-gray-900/10 ring-1 ring-black/[0.06] backdrop-blur-sm dark:border-gray-600/90 dark:bg-gray-900/95 dark:shadow-black/40 dark:ring-white/[0.08]"
                        role="menu"
                      >
                        {selectedItems.size > 0 ? (
                          <p className="mx-3 mb-2 rounded-md bg-teal-50 px-2 py-1.5 text-[11px] text-teal-900 dark:bg-teal-950/40 dark:text-teal-100">
                            Ekspor memakai {selectedItems.size} baris tercentang (jika ada).
                          </p>
                        ) : null}
                        <p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Ekspor</p>
                        <button
                          type="button"
                          role="menuitem"
                          className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-gray-800 hover:bg-gray-50 dark:text-gray-100 dark:hover:bg-gray-700/80"
                          onClick={() => {
                            setIsExportOffcanvasOpen(true)
                            setTableActionsMenuOpen(false)
                          }}
                        >
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-green-600 text-white">
                            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
                          </span>
                          <span>
                            <span className="font-medium block">Eksport Excel…</span>
                            <span className="text-[11px] text-gray-500 dark:text-gray-400">Pilih kolom di offcanvas</span>
                          </span>
                        </button>
                        <button
                          type="button"
                          role="menuitem"
                          className="w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/80"
                          onClick={() => {
                            handleQuickExcel('')
                            setTableActionsMenuOpen(false)
                          }}
                        >
                          Excel cepat (sesuai tampilan)
                        </button>
                        <button
                          type="button"
                          role="menuitem"
                        ...
                        <p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Filter pembayaran (tabel)</p>
                        <button
                          type="button"
                          role="menuitem"
                          className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm ${paymentQuickFilter === '' ? 'bg-gray-100 font-medium dark:bg-gray-700/80' : 'text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/80'}`}
                          onClick={() => {
                            setPaymentQuickFilter('')
                            setTableActionsMenuOpen(false)
                          }}
                        >
                          Semua
                          {paymentQuickFilter === '' ? <span className="text-teal-600 dark:text-teal-400">✓</span> : null}
                        </button>
                        <button
                          type="button"
                          role="menuitem"
                          className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm ${paymentQuickFilter === 'lunas' ? 'bg-emerald-50 font-medium dark:bg-emerald-950/40' : 'text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/80'}`}
                          onClick={() => {
                            setPaymentQuickFilter('lunas')
                            setTableActionsMenuOpen(false)
                          }}
                        >
                          Lunas
                          {paymentQuickFilter === 'lunas' ? <span className="text-teal-600 dark:text-teal-400">✓</span> : null}
                        </button>
                        <button
                          type="button"
                          role="menuitem"
                          className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm ${paymentQuickFilter === 'belum_lunas' ? 'bg-amber-50 font-medium dark:bg-amber-950/40' : 'text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/80'}`}
                          onClick={() => {
                            setPaymentQuickFilter('belum_lunas')
                            setTableActionsMenuOpen(false)
                          }}
                        >
                          Belum lunas
                          {paymentQuickFilter === 'belum_lunas' ? <span className="text-teal-600 dark:text-teal-400">✓</span> : null}
                        </button>

                        <div className="my-2 border-t border-gray-100 dark:border-gray-700" />
                        <p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Baris per halaman</p>
                        <div className="px-3 pb-1">
                          <select
                            ...
                            className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-2 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-teal-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
                          >
                        ...
                        {dataPendaftarEdit && selectedItems.size > 0 ? (
                          <>
                            ...
                            <button
                              type="button"
                              role="menuitem"
                              className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm font-medium text-teal-800 hover:bg-teal-50 dark:text-teal-200 dark:hover:bg-teal-950/40"
                              onClick={() => {
                        ...
                        {selectedItems.size > 0 ? (
                          <button
                            type="button"
                            role="menuitem"
                            className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/80"
                            onClick={() => {
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                    {paymentQuickFilter === 'lunas' ? (
                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 font-medium text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-200">Filter: lunas</span>
                    ) : null}
                    {paymentQuickFilter === 'belum_lunas' ? (
                      <span className="rounded-full bg-amber-100 px-2 py-0.5 font-medium text-amber-900 dark:bg-amber-900/40 dark:text-amber-100">Filter: blm lunas</span>
                    ) : null}
                    <span className="tabular-nums">
                      {filteredList.length.toLocaleString('id-ID')} baris tampil
                      ...
                      <tr>
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Status
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="keterangan_status" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Nama
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="nama" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Pendaftar
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="status_pendaftar" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Status Santri
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="status_santri" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Formal
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="daftar_formal" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Status Murid
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="status_murid" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Diniyah
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="daftar_diniyah" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Gel
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="gelombang" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Wajib
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="wajib" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Bayar
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="bayar" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Prodi
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="prodi" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            NIK
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="nik" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            NIS
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="id" />
                          </div>
                        </th>
                        <th
                        ...
                        >
                          <div className="flex items-center gap-2">
                            Tahun Ajaran
                            <PendaftarSortIcon sortKey={sortConfig.key} direction={sortConfig.direction} columnKey="tahun_hijriyah" />
                          </div>
                        </th>
                      </tr>
                    </thead>
            ...
            {createPortal(
              <>
                  ...
                  {isDetailOffcanvasOpen && selectedPendaftar && (
                    <>
                      <motion.div
                        ...
                        onClick={closeDetailOffcanvasWithBack}
                        aria-hidden="true"
                      />
                      <motion.div
                        initial={{ x: '100%' }}
                      ...
                      >
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                          ...
                          <div>
                            <dt className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Tempat, tanggal lahir</dt>
                            <dd className="mt-0.5 text-sm text-gray-700 dark:text-gray-300">
                              {formatTempatTanggalLahirPendaftar(selectedPendaftar)}
                            </dd>
                          </div>
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => checkPhoneNumberTelpon(
                                  selectedPendaftar.no_telpon,
                                  { no_telpon: selectedPendaftar.no_telpon },
                                  { id_santri: selectedPendaftar.id || selectedPendaftar.id_santri }
                                )}
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setRiwayatChatMeta({
                                      ...
                                      idSantri: String(selectedPendaftar.id ?? selectedPendaftar.id_santri ?? ''),
                                      namaSantri: String(selectedPendaftar.nama ?? ''),
                                      phoneField: 'no_telpon',
                                    })
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => checkPhoneNumberWaSantri(
                                  selectedPendaftar.no_wa_santri,
                                  { no_wa_santri: selectedPendaftar.no_wa_santri },
                                  { id_santri: selectedPendaftar.id || selectedPendaftar.id_santri }
                                )}
                              ...
                              {lembagaAktif && kelasAktif && (
                                <>
                                  ...
                                  {kelOptionsAktif.length === 0 ? (
                                    ...
                                    <ul className="max-h-56 space-y-1 overflow-y-auto">
                                      {kelOptionsAktif.map((kelItem, idx) => {
                                        const kelVal = kelItem != null && typeof kelItem === 'object'
                                          ? String(kelItem.kel ?? '')
                                          : String(kelItem ?? '')
                                        const rombelOptId = kelItem != null && typeof kelItem === 'object' && kelItem.id != null
                                          ? String(kelItem.id)
                                          : ''
                                        ...
                                        return (
                                          <li key={rombelOptId || `${kelVal}-${idx}`}>
                                            <button
                                              ...
                                              onClick={() => {
                                                setKelAktif(kelVal)
                                                if (rombelOptId) setKamarAktif(rombelOptId)
                                              }}
                                              className={`flex w-full items-center justify-between rounded-lg border px-3 py-2.5 text-left text-sm transition-colors ${
                                                selected
                                                  ? 'border-emerald-400 bg-emerald-50 text-emerald-800 dark:border-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-200'
                ...
                <RiwayatChatOffcanvas
                  ...
                  phoneField={riwayatChatMeta.phoneField || 'no_wa_santri'}
                />
              </>,
              document.body
            )}
