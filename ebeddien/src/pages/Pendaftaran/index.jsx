...
import PendaftaranPageToolbar from './components/PendaftaranPageToolbar'
...
function Pendaftaran() {
  ...
  const [bioToolbar, setBioToolbar] = useState(null)
  const [payToolbar, setPayToolbar] = useState(null)
  const urlTahunHijriyah = searchParams.get('tahun_hijriyah') || ''
  const urlTahunMasehi = searchParams.get('tahun_masehi') || ''
  const scrollToNama = searchParams.get('scroll') === 'nama'

  const {
    selectedHijriyah: pageTahunHijriyah,
    setSelectedHijriyah: setPageTahunHijriyah,
    selectedMasehi: pageTahunMasehi,
    setSelectedMasehi: setPageTahunMasehi,
    hijriyahOptions: pageHijriyahOptions,
    masehiOptions: pageMasehiOptions,
    psbDefaultReady
  } = usePageTahunAjaranFilter({
    urlHijriyah: urlTahunHijriyah,
    urlMasehi: urlTahunMasehi,
    defaultFromPengaturan: true
  })

  // Pertahankan tahun ajaran halaman di URL (mis. dari Data Pendaftar → Edit)
  // Tunggu default pengaturan PSB selesai agar tidak menulis tahun ajaran header/rentang ke URL.
  useEffect(() => {
    if (!psbDefaultReady || isUpdatingUrlRef.current) return
    const h = String(pageTahunHijriyah || '').trim()
    const m = String(pageTahunMasehi || '').trim()
    if (!h || !m) return
    if (searchParams.get('tahun_hijriyah') === h && searchParams.get('tahun_masehi') === m) return
    const next = new URLSearchParams(searchParams)
    next.set('tahun_hijriyah', h)
    next.set('tahun_masehi', m)
    setSearchParams(next, { replace: true })
  }, [pageTahunHijriyah, pageTahunMasehi, searchParams, setSearchParams, psbDefaultReady])
  ...
  return (
    <div className="p-2 sm:p-3 h-full min-h-0 overflow-hidden flex flex-col">
      ...
      >
        <PendaftaranPageToolbar
          bioToolbar={bioToolbar}
          payToolbar={payToolbar}
          tahunHijriyah={pageTahunHijriyah}
          tahunMasehi={pageTahunMasehi}
          onTahunHijriyahChange={setPageTahunHijriyah}
          onTahunMasehiChange={setPageTahunMasehi}
          hijriyahOptions={pageHijriyahOptions}
          masehiOptions={pageMasehiOptions}
        />

        {/* Tab HP/Tablet: di bawah toolbar (ikon + tahun ajaran) */}
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:grid-rows-1 gap-6 flex-1 min-h-0 overflow-hidden">
          ...
          >
            <BiodataPendaftaran
              ...
              hijriyahOptions={pageHijriyahOptions}
              masehiOptions={pageMasehiOptions}
              scrollToNamaOnLoad={scrollToNama}
              onOpenSearch={() => setIsSearchOpen(true)}
              hideExternalToolbar
              onToolbarState={setBioToolbar}
              hideBerkasSection
          ...
          <div ref={rightColumnRef} className="col-span-1 flex flex-col min-h-0 overflow-hidden">
            ...
            >
              <PembayaranBox
                ...
                onToolbarState={setPayToolbar}
                onTahunAjaranChanged={(h, m) => {
                  setPageTahunHijriyah(h)
                  setPageTahunMasehi(m)
                }}
              />
            </div>
          </div>
