...
function DashboardPendaftaran() {
  ...
  useEffect(() => {
    loadDashboardData()
  }, [tahunAjaran, tahunAjaranMasehi, lembagaFilter, statusPendaftarFilter])
  ...
  const loadDashboardData = async () => {
    ...
    try {
      const lembaga = lembagaFilter || null
      const statusPendaftar = statusPendaftarFilter || null
      ...
      const [dashboardResult, pendaftarResult] = await Promise.all([
        pendaftaranAPI.getDashboard(tahunAjaran, tahunAjaranMasehi, lembaga, statusPendaftar),
        pendaftaranAPI.getLastPendaftar(tahunAjaran, tahunAjaranMasehi, lembaga, statusPendaftar)
      ])
    ...
    <div className="h-full overflow-hidden" style={{ minHeight: 0 }}>
      <div className="h-full overflow-y-auto" style={{ minHeight: 0 }}>
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          >
            {/* Tahun ajaran + filter lembaga / status pendaftar */}
            <motion.div
              className="mb-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-end"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              />
              <div className="flex flex-col gap-1 min-w-[min(100%,200px)] sm:ml-auto">
              ...
              <div className="flex flex-col gap-1 min-w-[min(100%,200px)]">
                <label htmlFor="dashboard-psb-status-pendaftar" className="text-xs font-medium text-gray-600 dark:text-gray-400">
                  Status pendaftar
                </label>
                <select
                  id="dashboard-psb-status-pendaftar"
