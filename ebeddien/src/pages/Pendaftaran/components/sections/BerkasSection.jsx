...
function BerkasSection({
  ...
  jenisBerkasOptions: jenisBerkasOptionsProp = null,
  /** 'santri' = ID 7 digit (NIS lokal pendaftaran); selain itu = ID numerik PK (santri.id / mahrom) */
  localIdKind = 'santri',
  /** Opsional: buka kamera scan langsung (jenisBerkas) — tampil di mode standalone */
  onScanCamera = null,
}) {
  ...
  const jenisBerkasOptions = jenisBerkasOptionsProp ?? jenisBerkasOptionsBase
  ...
  const isValidLocalId =
    localIdKind === 'santri'
      ? Boolean(localId && /^\d{7}$/.test(String(localId).trim()))
      : Boolean(localId && Number(localId) > 0)
                      ...
                      {existingBerkas ? (
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg bg-teal-600 hover:bg-teal-700 text-white transition-colors"
                            title="Upload file"
                          >
                          ...
                          {typeof onScanCamera === 'function' && (
                            <button
                              type="button"
                              onClick={() => onScanCamera(jenisBerkas)}
                              className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg border border-teal-600 text-teal-700 dark:text-teal-300 hover:bg-teal-50 dark:hover:bg-teal-900/30 transition-colors"
                              title="Scan kamera"
                            >
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
