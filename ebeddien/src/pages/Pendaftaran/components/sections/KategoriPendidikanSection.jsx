 ...
 * @param {Array<{id, tingkatan, kelompok}>} props.lttqTingkatanOptions - Opsi tingkatan LTTQ (master)
 */
function KategoriPendidikanSection({
  ...
  return (
    <div ref={sectionRef} className="mt-16 pt-8 border-t-4 border-teal-600 dark:border-teal-400">
      ...
      {/* LTTQ */}
      <div className="mb-4">
        <label className={getLabelClassName('id_lttq_tingkatan')}>
          Tingkatan LTTQ
        </label>
        <select
          value={formData.id_lttq_tingkatan != null && formData.id_lttq_tingkatan !== '' ? String(formData.id_lttq_tingkatan) : ''}
          onChange={(e) => onFieldChange('id_lttq_tingkatan', e.target.value)}
          onFocus={() => onFocus('id_lttq_tingkatan')}
          onBlur={onBlur}
          className="w-full p-2 border-b-2 border-gray-300 dark:border-gray-600 focus:border-teal-500 dark:focus:border-teal-400 focus:outline-none bg-transparent text-gray-900 dark:text-gray-100"
        >
