  ...
  const itemsByCategory = useMemo(() => {
    ...
    Object.keys(acc).forEach((cat) => {
      acc[cat].sort((a, b) => {
        ...
        return String(a.nama_item || '').localeCompare(String(b.nama_item || ''), 'id')
      })
    })
    return acc
  }, [filteredList])
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {sortedCategoryKeys.map((category) => (
                    <div key={category} className="space-y-0 overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between gap-2 bg-teal-50 px-3 py-2 dark:bg-teal-900/20">
                      ...
                      {itemsByCategory[category].map((item, idx) => {
                        ...
                        const isLast = idx === itemsByCategory[category].length - 1
                        return (
                          <div
                            key={item.id}
                            onClick={() => handleAddItem(item)}
                            className={`flex cursor-pointer items-center justify-between gap-2 p-3 transition-colors hover:bg-teal-50 dark:hover:bg-gray-700/50 ${
