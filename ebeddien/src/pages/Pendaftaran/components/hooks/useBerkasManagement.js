...
export const useBerkasManagement = (localId, options = {}) => {
  ...
  const api = options.api ?? pendaftaranAPI
  const isValidId = options.isValidId ?? ((id) => id && /^\d{7}$/.test(String(id)))
  ...
  // Fetch list berkas
  const fetchBerkasList = async (id) => {
    if (!isValidId(id)) {
      setBerkasList([])
      return
    }
    ...
    try {
      const result = await api.getBerkasList(id)
  ...
  const downloadForPreview = async (idBerkas, namaFile) => {
    return await api.downloadBerkas(idBerkas)
  }
  ...
  const handleUpdateBerkas = async (e) => {
    ...
    try {
      ...
      const result = await api.updateBerkas(formData)
      if (result.success) {
  ...
  const handleDeleteConfirmBerkas = async () => {
    ...
    try {
      const result = await api.deleteBerkas(berkasToDelete.id)
      if (result.success) {
