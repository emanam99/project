...
import { pendaftaranAPI, mahromAPI } from '../../../services/api'
...
function BerkasOffcanvas({
  ...
  overlayZIndex = 100,
  /** 'santri' (default) atau 'mahrom' — menentukan API upload */
  berkasTarget = 'santri',
}) {
  const berkasApi = berkasTarget === 'mahrom' ? mahromAPI : pendaftaranAPI
  const entityLabel = berkasTarget === 'mahrom' ? 'mahrom' : 'santri'
  ...
  const [jenisBerkas, setJenisBerkas] = useState(berkasTarget === 'mahrom' ? 'KTP' : 'Ijazah SD Sederajat')
  ...
  useEffect(() => {
    if (isOpen && idSantri) {
      ...
      if (existingBerkas) {
        ...
        const fallbackJenis = berkasTarget === 'mahrom' ? 'KTP' : 'Ijazah SD Sederajat'
        setJenisBerkas(defaultJenisBerkas || dariEditor || fallbackJenis)
        setKeterangan('')
      }
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (existingBerkas) {
        ...
        result = await berkasApi.updateBerkas(formData)
      } else {
        result = await berkasApi.uploadBerkas(idSantri, jenisBerkas, selectedFile, keterangan || null)
      }
      if (result.success) {
              ...
              {previewUrl ? (
                ...
                <div className="flex-1 relative flex items-center justify-center p-8 min-h-0">
                  ...
                  <div className="absolute bottom-6 left-6 right-6 text-white text-sm font-medium opacity-90">
                    {existingBerkas ? 'Ganti berkas' : `Upload berkas ${entityLabel}`}
                  </div>
                </div>
              )}
            </div>
