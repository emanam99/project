...
function RiwayatChatOffcanvas({
  ...
  const currentUserDisplayName = () => {
    const n = user?.nama && String(user.nama).trim()
    if (n) return n
    const u = user?.username && String(user.username).trim()
    return u || null
  }

  const normalizePengurusId = (id) => (id == null || id === '' ? null : String(id))

  /** Nama pengirim pesan keluar: dari API (lookup pengurus) atau sesi login jika cocok. */
  const namaPengirim = (item) => {
    const fromApi = item.nama_pengirim && String(item.nama_pengirim).trim()
    if (fromApi) return fromApi
    if ((item.arah || 'keluar') === 'masuk') return ''
    const cur = currentUserDisplayName()
    const itemPid = normalizePengurusId(item.id_pengurus)
    const userPid = normalizePengurusId(user?.id_pengurus ?? user?.user_id ?? user?.id)
    if (cur && itemPid && userPid && itemPid === userPid) return cur
    if (itemPid) return 'Staf'
    return 'Sistem'
  }

  const nomor = (nomorTujuan || '').trim()
  const nomorMasked = nomor.includes('*')
  const idSantriNum = idSantri != null && String(idSantri).trim() !== '' ? Number(idSantri) : 0
  const canUseBackendPhone = idSantriNum > 0
  const hasChatTarget = canUseBackendPhone || (nomor !== '' && !nomorMasked)
  ...
  const fetchRiwayat = useCallback(async (beforeDate = null, append = false) => {
    ...
    try {
      const opts = idSantriNum > 0
        ? { id_santri: idSantriNum, phone_field: phoneField || 'no_wa_santri' }
        : null
      const res = await chatAPI.getChatByNomor(nomorMasked ? '' : nomor, CHAT_PAGE_SIZE, beforeDate, opts)
  ...
  }, [hasChatTarget, nomor, nomorMasked, idSantriNum, phoneField])

  useEffect(() => {
    if (isOpen && hasChatTarget) {
      fetchRiwayat()
      if (!syncOnceDoneRef.current) {
        ...
        const opts = idSantriNum > 0
          ? { id_santri: idSantriNum, phone_field: phoneField || 'no_wa_santri' }
          : null
        chatAPI.syncFromWa(nomorMasked ? '' : nomor, 50, opts).then((res) => {
  ...
  }, [isOpen, hasChatTarget, nomor, nomorMasked, idSantriNum, phoneField, fetchRiwayat])

  /** Auto refresh: hanya saat offcanvas terbuka dan target chat ada. Skip merge saat baru saja kirim agar tidak duplikat. */
  useEffect(() => {
    if (!isOpen || !hasChatTarget) return undefined
    const opts = idSantriNum > 0
      ? { id_santri: idSantriNum, phone_field: phoneField || 'no_wa_santri' }
      : null
    const t = setInterval(() => {
      if (skipMergeRef.current) return
      chatAPI.getChatByNomor(nomorMasked ? '' : nomor, CHAT_PAGE_SIZE, null, opts).then((res) => {
        if (skipMergeRef.current) return
        const data = Array.isArray(res?.data) ? res.data : []
        setList((prev) => {
          const byId = new Map()
  ...
  }, [isOpen, hasChatTarget, nomor, nomorMasked, idSantriNum, phoneField])
  ...
  const handleSaveEdit = async (item) => {
    const msgId = item.message_id || item.messageId
    if (!msgId || !hasChatTarget || !editDraft.trim()) {
      handleCancelEdit()
      return
    }
    setEditSending(true)
    try {
      const res = await waAPI.edit(nomorMasked ? '' : nomor, msgId, editDraft.trim(), {
        ...(idSantriNum > 0 ? { id_santri: idSantriNum, phone_field: phoneField || 'no_wa_santri' } : {}),
      })
      if (res?.success) {
        setList((prev) =>
          prev.map((it) =>
  ...
  const handleSyncFromWa = async () => {
    if (!hasChatTarget || syncingFromWa) return
    setSyncingFromWa(true)
    try {
      const opts = idSantriNum > 0
        ? { id_santri: idSantriNum, phone_field: phoneField || 'no_wa_santri' }
        : null
      const res = await chatAPI.syncFromWa(nomorMasked ? '' : nomor, 50, opts)
      if (res?.success) {
  ...
  const sendPesanToWa = async (pesan, { clearInput = false } = {}) => {
    const teks = (pesan || '').trim()
    if (!teks || !hasChatTarget) {
    ...
    const optimisticItem = {
      ...
      id_pengurus: user?.id_pengurus ?? user?.user_id ?? user?.id ?? null,
      nama_pengirim: currentUserDisplayName() || 'Staf',
      message_id: null,
    }
    skipMergeRef.current = true
    setList((prev) => [optimisticItem, ...prev])
    if (clearInput) setInputPesan('')
    try {
      const idPengurus = user?.id_pengurus ?? user?.user_id ?? user?.id ?? null
      const sendRes = await waAPI.send(nomorMasked ? '' : nomor, teks, 'uwaba1', {
        ...(idSantriNum > 0 ? { id_santri: idSantriNum, phone_field: phoneField || 'no_wa_santri' } : {}),
  ...
  const handleKirim = async () => {
    ...
    setSending(true)
    try {
      await sendPesanToWa(inputPesan, { clearInput: true })
    } finally {
      setSending(false)
    }
  }

  const handleKirimUlang = async (item) => {
    if (!item || item.arah === 'masuk' || resendingId || sending) return
    const teks = (item.pesan || '').trim()
    if (!teks) return
    if (!window.confirm('Kirim ulang pesan ini ke WhatsApp?')) return
    setResendingId(item.id)
    try {
      const ok = await sendPesanToWa(teks, { clearInput: false })
      if (ok) {
        showNotification('Pesan dikirim ulang', 'success')
      }
    } finally {
      setResendingId(null)
    }
              ...
              <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    ...
                    disabled={syncingFromWa || !hasChatTarget}
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    {[...list].reverse().map((item) => {
                      ...
                      const showResendButton = !isMasuk && !isEditingThis && !!(item.pesan || '').trim()
                      const isResendingThis = resendingId === item.id
                      return (
                        <div
                          key={item.id}
                          ...
                          {!isEditingThis && (
                            <div className={`flex items-center gap-2 mt-0.5 text-xs text-gray-500 dark:text-gray-400 ${isMasuk ? 'flex-row' : ''}`}>
                              ...
                              {showResendButton && (
                                <button
                                  type="button"
                                  onClick={() => handleKirimUlang(item)}
                                  disabled={isResendingThis || sending || !!resendingId || !hasChatTarget}
                                  className="text-teal-600 dark:text-teal-400 hover:underline disabled:opacity-50"
                                  title="Kirim ulang isi pesan ini ke WhatsApp"
                                >
                                  {isResendingThis ? 'Mengirim…' : 'Kirim ulang'}
                                </button>
                              )}
                              {showEditButton && (
                                <button
              ...
              <div className="p-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex-shrink-0">
                ...
                <div className="flex gap-2 items-end">
                  <textarea
                    ...
                    disabled={sending || !!resendingId || !hasChatTarget}
                  />
                  <button
                    type="button"
                    onClick={handleKirim}
                    disabled={sending || !!resendingId || !inputPesan.trim() || !hasChatTarget}
