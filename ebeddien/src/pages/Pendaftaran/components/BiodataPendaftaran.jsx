...
import { santriAPI, pendaftaranAPI, santriBiodataAPI } from '../../../services/api'
...
import { toHtmlDateInputValue } from '../../../utils/htmlDateInputValue'
import { biodataPersonName } from '../../../utils/biodataPersonName'
...
function BiodataPendaftaran({
  onDataChange,
  externalSantriId,
  onOpenSearch,
  onBiodataSaved,
  hideBerkasSection = false,
  tahunHijriyah: tahunHijriyahProp,
  tahunMasehi: tahunMasehiProp,
  onTahunHijriyahChange,
  onTahunMasehiChange,
  hijriyahOptions: hijriyahOptionsProp,
  masehiOptions: masehiOptionsProp,
  scrollToNamaOnLoad = false,
  hideExternalToolbar = false,
  onToolbarState
}) {
  ...
  const didScrollToNamaRef = useRef(false)
  ...
  const handleOpenDeleteModalRef = useRef(null)
  ...
  const showPageTahunFilter = typeof onTahunHijriyahChange === 'function' && !hideExternalToolbar
  ...
  useEffect(() => {
    ...
    if (!/^\d{7}$/.test(nis)) return
    if (!String(tahunAjaran || '').trim() || !String(tahunAjaranMasehi || '').trim()) return
    void loadDataRef.current(nis)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tahunAjaran, tahunAjaranMasehi])

  useEffect(() => {
    didScrollToNamaRef.current = false
  }, [externalSantriId, scrollToNamaOnLoad])

  useEffect(() => {
    if (!scrollToNamaOnLoad || didScrollToNamaRef.current) return
    if (isLoading) return
    const nis = String(formData?.id || '').trim()
    if (!/^\d{7}$/.test(nis)) return
    didScrollToNamaRef.current = true
    const timer = window.setTimeout(() => {
      const sectionEl = sectionRefs.dataDiri?.current
      if (!sectionEl) return
      const scrollParent = sectionEl.closest('.overflow-y-auto')
      if (scrollParent) {
        const top = sectionEl.offsetTop - scrollParent.offsetTop
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scrollToNamaOnLoad, isLoading, formData?.id])
      ...
      try {
        ...
        if (biodataResponse.success && biodataResponse.data) {
          ...
          const newFormData = {
            ...
            tanggal_lahir: toHtmlDateInputValue(biodata.tanggal_lahir),
            nisn: biodata.nisn || '',
            no_kk: biodata.no_kk || '',
            ...
            wali: biodataPersonName(biodata.wali),
            nik_wali: biodata.nik_wali || '',
            tempat_lahir_wali: biodata.tempat_lahir_wali || '',
            tanggal_lahir_wali: toHtmlDateInputValue(biodata.tanggal_lahir_wali),
            ...
            id_lttq_tingkatan: biodata.id_lttq_tingkatan != null ? String(biodata.id_lttq_tingkatan) : '',
            prodi: biodata.prodi || '',
          }
  ...
  const handleFieldChange = (field, value) => {
    ...
    if (field === 'nik') {
      ...
      setFormData(prev => {
        ...
        // Setiap ubah gender: perbarui kategori (Laki-laki â†’ Banin, Perempuan â†’ Banat)
        if (field === 'gender') {
          if (value === 'Laki-laki') {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const biodataPayload = {
        ...formData,
        ayah: biodataPersonName(formData.ayah),
        ibu: biodataPersonName(formData.ibu),
        wali: biodataPersonName(formData.wali),
        id_admin: user?.id || null,
        tahun_hijriyah: tahunAjaran || null,
        tahun_masehi: tahunAjaranMasehi || null
      }
      delete biodataPayload.lttq
      delete biodataPayload.kelas_lttq
      delete biodataPayload.kel_lttq
      if (biodataPayload.id_lttq_tingkatan === '' || biodataPayload.id_lttq_tingkatan == null) {
        biodataPayload.id_lttq_tingkatan = null
      } else {
        biodataPayload.id_lttq_tingkatan = Number(biodataPayload.id_lttq_tingkatan)
      }
  ...
  handleOpenDeleteModalRef.current = handleOpenDeleteModal
  ...
  useEffect(() => {
    ...
    onToolbarState({
      ...
      setIsSidebarOpen,
      localId,
      onIdChange: (value) => handleFieldChangeRef.current?.('id', value),
      onSave: () => handleSaveRef.current?.(),
      isSaving,
      isLoading,
      formDataId: formData.id,
      hasChanges,
      onOpenSearch: () => onOpenSearchRef.current?.(),
      onOpenNewModal: () => setShowNewModal(true),
      onOpenDeleteModal: () => handleOpenDeleteModalRef.current?.(),
          ...
          >
            <div className="p-6">
              <div className="mb-4">
                ...
                <p className="text-sm text-red-600 dark:text-red-400 font-medium">âš ï¸ Tindakan ini tidak dapat dibatalkan!</p>
              </div>
      ...
      <div className={`flex-1 overflow-y-auto min-h-0 p-6 transition-all duration-300 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600 dark:hover:scrollbar-thumb-gray-500 ${
        ...
        >
          ...
          <KategoriPendidikanSection
            ...
            lttqTingkatanOptions={lttqTingkatanOptions}
          />
