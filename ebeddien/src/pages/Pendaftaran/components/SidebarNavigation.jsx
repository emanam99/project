 ...
 * @param {string} props.activeSection - Section yang sedang aktif
 * @param {function} props.scrollToSection - Function untuk scroll ke section
 */
function SidebarNavigation({ isOpen, activeSection, scrollToSection, excludeSections = [] }) {
  // Calculate indicator position (pakai filtered config agar posisi sesuai tombol yang tampil)
  const getIndicatorY = (filteredConfig) => {
    const index = filteredConfig.findIndex((s) => s.key === activeSection)
    ...
    // Adjust: reduce by 6px to better align with icon center (icon is visually centered in button)
    let baseY = 16 + (index * 48) + 18 - 12 - 14
    
    // Special adjustment for "berkas" section (last item) - indicator was appearing above icon
    if (activeSection === 'berkas') {
      baseY += 6 // Move indicator down slightly to better align with icon center
    }
  ...
  const sectionConfig = [
    ...
    { key: 'kategoriPendidikan', title: 'Kategori & Pendidikan', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
      </svg>
    )},
    { key: 'berkas', title: 'Berkas', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const filteredConfig = excludeSections.length
    ? sectionConfig.filter((s) => !excludeSections.includes(s.key))
    : sectionConfig
    ...
    <div className={`absolute left-0 top-0 bottom-0 w-12 bg-gray-200 dark:bg-gray-700/50 border-r-2 border-gray-300 dark:border-gray-600 z-10 transition-transform duration-300 ${
      ...
      <div className="flex flex-col items-center py-4 gap-3 h-full overflow-y-auto relative">
        ...
        <motion.div
          ...
          initial={false}
          animate={{
            y: getIndicatorY(filteredConfig)
          }}
