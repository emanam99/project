...
import { getSlimApiUrl, getAuthHeaders } from '../../../services/api'
...
function PrintPendaftaran({
  ...
  const { hijriyah: tahunAjaranDefault } = usePendaftaranDefaultTahunAjaran()
  ...
  const effectiveHijriyah = useMemo(
    () => String(tahunHijriyahProp ?? searchParams.get('tahun_hijriyah') ?? '').trim(),
    [tahunHijriyahProp, searchParams]
  )
  const effectiveMasehi = useMemo(
    () => String(tahunMasehiProp ?? searchParams.get('tahun_masehi') ?? '').trim(),
    [tahunMasehiProp, searchParams]
  )

  const pendaftaranBlocks = useMemo(() => {
    if (!pendaftaranData?.pendaftaran_multi || !Array.isArray(pendaftaranData.pendaftaran_blocks)) {
      return null
    }
    return pendaftaranData.pendaftaran_blocks.length ? pendaftaranData.pendaftaran_blocks : null
  }, [pendaftaranData])

  /** Satu kwitansi per tahun ajaran aktif halaman (bukan semua registrasi PSB). */
  const kwitansiBlocks = useMemo(() => {
    if (pendaftaranBlocks?.length) {
      if (effectiveHijriyah && effectiveMasehi) {
        const matched = pendaftaranBlocks.filter((block) => {
          const rh = String(block.registrasi?.tahun_hijriyah ?? '').trim()
          const rm = String(block.registrasi?.tahun_masehi ?? '').trim()
          return rh === effectiveHijriyah && rm === effectiveMasehi
        })
        if (matched.length) return matched
      }
      return pendaftaranBlocks
    }
    return [null]
  }, [pendaftaranBlocks, effectiveHijriyah, effectiveMasehi])

  const registrasiUntukForm = useMemo(() => {
    if (!pendaftaranData) return null
    if (pendaftaranBlocks?.length) return pendaftaranBlocks[0].registrasi
    return pendaftaranData.registrasi
  }, [pendaftaranData, pendaftaranBlocks])

  const tahunAjaranLabelHijriyah = useMemo(() => {
    if (pendaftaranData?.pendaftaran_multi && (pendaftaranBlocks?.length ?? 0) > 1) {
      return 'Semua tahun ajaran (PSB)'
    }
    const ta =
      effectiveHijriyah ||
      registrasiUntukForm?.tahun_hijriyah ||
      tahunAjaranDefault ||
      '1446-1447'
    ...
    return `${ta} H`
  }, [pendaftaranData, pendaftaranBlocks, tahunAjaranDefault, effectiveHijriyah, registrasiUntukForm])
  ...
  const getCurrentUrlWithParams = () => {
    const b = pendaftaranData?.biodata || {}
    return getMybeddienKwitansiQrUrl('pendaftaran', {
      nis: b.nis,
      id: b.id || santriId || searchParams.get('nis') || searchParams.get('id'),
    })
  }
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (effectiveHijriyah) params.set('tahun_hijriyah', effectiveHijriyah)
        if (effectiveMasehi) params.set('tahun_masehi', effectiveMasehi)
    ...
    loadData()
  }, [santriId, searchParams, effectiveHijriyah, effectiveMasehi])
      ...
      <div className="print-container-wrapper">
        ...
        {!loading && !error && pendaftaranData && (
          <>
            {printKwitansi && (
              <div id="content">
                {kwitansiBlocks.map((block, blockIdx) => {
                  const receiptData = block
                    ? {
                        ...pendaftaranData,
                        biodata: block.biodata,
                        tunggakan: block.tunggakan,
                        pembayaran: block.pembayaran,
                        registrasi: block.registrasi
                      }
                    : pendaftaranData
                  const blockKey = block?.registrasi?.id ?? 'single'
                  return (
                    <div
                      key={blockKey}
                      style={blockIdx > 0 ? { pageBreakBefore: 'always', marginTop: '12px' } : undefined}
                    >
                      {block && (block.registrasi?.tahun_hijriyah || block.registrasi?.tahun_masehi) ? (
                        <p
                          className="no-print"
                          style={{ fontSize: '13px', fontWeight: 600, color: '#0f766e', marginBottom: '8px' }}
                        >
                          PSB — Hijriyah {block.registrasi.tahun_hijriyah || '—'} · Masehi {block.registrasi.tahun_masehi || '—'}
                        </p>
                      ) : null}
                      <div className={columnCount === '2' ? 'layout-two-columns' : ''}>
                        {columnCount === '2' ? (
                          <>
                            <div className={`receipt-instance ${useColor ? 'receipt-blue' : ''}`}>
                              ...
                              <ReceiptContentTwoColumns data={receiptData} formatRupiah={formatRupiah} formatTanggal={formatTanggal} getCurrentUrlWithParams={getCurrentUrlWithParams} user={user} />
                            </div>
                          ...
                          <div className={`receipt-instance ${useColor ? 'receipt-blue' : ''}`}>
                            {useColor && <div className="top-border-line" style={{borderTop: '1pt solid #87CEEB', width: '100%', marginTop: '0', marginBottom: '12px'}}></div>}
                            <ReceiptContent data={receiptData} formatRupiah={formatRupiah} formatTanggal={formatTanggal} getCurrentUrlWithParams={getCurrentUrlWithParams} user={user} />
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {printRaporTes && (
              ...
              >
                <PrintRaporTesMadin
                  idSantri={santriId}
                  biodata={pendaftaranData.biodata}
                  tahunHijriyah={effectiveHijriyah}
                  tahunMasehi={effectiveMasehi}
            ...
            {printSuratKapdarEffective && (
              ...
              >
                <PrintSuratPerjanjianKapdar
                  biodata={pendaftaranData.biodata}
                  registrasi={registrasiUntukForm}
                />
              </div>
            )}
            {printPaktaIntegritasEffective && (
              ...
              >
                <PrintPaktaIntegritas biodata={pendaftaranData.biodata} registrasi={registrasiUntukForm} />
              </div>
            )}
          </>
        )}
          ...
          <div className="payment-info-section">
            <div className="qr-admin-row">
              <div className="qr-code-container">
                <a
                  href={getCurrentUrlWithParams()}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="qr-code-link"
                  title="Scan atau klik: riwayat PSB di myBeddien (login wajib)"
                >
