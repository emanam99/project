  ...
  async initialize() {
    try {
      ...
      const isPushUnavailable =
        ...
        /push service not available|registration failed/i.test(msg)
      if (isPushUnavailable) {
        if (import.meta.env.DEV) {
          console.debug('[PWA] Push tidak tersedia (normal di beberapa browser/HTTP):', msg || error?.name)
        }
        return { success: false, message: 'Push tidak tersedia di lingkungan ini' }
      }
