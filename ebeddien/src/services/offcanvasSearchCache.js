...
class OffcanvasSearchCacheDB extends Dexie {
  constructor() {
    ...
    // v4: GET /santri kini mengembalikan biodata lengkap (mis. nisn). Baris lama di IndexedDB
    // sering tidak punya key nisn karena cache dari query singkat + sinkron ?since= tidak menyentuh semua baris.
    // Kosongkan indeks sekali agar halaman Santri memuat ulang full list dengan bentuk baris terbaru.
    this.version(4).stores({
      ...
      await tx.table('santriRows').clear()
    })
  }
}
