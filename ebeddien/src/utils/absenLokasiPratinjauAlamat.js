...
/** Kunci stabil untuk animasi / memo saat teks pratinjau alamat berubah. */
export function alamatPreviewFingerprint(d) {
  if (!d || typeof d !== 'object') return ''
  const keys = ['_source', 'dusun', 'rt', 'rw', 'desa', 'kecamatan', 'kota', 'provinsi', 'display_name']
  return keys.map((k) => (d[k] == null ? '' : String(d[k]).trim())).join('\u241e')
}
...
/**
 * Satu elemen dari GET /api/absen-lokasi/pratinjau-alamat — tanpa nama lokasi.
 * @param {{ alamat?: Record<string, string>, jarak_meter?: number, punya_alamat_manual?: boolean }} item
 */
export function pratinjauAlamatApiItemToPreview(item) {
  if (!item || typeof item !== 'object') return null
  const alamat = item.alamat
  if (!alamat || typeof alamat !== 'object' || Object.keys(alamat).length === 0) return null
  const base = lokasiRowToAlamatPreviewShape(alamat)
  return {
    ...base,
    _source: 'lokasi_umum',
    _jarak_meter: typeof item.jarak_meter === 'number' ? item.jarak_meter : null
  }
}
