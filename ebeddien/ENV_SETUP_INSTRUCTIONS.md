...
**Build production** memuat `ebeddien/.env.production` (default `https://gambar.alutsmani.id`). Untuk dev lokal, `.env` boleh `VITE_GAMBAR_BASE=/gambar` (proxy ke aset lokal).

Set di `ebeddien/.env` (dev) atau pastikan `.env.production` untuk CI/deploy:

```env
VITE_GAMBAR_BASE=https://gambar.alutsmani.id
```
